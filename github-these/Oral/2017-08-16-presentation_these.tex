...
De par
\begin{itemize}
 \item{} les polynômes modulaires (algorithme SEA), 
 \item{} la cryptanalyse de cryptosystème (par exemple [Rostovtsev,~Stolbunov~'06]) 
\end{itemize}
on peut être amené à résoudre le problème suivant:
...
\begin{itemize}
\item{} [Atkin~'91],[Charlap, Colay, Robbins '91] ,
...
\pause
\bluebox{Algorithme de Couveignes'96 (calcul d'une $r$-isogénie $\phi:E\to E'$)}{
...
\rangle =E[\ell^k] $ alors $Q=R \Leftrightarrow \pi(P,Q)=\pi(P,R)$. 
\end{prop}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
...
%	\item Les points de torsion ne sont pas en général définis dans $\mathbb{F}_q$.	
	\item Nous travaillons dans des extensions $\ell$-adiques de $\mathbb F_q$
		...
		\begin{itemize}  
         ...
         méthodes mentionnées par [Enge-Morain '03] et développées par [DeFeo'10])
         \end{itemize}
