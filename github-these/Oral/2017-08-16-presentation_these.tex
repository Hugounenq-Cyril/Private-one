...
\includegraphics[scale=0.5]{amazon_google_and_smartcard.pdf} 
\end{frame}
...
{Pour toute courbe elliptique $E$ et tout entier $n$ il existe un polynôme $\psi_n$ tel que les abscisses des points de $n$-torsion soient les racines de ce polynôme.}
...
\item Deux courbes $E$ et $E'$ sont dites \emph{$\ell$-isogènes} s'il existe une $\ell$-isogénie qui les relie.
...
\bluebox{Problème du calcul de l'isogénie explicite}
{
 ...
 \`A partir de $E$, $E'$  deux courbes elliptiques \emph{ordinaires} $r$-isogènes définies sur 
...
\item Vérification si la fraction rationnelle $F$ déduite du polynôme d'interpolation définit une isogénie.\hfill $\color{blue!50!gray}{\tilde{O}(r)}$
	\begin{itemize}
	\item Test du degré du dénominateur $d$ de $F$;
	\item Test de la décomposition de $d$ en facteurs carré ;
	\item Test si $\psi_{r}^2= 0 \bmod d$;
	\end{itemize}
...
%\hfill $\tilde{O}(r)$
	...
	$(r, r-1)$ à l'aide d'un algorithme de reconstruction \\ 
	rationnelle et test de celle-ci.
...
\bluebox{$p$-adique}{
Soient $\langle P \rangle= E[p^k]$, $\langle P' \rangle= E'[p^k]$
\[
...
\bluebox{$\ell$-adique}{
Soient $ \langle P,Q \rangle = E[\ell^k]$, $\langle P',Q' \rangle = E'[\ell^k]$
\begin{gather*}
\begin{pmatrix}
P\\Q
\end{pmatrix}
...
%	\item Calcul d'une fraction rationnelle $F$ de degrés \\
...
\bluebox{Premier d'Elkies}{Nous appelons $\ell$ un \emph{premier d'Elkies} si le polynôme caractéristique de $\pi$ se factorise sur $\mathbb{Z}_{\ell}$ comme suit:
\[
...
\begin{frame}
\frametitle{Décomposition horizontale}
\textbf{Objectif :} Calculer une décomposition horizontale de $E[\ell^k]$. \\
Celle-ci est faite ici à l'aide de $\ell^k$-isogénies horizontales.
%Does it work the same with general volcanoes of $\ell$-isogenies?
%\pause
\begin{columns}
\begin{column}{5cm}
\bluebox{Solutions existantes}
{\begin{itemize}
\item{} [Fouquet,~Morain~'02] est plus coûteux, nécessite le calcul de $\ell+1$ chemins distincts; %me situer par rapport à eux
\item{} [Ionica,~Joux~'10] suppose la connaissance du cardinal de la courbe, possède des restrictions. %me situer par rapport à eux
\end{itemize}}
\end{column}
\begin{column}{5cm}
\begin{center}
\tikzstyle{mu1}=[mu,draw=blue!60!green]
\tikzstyle{lambda1}=[lambda,draw=red]
...
\draw [mu] (A)--(B) node[midway,below]{$\color{red} \lambda$}; \draw[mu] (B)--(C) node[midway,below]{$\color{red} \lambda$}; \draw[mu] (C)--(D) node[midway,above]{$\color{red} \lambda$};
\draw [lambda] (A)--(F) node[midway,below]{$\color{blue} \mu$} ; \draw[lambda] (F)--(E) node[midway,above]{$\color{blue} \mu$}; \draw[lambda] (E)--(D) node[midway,above]{$\color{blue} \mu$};
\node [fill=white] at (A) {$E$};
\end{tikzpicture}
\end{center}
\end{column}
\end{columns} 
...
%Slide rappel volcan $h=0$ et base diagonale. \\

\begin{nota}
La notation $M_{\pi}(P,Q)$ désigne la matrice représentant l'action du Frobenius dans une base $\langle P,Q \rangle = E[\ell^k]$.
\end{nota}
...
$
signifie que $\pi(P_0)=[\color{red}{\lambda} \color{black}]P_0$ et $\pi(Q_0)=[\color{blue} \mu \color{black}]Q_0$.\\ 
\smallskip

\bluebox{Algorithme de diagonalisation}{
\begin{algorithmic}[1]
\REQUIRE $P_0,Q_0$ une base quelconque de $E[\ell^k]$ avec $E$ sur le cratère.  
\ENSURE $\langle P,Q \rangle =  E[\ell^k]$ tels que $M_{\pi}(P,Q)=\left( \begin{smallmatrix} \color{red}{\lambda} & \color{black} 0 \\
0 & \color{blue} \mu \end{smallmatrix} \right)$.
\end{algorithmic}
\begin{itemize}
\item Algébre linéaire sur $M_{\pi}([\ell^{i}]P_0,[\ell^{i}]Q_0)$
\item Relèvement ($i \leftarrow i-1$)
\end{itemize}}
Coût de la diagonalisation: $\tilde{O}(\ell^{2k+O(1)})$.
...
\begin{itemize}
\item[$\rightarrow$] suffisant pour le cas $h=0$
\uncover<3>{\item[$\rightarrow$] seulement $\langle [\ell^h]P,[\ell^h]Q \rangle = \color{red}{E[\ell^{k-h}]_{{\lambda}}} \color{black} \oplus \color{blue}{E[\ell^{k-h}]_{{\mu}}}$.}
\end{itemize}
\vspace{-1.0mm}
\begin{columns}
\begin{column}{5cm}
...
$\ker(\pi- \blu \mu\; | \; E[\ell^k]) \simeq \mathbb Z/\ell^k$.
\end{column}
\begin{column}{5.5cm}
...
\textbf{Problème:} pour $h \neq 0$
$\ker(\pi- \blu \mu  \;| \; E[\ell^k])
...
\end{frame}
\begin{frame}
\frametitle{Calcul d'une décomposition horizontale}
%tu répètes x fois l'algo de diagonalisation en te déplaçant sur le
%cratère, et pourquoi le surcoût est négligeable par rapport à h=0. \\

\bluebox{Algorithme de décomposition horizontale}{
\begin{algorithmic}[1]
\REQUIRE $P_0,Q_0$ une base quelconque de $E[\ell^k]$ avec $E$ sur le cratère.  
\ENSURE $\langle P,Q \rangle =  E[\ell^k]$ tels que $\langle \red P , \blu Q \rangle = \color{red}{E[\ell^k]_{{\lambda}}} \color{black} \oplus \color{blue}{E[\ell^k]_{{\mu}}} $.
\end{algorithmic}
\begin{itemize}
\item Calcul de $\color{red}{E[\ell^{k-h}]_{{\lambda}}} \color{black} \oplus 
\color{blue}{E[\ell^{k-h}]_{{\mu}}}$ à l'aide de \textbf{Diagonalisation};
\item $L \leftarrow L \cup \ell^{k-h}$-isogénies horizontales;
\item Déplacement sur le cratère.
\end{itemize}}
$\Rightarrow$Pas de surcoût majeur par rapport au cas $h=0$ car nous utilisons uniquement 
des bases diagonales de $E[\ell^k]$.

\begin{itemize}
\item Notre algorithme a un coût de $\tilde{O}(\ell^{2k+O(1)})$;
\item L'algorithme de [Fouquet,~Morain~'02] a un coût de $\tilde{O}(\ell^{3k+O(1)})$.
\end{itemize}
...
%\begin{frame}
%\frametitle{Calcul d'une $\ell$-isogénie horizontale de direction $\mu$}
%%
%%\bluebox{Fait}{
...
%\begin{itemize}
%\item Calcul d'une base diagonale $\langle P,Q \rangle $ de $E[\ell^3]$ telle que $\pi(Q)=[\mu]Q$
%\item Renvoi de l'isogénie $\psi$ de noyau $\langle \ell^{2}Q \rangle$
%\end{itemize}
%L'isogénie de noyau $\langle Q \rangle$ étant rationnelle de degré
%$\ell^3$ sur un volcan de hauteur $2$ alors $\langle \ell^{2}Q \rangle$ est le noyau d'une isogénie horizontale.
%\vspace{-5mm}
%\begin{center}
...
%%			\only<1>{
%			\draw (A) node[fill=yellow!50!white,shape=circle]{$E$};
%%			}
%			\draw (C) node[fill=white]{$\bullet$};
%			\draw (CA) node[fill=white]{$\bullet$};
%			\draw (CB) node[fill=white]{$\bullet$};
...
%\end{center}
%\vspace{-4mm}
%Pour le calcul d'une $\ell^k$-isogénie horizontale:
%\begin{itemize}
%\item Notre algorithme a une complexité de $\tilde{O}(\ell^2k \ell^{O(1)})$ (à vérifier);
%\item L'algorithme de [Fouquet,~Morain~'02] a une complexité de $O()$ (à compléter).
%\end{itemize}
...
\bluebox{Un algorithme de Couveignes $\ell$-adique ($\ell$ un premier d'Elkies)}{
\begin{algorithmic}[1]
\REQUIRE $E, E',r$ tels que $E$ et $E'$ soient deux courbes $r$-isogènes sur $\mathbb{F}_{q}$ 
...
%   l'algorithme.
   ...
   \item Calcul et test d'un candidat à l'aide d'un polynôme\\ 
   d'interpolation $L$ qui envoie $\color{red}P\mapsto [a]P'$ et 
   $\color{blue}Q\mapsto [b]Q'$. \hfill\uncover<2>{$\color{blue!50!gray}{\tilde{O}(r \ell^{O(1)})}$}
  \end{enumerate}
\end{enumerate}
\end{frame}
...
\frametitle{Limitations du cas Elkies}
 \bluebox{Trouver un premier $\ell$ d'Elkies}{
  \begin{itemize}
  \item La complexité de l'algorithme dépend polynomialement du paramètre $\ell$.
  \item Heuristiquement la moitié des paramètres $\ell$ sont supposés être des premiers d'Elkies pour une courbe $E$ fixée.
  \item En théorie nous pouvons seulement prouver qu'un tel $\ell \leqslant 
  O(\log(q))$ \textbf{pour presque tous les $q$ et courbes $E,E'$} (voir 
[Shparlinski,~Sutherland~'14]).\\
 $\Rightarrow$ Algorithme quasi-quadratique de Couveignes $\ell$-adique sans dépendance polynomiale en la caractéristique.
\end{itemize}
  }
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
\bluebox{Premier d'Atkin}{Un nombre premier $\ell$ est dit un \emph{premier d'Atkin} si le polynôme caractéristique de $\pi$: 
\[
\pi^2 - t\pi + q 
\]
est irréductible sur $\mathbb{Z}_{\ell}$}
%Thus we are not able to distinguish two different eigenspaces, 
Ainsi on ne peut se servir d'isogénies horizontales comme ce fut le cas avec des \emph{premiers d'Elkies}.  
\begin{figure}
\begin{center}
...
\vspace{-0.5mm}
\begin{prop}
Soit $E$ située sur un volcan de hauteur $0$, $\ell$ un nombre premier d'Atkin,
$P,Q,R$ trois points de $E$  tels que $\langle P,Q \rangle = \langle P,R 
\rangle =E[\ell^k] $ alors
\[
Q=R \Leftrightarrow M_{\pi}(P,Q)=M_{\pi}(P,R).
\]
\end{prop}
Le coût du calcul de $M_{\pi}(P,Q)$ est de $\tilde{O}(\ell^{k+O(1)})$.
\begin{cor}
Soient $E, E'$ deux courbes $r$-isogènes situées sur un volcan de hauteur $0$, 
$\ell$ un nombre premier d'Atkin, $\langle P,Q \rangle = E[\ell^k]$, $R' \in 
E'[\ell^k]$ et $\phi:E \mapsto E'$ une isogénie de degré $r$, alors 
\[
R'=\phi(Q)
 \Leftrightarrow M_{\pi}(P,Q)=M_{\pi'}(\phi(P),R').
\]
\end{cor}

Le coût de la recherche d'un tel point $R'$ est de $\tilde{O}(\ell^{k+O(1)})$.
\end{frame}
...
\begin{enumerate}
\medskip
  \item Calcul de $R'$ tel que $M_{\pi'}(S',R')=M_{\pi}(P,Q)$; \hfill\only<2>{$\color{blue!50!gray}{\tilde{O}(\sqrt{r} \ell^{O(1)})}$}
  \medskip
...
\item Calcul et test d'un candidat à l'aide d'un polynôme\\ 
      d'interpolation $L$ qui envoie $P\mapsto S'$ et $Q\mapsto R'$. \hfill\only<2>{$\color{blue!50!gray}{\tilde{O}(r \ell^{O(1)})}$}
   \end{enumerate}
\end{enumerate}
\end{frame}
...
\begin{itemize}
\item Si $\ell$ est un nombre d'Atkin ou d'Elkies avec $h=0$ alors on obtient la complexité énoncée;
...
\begin{frame}
\frametitle{Corps de définition de $E[\ell^i]$}
\begin{columns}
\begin{column}{2cm}
\begin{center}
\begin{figure}
...
\draw (E) node {$\bullet$} node[left] {$\mathsf{F}_4$};
\draw (D)--(E) node[midway,left]{$\ell$} ;
...
\bluebox{}{
Pour $E$ une courbe elliptique définie sur $\mathbb{F}_q$, $\ell~\nmid~q$, nous définissons 
une tour d'extensions de $\mathbb{F}_q$: $\mathbb{F}_q, \mathsf{F}_{1}, 
\mathsf{F}_{2}, \mathsf{F}_{3}, \cdots$ où $\mathsf{F}_i$ est le plus
petit corps tel que $E[\ell^i] \subset E(\mathsf{F}_{i})$
\begin{itemize}
...
degré divisant $ \ell^2-1$;
\item Il existe $i_0 \geqslant 1$ tel que $\mathsf{F}_{i_0} \simeq \mathsf{F}_{i_0-1} \simeq \cdots \simeq \mathsf{F}_{1}$ et $\mathsf{F}_{i_0} \not \simeq \mathsf{F}_{i_0+1}$; 
\item $\mathsf{F}_{i}$ pour $i \geqslant i_0+1$ est une extension de 
...
\begin{frame}
  ...
  \frametitle{Calculs dans une tour d'extensions de corps finis}


  \begin{itemize}
%	\item Les points de torsion ne sont pas en général définis dans $\mathbb{F}_q$.	
	\item Nous généralisons les constructions des tours d'extensions de 
	    $\mathbb F_q$ [De~Feo,~Doliskani, Schost '13], 
        [Doliskani,~Schost '15] et où en particulier nous avons un calcul rapide 
		\begin{itemize}  
		 \item du Frobenius (adapté depuis [Doliskani-Schost'15]);
		 \item du polynôme d'interpolation (adaptant des 
         techniques de [Enge,Morain~'03] développées par [De Feo~'10]);
		 \item des points d'ordre $\ell^k$ d'une courbe elliptique.       
         \end{itemize}
\end{itemize}
...
\bluebox{Contributions}{
\begin{itemize}
\item De nouveaux outils pour se déplacer dans le volcan d'isogénies. 

\item Une variante de l'algorithme de Couveignes sans dépendance polynomiale en la caractéristique.
...
\item Estimer le coût de calcul de l'anneau des endomorphismes en modifiant l'algorithme de Kohel  avec l'étude de l'action de l'endomorphisme de Frobenius et l'implanter.
...
\end{algorithmic}}
\only<1>{
\bluebox{}{\textbf{Fait:} $M_{\pi}(P,R)=M_{\pi}(P,Q)$ ssi $\color{red}[\ell^h]\color{black}R=\color{red}[\ell^h]\color{black}Q$ \\
avec $\langle P,R \rangle = \langle P,Q \rangle = E[\ell^{h+i}]$ avec $i>0$.}
}
\only<2>{
\bluebox{}{\textbf{Fait:} $M_{\pi}(P,R)=M_{\pi}(P,Q)$ ssi $[\ell^h]R=[\ell^h]Q$ \\
avec $\langle P,R \rangle = \langle P,Q \rangle = E[\ell^{h+i}]$ avec $i>0$.}
}
\begin{enumerate}
\item Choix du plus petit entier $k$ tel que $\ell^{2k}>4r$;
...
%\item Deduce $\to$ $\langle{\color{red}P},{\color{blue}Q}\rangle=E[\ell^k]$ and $\langle{\color{red}P'},{\color{blue}Q'}\rangle=E'[\ell^k]$;
\medskip
\item Calcul de la matrice $M_{\pi}(P,Q)$; \hfill {$\color{blue!50!gray} \tilde{O}(\sqrt{r} \ell^{O(1))})$}
\medskip
\item Pour tout point $S'$ d'ordre $\ell^k$ de $E'$: \hfill {$\color{blue!50!gray}{O(r)}$}
\begin{enumerate}
\medskip
  \item Calcul de $R'$ tel que $M_{\pi'}(S',R')=M_{\pi}(P,Q)$; \hfill {$\color{blue!50!gray}{\tilde{O}(\sqrt{r} \ell^{O(1)})}$}
  \medskip
  \only<1>{
...
\item Calcul et test d'un candidat à l'aide d'un polynôme\\ 
	d'interpolation $L$ qui envoie $P\mapsto S'$ et $Q\mapsto U'$. \hfill $\color{blue!50!gray}{\tilde{O}(r \ell^{O(1)})}$
  \end{enumerate}
...
\medskip
\item Détermination à partir de $d_{\pi}$ si $\ell_i$ est un nombre premier d'Atkin ou d'Elkies 
\medskip 
...
{
Un élément de $F_k$ est dit sous forme univariée s'il est écrit à l'aide d'un polynôme $G \in \mathbb{F}_q[X]$ de degré inférieur à $d_1\ell^{k-1}$, avec $d_1=[\mathsf{F}_1:\mathbb{F}_q]$, évalué en l'élément primitif $x_k$ de $F_k$.
}
...
{
Un élément de $\mathsf{F}_k$ est dit sous forme bivariée s'il est écrit à l'aide des éléments primitifs $x_k$ de $\mathsf{F}_k$ et $x_{k-1}$ de $\mathsf{F}_{k-1}$ sous la forme: 
\begin{equation*}
