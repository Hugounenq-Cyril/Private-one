...
Les isogénies séparables de degré $\ell$ sont représentées par des fonctions
...
La connaissance du noyau de l'isogénie permet d'expliciter celle-ci
...
\begin{figure}[h]
		...
		à cratères cycliques (cas Elkies), puis les volcans avec cratères réduit à un
		point (cas Atkin).
%\begin{figure}[hbtp]
%\centering
...
\bluebox{Atkin prime}{Un nombre premier $\ell$ est dit un \emph{premier de Atkin} si le polynôme caractéristique de $\pi$: 
\[
\pi^2 - t_{\pi}\pi + q 
\]
...
\bluebox{Un algorithme de Couveignes $\ell$-adique (Cas Atkin)}{
\begin{algorithmic}[1]
\REQUIRE $E, E'$ deux courbes $r$-isogènes sur $\mathbb{F}_{q}$ 
\ENSURE $\phi: E \rightarrow E'$ de degré $r$
\end{algorithmic}}
...
\medskip
  ...
  \medskip
  \item Pour tout point $U' \in E'$ tel que $[\ell^h]R'=[\ell^h]U'$ \hfill\only<2>{$O(\ell^h)$}
  \begin{enumerate}
...
\item L'impossible... Couveignes en O(r)
\end{itemize}}
...
\begin{frame}
\frametitle{Bonus: points ascendants horizontaux}
\begin{figure}
\begin{center}
