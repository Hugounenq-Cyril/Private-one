...
\item $\mathsf{F}_{i}=\mathsf{F}_{i-1}[X_{i-1}]/(X^{\ell}-x_{i-1})$ pour $i \geqslant 2$.
\end{itemize}
}
\end{frame}
...
{
On dit qu'un élément de $F_k$ est sous forme univariée si il est écrit à l'aide d'un polynôme $G \in \mathbb{F}_q[X]$ de degré $d_1\ell^{k-1}$ évalué en l'élément primitif $x_k$ de $F_k$.
}

\begin{exe}
Soit $\mathbb{F}_q, \mathsf{F}_1, \mathsf{F}_2, \cdots, \mathsf{F}_k$ une tour d'extensions $5$-adiques telle que $\mathbb{F}_q \cong \mathsf{F}_1$, soit $\alpha \in \mathsf{F}_4$, $x_4$ un élément primitif de $F_4$ alors une représentation univariée d'un élément de $\mathsf{F}_4$ est:
\begin{equation}
