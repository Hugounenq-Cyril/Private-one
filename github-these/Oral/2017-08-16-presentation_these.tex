...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Introduction - motivation cryptographique}
...
\only<1-1>{
\begin{center}
\includegraphics[scale=0.32]{Images/communication_Alice_Bob.jpg} 
\end{center}
}
\only<2-2>{
\begin{center}
\includegraphics[scale=0.32]{Images/communication_Alice_Bob_chiffre.jpg} 
\end{center}
}
\only<3-3>{
\begin{center}
\includegraphics[scale=0.32]{Images/communication_Alice_Bob_dechiffre.jpg} 
\end{center}
}
...
\begin{tikzpicture}[scale=0.50,domain=-10/3:4.5,samples=100,yscale=1/2]
          \begin{scope}[fixed point arithmetic]
            \begin{scope}[fixed point arithmetic]
...
\end{matrix}\right ), \quad \text{with }a,d \text{ primes with }\ell, \ell^{h} |a-d \]  %with $a,d $ primes with $\ell$,
 and $e = 0$ iff~$E$ lies on the crater. Moreover 
\begin{itemize}
\item either $b$ and $c$ are primes with $\ell$, 
\item either one of them is prime with $\ell$ and the other has its $\ell$-adic valuation which belongs to $[2,2e]$.
...
Dans le cas Atkin on ne peut comme dans le cas Elkies distinguer des espaces propres pour le Frobenius
\pause
\begin{prop}
...
\pause 
permet de distinguer les différents sous-groupes une fois que l'on en a fixé un ...
\end{frame}
...
Couveignes'algorithm dans le cas Atkin
...
\begin{frame}
\frametitle{Bonus: An $\ell$-adic algorithm in the Atkin case}
\begin{figure}
\begin{center}
\begin{tikzpicture}[scale=0.40]
