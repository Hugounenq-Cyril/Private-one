...
\bluebox{}{\textbf{Fact:} For $\langle P, R \rangle = E[\ell_i^{r_i+1}]$ with $r_i=v_{\ell_i}(d_{\pi})$ from  $\pi(P,R)$ we deduce $v_{\ell_i}[\mathcal{O}:d_{\pi}]$.}

\begin{enumerate}
\item Compute the prime factorisation of $d_{\pi}=\prod_{i=1}^j\ell_i^{r_i}$
\item For each $\ell_i$:
\begin{enumerate}
...
\item Compute $\pi(P,Q)$
\medskip
\item Deduce $s_i=v_{\ell_i}[\mathcal{O}:d_{\pi}]$ 
\end{enumerate}
\item Compute $\mathcal{O}=\mathbb{Z}[\pi]/\prod_{i=1}^j\ell_i^{s_i}$
