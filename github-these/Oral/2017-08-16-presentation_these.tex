...
\begin{itemize}
 	...
 	\item Calcul du polynôme d'interpolation $L$ tel que \\
   $L (x ([u] P)) = x([u]P' )$ pour tout $u \in \mathbb{Z}/p^k \mathbb{Z}$;
...
\item Vérification si l'interpolation définit une isogénie.\hfill $\color{blue!50!gray}{\tilde{O}(r)}$;
	\begin{itemize}
	\item Test de la forme du dénominateur $d$ de $F$ et si $\mathrm{pgcd}(d,\omega_{\ell}) \neq 1$;
	\end{itemize}
...
\bluebox{$p$-adique}{
Soient $P\in E\quad$ et $\quad P'\in E'$ tels que $\langle P \rangle= E[p^k]$ et $\langle P' \rangle= E'[p^k]$
\[
...
\bluebox{$\ell$-adique}{
Soit $P,Q$, $ \langle P,Q \rangle = E[\ell^k]\quad$ et $\quad P',Q'$, $ \langle P',Q' \rangle = E'[\ell^k]$
\begin{gather*}
\begin{pmatrix}
P\\Q
\end{pmatrix}
...
\begin{figure}[h]
		...
		\end{center} 
		\end{figure}
		Durant le reste de cet exposé nous étudierons tout d'abord le cas Elkies, puis le cas Atkin.
%\begin{figure}[hbtp]
%\centering
...
\hfill\uncover<2>{$\color{blue!50!gray}{\tilde{O}(r)}$}
  \item Si $F$ définit une isogénie de degré $r$, on la retourne et on termine
   l'algorithme. \hfill\uncover<2>{$\color{blue!50!gray}{\tilde{O}(r)}$}
  \end{enumerate}
\end{enumerate}
\end{frame}
