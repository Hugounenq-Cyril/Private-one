...
\begin{frame}
\section{Towers of field extensions}
...
\bluebox{Construction d'une tour \textit{$\ell$}-adique}
{
\begin{itemize}
\item $\mathsf{F}_{1}=\mathbb{F}_q[X]/P_0(X)$ tel que $x_1$ l'image de $X$ dans $\mathsf{F}_1$ est un non-résidu $\ell$-adique, 
\item $\mathsf{F}_{i}=\mathsf{F}_{i-1}[X]/(X^{\ell}-x_{i-1})$ pour $i \geqslant 2$, $x_i$ est alors l'image de $X$ dans $\mathsf{F}_{i}$.
\end{itemize}
}
\end{frame}
...
{
Un élément de $F_k$ est dit sous forme univariée si il est écrit à l'aide d'un polynôme $G \in \mathbb{F}_q[X]$ de degré inférieur à $d_1\ell^{k-1}$ évalué en l'élément primitif $x_k$ de $F_k$. Avec $d_1=[\mathsf{F}_1:\mathbb{F}_q]$.
}

\begin{exe}
Soit $\mathbb{F}_q, \mathsf{F}_1, \mathsf{F}_2, \cdots, \mathsf{F}_k$ une tour d'extensions $5$-adiques telle que $\mathbb{F}_q \cong \mathsf{F}_1$ (id est $d_1=1$), soit $\alpha \in \mathsf{F}_4$, $x_4$ un élément primitif de $F_4$ alors une représentation univariée d'un élément $\beta \in \mathsf{F}_4$ est:
\begin{equation*}
 \beta_0 + \beta_1 x_4+ \cdots +\beta_{5k}x_4^{5k} + \beta_{5k+1}x_4^{5k+1}+\beta_{5k+2}x_4^{5k+2}+ \beta_{5k+3}x_4^{5k+3}+ \cdots + \beta_{124}x_4^{124}
\end{equation*}
\end{exe}
\end{frame}
