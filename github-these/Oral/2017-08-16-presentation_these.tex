...
\bluebox{Polynômes de $\ell$-division}
...
$p$. Soit les conditions suivantes équivalentes sont vérifiées:
\begin{enumerate}
...
\end{enumerate}
$E$ est alors dite supersingulière.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Algorithme de Couveignes[96]}
\begin{frame}
...
\item Interpolation de $\phi$, en supposant que l'on a la correspondance $P\mapsto P'$ pour tout $u \in \mathbb{Z}/p^k\mathbb{Z}$;
\item Vérification si l'interpolation définit une isogénie.\\
    Si ce n'en est pas une, on remplace $P'$ par un multiple $[a]P'$ et on recommence à nouveau.
\end{itemize}
...
\item Calcul des générateurs $P$ de
  $E[p^k]$ et $P'$ de $E'[p^k]$; \hfill $\color{blue!50!gray}{O()}$
\item Calcul de $T=\prod(X-x([u]P))$ avec $1\le u \le \frac{p^k-1}{2}$; \hfill $\color{blue!50!gray}{O()}$
...
%\hfill $\tilde{O}(r)$
	...
	$(r, r-1)$  à l'aide d'un algorithme de reconstruction rationnelle; 
...
\begin{enumerate}
\item  \textbf<2>{descendante} si $[\mathcal{O} : \mathcal{O}']=\ell$,
\item \textbf<3>{ascendante} si $[\mathcal{O}':\mathcal{O}]=\ell$,
\item \textbf<4>{horizontale} si \\
 $\mathcal{O}=\mathcal{O}'$.
\end{enumerate}
\end{lem}
%\begin{defi}
...
\coordinate (AB) at (0,-7.00);
\coordinate (B) at (0,-3.75);
\coordinate (C) at (0,0);
\draw (AB) node[above,left,text width=1.7cm]{
$
...
\section{Algorithme de Couveignes[96] $\ell$-adique}
...
\item Calcul de $\langle \red P , \blu Q \rangle = \color{red}{E[\ell^k]_{{\lambda}}} \color{black} \oplus \color{blue}{E[\ell^k]_{{\mu}}} $\\
 et $\langle \color{red} P' \color{black}, \color{blue} Q'\color{black} \rangle = \color{red}{E'[\ell^k]_{{\lambda}}} \color{black} \oplus \color{blue}{E'[\ell^k]_{{\mu}}} $ \hfill\only<2>{$\color{blue!50!gray}{\tilde{O}(r)}$}
...
\hfill\only<2>{$\color{blue!50!gray}{O(r)}$}
    \begin{enumerate}
  \item Calcul du polynôme d'interpolation 
    $L$  qui envoie\\
...
\medskip
\item Calcul de $\langle \red P , \blu Q \rangle = \color{red}{E[\ell^k]_{{\lambda}}} \color{black} \oplus \color{blue}{E[\ell^k]_{{\mu}}} $\\
 et $\langle \color{red} P' \color{black}, \color{blue} Q'\color{black} \rangle = \color{red}{E'[\ell^k]_{{\lambda}}} \color{black} \oplus \color{blue}{E'[\ell^k]_{{\mu}}} $ générateurs de $\ell^k$-isogenies horizontales de directions $\color{red} \lambda \color{black}, \color{blue} \mu$. \hfill\only<2>{$\color{blue!50!gray}{\tilde{O}(r)}$}  
...
%   l'algorithme.
   ...
   $L$ qui envoie $\color{red}P\mapsto [a]P'$ et $\color{blue}Q\mapsto [b]Q'$; \hfill\only<2>{$\color{blue!50!gray}{\tilde{O}(r \ell^{O(1)})}$}
  \end{enumerate}
\end{enumerate}
\end{frame}
...
\begin{frame}
\frametitle{Titre abandonnée}
\begin{nota}
La notation $\pi(P,Q)$ désigne l'action du Frobenius dans une base $\langle P,Q \rangle = E[\ell^k]$
\end{nota}

\textbf{Exemple :}Pour deux points $P_0$ et $Q_0$ générateurs de $\ell^k$-isogénies horizontales 
\[
\pi(P_0,Q_0)= \left( \begin{smallmatrix} \color{red}{\lambda} & \color{black} 0 \\
0 & \color{blue} \mu \end{smallmatrix} \right)
\]

\begin{prop}
Soit $\ell$ un nombre premier de Atkin soit $P,Q,R$ trois points de $E$ située 
sur un volcan de hauteur $0$ tels que $\langle P,Q \rangle = \langle P,R 
\rangle$ alors $Q=R \Leftrightarrow \pi(P,Q)=\pi(P,R)$. 
\end{prop}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
...
\bluebox{}{\textbf{Fait:} $\pi(P,R)=\pi(P,Q)$ ssi $R=Q$ \\
...
\medskip
\item Pour tout point $S'$ d'ordre $\ell^k$ de $E'$: \hfill\only<2>{$\color{blue!50!gray}{O(r)}$}
\begin{enumerate}
\medskip
  \item Calcul de $R'$ tel que $\pi(S',R')=\pi(P,Q)$ \hfill\only<2>{$\color{blue!50!gray}{O(r \ell^{O(1)})}$}
  \medskip
...
\begin{enumerate}
\medskip
  \item Calcul de $R'$ tel que $\pi(S',R')=\pi(P,Q)$ \hfill {$\color{blue!50!gray}{O(r \ell^{O(1)})}$}
  \medskip
  \only<1>{
...
\begin{thm}
Soient deux courbes elliptiques $E,E'$, un entier $r$ tel que $E$ et $E'$ soient 
$r$-isogènes, alors peut calculer la $r$-isogénie en temps quasiquadratique en 
$r$ et polynomial en un paramètre $\ell$ borné par $O(\log(q))$.
\end{thm}
...
$\ell$-adiques de $\mathbb{F}_q$ est la suite d'extensions: $\mathbb{F}_q, 
...
L'algorithme (cas Elkies) a été implanté sur SageMath v7.1 seulement pour $\ell=2$, le code ($XXXXX$ lignes) est disponible sur GitHub: \url{https://github.com/Hugounenq-Cyril/Two_curves_on_a_volcano}
\begin{figure}[hbtp]
\centering
...
\item Estimer le coût de calcul de l'anneau des endomorphismes en modifiant l'algorithme de Kohel  avec l'étude de l'action de l'endomorphisme du Frobenius et l'implanter.
