...
\item Vérification si la fraction rationnelle $F$ candidate définit une isogénie.\hfill $\color{blue!50!gray}{\tilde{O}(r)}$
	\begin{itemize}
	\item Test du degré du dénominateur $d$ de $F$;
	\item Test de la décomposition de $d$ en facteurs carré ;
	\item Test si $\omega_{\ell}^2= 0 \bmod d$;
	\end{itemize}
...
\begin{frame}
\frametitle{Décomposition horizontale pour le cas $h=0$.}
...
\smallskip
En pratique le calcul d'une décomposition horizontale est fait à l'aide de: 

\bluebox{Diagonalisation}{
\begin{algorithmic}[1]
\REQUIRE $P_0,Q_0$ une base quelconque de $E[\ell^k]$.  
...
\]
\end{prop}
Le coût de la détermination de $M_{\pi}(P,Q)$ est de $\tilde{O}(\sqrt{r}\ell^{O(1)})$
\begin{cor}
...
Le coût de la recherche d'un tel point $R'$ est de $O(\tilde{O}(\sqrt{r} \ell^{O(1)}))$
\end{frame}
...
\end{thm}
\textbf{Idée de preuve:} Les entiers $\ell$ pour lesquels notre algorithme n'a 
pas la complexité espérée sont des diviseurs de $d_{\pi}$ borné par $O(q)$.
\end{frame}
...
tour d'extensions $\ell$-adiques de 
$\mathbb{F}_q$ est une suite d'extensions: $\mathbb{F}_q, \mathsf{F}_{1}, 
