...
\end{matrix}\right ), \quad \text{with }a,d \text{ primes with }\ell, \]  %with $a,d $ primes with $\ell$,
$\ell^{h} |a-d$ and $e = 0$ iff~$E$ lies on the crater. Moreover either $b$ and $c$ are primes with $\ell$, either one of them is prime and the other has its $\ell$-adic valuation which belongs to $[2,2e]$.
