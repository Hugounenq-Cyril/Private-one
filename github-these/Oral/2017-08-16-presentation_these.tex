...
\begin{frame}
\frametitle{An $\ell$-adic algorithm in the Atkin case}
\begin{figure}
\begin{center}
\begin{tikzpicture}[scale=0.40]
...
Dire quelque part que l'on parle de courbes elliptiques ordinaires !!!

Motivation calcul d'anneau d'endomorphisme

Parler de Kohel

Parler de Bisson Sutherland

Faire une slide qui parle de calcul d'anneau d'endomorphisme... et rattacher tout cela à ce que l'on a fait auparavant...

Dire que $\mathcal{O}_K=\mathbb{Z} \left[ \frac{\pi-a}{g} \right] $
...
\begin{enumerate}
\item Compute the prime factorisation of $d_{\pi}$ and deduce from it the conductor  $g=\prod_{i=1}^j\ell_i^{t_i}$ such that $d_{\pi}=g^2d_K$ and $a$ such that: $\mathcal{O}_K=\mathbb{Z} \left[ \frac{\pi-a}{g} \right] $  
\item For each $\ell_i$:
\begin{enumerate}
\item Compute $s_i= v_{\ell_i}([\mathcal{O}:\mathbb{Z}[\pi]]) $
\end{enumerate}
\item Compute $\mathcal{O}=\mathbb{Z}+\frac{\pi-a}{\prod_{i=1}^j\ell_i^{s_i}} \mathbb{Z}$
\end{enumerate}
...
\begin{enumerate}
\item Compute from $d_{\pi}$ the prime factorisation of the conductor $g=\prod_{i=1}^j\ell_i^{t_i}$ of $\mathbb{Z}[\pi]$
\item For each $\ell_i$:
\begin{enumerate}
