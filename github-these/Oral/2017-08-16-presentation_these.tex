...
\item Vérification si $\phi$ est une isogénie.\\
...
\frametitle{Limitations du cas Elkies}
 \bluebox{Trouver un premier $\ell$ de Elkies}{
  \begin{itemize}
  \item La complexité de l'algorithme dépend de polynomialement du paramètre $\ell$.
...
\bluebox{}{\textbf{Fait:} $\pi(P,R)=\pi(P,R')$ ssi $[\ell^h]R=[\ell^h]R'$ avec $\langle P,R \rangle = \langle P,R' \rangle = E[\ell^{h+i}]$ avec $i>0$.}

\begin{enumerate}
\item Choix du plus petit entier $k$ tel que $\ell^{2k}>4r$ avec $h<k$.
\medskip
\item Calcul de $\langle P , Q \rangle = E[\ell^k]$ et $\langle P' , Q' \rangle = E'[\ell^k]$
%\item Deduce $\to$ $\langle{\color{red}P},{\color{blue}Q}\rangle=E[\ell^k]$ and $\langle{\color{red}P'},{\color{blue}Q'}\rangle=E'[\ell^k]$;
\medskip
