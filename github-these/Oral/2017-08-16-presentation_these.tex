...
\begin{prop}
Soit $E$ une courbe elliptique ordinaire, alors on a $\mathbb{Z} \left[ \pi \right] \subset \mathcal{O} \subset \mathcal{O}_K$ avec $\mathcal{O}_K$ l'anneau des entiers de $K$ le corps quadratique imaginaire tel que $\mathcal{O} \subset K$
\end{prop}
...
\begin{enumerate}
\item Compute the prime factorization of $d_{\pi}$ and deduce from it the conductor  $g=\prod_{i=1}^j\ell_i^{t_i}$ such that $d_{\pi}=g^2d_K$ 
\item For each $\ell_i$:
\begin{enumerate}
\item Compute $s_i= v_{\ell_i}([\mathcal{O}:\mathbb{Z}[\pi]]) $
\end{enumerate}
\item Return $\mathcal{O}=\mathbb{Z}+\frac{\pi-a}{\prod_{i=1}^j\ell_i^{s_i}} \mathbb{Z}$
\end{enumerate}
