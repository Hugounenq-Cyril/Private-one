...
\]
Signifie que $\pi(P_0)=[\color{red}{\lambda} \color{black}]P_0$ et $\pi(Q_0)=[\color{blue} \mu \color{black}]Q_0$ générateurs de $\ell^k$-isogénies horizontales\\ 
\smallskip
Pour le calcul d'une décomposition horizontale en pratique on effectue l'opération suivante: 
...
\item[$\rightarrow$] seulement $\langle [\ell^h]P,[\ell^h]Q \rangle$ s les $k-h$ premières $\ell$-isogénies peuvent être certifiées horizontales (reformuler). 
\end{itemize}
\begin{columns}
\begin{column}{5cm}
...
$\ker(\pi- \blu \mu\; | \; E[\ell^3])$ est un groupe cyclique de taille $\ell^3$.
\end{column}
\begin{column}{5.5cm}
...
\textbf{Problème:} pour $h =2$
alors $\ker(\pi- \blu \mu  \;| \; E[\ell^3])
\simeq (\mathbb Z/\ell^3) \!\times\! (\mathbb Z/\ell^2)$ n'est pas cyclique,
et contient $\ell^2$ sous-groupes cycliques d'ordre $\ell^3$.
...
\begin{cor}
Soit $E$ située sur un volcan de hauteur $0$, $\ell$ un nombre premier de 
Atkin, $\langle P,Q \rangle = E[\ell^k]$, $R' \in E'[\ell^k]$ et $\phi$ une 
isogénie de degré $r$, alors $R'=\phi(Q) \Leftrightarrow 
M_{\pi}(P,Q)=M_{\pi}(\phi(P),R')$.
\end{cor}
\end{frame}
