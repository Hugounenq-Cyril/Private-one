...
\section{Notations} \todo{A eventuellement enrichir}
La multiplication scalaire d'un point $P$ par $x$ se note $[x]P$. Les isogénies horizontales sont notées $\psi$, ascendantes $\widehat{\varphi}$, descendantes $\varphi$, l'isogénie de degré $r$ que l'on veut calculer est notée $\phi$. On note $\mathsf{F}_i$ la tour d'extension sur laquelle on travaille.
...
\todo{enlever les pmod et mettre des bmod ou l'inverse...}
...
\begin{prop}
L'algorithme \ref{alg:bas:tri} calcule une base triangulaire de $E[\ell^k]$ située au niveau $h-e<h$ dans le volcan de $\ell$-isogénies avec une complexité espérée de $O(\mathsf{R}(k-\beta+e) + \ell^2\mathsf{M}(\ell^{k-\beta+e}) + \ell\mathsf{M}(\ell^2)\log(\ell)\log(\ell q))$ opérations sur $\mathbb{F}_q$ lorsque $E$ est située sur un volcan régulier et de $O(\mathsf{R}(k-\beta) + \ell^2\mathsf{M}(\ell^{k-\beta}) + \ell\mathsf{M}(\ell^2)\log(\ell)\log(\ell q))$ opérations sur $\mathbb{F}_q$ lorsque $E$ est au dessus du niveau de stabilité.
\end{prop}

\begin{proof}
L'équation à l'étape \ref{alg:trigonal:solve1} lorsque elle concerne une courbe située au niveau $h-e$ peut être simplifiée par $\ell^{h-e}$ avant d'être résolue modulo $\ell^{e+1}$. Pour $i<h-e-1$ comme $a=b$ et $c=d=0$ alors le calcul est trivial.
...
opérations en utilisant l'algorithme de Cantor-Zassenhaus.
  ...
  multiplication-par-$\ell$ par un produit de deux $\ell$-isogenies.On note 
  tout d'abord que pour $E$ au niveau $h-e$ dans le volcan on a $E[\ell^k]$
   qui est définie dans $\mathsf{F}_{k-\beta+e}$ lorsque l'on est sur un volcan
  régulier et définie sur $\mathsf{F}_{k-\beta}$ lorsque $E$ est au-dessus du
  niveau de stabilité. Pour tout $P$ d'ordre $\ell^i$ défini dans $E(\mathsf{F}_{i-\beta+e})$, 
  le calcul de $\mathsf{divise}(\ell, P)$ à l'étape ~\ref{alg:point:divide} 
  coûte $O(\mathsf{R}(i-\beta+e+1))$ opérations.
  L'évaluation de~$\pi(P')$ à l'étape~\ref{alg:trigonal:frobenius} a un côut
  de~$O(\ell^{i-\beta+e}\mathsf{M}(\ell))$ par le théorème \ref{thm:frob-ell}.
  L'écriture~$\pi(P')$ comme une combinaison linéaire~$[u] P' + [v] Q'$ nécessite au
  plus~$\ell^2$ additions de points, avec un côut de~$\ell^2
  \mathsf{M}(\ell^{i-\beta+e})$.
...
\`A partir d'une base de la $\ell^k$ torsion pour une courbe $E$ située en dessous du cratère on peut se servir de celle-ci pour calculer une base diagonale de la $\ell^k$ torsion pour la courbe sommet (voir définition \ref{def:cou:som} pour un rappel) $E_s$. Afin de permettre au lecteur de se rendre compte du niveau dans le volcan auquel les points sont définis on notera $P^{(h-e+i)}$ pour signifier que le point $P$ appartient à une courbe elliptique située au niveau $h-e+i$ du volcan de $\ell$-isogénies quand cela n'est pas évident de par le contexte. \todo{A mettre dans une définition ???} Nous présentons maintennant l'algorithme qui calcule une base la $\ell^k$ torsion de $E_s$:
...
\STATE\label{alg:rem:solve1}
 ...
 $y\ell^{k-1}(\mu - \lambda) + c \ell^{k-1}=0$; \textbf{end if}.
\STATE
...
\`A l'étape~\ref{alg:rem:divide} dans le pire des cas où l'on serait sur un volcan régulier on aurait les points $P$ et $Q$ situés au niveau $h-e+i$ définis dans $E(\mathsf{F}_{k-\beta+e-i})$ 
...
Maintenant on présente l'algorithme~\ref{alg:ulti:fro} qui prend en entrée une courbe située à un niveau $h-e$ dans le volcan de $\ell$-isogénies et qui retourne une base diagonale de sa courbe sommet.
...
tels que $\pi(P_k)= [\lambda] P_k$, $ \pi(Q_k)= [\mu] Q_k$.
% \STATE Calcul $(P_1,Q_1)$, a basis of $E[\ell]$
\STATE{$\lambda \leftarrow 0$; $\mu \leftarrow 0$;$i \leftarrow 0$; $\mathsf{trigonal} \leftarrow \mathsf{Vrai} $; $P', Q' \leftarrow $ tels que~$\langle P',Q' \rangle =  E[\ell]$.}
\STATE calcul $\pi|(P',Q')=\left( \begin{smallmatrix}
\lambda + a\ell^{i} & b\ell^{i}\\
c\ell^{i} & \mu + d\ell^{i} \end{smallmatrix} \right) \pmod {\ell^{i+1}}.$
\WHILE {$\pi|(P',Q')= (\lambda + a\ell^i) \cdot I_2$} \label{alg:ult:scalaire:debut}
...
\WHILE{ $\mathsf{trigonal}$} \label{alg:ult:trigonal:debut}
\STATE \textbf{if} $c = 0$ \textbf{then} $y \leftarrow 0$;
\STATE\label{alg:ult:trigonal:solve1} \textbf{else}  résoudre equation 
...
\STATE\label{alg:ult:rem:divide}
  $P' \leftarrow \mathsf{divise}(\ell, \widehat{\varphi}(P_{h-e+i+1}^{(h-e+i)}))$; $Q' \leftarrow  \widehat{\varphi}(Q_{h-e+i+1}^{(h-e+i)})$.
\STATE\label{alg:ult:rem:frobenius}
  Calcul $\pi|(P',Q')=\left( \begin{smallmatrix}
\lambda  & \ell \delta \\
c\ell^{h-e+i} & \mu  \end{smallmatrix} \right) \pmod {\ell^{h-e+i+1}}.$
\STATE 
  \textbf{if} $c=0$ \textbf{then} $y \leftarrow 0 $; 
\STATE\label{alg:ult:rem:solve1}
  \textbf{else} résoudre équation
 $y\ell^{h-e+i}(\mu - \lambda) + c \ell^{h-e+i}$; \textbf{end if}.
\STATE
...
c\ell^{h-e+i} & \mu + d\ell^{h-e+i} \end{smallmatrix} \right) \pmod {\ell^{h-e+i+1}}.$  
...
\WHILE{$i<k $} \label{alg:ult:diagonal:debut}
\STATE
  $P' \leftarrow \mathsf{divise}(\ell, P_{i})$; $Q' \leftarrow \mathsf{divise} (\ell, Q_{i})$.
\STATE calcul $\pi|(P',Q')=\left( \begin{smallmatrix}
\lambda + a\ell^{i} & b\ell^{i} \\
...
\begin{lem}
\label{lem:alg:rat}
Dans l'algorithme~\ref{alg:ulti:fro} avec comme entrée une courbe située sur un volcan de $\ell$-isogénie régulier les points utilisés et calculés entre les étapes \ref{alg:ult:trigonal:debut} et \ref{alg:ult:trigonal:fin} sont définis dans $\mathsf{F}_{h+1-\beta}$.
\end{lem}
...
$\ell^{h-e+i+1}$ et définis dans $\mathsf{F}_{h-(e-i)+1-\beta+(e-i)}=
\mathsf{F}_{h+1-\beta}$ par la proposition \ref{pro:niv:str}. En particulier les 
 ...
 $\mathsf{divise}(\ell, P')$ et $\pi(P')=[u]P'+[v]Q'$ sont donc faites dans 
 $ \mathsf{F}_{h+1-\beta}$. De même pour les points $Q'$ car à l'étape 
...
\begin{enumerate}
\item 
 $O(R(h-\beta)+\ell M(\ell^{h-\beta})+\ell\mathsf{M}(\ell^2)\log(\ell)\log(\ell q)+e(R(h+1-\beta)+\ell^{h-\beta}M(\ell)+\ell^2 M(\ell^{h+1-\beta}))+\mathsf{R}(k-\beta) + \ell^2\mathsf{M}(\ell^{k-\beta}))$ opérations sur $\mathbb{F}_q$ pour une courbe en entrée située sur un volcan de $\ell$-isogénies régulier,  
\item  $O(\mathsf{R}(k-\beta) + \ell^2\mathsf{M}(\ell^{k-\beta}) + \ell\mathsf{M}(\ell^2)\log(\ell)\log(\ell q))$ opérations sur $\mathbb{F}_q$ pour une courbe en entrée située au dessus du niveau de stabilité.
...
que l'on travaille avec des points d'ordre $\ell^{h-e+1}$, ainsi cette première
 ...
 qui sont définis dans $\mathsf{F}_{i+1+e-\beta}$ par la proposition 
 \ref{pro:niv:str}, ainsi le coût de calcul d'un tel point $P$ à l'étape 
 \ref{alg:ult:identical:divide} est de $R(i+1+e-\beta)$. Le coût de calcul de 
 $\pi(P)$ est de $O(\ell^{i+e-\beta}M(\ell))$. L'écriture de $\pi(P)=[u]P$, on 
 se limite à ce cas là car on est dans le cas où la matrice représentant 
 l'action du Frobenius est scalaire, a un coût de $O(\ell M(\ell^{i+1+e-\beta}))$. 
 ...
 total de $O(R(h-\beta)+\ell M(\ell^{h-\beta}))$ auquel il faut ajouter le coût
  de calcul de la base de $\ell$-torsion: 
  $O(\ell\mathsf{M}(\ell^2)\log(\ell)\log(\ell q))$.

Ensuite on passe à la partie entre les étapes \ref{alg:ult:trigonal:debut} et \ref{alg:ult:trigonal:fin} où l'on calcule à chaque boucle une base de la $\ell^{h-e+i}$ torsion dans laquelle l'action du Frobenius est triangulaire, et l'on remonte d'un niveau dans le volcan et l'on trigonalise à nouveau la $\ell^{h-e+i}$ torsion. Cette étape est répétée du niveau $h-e$ où l'on exprime l'action du Frobenius dans une base de la $\ell^{h-e+1}$ torsion au niveau $h$ où l'on exprime l'action du Frobenius dans une base de la $\ell^{h+1}$ torsion. Par le lemme~\ref{lem:alg:rat} les points avec lesquels on travaille sont toujours définis dans le même corps $\mathsf{F}_{h+1-\beta}$.
Ainsi le coût des étapes \ref{alg:ult:rem:divide} et \ref{alg:ult:trigonal:divide} est de $R(h+1-\beta)$, le coût de calcul de $\pi(P)$ est de $O(\ell^{h-\beta}M(\ell))$. L'écriture de $\pi(P)=[u]P+[v]Q$ aux étapes \ref{alg:ult:rem:frobenius} \ref{alg:ult:trigonal:frob} a un coût de $O(\ell^2 M(\ell^{h+1-\beta}))$. Lors de la boucle $h-e+i$, les étapes \ref{alg:ult:trigonal:solve1} et \ref{alg:ult:rem:solve1} sont résolues à l'aide d'une recherche exhaustive et ont un coût négligeable par rapport aux autres opérations. 
 Le coût étant le même pour chaque boucle on a un coût total pour cette boucle de trigonalisation de $O(e(R(h+1-\beta)+\ell^{h-\beta}M(\ell)+\ell^2 M(\ell^{h+1-\beta})))$.

Pour la dernière boucle entre les étapes \ref{alg:ult:diagonal:debut} et \ref{alg:ult:diagonal:fin} par une analyse similaire à celle faite dans la preuve de la proposition \ref{pro:alg:diagonal}, cette boucle a un coût de $O(\mathsf{R}(k-\beta) + \ell^2\mathsf{M}(\ell^{k-\beta}) )$ opérations sur $\mathbb{F}_q$.

\item 
Pour une courbe située au dessus du niveau de stabilité, le corps de définition des points ne dépend que de  leur ordre et non pas de la courbe sur laquelle ils sont situés dans ces conditions, on peut faire une analyse de complexité de cet algorithme similaire à celle donnée dans la preuve de la proposition \ref{pro:alg:diagonal} pour obtenir une complexité totale de $O(\mathsf{R}(k-\beta) + \ell^2\mathsf{M}(\ell^{k-\beta}) + \ell\mathsf{M}(\ell^2)\log(\ell)\log(\ell q))$ opérations sur $\mathbb{F}_q$.
\end{enumerate}
\end{proof}
...
\begin{rem}
Pour savoir le coût des algorithmes dépendant de la profondeur de la courbe dans le volcan de $\ell$-iogénies régulier tels que les algorithmes~\ref{alg:bas:tri}, \ref{alg:rem:tri} et \ref{alg:ulti:fro}  appliqué à une courbe sur un volcan non-régulier il suffit uniquement de prendre en compte le nombre de niveaux en dessous du premier niveau de stabilité de la courbe. Si l'on utilise un algorithme qui monte de niveaux dans le volcan tels que les algorithmes~\ref{alg:rem:tri} et \ref{alg:ulti:fro} il faut alors faire une distinction des cas et adapter la complexité lorsque la courbe avec laquelle on travaille se situe au-dessus du premier niveau de stabilité.
\end{rem}
...
%\begin{block}
%Problème du calcul explicite d'isogénie} \label{prob:isogeny-problem}
%Soient deux $j$-invariants $j$ et $j'$ et un entier positif $r$. Si les $j$-invariants sont $r$-isogènes, c'est à dire il existe deux courbes elliptiques $E_a$ et $E_b$ $r$-isogènes telles que $j(E_a)=j$ et $j(E_b)=j'$, calculer les courbes $E$, $E'$ telles que $j(E)=i$ et $j(E')=j'$, et la fonction rationnelle qui définit la $r$-isogénie $\phi:E \rightarrow E'$.
%\end{block}


\section{Elkies}
Cette section s'inspire de travaux effectués en collaboration avec Luca De Feo, J\'erome Plut et \'Eric Schost qui ont été publié \cite{Defeo_Plut_Schost_2016}.

Soient $E$, $E'$ deux courbes elliptiques de $j$-invariants $j$ et $j'$ reliées par une isogénie $\phi$ de degré $r$, alors pour deux bases diagonales $(P,Q),(P',Q')$ de $E$ de $E[\ell^k]$ et $E'[\ell^k]$
on pourrait croire que cela est suffisant cependant comme l'action du Frobenius sur la $\ell^h$ torsion est égal à l'action d'une matrice scalaire alors
 alors pour deux bases $(\color{red}P_{\lambda} \color{black},\color{blue}Q_{\mu}\color{black}),(\color{red}P'_{\lambda}\color{black},\color{blue}Q'_{\mu}\color{black})$ (partiellement) horizontales de $E[\ell^k]$ et $E'[\ell^k]$ on a l'action de $\phi$ sur la base $(\color{red}P_{\lambda} \color{black},\color{blue}Q_{\mu}\color{black})$ qui peut être représentée comme une matrice diagonale car par la proposition~\ref{pro:par:iso} et le corollaire~\ref{cor:par:hor} les points de même direction sont mis en correspondance:
\[
\left(
\begin{matrix}
...
polynôme d'interpolation $A$ tel que 
\[
A(x([u]\color{red}P_{\lambda}
...
\]
on se sert ensuite du polynôme $A$ pour calculer la fraction rationnelle $F$ telle que:
\[
F=A \bmod T
\]
...
On doit également avoir $k \geqslant h +1$ pour pouvoir calculer des points 
(partiellement) horizontaux sur le volcan de $\ell$-isogénies par la 
proposition~\ref{pro:dia:hor}.

On peut donc maintenant énoncer l'algorithme complet pour calculer une 
$r$-isogénie entre deux courbes elliptiques $E$, $E'$ définis sur des volcans 
de $\ell$-isogénies à cratère cyclique: 

\begin{algorithm}
\caption{\label{alg:cou:ell-adique} Algorithme de Couveignes $\ell$-adique.}
\begin{algorithmic}[1]
\REQUIRE $E,E'$: deux courbes elliptiques $r$-isogènes ordinaires situées au niveau $e$ d'un volcan de $\ell$-isogénies avec cratère cyclique.
...
Calcul de $(P,Q),(P',Q')$ bases diagonales de $E_s[\ell^k],E'_s[\ell^k]$ à l'aide l'algorithme \ref{alg:ulti:fro};
\STATE Calcul de $(\color{red}P_{\lambda}\color{black},\color{blue}Q_{\mu}\color{black}),(\color{red}P'_{\lambda}\color{black},\color{blue}Q_{\mu}'\color{black})$ bases horizontales de $E_s[\ell^k],E'_s[\ell^k]$ à l'aide l'algorithme \ref{alg:hor:poi};
\IF{$(E_s,E'_s) \neq (E_s,E'_s)$}
...
\STATE Réordonancement de la liste $L'$ en $L'_{a,b}$ afin de respecter la correspondance induite par le choix de $M$;
\STATE Calcul des des polynômes $A_{a,b}$ et $T$ par la méthode décrite dans la proposition~\ref{prop:interpol}section (\ref{sec:interpolation});
\STATE Calcul de la fraction rationnelle $F=A_{a,b} \bmod T$ à l'aide d'une interpolation de Cauchy;
\IF{$\mathsf{Test}(F)$}
\RETURN $F$
\ENDIF
\ENDFOR 
...
\begin{prop}
  ...
  Assuming that $\ell^h<\sqrt{r}$, the algorithm~\ref{alg:cou:ell-adique} above
  computes an isogeny ${\phi:E \rightarrow E'}$ in expected time
...
Par définition de ~$k$, on a ~$\ell^{2k+1} \in O(r\ell^2)$.
  Par la proposition~\ref{pro:clas:fro}, il existe $\beta<h$ tel que
  $E[\ell^k]$ est inclus dans $E(F_n)$ avec $n=k-\beta$.On construit donc une
  tour d'extension $\ell$-adiques $\mathsf{F}_0\subset\cdots\subset 
  \mathsf{F}_n$, et l'on effectue les précalculs nécessaires au 
  ...
  l'algorithme~\ref{alg:ulti:fro} qui a un coût de $O()$ par la proposition~\ref{pro:alg:ulti}.

  Pour $h$ inférieur à $k-1$, l'étape ~\ref{alg:hor:poi} coûte en moyenne 
  $O(k\mathsf{R}(k-\beta) + k\ell^2\mathsf{M}(\ell\sqrt{r}) + \ell\mathsf{M}(\ell^2)\log(\ell)\log(\ell q))$ d'après la 
  proposition~\ref{pro:alg:hor} et le théorème~\ref{th:horizontal}.
  En utilisant le  résultat  donné par ~\cite[Chapter~14.5]{vzGJG03} pour le coût de $R$, 
  on a alors un coût borné par 
  $O(\mathsf{M}(r\ell^3)\log(r)\log(\ell)\log(\ell q))$.
  
  Il faut éventuellement rajouter le coût de calcul des points partiellement 
  horizontaux si les courbes en entrée ne se situent pas sur le cratère celui-ci est de
  $O()$ 
  par la proposition~\ref{pro:par:hor}.
  ...
  \color{blue}Q_{\mu}\color{black})$ en multipliant les deux points définis 
  ...
  $O(k\mathsf{M}(\ell^{2k})\log(\ell^k)) \subset O(\mathsf{M}(r\ell^2)\log(r)^2)$
  ...
  et~$A_{a,b}$ à un coût de~$O(\mathsf{M}(r \ell^4)\log(r)\log(\ell))$.  Le coût
  de calcul de $F_{a,b}$, et l'identification de l'isogénie, est dominé par
  celui du calcul de~$L_{a,b}$~\cite[§3.3]{defeo10}.  Enfin, 
  approximately ${\ell^{2k}=O(r \ell^2)}$~matrices candidates doivent être testées
   avant de trouver l'isogénie.
\end{proof}


 à l'aide d'une 
interpolation de Cauchy la fraction rationnelle représentant l'action de 
l'isogénie. La fraction rationnelle représentant une isogénie de degré $r$ 
étant de de degré $(r,r-1)$ alors elle est entièrement déterminée par $2r$ 
coefficients. En particulier on teste si la matrice $M$ est bonne en regardant si 
le résultat de l'interpolation de Cauchy nous donne une fraction rationnelle de
 degré $(r,r-1)$ ensuite on regarde si le dénominateur est un carré, enfin on 
peut éventuellement regarder si le dénominateur est un facteur du polynôme de 
$\ell$-divisions. Comme on calcule le polynôme d'interpolation à partir de
 couples de points (et plus particulièrement d'abscisses) 
de $E$ et $E'$ d'ordre $\ell^k$ et qu'il y a $\ell^{2k-2}(\ell+1)$ tels 
points alors on a besoin de choisir $k$ tel que le nombre d'abscisses de points
 d'ordre $\ell^k$: $\frac{\ell^{2k-2}(\ell+1)}{2}$soit strictement plus grand 
que $2r$ pour avoir suffisament d'information. On doit également avoir 
$k \geqslant h +1$ pour pouvoir calculer des points (partiellement) horizontaux
 sur le volcan de $\ell$-isogénies par la proposition~\ref{pro:dia:hor}.
 
 , on teste donc si l'on obtient bien une $r$-isogénie. En effet on cherche à calculer une fraction de degré $(r,r-1)$ à l'aide des abscisses des points d'ordre $\ell^k$ (voir discussion sur les points partiellement horizontaux dans la section \ref{sub:ss-gr}) il y en a $\ell^{2k-2}(\ell+1)$ or la fracion rationnelle est déterminée par $2r$ coefficients on a donc besoin d'avoir $k$ tel que $k\geqslant h+1$ et $\ell^{2k-2}(\ell+1)> 2r$.
