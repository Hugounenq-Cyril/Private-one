...
Soit $P \in E(\mathbb{L}_1)$ un point d'ordre $p$ de la courbe elliptique $E$ définie sur $\mathbb{F}_q$ telle que $E(\mathbb{F}_q)[p]=\{O_E\}$, soit $\mathbb{L}_1$ tel que $[\mathbb{L}_1:\mathbb{F}_q]=d_0$ avec $d_0 \mid p-1$. Alors les orbites des points d'ordre $p$ sous l'action de $\mathsf{Gal}(\mathbb{L}_1:\mathbb{F}_q)$ est de taille de $d_0 $ et il y a $f$ orbites telles que $f \times d_0  = (p-1)/2$.
\end{prop}


\begin{cor}
Soit $k \geqslant 1$, $P \in E(\mathbb{L}_{k})$ un point d'ordre $p^k$ de la courbe elliptique $E$ définie sur $\mathbb{F}_q$ telle que $E(\mathbb{F}_q)[p]=\{O_E\}$, soit $\mathbb{L}_1$ tel que $[\mathbb{L}_1:\mathbb{F}_q]=d_0$ avec $d_0 \mid p-1$. Alors les orbites des points d'ordre $p^k$ sous l'action de $\mathsf{Gal}(\mathbb{L}_{k}:\mathbb{F}_q)$ est de taille de $d_0 p^{k-v_p(E(\mathbb{L}_1))}$ et il y a $f$ orbites telles que $f \times d_0 p^{k-v_p(E(\mathbb{L}_1))} = p^{k-1}(p-1)/2$.
\end{cor}
...
\'A l'aide d'une représentation de ce calcul sur un arbre binaire, on voit sur la figure \ref{fig:orb:T} que le calcul de $T_0$  n'est effectué qu'en travaillant avec une branche de celui-ci à l'aide de la méthode décrite ci-dessus.
