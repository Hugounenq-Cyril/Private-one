...
\begin{prop}
L'algorithme~\ref{alg:atk:bas:mat} a une complexité de $O(\ell^4(\mathsf{M}(\ell^{i+1})+(h-e)\mathsf{M}(\ell)))$ opérations sur $\mathbb{F}_q$.
\end{prop}

\begin{proof}
Par la proposition~\ref{pro:clas:fro:atk} les points $P_j,R_j$ sont définis
dans $\mathsf{F}_{i-j}$ lorsque $i-j>1$ et définis dans $\mathsf{F}_1$ sinon.
L'étape~\ref{alg:atk:bas:niv} est fait en tant que pré-calcul, ainsi on calcule
successivement des multiplications par $\ell$ de points définis dans 
$\mathsf{F}_{i-j}$ à $\mathsf{F}_{1}$ en décomposant la multiplication par 
$\ell$ par deux isogénies de degré $\ell$ qui coûte chacune 
$O(\mathsf{M}(\ell^{\max(i-j+1,1)}))$ opérations sur $\mathbb{F}_q$. On a donc 
un coût total pour l'étape~\ref{alg:atk:bas:niv} de 
$O(\mathsf{M}(\ell^{i+1})+(h-e)\mathsf{M}(\ell))$.
\`A l'étape~\ref{alg:atk:coef:int} le calcul de $R_j$ peut être actualisé à 
chaque itération de la boucle pour à l'aide 
de $\ell^2$ additions de points définis dans $\mathsf{F}_{i-j}$ lorsque $i-j>1$
ou $\mathsf{F}_1$ sinon. Ainsi le calcul de $R_j$ a un coût total de 
$O(\ell^{2}(\mathsf{M}(\ell^{i+1})+(h-e)\mathsf{M}(\ell)))$. Le calcul du 
Frobenius sur un point $R_j$ fixé défini dans $\mathsf{F}_{i-j}$ lorsque $i-j>1$, 
$\mathsf{F}_1$ sinon a un coût total de $O(\ell^{\max(i-j,1)}\mathsf{M}(\ell))$ par le 
théorème~\ref{thm:frob-ell}. Le coût total du Frobenius pour cet algorithme est
donc de $O(\ell^2\mathsf{M}(\ell)(\ell^{i+1}+(h-e)\ell))$ opérations sur 
$\mathbb{F}_q$.
...
$\ell^{j}$ définis dans dans $\mathsf{F}_{i-j}$ lorsque $i-j>1$, $\mathsf{F}_1$
sinon. Ce test d'égalité a donc un coût total de 
$O(\ell^4(\mathsf{M}(\ell^{i+1})+(h-e)\mathsf{M}(\ell)))$ opérations sur 
$\mathbb{F}_q$.
...
\STATE Calcul de $\pi(P,Q)$;
\FOR{$S' \in E'$ d'ordre $\ell^k$}
\STATE Calcul de $R' \in E'$ tel que $\pi(S',R')=\pi(P,Q)$ à l'aide de l'algorithme~\ref{alg:atk:bas:mat}
...
\end{prop}
\begin{proof}
Le coût de calcul des bases de la $\ell^k$-torsion se fait à partir d'une base de la $\ell$-torsion puis on inverse la multiplication l'aide de deux isogénies de degré $\ell$
\end{proof}
