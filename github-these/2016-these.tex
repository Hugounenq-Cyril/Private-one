...
\begin{lem}
\label{lem:div:cou}
Soit $P$ un point de $E$ tel que $P \in E(\mathsf{F}_{n})$, alors le coût de calcul de $E[\ell]$ est de $O(\ell \mathsf{M}(\ell^2)\log(\ell)\log(\ell q))$. Le coût de calcul de $\mathsf{divise}(\ell,P)$ est de $\mathsf{R}(n)$.
\end{lem}
...
opérations en utilisant l'algorithme de Cantor-Zassenhaus.
  ...
  multiplication-par-$\ell$ par un produit de deux $\ell$-isogenies. Ainsi on
   calcule un antécédent d'un point par la multiplication par $\ell$ en 
   calculant successivement antécédents de ce point $P$ par les isogénies de 
   degré $\ell$ qui composent la multiplication par $\ell$. Pour calculer ces 
   antécédents on a donc besoin de calculer les racines d'un polynôme de degré 
   $\ell$, ainsi le coût de $\mathsf{divise}(\ell,P)$ est $O(R(n))$ car $R(i)$ 
   est défini comme le coût de résoudre un polynôme de degré $\ell$ à 
...
\begin{prop}
L'algorithme \ref{alg:bas:tri} calcule une base triangulaire de $E[\ell^k]$ située au niveau $h-e<h$ dans un volcan de $\ell$-isogénies régulier avec une complexité espérée de $O(\mathsf{R}(k-\beta+e) + \ell^2\mathsf{M}(\ell^{k-\beta+e})+ \ell^{k-\beta+e}\mathsf{M}(\ell) + \ell\mathsf{M}(\ell^2)\log(\ell)\log(\ell q))$ opérations sur $\mathbb{F}_q$ et de $O(\mathsf{R}(k-\beta)+ \ell^{k-\beta}\mathsf{M}(\ell) + \ell^2\mathsf{M}(\ell^{k-\beta}) + \ell\mathsf{M}(\ell^2)\log(\ell)\log(\ell q))$ opérations sur $\mathbb{F}_q$ lorsque $E$ est au dessus du niveau de stabilité.
\end{prop}
...
opérations en utilisant l'algorithme de Cantor-Zassenhaus.
  ...
  tout d'abord que pour $E$ au niveau $h-e$ dans le volcan on a $E[\ell^{k-1}]$
   qui est définie dans $\mathsf{F}_{k-1-\beta+e}$ lorsque l'on est sur un volcan
  régulier et définie sur $\mathsf{F}_{k-\beta}$ lorsque $E$ est au-dessus du
 premier niveau de stabilité. Pour tou point $P$ d'ordre $\ell^i$ défini dans 
 $E(\mathsf{F}_{i-\beta+e})$, le calcul de $\mathsf{divise}(\ell, P)$ à 
...
\begin{rem}
Il est important de noter que dans le cas des volcans réguliers on a $\beta=h$ voir proposition~\ref{pro:niv:str} et l'on a $e \leqslant h$, ainsi certains algorithmes dépendant de la profondeur  ne seront pas forcéments plus coûteux à cause d'une hauteur élevée notamment dans les algorithmes qui remontent dans le volcan (tels \ref{alg:rem:tri}, \ref{alg:ulti:fro}), le lemme~\ref{lem:alg:rat} en est une illustration.
\end{rem}
...
régulier on a les points $P$ et $Q$ d'ordre $\ell^k-1$ situés au niveau $h-e+i-1$
 définis dans $E(\mathsf{F}_{k-\beta+e-i-1})$ alors le côut de $\mathsf{divise}$
...
\begin{prop}
Soit $E$ une courbe au niveau $h-e$ dans un volcan de $\ell$-isogénies, situé au-dessus du premier niveau de stabilité alors l'algorithme \ref{alg:rem:tri} calcule une base diagonale de $E_s[\ell^k]$ avec une complexité espérée de $O(e( \mathsf{R}(k-\beta) + \ell^{k-\beta}\mathsf{M}(\ell) + \ell\mathsf{M}(\ell^{k-\beta})))$ opérations sur $\mathbb{F}_q$.
\end{prop}
...
\begin{prop}\label{pro:alg:diagonal}
  ...
  $O(\mathsf{R}(k-\beta) + \ell^2\mathsf{M}(\ell^{k-\beta}) + \ell^{k-\beta}\mathsf{M}(\ell) + \ell\mathsf{M}(\ell^2)\log(\ell)\log(\ell q))$ opérations dans $\mathbb{F}_q$.
\end{prop}
...
\begin{enumerate}
\item 
 $O(\ell\mathsf{M}(\ell^2)\log(\ell)\log(\ell q)+(e+1)(R(h+1-\beta)+\ell^{h+1-\beta}\mathsf{M}(\ell)+\ell^2 \mathsf{M}(\ell^{h+1-\beta}))+\mathsf{R}(k-\beta) + \ell^{k-\beta}\mathsf{M}(\ell) + \ell^2\mathsf{M}(\ell^{k-\beta}))$ opérations sur $\mathbb{F}_q$ pour une courbe en entrée située sur un volcan de $\ell$-isogénies régulier,  
\item  $O(\mathsf{R}(k-\beta) + \ell^{k-\beta}\mathsf{M}(\ell) + \ell^2\mathsf{M}(\ell^{k-\beta}) + \ell\mathsf{M}(\ell^2)\log(\ell)\log(\ell q))$ opérations sur $\mathbb{F}_q$ pour une courbe en entrée située au dessus du premier niveau de stabilité.
...
que l'on travaille avec des points d'ordre $\ell^{h-e+1}$, ainsi cette première
 ...
 Frobenius sur la $\ell^{i+1}$ torsion, on a les points d'ordre $\ell^{i}$ 
 ...
 \ref{alg:ult:identical:divide} est de $\mathsf{R}(i+e-\beta)$. Le coût de calcul de 
 $\pi(P)$ est de $O(\ell^{i+e-\beta}\mathsf{M}(\ell))$ par le théorème 
 \ref{thm:frob-ell}. L'écriture de $\pi(P)=[u]P$, on 
 se limite à ce cas là car on est dans le cas où la matrice représentant 
 l'action du Frobenius est scalaire, a un coût de $O(\ell \mathsf{M}(\ell^{i+e-\beta}))$. 
...
Ainsi le coût des étapes \ref{alg:ult:rem:divide} et \ref{alg:ult:trigonal:divide} est de $\mathsf{R}(h+1-\beta)$, le coût de calcul de $\pi(P)$ est de $O(\ell^{h+1-\beta}\mathsf{M}(\ell))$. L'écriture de $\pi(P)=[u]P+[v]Q$ aux étapes \ref{alg:ult:rem:frobenius} et \ref{alg:ult:trigonal:frob} a un coût de $O(\ell^2 \mathsf{M}(\ell^{h+1-\beta}))$. Lors de la boucle $h-e+i$, les étapes \ref{alg:ult:trigonal:solve1} et \ref{alg:ult:rem:solve1} sont résolues à l'aide d'une recherche exhaustive et ont un coût négligeable par rapport aux autres opérations. 
...
Calcul de $(\color{red}P_{\lambda}\color{black},\color{blue}Q_{\mu}\color{black}),(\color{red}P'_{\lambda}\color{black},\color{blue}Q_{\mu}'\color{black})$ bases horizontales de $E_s[\ell^k],E'_s[\ell^k]$ à l'aide l'algorithme \ref{alg:hor:poi};
\IF{$(E_s,E'_s) \neq (E_s,E'_s)$}
...
\STATE Calcul des des polynômes $A_{a,b}$ et $T$ par les méthodes décrites dans la section~\ref{sec:interpolation};
...
Par définition de ~$k$, on a ~$\ell^{2k+1} \in O(r\ell^2)$.
  ...
  On se place désormais dans le cas où $k$ strictement plus grand que $h$ 
  le coût de calcul des bases 
  diagonales de courbes sommets à l'étape~\ref{alg:cou-ell:bdiag} se fait à 
  l'aide de l'algorithme~\ref{alg:ulti:fro} qui a un coût dans le pire des cas
  de $O(\ell\mathsf{M}(\ell^2)\log(\ell)\log(\ell q)+k(\mathsf{R}(k-\beta) + \ell^{k-\beta}\mathsf{M}(\ell) + \ell^2\mathsf{M}(\ell^{k-\beta})))$ 
  par la proposition~\ref{pro:alg:ulti}. Cette borne est très grossière car 
  elle est prise pour la cas où l'on aurait $e=h=k-1$ et dans ce cas où le 
  volcan est régulier on devrait avoir $\beta \geqslant h-1$. 
  Dans le cas où la courbe se situe sur 
  un volcan régulier et de $O(\ell\mathsf{M}(\ell^2)\log(\ell)\log(\ell q)+\mathsf{R}(k-\beta) 
  + \ell^{k-\beta}\mathsf{M}(\ell) + \ell^2\mathsf{M}(\ell^{k-\beta}))$ dans le
   cas où la courbe se situe au-dessus du premier niveau de stabilité.

  L'étape~\ref{alg:cou-ell:bhor} qui utilise 
  l'algorithme~\ref{alg:hor:poi} coûte en moyenne 
  ...
  coût de $\mathsf{R}$, on a alors un coût borné par 
  ...
  $O(k\mathsf{M}(\ell^{k-\beta+1}))$ par la proposition~\ref{pro:par:hor}.
  ...
  celui du calcul de~$L_{a,b}$~\cite[§3.3]{DeFeo10}.  Enfin, en moyenne 
   ${\ell^{2k}=O(r \ell^2)}$~matrices candidates doivent être testées
