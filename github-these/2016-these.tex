...
similaire à la section~\ref{sec:interpolation}, ainsi le coût de 
...
$\ell_{n-i}(k_{n-i}+1)$ fois et donc on majore ce coût par $O(n\mathsf{M}
(p_n)p_{n-1}^{(\omega-1)/2}\max_i(\ell_{i}(k_{i}+1)))$.
...
Cependant comme on a vu précédemment le coût de calcul des $A_i$ est de 
$O(n\mathsf{M}(t_i\max_i(d_i\ell_i^{k_i}))(p_{n-1})^{(\omega-1)/2}
\max_i(\ell_i))$ on majore donc le cout total par $O(n\mathsf{M}(t\max_i(d_i
\ell_i^{k_i}))(p_{n-1})^{(\omega-1)/2}\max_i(\ell_i(k_i+1)))$ par superlinéarité de 
$\mathsf{M}$.

On peut donc majorer le coût de calcul de la partie de l'interpolation de cette
version $\ell$-adique de l'algorithme de Couveignes. On répète l'interpolation
$O(r \prod_{i=1}^n\ell_i)$ en moyenne. On peut donc estimer le coût de cet 
algorithme par $O(r (\prod_{i=1}^n\ell_i)n\mathsf{M}(r (\prod_{i=1}^n \ell_i^2)
\max_i(d_i\ell_i^{k_i}))(p_{n-1})^{(\omega-1)/2}\max_i(\ell_i (k_i +1)) )$ or $p_n \in 
O(r(\prod_{i=1}^n\ell_i)^{3,5})$ ce qui nous donne comme estimation 
\[ 
O(r(\prod_{i=1}^n\ell_i)n\mathsf{M}(r (\prod_{i=1}^n \ell_i^{3,5})
\max_i(d_i\ell_i^{k_i}))(\sqrt{r (\prod_{i=1}^n \ell_i^{3,5})})^{(\omega-1)/2}
\max_i(\ell_i(k_i+1)) )
\]
...
l'apport d'opérations de changement de représentations non linéaire en la 
...
réduire les coûts apportés par l'étape d'interpolation dûs au choix du(des)
nombre(s) de Elkies.
