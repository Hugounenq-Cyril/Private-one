...
\label{def:tour-ell}
Soit $\mathbb{F}_q$ un corps fini à $q$ éléments de caractéristique $p$, soit $\ell$ un nombre premier différent de $p$, on appelle tour d'extensions $\ell$-adiques du corps fini $\mathbb{F}_q$ la suite d'extensions: $\mathbb{F}_q, \mathsf{F}_{0}, \mathsf{F}_{1}, \mathsf{F}_{2}, \cdots$ pour lesquels:
\begin{itemize}
...
\begin{thm}
Soit $\mathbb{F}_q$ un corps fini de caractéristique $p$, soit $\ell$ un nombre premier différent de $p$,  alors une tour d'extensions $\ell$-adiques du corps $\mathbb{F}_q$: $\mathbb{F}_q, \mathsf{F}_0, \mathsf{F}_1, \mathsf{F}_2, \cdots$ est calculée en $O(d_0\mathsf{M}(d_0)\log(d_0)\log(d_0q)+\mathsf{M}(\ell)\log(\ell)+d_0^2)$ opérations en moyenne sur $\mathbb{F}_q$.
\end{thm}
...
\begin{rem}
Sachant que $d_0 \mid \ell-1$ ce coût peut être borné à l'aide de $\ell$ de la façon suivante: $O(\ell\mathsf{M}(\ell)\log(\ell)\log(\ell q)+\mathsf{M}(\ell)\log(\ell)+\ell^2)$ opérations en moyenne sur $\mathbb{F}_q$.
\end{rem}
...
\section{Représentation des éléments de la tour d'extensions $\ell$-adiques}%titre a retravailler
...
Soit $P_0$ le polynôme minimal défini sur $\mathbb{F}_q$ d'un résidu non $\ell$ adique défini sur une extension de degré $d_0$, à l'aide de celui-ci sont définis les suites de polynômes $T_0,T_1,T_2,\cdots, T_k$ en les indéterminées $X_0,X_1,X_2, \cdots X_k$ de la façon suivante:
\[
T_0=P_0(X_0) \quad T_1=X_1^{\ell}-T_0(X_0) \quad \text{et} \quad T_k=X_k^{\ell}-X_{k-1} \quad \text{avec $k$ entier naturel non nul}
...
Il a été montré dans \cite{lang2002algebra} l'égalité entre les idéaux suivants :
\begin{equation}
...
\end{equation}
Cet idéal premier est appelé $A_k$ et l'on a:
\[
...
La $\mathbb{F}_q$ base monovariée de $\mathsf{F}_k$ est:
\begin{equation*}
...
\begin{proof}
Sans perte de généralité on peut supposer que l'on a $j<i$, le résultat étant sinon $a$ lui-même. Notons $s= |\mathsf{F}_j|$ et rappelons que $[\mathsf{F}_i:\mathsf{F}_0]=\ell^{i}$. Soit $x_i$ l'image de $x$ dans $F_i=\mathsf{F}_0[x]/P_i(x)$, on a en particulier $x_i^{\ell^i}=x_0$.

La première étape est de calculer $y=x_i^s$. En écrivant $s=u\ell^i+v$ avec $0 \leqslant v < \ell^i$ on a alors $y=x_0^{u \bmod |F_0|-1}x_i^v$. 
...
\end{equation*}
avec les $a_w=a_w^* + \sum_{k=1}^{\lfloor d_0\ell^i-1/d \rfloor}a_{w+dk}^*x_0^k \in \mathsf{F}_0$ pour $w$ appartenant à $[0; \ell^i-1]$. Pour obtenir cette réécriture on s'est servi du fait que pour $t$ appartenant à $[1; d_0\ell^i-1]$ tel que $t=\ell^i u_t+v_t$ avec $0 \leqslant v_t<\ell^i$ on a $x_w^t=x_0^{u_t \bmod |\mathsf{F}_0|-1}x_w^{v_t}$ . Ainsi cette opération est juste un réarrangement des coefficients de $a$.
...
\begin{thm}
\label{thm:BMSS}
Soient $E, \tilde{E}$ deux courbes elliptiques ordinaires $\ell$-isogènes définies sur $\mathbb{F}_q$ sous forme de Weierstrass normalisées, avec $\ell$ différent de $p$ la caractéristique de $\mathbb{F}_q$ qui peut être nulle ou doit vérifier $p >> \ell$. Alors:
\begin{itemize}
...
\end{equation}
Le développement de Taylor de $H$ en $f_1$ nous donne l'équation différentielle linéaire:
\begin{equation}
...
\end{equation}
que l'on détermine successivement dans l'algorithme \ref{alg:BMSS:total}. La dernière étape de \ref{alg:BMSS:total} est une reconstruction rationnelle \cite[11]{vzGJG03} (\textcolor{red}{mettre référence}) qui a un cout de $O(\mathsf{M}(\ell)\log(\ell))$.
...
\begin{rem}
Il est à noter que si $p_1$ est connu alors on peut effectuer la reconstruction rationnelle comme précisé dans les remarques \ref{rem:ELK:h-p} \ref{rem:pol:rec} et ainsi il est suffisant de calculer $S$ à précision $x^{2\ell}$ car on a pas besoin d'effecture une reconstruction rationnelle.
\end{rem}
...
\begin{prop}
\label{pro:ord:fro}
Pour une tour de \emph{niveau} $p$-adique de $\mathbb{F}_q$ définie comme dans \ref{def:p-adic} on a:
\begin{itemize}
...
On voit donc à travers cette preuve que si $E[p] \subset E(\mathbb{F}_q)$ alors il est possible que $\mathbb{L}_0=\mathbb{L}_1=\mathbb{L}_2$, de même lorsque l'on a pas $E[p] \subset E(\mathbb{F}_q)$ il est possible que $\mathbb{L}_1=\mathbb{L}_2$, dés lors la distinction suivante est nécessaire.
...
\begin{defi}
Soit $\mathbb{L}_0, \mathbb{L}_1, \mathbb{L}_2, \cdots, $ une tour de \emph{niveau} $p$-adique de $\mathbb{F}_q$ définie comme dans \ref{def:p-adic} alors on définit
\begin{itemize}
...
\end{equation*}
pour plus de détails sur le calcul de complexité voire \cite{DeFeo11}, la démarche étant la même que celle décrite dans la sous section précédente, il est à noter que pour ce calcul de complexité $d_1$ a été majoré par $p$ car $d_1 \mid p-1$. 
...
\begin{defi}
Soit $\sigma \in \mathsf{Gal}(\mathbb{U}_k:\mathbb{F}_q)$, $T=\sum_{i=0}^{d}t_iX^i \in \mathbb{U}_k[X]$ alors:
\[
...
Soit $P$ un point d'ordre $p^k$ alors $x(P) \in \mathbb{U}_k \setminus \mathbb{U}_{k-1}$ par définition de $\mathbb{U}_k$, alors pour tout $n \in \left( \mathbb{Z}/p^k\mathbb{Z} \right)^*$ $x([n]P) \in \mathsf{orb}(x(P))$. On a donc:
\begin{equation*}
T_{0}(x)=\prod_{x([n]P) \in \mathsf{orb}(x(P))}(x-x([n]P))=\prod_{i=0}^{d_1p^{k-1}-1}(x-\pi^i(x(P)))=\prod_{i=0}^{d_1p^{k-1}-1}\pi^i(x-x(P))
\end{equation*}

On note $T_0^{(0)}=x-x(P)$, $\sigma_i$ un élément générateur de $\mathsf{Gal}(\mathbb{U}_i:\mathbb{U}_{i-1})$ pour $i \geqslant 1$, supposons que l'on connaisse $T_0^{(i)}$ dés lors on définit $T_0^{(i,j)}=\sigma_i^j(T_0^{(i)})$ et l'on a 
\[
T_0^{(i+1)}=\prod_{j=0}^{b} \sigma_i^j(T_0^{(i)}) \quad  b = \begin{cases}
    p-1 &\text{si $i<k-1$,}\\
    d_1 &\text{si ( $i=k-1$ et $2 \nmid d_1$)}\\
    d_1/2 &\text{si ( $i=k-1$ et $2 \mid d_1 $)}.
  \end{cases}
\]
 $T_0^{(k)}$ est égal à $T_0$ le polynôme minimal de $x(P)$ sur $\mathbb{F}_q$.

\'A l'aide d'une représentation de ce calcul sur un arbre binaire, on voit sur la figure \ref{fig:orb:T} que le calcul de $T_0$  n'est effectué qu'en travaillant avec une branche de celui-ci.
...
\begin{rem}
La figure \ref{fig:orb:T} est à mettre en parallèle avec la figure \ref{fig:p-tor:fro} puisque l'arbre \ref{fig:orb:T} représente un seul des $f$ arbres de \ref{fig:p-tor:fro} et que les extrémités de l'arbre \ref{fig:orb:T} représentent les polynomes qui s'annulent en les abscisses de points des extrémités d'un des arbres de \ref{fig:p-tor:fro}.
\end{rem}

Ainsi avec l'utilisation du Frobenius on obtient le résultat suivant pour le calcul du polynome d'interpolation sur les points d'ordre $p^k$:
