...
On travaille donc avec points spécifiques que l'on va définir avant cela on a besoin d'introduire la notion suivante:
...
Vu que l'on se place dans le contexte de l'algorithme de Couveignes alors on veut calculer la une base horizontale (ou partiellement horizontale) de la $\ell^k$-torsion pour calculer une isogénie de degré $r$. On verra dans un autre chapitre \todo{spécifier ce chapitre} le lien entre $k$ et $r$, dans cette section on se place dans le cas Elkies où le polynôme caractéristique du Frobenius se scinde sur $\mathbb{Z}_{\ell}$, avec $h$ la valuation $\ell$-adique de la différence des valeurs propres. On a vu précédemment que l'on devait avoir $k \geqslant h+1$ \todo{mettre référence}.

On va donc travailler avec une courbe elliptique se trouvant sur un volcan de $\ell$-isogénies à cratère cyclique, et on va supposer dans un premier temps que l'on a $k>h$.

Ainsi pour une telle courbe lorsque l'on calcule l'action du Frobenius sur la $\ell^k$ torsion alors celle-ci a une forme triangulaire pour les courbes situées en dessous du cratère et diagonale pour une courbe située sur le cratère, ainsi la calcul d'une base de la $\ell^k$ torsion diffère dans ces deux cas. Ainsi on va voir dans un premier temps comment calculer une base de la $\ell^k$ torsion dans laquelle l'action du Frobenius est triangulaire.

\todo{A mettre ici ? Peut etre dans le chapitre \ref{cha:tour}}
On notera par $\mathsf{R}(i)$ une borne sur le coût espéré du calcul de racine d'un polynôme de degré $\ell$ à coefficients dans $\mathsf{F}_i$.
...
On va noter par $\mathsf{divise}(\ell,P)$ l'algorithme qui donne un des antécédents par la multiplication par $\ell$ du point $P$.
\begin{algorithm}
\caption{\label{alg:bas:tri}Calcul d'une base de $E[\ell^k]$ dans laquelle l'action du Frobenius est triangulaire pour $E$ non situéee au niveau du cratère}
\begin{algorithmic}[1]
\REQUIRE $E$: une courbe elliptique ordinaire non située sur le cratère d'un volcan de $\ell$-isogénies avec cratère cyclique;
...
\begin{prop}
L'algorithme \ref{alg:bas:tri} calcule une base triangulaire de $E[\ell^k]$ avec une complexité de $O(\mathsf{R}(k-\beta) + \ell^2\mathsf{M}(\ell^{k-\beta}) + \ell\mathsf{M}(\ell^2)\log(\ell)\log(\ell
  q))$ oéprations sur $\mathbb{F}_q$.
\end{prop}

\begin{proof}
L'équation à l'étape \ref{alg:trigonal:solve1} peut déjà être simpliée par $\ell^i$ avant d'être résolue modulo $\ell$. Pour $i<h-e-1$ comme $a=b$ et $c=d$ alors le calcul est trivial.
...
Pour~$i = 0$, la base de~$E[\ell](F_1)$ à l'étape~\ref{alg:point:divide}
...
opérations en utilisant l'algorithme de Cantor-Zassenhaus.
  ...
  multiplication-par-$\ell$ par un produit de deux $\ell$-isogenies. Pour tout
   $P$ défini dans $E(F_{i-\beta})$, le calcul de
  $\mathsf{divise}(\ell, P)$ à l'étape ~\ref{alg:point:divide} coute $O(\mathsf{R}(i-\beta+1))$
  opérations.
  L'évaluation de~$\pi(Q')$ à l'étape~\ref{alg:trigonal:frobenius} a un cout
  de~$O(\ell^{i-\beta}\mathsf{M}(\ell))$ par le théoèreme \ref{thm:frob-ell}.
  L'écriture~$\pi(P')$ comme une combinaison linéaire~$[u] P' + [v] Q'$ nécessite au
  plus~$\ell^2$ additions de points, avec un cout de~$\ell^2
  \mathsf{M}(\ell^{i-\beta+1})$.
  Le cout de résoudre l'équation à l'étape~\ref{alg:trigonal:solve1}
  par recherche exhaustive est négligeable, comme les 
  opérations restantes. Comme le cout de chaque boucle est géométrique
  la dernière boucle domine la complexité et donne le résultat.
\end{proof}

\`A partir d'une base de la $\ell^k$ torsion pour une courbe $E$ située en dessous du cratère on peut se servir de celle-ci pour calculer une base diagonale de la $\ell^k$ torsion pour la courbe "nom à trouver" \todo{Définir cela!} $Ec$  à l'aide de l'algorithme suivant:

\begin{algorithm}
\caption{\label{alg:rem:tri}Calcul d'une base de $E_c[\ell^k]$ pour $E_c$ situéee au niveau du cratère à partir de $E$ non située sur le cratère et de l'action de $\pi$ dans celle-ci}
\begin{algorithmic}[1]
\REQUIRE $E$: une courbe elliptique ordinaire non située sur le cratère d'un volcan de $\ell$-isogénies avec cratère cyclique;
...
\ENSURE $(P_k, Q_k )$: une base de $E_c[\ell^k]$;
$\lambda, \mu \in \mathbb{Z}/\ell^k \mathbb{Z}$
tels que $\pi(P_k)= \lambda P_k$, $ \pi(Q_k)=  \mu Q_k$.
\FOR {$i=1$ to $h-v_{\ell}(\delta )$}
\STATE Calcul de l'iosgénie $\varphi$ de noyau $\langle \ell^{k-1}P \rangle$
\STATE\label{alg:rem:divide}
  $P' \leftarrow \mathsf{divise}(\ell, \varphi(P))$; $Q' \leftarrow \mathsf{divise} (\ell, \varphi(Q))$.
...
\STATE
  $P \leftarrow P' + y Q'$.
  \STATE{$\lambda \leftarrow \lambda + y (\ell \delta)$; $\mu \leftarrow \mu - y (\ell \delta) $; $\delta \leftarrow \ell \delta$.}
\ENDFOR
\RETURN $(P_{k},Q_{k},\lambda,\mu).$
\end{algorithmic}
\end{algorithm}
...
\begin{prop}
Soit $E$ une courbe au niveau $h-e$ dans le volcan alors L'algorithme \ref{alg:rem:tri} calcule une base diagonale de $E_c[\ell^k]$ avec une complexité de $O(e( \mathsf{R}(k) + \ell^2\mathsf{M}(\ell^{k})) )$ oéprations sur $\mathbb{F}_q$.
\end{prop}

\begin{proof}
L'équation à l'étape \ref{alg:rem:solve1} peut déjà être simplifiée par $\ell^{k-1}$ avant d'être résolue modulo $\ell$. Un calcul direct montre qu'à chaque étape $(P_i,Q_i)$ est une base dans laquelle le coefficient en haut à droite de la matrice de$\pi$ augmente de $1$ à chaque étape. 


A l'étape~\ref{alg:rem:divide} comme $P$ et $Q$ sont définis dans $E(\mathsf{F}_{k-i})$ 
alors le cout de $\mathsf{divise}$ est de $O(\mathsf{R}(k-i))$
  opérations.

  L'évaluation de~$\pi(Q')$ et $\pi(P')$ à l'étape~\ref{alg:rem:frobenius} a un cout
  de~$O(\ell^{k}\mathsf{M}(\ell))$ par le théoèreme \ref{thm:frob-ell}.
  L'écriture~$\pi(P')$ comme une combinaison linéaire~$[u] P' + [v] Q'$ nécessite au
  plus~$\ell^2$ additions de points, avec un cout de~$\ell^2
  \mathsf{M}(\ell^{k})$.
  Le cout de résolution de l'équation à l'étape~\ref{alg:trigonal:solve1}
  par recherche exhaustive est négligeable, comme les 
  opérations restantes. 
\end{proof}

Il est possible que la courbe elliptique $E$ avec laquelle on travaille soit déjà située sur le cratère alors dans un tel cas on peut directement utiliser l'algorithme suivant qui permet de calculer une base diagonale de la $\ell^k$ torsion.

\begin{algorithm}
\caption{\label{alg:bas:diag}Calcul d'une base de $E[\ell^k]$ pour $E$ au niveau du cratère et de l'action de $\pi$ dans celle-ci}
\begin{algorithmic}[1]
...
tels que $\pi(P_k)= \lambda P_k$, $ \pi(Q_k)= \mu Q_k$.
% \STATE Compute $(P_1,Q_1)$, a basis of $E[\ell]$
\STATE{$\lambda \leftarrow 0$; $\mu \leftarrow 0$; $P_0, Q_0 \leftarrow $ deux éléments tes que~$\langle P_0,Q_0 \rangle  E[\ell]$.}
\FOR {$i=0$  to  $k-1$}
...
\begin{prop}\label{th:alg:diagonal}
...
\end{prop}
\begin{proof}
L'équation à la ligne~\ref{alg:diagonal:solve1} or~\ref{alg:diagonal:solve2}
is first divided out by the largest power of~$\ell$ possible,
which is~$\ell^{\min (h, i)}$, then solved modulo $\ell$.
For~$i \leqslant h-1$, since~$a = d$ and~$b = c = 0$,
...
For~$i = 0$, the basis of~$E[\ell](F_1)$ at step~\ref{alg:diagonal:divide}
is computed by factoring the $\ell$-division polynomial
at an expected cost of $O(\ell\mathsf{M}(\ell^2)\log(\ell)\log(\ell q))$
operations using the Cantor-Zassenhaus algorithm.
  Once $E[\ell]$ has been computed, we can factor the
  multiplication-by-$\ell$ map as a product of two $\ell$-isogenies. Then,
  for any $P$ defined in $E(F_{i-\beta})$, the computation of
  $\mathsf{divise}(\ell, P)$ at Step~\ref{alg:diagonal:divide} costs $O(\mathsf{R}(i-\beta+1))$
  operations.
  Evaluating~$\pi(P')$ in Step~\ref{alg:diagonal:frobenius} has a cost
  of~$O(\ell^{i-\beta}\mathsf{M}(\ell))$.
  Writing~$\pi(P')$ as a linear combination~$\alpha P' + \beta Q'$ needs at
  most~$\ell^2$ point additions, with a cost of~$\ell^2
  \mathsf{M}(\ell^{i-\beta+1})$.
...
%\begin{algorithm}
%\caption{\label{alg:diagonal}Montée d'un niveau dans le volcan et calcul d'une base de $E[\ell^k]$ et de l'action de $\pi$ dans celle-ci}
%\label{alg:bas:fro}
%\begin{algorithmic}[1]
%\REQUIRE $E_i$: une courbe elliptique ordinaire située sur un volcan de $\ell$-isogénies avec cratère cyclique; $P_i,Q_i$ une base de $E[\ell^k]$; $\Pi$ la matrice de $\pi$ exprimée dans la base $P,Q$; $\widehat{\varphi}$ la $\ell$-isogénie de noyau engendré par $[\ell^{k-1}]P$
%\ENSURE $(P_{i+1}, Q_{i+1} )$: une base de $E_{i+1}[\ell^k]$ dans laquelle l'action du Frobenius est trigonalisée: $\left( \begin{smallmatrix} \lambda & 0 \\ c \ell^{k-1} & \mu \end{smallmatrix} \right)$ \;
%$\lambda, \mu, c\ell^{k-1}  \in \mathbb{Z}/\ell^k \mathbb{Z}$.
%% \STATE Compute $(P_1,Q_1)$, a basis of $E[\ell]$
%\STATE{$\lambda, \mu, c \leftarrow \Pi$}
%\STATE\label{alg:diagonal:divide}
%  $P' \leftarrow \mathsf{divise}(\ell,\widehat{\varphi} (P_{i}))$; $Q' \leftarrow  \widehat{\varphi}(Q_{i})$.
%\STATE\label{alg:diagonal:frobenius}
%  compute $\pi|(P',Q')=\left( \begin{smallmatrix}
%\lambda & b\ell^{k-1}\\
%c\ell^{k-1} & \mu \end{smallmatrix} \right) \pmod {\ell^{i+1}}.$
%\STATE\label{alg:diagonal:solve1}
%  \textbf{if} $b c= 0$ \textbf{then} $y \leftarrow 0$;
%\STATE\label{alg:diagonal:solve2}
%  \textbf{else} solve equation
%  $c \ell^{i} x^2 + ((d-a) \ell^{i}+ \mu-\lambda) x - b \ell^{i} = 0$; \textbf{end if}.
%\STATE\label{alg:diagonal:upd-P}
%  $P_{i+1} \leftarrow P' + y Q'$; $Q_{i+1} \leftarrow x P' + Q'$.
%\STATE{$\lambda \leftarrow \lambda + \ell^{i} (a + bx)$; $\mu \leftarrow \mu + \ell^{i} (d + cy)$.}
%\RETURN $(P_{k},Q_{k},\lambda,\mu).$
...
\begin{algorithm}
\caption{\label{alg:horizontal}Computing a horizontal point of order~$\ell^k$}
\begin{algorithmic}[1]
...
\STATE\label{alg:horizontal:frob} Write~$\pi(P') = \lambda P' + b Q_i$ for~$b \in \mathbb{Z}/\ell\mathbb{Z}$ and
let $P_{i} \gets P' - (b/\mu) Q_i$.
\ENDFOR
...
\begin{prop}\label{th:horizontal}
  ...
  using an expected $O(\mathsf{R}(k-\beta) + k\mathsf{R}(h-\beta+1) + k\ell^2\mathsf{M}(\ell^{h-\beta+1}))$
  operations in $\mathbb{F}_q$.
\end{prop}
\begin{proof}
Let $E_i$ be the image curve of $\varphi_i$.
We check that at step~$i$ of the loop,
the points~$(P_i, Q_i)$ form a diagonal basis of~$E_i[\ell^{h+1}]$,
and $\varphi_i$~has direction~$\lambda$.
...
costing respectively $O(\mathsf{R}(h-\beta+1))$ and $O(\ell^2\mathsf{M}(\ell^{h-\beta+1}))$, as
discussed in the proof of Proposition~\ref{th:diagonal}. They are
repeated $k$ times. Finally, Step~\ref{alg:horizontal:final} is
dominated by the last $\mathsf{divise}$ operation, which costs $O(\mathsf{R}(k-\beta))$.
\end{proof}


\begin{algorithm}
\caption{\label{alg:bas:fro}Calcul d'une base de $E[\ell^k]$ et de l'action de $\pi$ dans celle-ci}
\begin{algorithmic}[1]
...
\STATE compute $\pi|(P',Q')=\left( \begin{smallmatrix}
\lambda + a\ell^{i} & b\ell^{i}\\
c\ell^{i} & \mu + d\ell^{i} \end{smallmatrix} \right) \pmod {\ell^{i+1}}.$
...
\todo{Cout de détermination de la forme du cratère à mettre !}
