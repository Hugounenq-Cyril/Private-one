...
Le coût de calcul de $\beta$ est de $O(M(d_0)\log(q)+C(d_0)\log(d_0))$ opérations en moyenne sur $\mathbb{F}_q$ avec $C(n)=O(n^{1.67})$ d'après \cite{Doliskani-Schost14}. Ainsi seul le coût du calcul de $\eta$ est crucial pour obtenir une complexité satisfaisante.
...
\end{defi}
\todo{mettre ici une figure pour les niveaux et une pour l'extension quadratique}
\begin{figure}
\begin{center}
...
\end{tikzpicture}
\end{center}
\caption{\label{fig:ex:niv:ext} Exemples de structures de la torsion par rapport au niveau des courbes dans le volcan et évolution de celle-ci sur une extension $\ell$-adique (à droite).}
\end{figure}
\begin{cor}
\label{cor:des:str}
...
On peut donc résumer les résultats de cette section sur les volcans \emph{réguliers} 
à l'aide de la figure~\ref{fig:ex:niv:ext}. On observe le résultat de la 
proposition~\ref{pro:niv:str} et l'évolution de structure de 
$E(\mathsf{F})[\ell^{\infty}]$ en fonction du niveau de la courbe dans le 
volcan. Le volcan représenté à droite est le volcan obtenu à partir du volcan 
de gauche en travaillant avec les courbes dans une extension $2$-adique du 
corps sur lequel étaient définies les courbes dans le volcan de gauche.
La seule différence à observer avec   
\todo{Faire un volcan non régulier}
...
allant d'une taille de $7$ bits à $252$ bits. On donne un exemple de la 
structure de $E(\mathbb{F}_p)[2^{\infty}]$ selon le niveau dans le volcan de 
$\ell$-isogénies dans la figure~\ref{fig:elk:dif:niv:tor} pour un des exemples
ayant servi aux calculs montré dans la figure~\ref{fig:exp:dif}.
\begin{figure}
\begin{center}
...
\caption{\label{fig:elk:dif:niv:tor}Exemple de structure d'un volcan utilisé lors des test montrés à la figure~\ref{fig:exp:dif}.}
\end{figure}
\begin{figure}
...
\caption{\label{fig:exp:niv} Comparaison d'une phase d'interpolation pour $r$ croissant pour différentes courbes définies sur les corps finis $\mathbb{F}_{521}$ et $\mathbb{F}_{1033}$. La courbe est en échelle logarithmique.}
\end{figure}
