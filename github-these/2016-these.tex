...
Dans la tour d'extensions $\ell$-adiques les éléments de la tour peuvent être représentés sous forme $G(x_k)$ avec $G$ un polynôme de degré $d$ à coefficients dans $\mathsf{F}_0$, or un élément $G(x_k)$ peut aussi être représenté sous forme $H(x_k)$ avec $H$ un polynôme de degré $d$ à coefficients dans $\mathbb{F}_q$. \'A travers le calcul du Frobenius on va expliciter ces changements de représentation. On se place dans le cas où $\mathbb{F}_q \ncong \mathsf{F}_0$ avec $a$ représenté sous la forme d'un polynôme en $x_i$ de degré inférieur ou égal à $[\mathsf{F}_i:\mathbb{F}_q]$ : 
\begin{equation*}
