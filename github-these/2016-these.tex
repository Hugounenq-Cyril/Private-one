...
\begin{prop}
Soit $P \in E[\ell^k]$ un vecteur propre de $\pi$, $i\leqslant k$ tel que $[\ell^i]P$ soit horizontal, $\psi:E \mapsto E_i$ une isogénie descendante de degré $\ell^i$, on note $P'=\psi(P)$, alors:
\begin{enumerate}
\item  $\langle [\ell^{k-i}]P' \rangle$ est le noyau d'une isogénie montante de degré $\ell^{i}$ duale de $\psi$ notée $\widehat{\psi}$ 
\item  $\langle \widehat{\psi}(P') \rangle$ est le noyau d'une isogénie horizontale de degré $\ell^{k-i}$.  
\end{enumerate}
\end{prop}
...
\begin{cor}
Soient $\phi: E \rightarrow E'$ une $r$-isogénie avec $E$ et $E'$ qui se situent à une profondeur $e$ du cratère, soient $E_c$ et $E'_c$ les courbes les plus proches de $E$ et $E'$ situées sur le cratère soient $P \in E_c$ et $P' \in E'_c$ deux points d'ordre $\ell^k$ avec $k \geqslant e \geqslant 1$ tels que $[\ell^e]P$ et $[\ell^e]P'$ soient horizontaux de même direction. On note $\varphi:E_c \rightarrow E$ et $\varphi':E'_c \rightarrow E'$ les deux $\ell^e$ isogénies descendantes alors:
\begin{itemize}
\item $\phi(\varphi(P)) \in \langle \varphi'(P') \rangle$
\end{itemize} 
\end{cor}

Ainsi à l'aide de cette méthode on est capable à l'aide de bases horizontales et d'isogénies montantes de déterminer une correspondance entre deux groupes cycliques d'ordre $\ell^k$ de courbes $r$-isogénes qui respectent l'action de la $r$-isogénie.
