...
%Assume $\lambda \cong \mu \cong 1 \pmod{4}$ and let~$\alpha = v_2(\lambda-1), \beta=v_2(\mu-1)$.
%Write~$ν_2(x, y) = \min (x+y, x+\beta-2, y+\alpha-2)$
%and~$ρ_2(x, y) = x+y - ν_2(x, y) = \max (0, x-\alpha+2, y-\beta+2)$.
%The decomposition of the group~$E[2^k]$ in Galois classes is as follows:
%\begin{enumerate}
%\item for~$i, j = 1, …, k-2$:
%$4 \cdot 2^{ν_2(i,j)}$ classes of size~$2^{ρ_2(i,j)}$;
%\item for~$i = 1, …, k-2$:
