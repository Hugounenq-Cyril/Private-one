...
\begin{defi}
Sur un volcan de $\ell$-isogénie on appelle le niveau à partir duquel la structure de la $\ell$-torsion ne change pas en même temps que les niveaux du volcan le premier niveau de stabilité.

Un volcan dont tous les niveaux ont des structures de $\ell$-torsion disctinctes est dit un volcan régulier.
\end{defi}
...
\end{proof}
Ainsi avec cette proposition on a un moyen de déterminer la hauteur d'une courbe dans un volcan de $\ell$-isogénies à l'aide de l'étude de l'action du Frobenius sur la $\ell$-torsion.
On voit aussi que cette détermination n'est pas sujette à l'existence de niveau de stabilité vu que la deuxième partie de la preuve se fait à l'aide des ordres et non de la structure de $\ell$-torsion.
...
On va donc travailler avec points spécifiques que l'on va définir avant cela on a besoin d'introduire la notion suivante:

\begin{defi}
Soit $E$ un courbe située sur le cratère d'un volcan de $\ell$-isogénies, $\phi$ une isogénie de degré $\ell^k$ de domaine $E$, alors $\phi$ est dite une isogénie horizontale si il existe $\phi_1, \cdots, \phi_k$ $k$ $\ell$-isogénies horizontales, dans le sens de \ref{def:iso:nom},telles que $\phi=\phi_1 \circ \phi_2 \circ \cdots \circ \phi_k=\phi$.
\end{defi}
...
Montrons tout d'abord pour $\phi:E \mapsto E'$ comment calculer l'image de la matrice du Frobenius $\pi|T_{\ell}(E)$ dans une base de $T_{\ell}(E')$.

\begin{lem}
\label{lem:iso:con}
Soient $\langle P, Q \rangle$ une base de $E[\ell^k]$, $\phi$ l'isogénie de noyau engendré par $[x]P+[y]Q$  avec $x\wedge \ell =1$ et $y=\ell^my'$ avec $y' \wedge \ell =1$, $M_R$ la matrice de forme normale réduite de Hermite qui correspond au réseau engendré par $R$,  $\Pi$ la matrice du Frobenius sur $T_{\ell}(E)$ alors la matrice du Frobenius sur $T_{\ell}(E')$ est donnée par $M_R^{-1} \cdot \Pi \cdot M_R$ à conjugaison près.
\end{lem}
...
Soit~$\psi: E \rightarrow E'$ une isogénie de degré~$r$ premier avec~$\ell$.
\begin{enumerate}
\item les courbes~$E$,~$E'$ ont la même profondeur dans leur volcan de
 $\ell$-isogénies,
\item\label{pro:par:fun} pour tout point~$P \in E[\ell^k]$,
les isogénies de noyau $\langle P \rangle$ et~$\langle \psi(P) \rangle$ sont de même type
...
\end{prop}
\begin{proof}
Les points (1) et (2) découlent de la proposition \ref{pro:mat:fro} et du fait que pour $\psi$ isogénie rationnelle de degré $r$ premier avec $\ell$ on a un isomorphisme de modules de Tates qui commute avec le Frobenius. Pour le point (3) il suffit de remarquer qu'il existe un unique sous-groupe d'ordre $\ell$ qui génère soit une isogénie ascendante soit une isogénie de direction fixée, ainsi on doit avoir $\langle P' \rangle=\langle \psi(P) \rangle$.
...
\begin{prop}
Soit $P \in E[\ell^k]$ un vecteur propre de $\pi$, $i\leqslant k$ tel que $[\ell^i]P$ soit horizontal de direction $\lambda$, $\psi:E \mapsto E_i$ une isogénie descendante de degré $\ell^i$, on note $P'=\psi(P)$, alors:
\begin{enumerate}
\item  $\langle [\ell^{k-i}]P' \rangle$ est le noyau d'une isogénie montante de degré $\ell^{i}$ duale de $\psi$ notée $\widehat{\psi}$, 
\item  $\langle \widehat{\psi}(P') \rangle$ est le noyau d'une isogénie horizontale de direction $\lambda$ de degré $\ell^{k-i}$.  
\end{enumerate}
\end{prop}


\begin{proof}
Pour le pont (1) si l'on note $\langle P_0, Q_0 \rangle =E[\ell^k]$ une base horizontale, alors on peut supposer sans perte de généralité que $P$ appartient à l'espace propre associé à  $P_0$ ainsi on a $P=[a]P_0+[\ell^{k-i}b]Q_0$ avec $a \wedge \ell =1$. Supposons que $\psi$ est de noyau $\langle [a'\ell^{k-i}]P_0+[b'\ell^{k-i}]Q_0 \rangle$ avec $b' \wedge \ell =1$ et $a' = 1 \bmod \ell$. Alors les images de $[\ell^{k-i}]P_0$ et $[\ell^{k-i}]Q_0$ par $E \rightarrow E/\langle [\ell^{k-i}]P,[a'\ell^{k-i}]P_0+[b'\ell^{k-i}]Q_0  \rangle $ sont l'élément neutre, l'image de $P$ par $\psi$ étant encore de degré $\ell^k$ on a le résultat.

Pour le point (2) il suffit d'écrire que $\widehat{\psi}(P')=\widehat{\psi}\circ \psi P= [\ell^i] P$ on conclut alors à l'aide des hypothèses sur $P$.
\end{proof}
...
Ainsi à l'aide de cette méthode on est capable à l'aide de bases horizontales et d'isogénies montantes de déterminer une correspondance entre deux groupes cycliques d'ordre $\ell^k$ de courbes $r$-isogénes qui respectent l'action de la $r$-isogénie et qui ne se situent pas forcément sur le cratère. 

La différence entre des points horizontaux $P_{\lambda}, P_{\mu}$ d'ordre $\ell^k$ et des points partiellement horizontaux c'est que pour deux points partiellement horizontaux $R_{\lambda}, R_{\mu}$ de profondeur $e$ de direction opposées on a $\langle [\ell^{e}]R_{\lambda} \rangle = \langle [\ell^{e}]R_{\mu} \rangle$ alors que l'on a vu que pour deux points horizontaux de direction opposées on avait $\langle [\ell^{k-1}]P_{\lambda} \rangle \neq \langle [\ell^{k-1}]P_{\mu} \rangle $. 

Ainsi en pratique on a vu que l'on était obligé de travailler avec la $\ell^{h+1}$ torsion et cette précédente discussion justifie l'utilité de se concentrer sur les points d'ordre $\ell^{k}$ avec $k \geqslant h+1$ plutôt que sur la $\ell^k$ torsion.

Objectif finir le Frobenius avec l'analyse des différentes classes pour l'endomorphisme de Frobenius. On a tout d'abord le premier résultat qui nous précise dans quel extension de corps on sera ammené à travailler pour avoir de la $\ell^k$ torsion qui soit définie.
...
sans perte de généralité que $\alpha \geq \beta$.  Comme $\lambda \neq \mu \pmod{\ell^{h+1}}$, il est
impossible que $\lambda \equiv \mu \equiv 1 \pmod{\ell^h}$ par la structure 
d'un volcan de $\ell$-isogénies, ainsi au moins une des deux
valuations~$\alpha, \beta$ est~$\leq h$, par conséquent~$\beta \leq h$.
\label{sub:classes}
\begin{prop}\label{prop:classes}
For any~$k$, let~$d_k$ be the degree of the smallest field extension $F/\mathbb{F}_q$
such that~$E[\ell^k]\subset E(F)$. Then:
\begin{enumerate}
\item The order of $q$ in $(\mathbb{Z}/ \ell \mathbb{Z})^\times$ divides $d_1$,
and $d_1$ divides~$(\ell-1)$.
\item If $\ell$~is odd then for all $k \geq 1$,
$ d_k = \ell^{\min (v_{\ell} (d_1), k - \beta)}$.
% $d_k = \mathrm {lcm} (d_1, \ell^{k-\beta})$.
\item If $\ell=2$ then $d_2 \in \{1,2\}$ and, for all~$k \geq 2$,
% $d_k = \mathrm{lcm}(d_2, \ell^{k-\beta})$.
$d_k = \ell^{\min (v_{\ell} (d_2), k - \beta)}$.
\item Let $[F:\mathbb{F}_q]=d_1\ell^n$, the group $E[\ell^{\infty}](F)$ is isomorphic to~$(\mathbb{Z}/\ell^{n+\alpha} \mathbb{Z}) \times (\mathbb{Z}/\ell^{n+\beta} \mathbb{Z})$.
\item\label{prop:classes:count} The group~$E[\ell^k]$ contains at most
$k \cdot \ell^{k+\beta}$ Galois conjugacy classes over~$F_1 = \mathbb{F}_{q^{d_1}}$.
\end{enumerate}
\end{prop}
\begin{proof}
...
on obtient donc~(i) en utilisant le fait que~$\lambda \cdot \mu = q$.
% We conclude using the multiplicative structure of~$(\mathbb{Z}/\ell^k \mathbb{Z})^\times$, and the fact
% that $\lambda\mu=q$.
% Note that in general the order of~$q$ is a strict divisor of~$d_1$,
% as is for example the case for~$\pi^2 - \pi + 29 = 0$ and~$\ell = 7$,
% where $q = 29 \cong 1 \pmod{7}$ and $d_1 = 6$.
Pour les points~(ii)--(v) on peut supposer que l'on a $d_1 = 1$.
...
et~$v_{\ell} (y) + \beta + v_{\ell} (N) \geq k$, cela montre~(iv).
Les plus grandes classes de Galois sont celles pour lesquelles
a~$v_{\ell} (y) = 0$ et leur taille est~$\ell^{k - \beta}$,
ce qui prouve~(ii) et~(iii).
De plus, pour tout~$i \leq k-\beta$ les points dans une orbite de taille~$\leq \ell^i$
sont ceux pour lesquels~$v_{\ell} (x) \geq k - \alpha - i$ et~$v_{\ell} (y) \geq k - \beta - i$;
il y a au plus $\ell^{\min(\alpha+i, k) + \min (\beta+i, k)}$ tels points,
par conséquent il y a $\ell^{\min(\alpha+i, k) + \min(\beta, k-i)} \leq \ell^{k-i+\beta}$
 classes correspondantes.
En additionnant ce résultat pour tous les ~$i$ on prouve~(v).
\end{proof}

Ainsi le point (v) nous donne une borne maximale sur le nombre de représentants des différentes orbites qe l'on a à calculer pour reconstruire tout $E[\ell^{\infty}](F)$, on a le résultat suivant qui précise la décomposition des différentes et leurs tailles:

We give here the full decomposition of $E[\ell^k]$ in Galois classes.
This is a more precise form of Proposition~\ref{prop:classes}~(v).
\begin{prop}\label{prop:orbites-l-torsion}
Let~$E$ be an elliptic curve with $\ell$-maximal endomorphism ring.
Assume $\ell \neq 2$, $\lambda \cong \mu \cong 1 \pmod{\ell}$ and let~$\alpha = v_{\ell}(\lambda-1), \beta=v_{\ell}(\mu-1)$.
Write~$\nu(x, y) = \min (x+y, x+\beta-1, y+\alpha-1)$
and~$\rho(x, y) = x+y - \nu(x, y) = \max (0, x-\alpha+1, y-\beta+1)$.
The decomposition of the group~$E[\ell^k]$ in Galois classes is as follows:
\begin{enumerate}
\item for~$i, j = 1, …, k-1$:
$(\ell-1)^2 \cdot \ell^{\nu(i,j)}$ classes of size~$\ell^{\rho(i,j)}$;
\item for~$i = 1, …, k-1$:
$(\ell-1) \cdot \ell^{\min (i, \alpha-1)}$ classes of size~$\ell^{\max (0, i-\alpha+1)}$, and
$(\ell-1) \cdot \ell^{\min (i, \beta-1)}$ classes of size~$\ell^{\max (0, i-\beta+1)}$;
\item the $\ell^2$ singleton classes of~$E[\ell]$.
% \item \todo{le cas où $\ell=2$}
\end{enumerate}
\end{prop}
\begin{proof}
Fix a basis~$(P, Q)$ of~$E[\ell^k]$ such that~$\pi(P)=\lambda P$, $\pi(Q)=\mu Q$.
Studying the Galois orbits of~$E[\ell^k]$
means studying the map~$\mathbb{Z}_\ell^2 \rightarrow \mathbb{Z}_\ell^2, (x, y) \mapsto (\lambda x, \mu y)$.
In other words, the orbits correspond to elements of~$\mathbb{Z}_\ell^2$
modulo the \emph{multiplicative} subgroup generated by~$(\lambda, \mu)$.
An easy way to describe this
is to consider a \emph{multiplicative lattice} in~$(\mathbb{Q}_{\ell}^{\times})^2$.

Let~$\varepsilon$ be a primitive $(\ell-1)$-th root of unity in~$\mathbb{Z}_{\ell}$.
Then by~\cite[Théorème II.3.2]{Serre.Arith},
the map~$f(x, y, z) = \ell^x \cdot \varepsilon^y \cdot \exp (\ell z)$
is a group isomorphism between~$\mathbb{Z} \times (\mathbb{Z}/(\ell-1) \mathbb{Z}) \times \mathbb{Z}_{\ell}$ and~$\mathbb{Q}_{\ell}^{\times}$.
For~$i \in [0,k-1 ]$ and~$c \in \mathbb{Z}/(\ell-1)\mathbb{Z}$,
let~$V(i,c)$ be the image in $\mathbb{Z}/\ell^k \mathbb{Z}$ of the map~$f(k-1-i,c,–)$:
then the multiplicative structure of $V(i, c)$
is that of a principal homogeneous space under~$\mathbb{Z}/\ell^i \mathbb{Z}$.
We also define~$W(i,j,c,d) = V(i, c) \cdot P \,+\, V(j, d) \cdot Q \subset E[\ell^k]$.

Since~$\lambda \cong 1 \pmod{\ell^{\alpha}}$, we may write~$\lambda = f(0,0,u\, \ell^{\alpha-1})$
and~$\mu = f(0,0, v\, \ell^{\beta-1})$
for some~$u, v \in \mathbb{Z}_{\ell}^{\times}$.
This implies that the set~$W(i,j,c,d)$ is stable under Galois.
Moreover, the orbits of~$W(i,j,c,d)$ correspond bijectively to
points of a fundamental domain of the lattice~$\Lambda_{i,j}$ generated by
the columns of~$\begin{smallmatrix} \ell^i & 0 & u \ell^{\alpha-1}\\ 0 & \ell^j & v \ell^{\beta-1}\end{smallmatrix}$,
whereas the size of each orbit is computed modulo~$[(\mathbb{Z}/\ell^i \mathbb{Z}) \times (\mathbb{Z}/\ell^j \mathbb{Z})\::\: \Lambda_{i,j}]$.
By using elementary column manipulations,
we find that the covolume of~$\Lambda_{i,j}$ is~$\ell^{\nu(i,j)}$,
hence the point~(i) of the proposition.
(The case~$i = j = 0$ yields singleton classes in~$E[\ell]$).

The union of all the sets~$W(j,i,c,d)$
is exactly the set of all~$x P + y Q$ for~$x, y \neq 0$.
We obtain the classes of~(ii) by considering
the sets~$V(i, c) \cdot P$ and~$V(j,d) \cdot Q$.
\end{proof}

We now state the equivalent proposition when~$\ell = 2$.
...
Assume $\lambda \equiv \mu \equiv 1 \pmod{4}$ and let~$\alpha = v_2(\lambda-1), \beta=v_2(\mu-1)$.
...
\item for~$i = 1, …, k-2$:
$4 \cdot 2^{\min (i, \alpha-2)}$ classes of size~$2^{\max (0, i-\alpha+2)}$, and
$4 \cdot 2^{\min (i, \beta-2)}$ classes of size~$2^{\max (0, i-\beta+2)}$.
\item the 16 singleton classes of~$E[4]$.
\end{enumerate}
\end{prop}
Note that if $\lambda$ or~$\mu \cong -1 \pmod{4}$ then
by replacing the base field by a quadratic extension,
we can always ensure that the condition $\lambda \cong \mu \cong 1 \pmod{4}$ is
satisfied.
...
\section{Cas Atkin}
\chapter{Rebus}
\section{Galois classes in~$E[\ell^k]$}
\label{ap:galois}



\begin{algorithm}
\caption{\label{alg:dichotomie} Dychotomic algorithm for research of point near of the border}
\begin{algorithmic}[1]
\REQUIRE $P_1, \cdots, P_m$ a list of point under a unique direction $\overrightarrow{P_1P_m}$,%\\
\ENSURE $S P_i,P_{i+1}$ two points such that $\mathrm{d}(P_i,P_{i+1}) \leqslant \sqrt{N}$ and such that $P_i \in \mathcal{P}$ and $P_{i+1} \not \in \mathcal{P}$,
\IF{$(\mathrm{d}(P_1,P_m)\leqslant \sqrt{n})$} 
\STATE \textbf{Return}$P_1$
\ELSE \STATE $\mathrm{Test}(P_{\lfloor m/2 \rfloor})$
\IF{$P_{\lfloor m/2 \rfloor} \in \mathcal{P}$}
\STATE \textbf{Return} $\mathrm{Dychotomic search}(P_{\lfloor m/2 \rfloor},P_m)$
\ELSE 
\STATE \textbf{Return} $\mathrm{Dychotomic search}(P_1,P_{\lfloor m/2 \rfloor})$ 
\ENDIF
\ENDIF 
\end{algorithmic}
\end{algorithm}
