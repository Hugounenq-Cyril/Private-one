...
\section{Calcul de polynôme d'interpolation}
...
Ainsi on note $T^{(0)}=(x-v)$, $\sigma_i$ l'application qui applique à tous les
 ...
 d'un élément générateur de $\mathsf{Gal}(F_{n-i-1} / \mathsf{F}_q)$. En 
 ...
 $T^{(i,j)}=\sigma^j(T^{(i)})$, et l'on pose:  
\begin{equation*}
...
$\mathsf{Gal}(\mathsf{F}_{n-i}/ \mathsf{F}_{n-i-1})$, il est alors défini dans
 ...
 coefficients dans $\mathsf{F}_{n-i}$ alors pour $j \in [0 ; \ell-1]$ on définit
 $A^{(i,j)}=\sigma^j(A^{(i)})$, et l'on pose:  
\begin{equation*}
A^{(i+1)}=\sum_{j=0}^bA^{(i,j)}\frac{T^{(i+1)}}{T^{(i,j)}} \qquad\text{avec}\qquad
  b = \begin{cases}
    \ell-1 &\text{if $i<n-1$,}\\
    d_0 &\text{sinon.}
  \end{cases}
...
\begin{proof}
  ...
  \`A l'étape $i$, on calcule la division Euclidienne d'un polynôme de degré
...
\section{Approche multi modulaire de l'algorithme}
...
\subsection{Aprroche avec le groupe de points isomorphe à $\oplus_{i=1}^n \mathbb{Z}/\ell_i^{k_i} \mathbb{Z} \times \mathbb{Z}/\ell_i^{k_i} \mathbb{Z}$}
...
\subsection{Approche avec le groupe de points isomorphe à $\prod_{i=1}^n \mathbb{Z}/\ell_i^{k_i} \mathbb{Z} \times \mathbb{Z}/\ell_i^{k_i} \mathbb{Z} $}
...
\subsubsection{Théorème des Restes Chinois Elliptique et son application à l'algorithme de Couveignes $\ell$-adique}
\begin{defi}
...
Soient $\ell_1, \dots, \ell_n$ $n$ nombres premiers distincts, les points $P_1 \in E[\ell_1^{k_1}], \dots, P_n \in E[\ell_n^{k_n}]$, alors le théorème des restes chinois revient à déterminer un point $P \in E[\prod_{i=1}^n \ell_i^{k_i}]$ tel que $[\overset{n}{\underset{i \neq i_0}{\underset{i = 0}{\prod}}}\ell_i^{k_i}] P=P_{i_0}$ pour $i_0 \in [1,n]$.
...
\FOR{$i=1$ à $n$}
\STATE $Q$ sortie de l'algorithme~\ref{alg:TRC:init:gen} tel que $\ell_i^{k_i}Q=0$ et $[\overset{n}{\underset{j \neq i}{\underset{j = 0}{\prod}}}\ell_j^{k_j}]Q=P_i$;
...
Soient $\ell_1, \dots, \ell_n$ $n$ nombres premiers de Elkies distincts, $P$ un
 point d'ordre $\prod_{i=1}^n\ell_i^{k_i}$ de $E$ construit à l'aide du 
 théorème des restes chinois (voir algorithme~\ref{alg:TRC}) tel que 
$[\overset{n}{\underset{j \neq i}{\underset{j = 0}{\prod}}}\ell_j^{k_j}]P=P_i$ 
avec $P_i$ des points (partiellement) horizontaux direction $\lambda_i$, alors 
...
$\ell_i$-isogénies pour $i \in [1,n]$. Alors pour $P$ un
 point d'ordre $\prod_{i=1}^n\ell_i^{k_i}$ de $E$ construit à l'aide de 
l'algorithme~\ref{alg:TRC} tel que 
$[\overset{n}{\underset{j \neq i}{\underset{j = 0}{\prod}}}\ell_j^{k_j}]P=P_i$ 
avec $P_i$ des points (partiellement) horizontaux direction $\lambda_i$, et 
$P'$ un point d'ordre $\prod_{i=1}^n\ell_i^{k_i}$ de $E'$ construit à l'aide de 
l'algorithme~\ref{alg:TRC} tel que 
$[\overset{n}{\underset{j \neq i}{\underset{j = 0}{\prod}}}\ell_j^{k_j}]P'=P'_i$ 
...
d'ordre $\prod_{i=1}^n\ell_i^{k_i}$ et de direction 
$(\lambda_1, \dots, \lambda_n)$ a pour image par $\phi$ un point $[a]P' \in E'$
avec $a \in (\mathbb{Z}/\prod_{i=1}^n \ell_i^{k_i} \mathbb{Z})^*$ et $P'$ un 
point d'ordre $\prod_{i=1}^n\ell_i^{k_i}$ et de direction 
$(\lambda_1, \dots, \lambda_n)$.
\end{rem}
\todo{réfléchir au fait qu'un point horizontal est aussi partiellement horizontal}
On a donc bien une correspondance entre les points définis à l'aide du Théorème
 ...
 , \ell_n$. Cependant il nous reste à voir dans quelle mesure la représentation
 ...
 Attardons-nous maintenant sur le second point. Afin de simplifier les 
 explications on va se concentrer sur la détermination de représentants 
 d'orbite sous l'action de groupes de Galois de points définis dans un groupe
 généré par un unique point de direction $(\lambda_1, \dots, \lambda_n)$. 
 \todo{faire du contexte quelque part...}
 Maintenant énonçons un résultat qui nous permette de déterminer les 
 représentants d'orbite sous l'action de groupes de Galois.
\begin{prop}\todo{definir a un endroit ce qu'est un représentant pour l'orbite du Frobenius}
Soit $P$ un point d'ordre $\prod_{i=1}^n \ell_i^{k_i}$ de $E$ obetnu à l'aide 
du théorème des restes chinois appliqué aux points de $E$ $P_i$ d'ordre 
$\ell_i^{k_i}$ tel que l'orbite généré par l'action de $\mathsf{Gal}(\mathsf{F}_{\ell_i^{k_i}}:\mathbb{F}_q)$ sur $P_i$ soit de taille maximale:$\ell_i^{o_i}$.  Alors l'orbite de $P$ est elle aussi de taille maximale:$\prod_{i=1}^n\ell_i^{o_i}$.
\end{prop} 

\begin{proof}
Soit $P$ un point de $E$ respectant les conditions de l'énoncé, supposons par 
l'absurde qu'il existe $i_0 \in [1,n]$ et $r_0 \in [1,o_{i_0}-1]$ 
tel quel'action d'un élément générateur de 
$\mathsf{Gal}(\mathsf{F}_{\ell_{i_0}\overset{n}{\underset{j \neq i_0}{\underset{j = 0}{\prod \ell_j^{k_j}}}}}:\mathsf{F}_{\overset{n}{\underset{j \neq i_0}{\underset{j = 0}{\prod}}}\ell_j^{k_j}})$ 
...
\times mathbb{Z}/\overset{n}{\underset{j \neq i_0}{\underset{j = 0}{\prod \ell_j^{k_j}}}}\mathbb{Z}$
à l'aide des deux points $P_{i_0}=[\overset{n}{\underset{j \neq i_0}{\underset{j = 0}{\prod \ell_j^{k_j}}}}]P$  et $Q=\ell_i^{k_i}P$. $Q$ étant définis dans $\mathsf{\overset{n}{\underset{j \neq i_0}{\underset{j = 0}{\prod \ell_j^{k_j}}}}}$ il est stable par l'action d'un élément générateur de 
$\mathsf{Gal}(\mathsf{F}_{\ell_{i_0}\overset{n}{\underset{j \neq i_0}{\underset{j = 0}{\prod \ell_j^{k_j}}}}}:\mathsf{F}_{\overset{n}{\underset{j \neq i_0}{\underset{j = 0}{\prod}}}\ell_j^{k_j}})$.
$P_{i_0}$ étant maximal par construction alors il ne peut être stable que sous 
l'action d'un élément générateur de $\mathsf{Gal}(\mathsf{F}_{\ell_{i_0}\overset{n}{\underset{j \neq i_0}{\underset{j = 0}{\prod \ell_j^{k_j}}}}}:\mathsf{F}_{\overset{n}{\underset{j \neq i_0}{\underset{j = 0}{\prod}}}\ell_j^{k_j}})$
à la puissance $o_{i_0}$, ainsi $r_0$ doit être un multiple de $o_{i_0}$.
\end{proof}
...
On voit donc avec cette proposition que pour déterminer des représentants pour
l'orbite selon l'action du groupe de Galois pour des points d'ordre 
$\prod_{i=1}^{n}\ell_i^{k_i}$ il suffit de calculer à l'aide du théorème des 
restes chinois tous les points $P$ d'ordre $\prod_{i=1}^{n}\ell_i^{k_i}$ 
tels que tous les choix possibles de représentants $P_i=[\overset{n}{\underset{j \neq i}{\underset{j = 0}{\prod}}}\ell_j^{k_j}]P_j$
 aiént été effectué. Ainsi le nombre de représentants est multiplicatif et est
 ...
 Une fois que toutes ces étapes ont été traité on peut modifier 
 l'algorithme~\ref{alg:cou:ell-adique} de telle que l'on travaille avec 
deux points d'ordre $\prod_{i=1}^n\ell_i^{k_i}$ de même direction.


\begin{algorithm}
\caption{\label{alg:cou:TRC-adique} Algorithme de Couveignes $\ell$-adique avec approche muti-modulaire à l'aide du TRC.}
\begin{algorithmic}[1]
...
\ENDFOR
\STATE Calcul de $P,Q \in E$ d'ordres $ \mathbb{Z}/\prod_{i=1}^n \ell_i^{k_i}\mathbb{Z}$ tel que $P$ est de direction $(\lambda_1, \dots, \lambda_n)$ et $Q$ de direction $(\mu_1, \dots, \mu_n)$ à l'aide de l'algorithme~\ref{alg:TRC};
\STATE Calcul de $P',Q' \in E'$ d'ordres $ \mathbb{Z}/\prod_{i=1}^n \ell_i^{k_i}\mathbb{Z}$ tel que $P'$ est de direction $(\lambda_1, \dots, \lambda_n)$ et $Q'$ de direction $(\mu_1, \dots, \mu_n)$ à l'aide de l'algorithme~\ref{alg:TRC};
\STATE \label{alg:mult-ell:rep} Calcul des listes de représentants $L_i,L_i'$ des orbites de points d'ordre $\ell_i^{k_i}$ sous l'action de $\mathsf{Gal}(\mathsf{F}_{\prod_{i=1}^n\ell_i^{k_i}}:\mathbb{F}_q)$;
\FOR{$M \in \mathsf{diag}(a,b)$  avec $a,b \in \left( \mathbb{Z}/\prod_{i=1}^n\ell_i^{k_i}\mathbb{Z} \right)^*$}
...
\subsubsection{Construction de compositas d'extensions $\ell_i$-adiques}

\begin{defi}
Soient $P$ un polynômes irréductible de degré $d_p$ de racines $r_i$ pour 
$i \in [1,d_p]$, $Q$ un polynômes irréductible de degré $d_q$ de racines $s_j$,
tels que $P$ et $Q$ aient des racines distinctes et des degrés $d_p,d_q$ 
premiers entre eux, alors on définit $R$ le \emph{produit composé} comme étant 
le polynôme de degré $d_pd_q$ unitaire et de racines $r_is_j$ avec $i \in 
[1,d_p], j \in [1,d_q]$. On note $R= P\bigodot Q$.  
\end{defi}

\begin{prop}[Dire comment on définit une composita de deux tours $\ell_i$-adiques]
Soient $\ell_1, \ell_2$ deux nombres premiers distincts, soient $P$, $Q$ deux 
polynômes irréductibles de degré $d_1\ell_1^{n_1}$ et $d_2\ell_2^{n_2}$ 
...
\cong \mathsf{F}_{(2,n_2)}$ le $n_2$  ème élément de la tour d'extension 
$\ell_1$-adique vu dans \ref{cha:tour}.
 Soit $R=P\bigodot Q$  le produit composé de $P$ et $Q$, alors 
 $\mathbb{F}_q[z]/\langle R \rangle$ est un corps fini de taille 
 $q^{d_1d_2\ell_1^{n_1}\ell_2^{n_2}}$ et est isomorphe à $\mathbb{F}_q[x,y]/
 \langle P,Q \rangle$.
\end{prop}
...
\todo{Corriger un jour les erreurs de biblio....}
Soient $\ell_1, \dots, \ell_n$ $n$ nombres premiers distincts 
On va supposer dans le reste de ce document que 
\begin{defi}
Définir la notation...
On note
\end{defi}

Dire tout d'abord comment on construit un simple composita 

\begin{defi}
Composita de tours d'extensions $\ell_i$-adiques
\end{defi}

\begin{nota}
Soient $\ell_1, \dots, \ell_n$ $n$ nombres premiers distincts, on va noter par 
$\mathbb{K}_m=\otimes_{i=1}^n \mathbb{K}_{d_i\ell_i^{n_i}}$ une extension de corps de 
$\mathbb{F}_q$ de degré $m=\prod_{i=1}^n d_i\ell_i^{n_i}$ (voir par exemple 
\cite{Schost15} pour un contexte), telle que celle-ci soit 
construite de façon compatible avec des plongements dans des sous-corps 
$\mathbb{K}_n \subset \mathbb{K}_m$ avec $n | m$ \todo{retravailler cela pour l'adapter à notre contexte}. De plus $n_i$ est défini pour
 tout $i \in [1,n]$ comme étant le degré de l'extension $\ell_i$-adique tel que
 $E[\ell_i^k] \subset E(\mathbb{K}_{\ell_i^{n_i}})$, en utilisant les notations
 de la proposition~\ref{pro:clas:fro} on a $n_i=k_i-\beta_i$. On va travailler
 avec la construction de composita montrée dans 
 \cite{DeFeoDoliskaniSchost14} qui permet notamment de représenter ses éléments
  sous forme univariée.Enfin afin de simplifier les notations on va 
 noter $m_i=\prod_{i=j}^n\ell_j^{n_j}/\ell_i^{n_i}$.
%$m_i = \overset{n}{\underset{j \neq i}{\underset{j = 0}{\prod \ell_j^{k_j}}}}$
\end{nota}

 On va supposer sans perte de généralité que pour tout $i \in [1,n]$ on a 
 $E[\ell_i] \subset E(\mathbb{F}_q)$ afin de simplifier les raisonnements pour
 l'interpolation à l'aide de l'action de groupe de Galois, le cas où l'on a
 $E[\ell_{i_0}] \not \subset E(\mathbb{F}_q)$ pour $i_0 \in [1,n]$ a déjà été
 traité dans la section~\ref{sec:interpolation}.
 
 Premièrement on peut calculer les points (partiellement) horizontaux sur les 
 volcans de $\ell_i$-isogénies à l'aide des méthodes vues dans le 
 ...
 on utilise donc le résultat suivant issu de \cite[Theorem 1]{DeFeoDoliskaniSchost14}
 \begin{prop}[De Feo, Doliskani, Schost]
 Soient $i \in [1,n] $, $\mathbb{K}_{\ell_i^{n_i}}\cong \mathbb{F}_q[x]/\langle P \rangle$, 
 $\mathbb{K}_{m} \cong \mathbb{F}_q[z]/\langle Q \rangle$ 
 des extensions de corps de $\mathbb{F}_q$ de degré $\ell_i^{n_i}$ 
 et $m$ avec $\ell_i | m$ et $P$ et $Q$ des polynômes irréductibles de degré 
 $\ell_i^{n_i}$ et $m$. Alors le plongement d'un élément de 
 $\mathbb{K}_{\ell_i^{n_i}} $  dans $\mathbb{K}_{m}$ a un coût de 
 $O\ell_i^{n_i}\mathsf{M}(m_i)+m_i\mathsf{M}(\ell_i^{n_i}))$ opérations sur 
 $\mathbb{F}_q$.
 La descente d'un élémént de 
 $\mathbb{K}_{m} $ vers un élément de $\mathbb{K}_{\ell_i^{n_i}} $  a un coût
 identique. 
 \end{prop}
...
\end{proof}  
 On sera aussi ammené à multiplier des polynômes à coefficients dans une 
 composita, ainsi on a besoin du résultat suivant.

 \begin{prop}
 La multiplication et la division euclidienne de polynômes de degré au plus $d$
 à coefficients dans $\mathbb{K}_{n} \cong \mathbb{F}_q[x]/ \langle P \rangle$
 avec $P$ un polynôme irréductible de degré $n$ est de $O(\mathsf{M}(nd))$ 
 opérations sur $\mathbb{F}_q$.
 \end{prop} 
 ...
 coefficients dans $\mathbb{K}_n$ comme un polynôme univarié de degré $2nd$ à 
 coefficients dans $\mathbb{F}_q$ dés lors la multiplication a un coût de 
 $O(\mathsf{M}(nd))$ opérations sur $\mathbb{F}_q$.
 \end{proof}
 
 On sera aussi ammené à trouvé une racine d'un polynôme de degré $d$ à 
 coefficients dans une composita de degré $n$, d'où le résultat suivant:
 
\begin{prop}
 Le coût de résolution  de polynômes de degré $d$ dans une composita de 
 taille $\mathbb{K}_{n} \cong \mathbb{F}_q[x]/ \langle P \rangle$
 avec $P$ un polynôme irréductible de degré $n$ a un coût espéré de 
 $O(n \mathsf{M}(nd)\log(d)\log(dq))$ opérations sur $\mathbb{F}_q$ \todo{trouver complexité bis}.
\end{prop}
...
\begin{prop}
\label{pro:iso:fie}
Soient $P(x),Q(y)$ deux polynômes irréductibles de degré $m,n$ premiers entre 
eux à coefficients dans $\mathbb{F}_q$ tels que $m \leqslant n$, $R=P \bigodot 
Q$ le produit composé de $P$ et $Q$, les corps$\mathbb{F}_q[x,y]/\langle P,Q 
\rangle, \mathbb{F}_q[x]/\langle R \rangle$ alors l'application de 
l'isomorphisme:
\begin{equation*}
\begin{alignedat}{1}
...
\end{alignedat}
\end{equation*}
a un coût de $O(m^2\mathsf{M}(n))$ opérations sur $\mathbb{F}_q$ et 
l'application de son inverse a un coût de $O(\mathsf{M}(mn)n^{1/2}+
\mathsf{M}(m)n^{(\omega+1)/2})$ opérations sur $\mathbb{F}_q$.
\end{prop}

Ainsi on peut travailler avec 

\begin{prop}
Soit $i \in [1,n]$, $P$ un polynôme irréductible de degré $d_i\ell_i$ tel que 
$\mathsf{F}_i=\mathbb{F}_q[x]/\langle P \rangle$, avec $\mathsf{F}_i$ un 
élément de la tour d'extensions $\ell_i$-adiques tel que vu dans le 
chapitre~\ref{cha:tour}, $Q$ un polynôme de irréductible de degré 
$p_j=\prod_{j=1}^{i-1}d_j\ell_j^{k_j}$, alors pour un élément $a$ de 
$\mathbb{K}_{p_j}$ le coût de calcul de $a$ à une puissance $q^{r p_j}$ est de
$O(p_{i+1})$) opérations dans $\mathbb{F}_q$ avec un précalclul qui a un coût 
booléen de $O(\log(rp_j)\log(\ell_i^{n_i}qd_i))$ et  
de $O( M(d_i) d_i\log(q))$ opérations sur $\mathbb{F}_q$.
\end{prop}

\begin{proof}
Par la proposition~\ref{pro:iso:fie} on peut écrire $a$ comme un élément de 
$\mathbb{F}_q[x,y]/\langle P,Q \rangle \cong \mathbb{F}_q[z]/\langle R \rangle$
 avec $P$ un polynôme irréductible de degré  $d_i\ell_i^{n_i}$ tel que $\mathsf{F}_{(i,n_i)}=\mathbb{F}_q[x]/\langle P \rangle=\mathbb{F}_q(x_i)$, 
avec $\mathsf{F}_{(i,n_i)}$ un élément de la tour d'extensions $\ell_i$-adiques tel que
 vu dans le chapitre~\ref{cha:tour}, $Q$ un polynôme de irréductible de degré 
$p_i=\prod_{j=1}^{i-1}d_j\ell_j^{n_j}$ et $R$ le produit composé de $P$ et $Q$.\todo{définir cela quelque part}
Ainsi en réécrivant $a$ de la façon suivante:
\begin{equation*}
a=\sum_{j=0}^{p_i-1}x_i^{j}f_{j}(y) \quad \text{avec } f_{j}(y) \in \mathbb{K}_{p_i}, x_i \in \mathsf{F}_{(i,n_i)}.
\end{equation*}
On a alors l'élévation à la puissance $q^{r p_j}$ de $a$ qui s'écrit:
\begin{equation*}
a^{q^{r p_j}}=\sum_{j=0}^{p_i-1}(x_i^{j})^{q^{r p_j}}f_{j}(y) \quad \text{avec } f_{j}(y) \in \mathbb{K}_{p_i}, x_i \in \mathsf{F}_{(i,n_i)}.
\end{equation*} 
On se concentre donc sur le calcul de $w=x_i^{q^{r p_j}}$ comme abordé dans la 
preuve du théorème~\ref{thm:frob-ell} on note $x_0=x_i^{\ell_i^{n_i}d_i}$. 
Calculons donc $x_i^{q^{r p_j}}$, on écrit $q^{r p_j}= u \ell_i^{n_i}+v$ avec 
$0 \leqslant v<\ell_i^{n_i}$. On a alors $w=x_0^{u \bmod qd_i-1}x_i^{v}$ (on 
...
booléen de $O(\log(rp_j)\log(\ell_i^{n_i}qd_i))$. Le calcul de $x_0^{u \bmod q^d_i-1}$ 
...
On écrit $a^{q^{r p_j}}$ sous la forme:
\begin{equation*}
\sum_{j=0}^{p_i-1}(w)^{j}f_{j}(y) \quad \text{avec } f_{j}(y) \in \mathbb{K}_{p_i}, w \in \mathbb{F}_{q}[x_i].
\end{equation*}
...
successives à l'aide de la précédente en $O(p_j)$ opérations sur 
$\mathbb{F}_q$. On a alors un coût total de $O(d_i\ell^{i}(p_i))$ 
($\mathsf{i.e.}O(p_{i+1})$) opérations dans $\mathbb{F}_q$.

\end{proof}


 
 Enfin pour les étapes concernant les méthodes décrites dans 
 la section~\ref{sec:interpolation}, on a besoin de les détailler pour montrer 
 comment elles s'appliquent à ce contexte. 
  
 Le contexte de notre travail est donc le suivant, soit deux éléments primitifs
  $v,w \in \mathsf{F}_{\prod_{i=1}^n\ell_i^{k_i}}$ on veut alors calculer le 
  polynôme $A \in \mathbb{F}_q[x]$ tel que:
\begin{equation}
A(v)=w \bmod T(v)
\end{equation}
...
minimal de $A$ dans la classe de polynômes $\mathbb{F}_q[x]/ \langle T \rangle$.
 ...
 d'abord $T$. On définit la suite de polynômes suivante $T_i$ d'ordre 
 $\prod_{j=1}^i\ell_i^{k_i}$ et à coefficients définis dans 
 $\mathsf{F}_{\prod_{j=i}^n\ell_j^{k_j}}[x]$. On définit $T^{(1)}(x)=(x-v)$. 
 Supposons que l'on ait calculé un élément $T_i$ alors on définit 
 $T^{(i,j)}=\sigma_{\mathsf{Gal}(j)}(T^{(i)})$ avec $\sigma_{\mathsf{Gal}(j)}$ 
 l'application qui à tous les coefficients du polynôme $T^{(i)}$ applique $j$ 
 fois l'action d'un élément générateur fixé de $\mathsf{Gal}(\mathsf{F}_{
 \prod_{r=i}^n\ell_r^{k_r}}:\mathsf{F}_{\prod_{r=i+1}^n\ell_r^{k_r}})$, 
 on définit alors récursivement $T^{(i)}$ de la façon suivante:
 \begin{equation*}
T^{(i+1)}=\prod_{j=0}^{\ell_i^{k_i}-1}T^{(i,j)}    
\end{equation*}
\todo{Faire un dessin à nouveau !}
 ...
 éléments primitifs de $\mathsf{F}_{\prod_{i=1}^n\ell_i^{k_i}}$. 
 \todo{peut etre utilisé la notation $\mathsf{L}_n$ au lieu de $\mathsf{F}_n$}
 \todo{dire que par souci de simplicité on considére que $E[\ell_i] \subset E(\mathbb{F}_q)$}
 Ainsi  pour faire ces étapes de façon optimisée il est crucial de calculer 
 l'action de $\mathsf{Gal}(\mathbb{F}_q:\mathbb{F}_q)$ à un coût raisonnable.
 Or en l'état actuel celui-ci est de $O()$ \todo{trouver complexité à nouveau}.

\subsubsection{Coût de l'algorithme de Couveignes $\ell_i$-adique avec le théorème des restes chinois elliptique}

 si l'on peut appliquer les méthodes de la 
 section~\ref{sec:interpolation} pour optimiser le calcul de la même manière 
 que dans l'algorithme~\ref{alg:cou:ell-adique}. 
 
 Tout d'abord
 
Ainsi pour s'intéresser à l'application des méthodes de la 
section~\ref{sec:interpolation} il faut s'intéresser à la représentantion des 
points avec lesquels on travaille. Comme dit précédemmennt pour définir des 
points d'ordre $\prod_{i=1}^n\ell_i^{k_i}$ on a besoin de travailler avec des
composita de tours d'extensions $\ell$-adiques. Ainsi une implantation telle 
que celle de l'article~\cite{DeFeoDoliskaniSchost14} permet déjà de représenter
les points horizontaux $P_i,Q_i$ dans la composita, mais le plongement a déjà 
un coût de $O()$\todo{Complexité à compléter}. En plus des coûts de calcul de
résolution de polynôme à coefficients dans la composita qui entrent dans les 
calculs de CRT il reste aussi.

Mais avec une telle approche il faudrait définir les 
points dans un composita de tour d'extensions $\ell$-adiques à la manière de \cite{DeFeoDoliskaniSchost14}. On pourrait dés lors
à l'aide des plongements utiliser les résultats du chapitre~\ref{cha:tour} et 
\ref{cha:act:fro} et travailler avec l 
$\prod_{i=1}^n\frac{\ell_i^{2k_{i}-2}(\ell_i+1)}{4}>r$. 'ensemble de points $S$ isomorphe à: $\oplus_{i=1}^n \mathbb{Z}/\ell_i^{k_i} \mathbb{Z} \times \mathbb{Z}/\ell_i^{k_i} \mathbb{Z} $
dés lors il faudrait fixer comme condition  $\sum_{i=1}^n\frac{\ell_i^{2k_{i}-2}(\ell_i+1)}{4}>r$
et construire le polynôme d'interpolations qui agit sur $S$ de façon similaire 
à celle décrite dans le chapitre~\ref{cha:tour}.
faudrait revoir le coût d'une opération telle que le Frobenius dans un tel 
contexte, certes les méthodes de 
plongement permettent d'utiliser les résultats du chapitre \ref{cha:tour} pour 
une tour d'extensions $\ell$-adiques mais il faut prendre en compte le p
faudrait trouver un autre moyen que celui déjà utilisé pour valider une 
correspondance entre deux bases horizontales de la $\ell_i^{h_i}$ torsion.

une $\ell^k$ torsion par rapport à l'action de la $r$-isogénie  de plus il ne 
faut pas oublier que l'on a la contrainte $k>h$.
...
\begin{rem}
Pour le calcul de chacune des étapes~\ref{alg:TRC:init:gen:div} à \ref{alg:TRC:init:gen:prim} il est bien évidemment recommandé de travailler itérativement avec les produits de nombre premier: $\overset{i}{\underset{j \neq i_0}{\underset{j = 0}{\prod}}}\ell_j^{k_j})$ pour $i$ allant de $1$ à $n$.
\end{rem}
\printnomenclature
