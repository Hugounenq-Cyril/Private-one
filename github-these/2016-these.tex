...
% \STATE Compute $(P_1,Q_1)$, a basis of $E[\ell]$
\STATE{$\lambda \leftarrow 0$; $\mu \leftarrow 0$;$i \leftarrow 0$; $P_0, Q_0 \leftarrow $ deux éléments tels que~$\langle P_0,Q_0 \rangle =  E[\ell]$.}
...
$ \delta \ell^i y^2 + y (\ell^i(a-d)+\lambda - \mu) - c\ell^i $; \textbf{end if}.
\STATE\label{alg:diagonal:upd-P}
