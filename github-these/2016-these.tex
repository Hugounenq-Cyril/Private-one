...
\todo{C'est là qu'on doit parler de la hauteur du volcan}
Soient $E$, $E'$ deux courbes elliptiques de $j$-invariants $j$ et $j'$ reliées par une $r$-isogénie $\phi$, alors pour $k$ plus grand que $h$ la hauteur du volcan de $\ell$-isogénies on peut calculer deux bases diagonales $(P,Q),(P',Q')$ de $E_c[\ell^k]$ et $E'_c[\ell^k]$ les courbes sommets respectives de $E$ et $E'$
\[
...
\begin{proof}
  ...
  empêche d'utiliser notre algorithme~\ref{alg:cou:ell-adique}, 
  ...
  nombres premier de Elkies satisfaisant $\ell^{h} < \sqrt{r}$ dans 
  l'intervalle  $[1,L]$, ainsi dans le pire des cas on a un nombre premier de
  Elkies $\ell \in O(\log(q))$ que l'on peut utiliser dans 
  l'algorithme~\ref{alg:cou:ell-adique}. On a alors la complexité annoncée en 
  remplaçant $\ell$ par $O(\log(q))$.
