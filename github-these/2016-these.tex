...
\begin{defi}
\label{def:p-adic}
Soit $\mathbb{F}_q$ un corps fini sur lequel on définit la tour d'extension $p$-adique $\mathbb{L}_0,  \mathbb{L}_1, \mathbb{L}_2, \cdots \mathbb{L}_i$ telle que $\mathbb{L}_i$ soit la plus petite extension de corps de $\mathbb{F}_q$ dans laquelle on a $E[p^i] \subset E(\mathbb{L}_i)$. On pose $\mathbb{L}_0=\mathbb{F}_q$.
\end{defi}
...
On voit donc à travers cette preuve que si $E[p] \subset E(\mathbb{F}_q)$ alors il est possible que $\mathbb{L}_0=\mathbb{L}_1=\mathbb{L}_2$, de même lorsque l'on a $E[p] \nsubset E(\mathbb{F}_q)$ il est possible que $\mathbb{L}_1=\mathbb{L}_2$, dés lors la distinction suivante est nécessaire.
...
\begin{defi}
Soit $\mathbb{L}_0, \mathbb{L}_1, \mathbb{L}_2, \cdots, $ une tour de \emph{niveau} $p$-adique de $\mathbb{F}_q$ définie comme dans \ref{def:p-adic} alors on définit l'indice de valuation rationnelle $p$-adique de la tour comme étant le plus grand indice $i$ tel que $\mathbb{L}_i \subset \mathbb{F}_q$. On le note $v_p(E(\mathbb{F}_q))$. On définit de la même manière l'indice de valuation $p$-adique de la courbe définie sur $\mathbb{L}_1$ comme étant le plus grand indice $i$ tel que $\mathbb{L}_i \subset \mathbb{L}_1$. On le note $v_p(E(\mathbb{L}_1))$.
\end{defi}
...
En effet l'ensemble des abscisses des points primitifs de $p^k$-torsion est de taille $p^{k-1}(p-1)/2$. La taille d'une orbite sous l'action du Frobenius est quant à elle égale à $[\mathbb{L}_k:\mathbb{L}_1]$ (ou $\frac{[\mathbb{L}_k:\mathbb{L}_1]}{2}$ voir corollaire \ref{cor:orb:fro}). Ainsi si l'on note $f$ la taille de ces orbites et $d$ le nombre de représentants on a $d \times f=p^{k-1}(p-1)/2$.

Détaillons cela, on a vu dans la proposition \ref{pro:ord:fro} que pour tout $i\geqslant 1$ l'extension $L_{i+1}$, dans laquelle est définie $E[p^{i+1}]$, est soit une extension de degré $p$ de $\mathbb{L}_{i}$ soit égale à $\mathbb{L}_{i}$. Ainsi les points d'ordre $p^{v_p(E(\mathbb{L}_1)}$ de $E$ sont par définition de $v_p(E(\mathbb{L}_1))$ définis sur $\mathbb{L}_1$ (qui n'est pas forcément égal à $\mathbb{F}_q)$, on note $d$ le nombre d'abscisses différentes de ces éléments. Dès lors les points d'ordre $p^{v_p(E(\mathbb{L}_1))+i+1}$ avec $i \geqslant 1$ sont définis comme des antécédents par $[p]$ (la multiplication par $p$) d'un point $P_i$ d'ordre $p^{v_p(E(\mathbb{L}_1))+i}$ et chaque point de cet orbite sous l'action du groupe de Galois $[\mathbb{L}_{v_E(p)+i}:\mathbb{L}_{v_E(p)+i-1}]$ est un antécédent de $P_i$ par $[p]$, il y en a donc $p$. Sur la figure \ref{fig:p-tor:fro} on peut voir une répartition des points de $p^{i+v_p(E(\mathbb{L}_1))}$-torsion selon l'action du Frobenius et les différentes orbites. $d$ le nombre de représentants des orbites est égal aux nombres d'abscisses de points d'ordre $p^{v_p(E(\mathbb{L}_1))}:\frac{p^{v_p(E(\mathbb{L}_1))-1}(p-1)}{2}$ et n'est généralement pas égal à $[\mathbb{L}_1: \mathbb{F}_q]$. En effet $\mathbb{L}_1$ est construit comme une extension de $\mathbb{F}_q$ qui contient les points de $p$-torsion de $E$ et donc $d \mid p-1$ par le théorème de Gunji \ref{thm:Gunji}.
...
\begin{cor}
\label{cor:orb:fro}
Soit $k$ tel que que $\mathbb{L}_k \neq \mathbb{F}_q$ avec $\mathbb{L}_k$ défini comme dans \textcolor{red}{ref à mettre}. 
...
\begin{proof}
Comme vu dans la preuve de la proposition \ref{pro:ord:fro} on peut considérer les points d'ordre $p^k$ commme des vecteurs propres pour le Frobenius associés à $t_{\pi}$. Ainsi lorsque  $2 \mid [\mathbb{L}_k:\mathbb{F}_q]$ on a $(t_{\pi}^{[\mathbb{L}_k:\mathbb{F}_q]/2})^2=t_{\pi}^{[\mathbb{L}_k:\mathbb{F}_q]}=1 \bmod p^k$. L'action de $\pi^{[\mathbb{L}_k:\mathbb{F}_q]/2}$ sur la $p^k$ torsion correspond donc à une multiplication par une racine carrée de $1$, or par définition $[\mathbb{L}_k:\mathbb{F}_q]$ est l'ordre de  $t_{\pi}$ dans $(\mathbb{Z}/p^k \mathbb{Z})^*$ et donc $t_{\pi}^{[\mathbb{L}_k:\mathbb{F}_q]/2}=-1 \bmod \mathbb{Z}/p^k \mathbb{Z}$.
\end{proof}
...
Dans l'algorithme original de Couveignes le coût minimal du calcul du polynôme d'interpolation a été sous-estimé. En effet on cherche à calculer un polynôme de degré $\frac{p^{k-1}(p-1)}{2}=\frac{\phi(p^k)}{2}$ à coefficients dans $\mathbb{L}_k$, ainsi ne serait ce que pour représenter ce polynôme on a besoin de $O(p^{2k})$ éléments de $\mathbb{F}_q$ et donc à minima un nombre de $O(p^{2k})=O(\ell^2)$ opérations à effectuer sur $\mathbb{F}_q$. De plus il faut calculer tous les points primitifs de $p^k$-torsion et donc effectuer $p^k$ additions de points de courbes elliptiques définis sur une extension de taille $[\mathbb{L}_k:\mathbb{F}_q] \in O(p^k) $, ce pré-calcul a un coût de $O(p^k\mathsf{M}(p^k)\log(p^k))$. Il faut donc utiliser des méthodes supplémentaires pour optimiser ce temps de calcul. Les travaux de \cite{DeFeo11} ont donc résolu cette question et seront présentés à \textcolor{red}{ref à mettre}.
...
\begin{enumerate}
\item $[\mathbb{F}_q :\mathbb{U}_0]= d$
...
Ainsi on va lier cette défintion de tours $p$-adiques à celle évoquée plus tôt définié à l'ide de la $p$-torsion de la courbe elliptique $E$ que l'on étudie.

\begin{rem}
Ces tours $p$-adique sont donc les constructions utilisées pour les tours de niveaux.
En effet on pose $\mathbb{L}_1=\mathbb{U}_1$, et dés lors pour tout $i\geqslant$ on pose $\mathbb{U}_{i+1}=\mathbb{L}_{v_p(E(\mathbb{L}_1))+i}$.
\end{rem}

A partir de maintenant on va utiliser uniquement la notation $\mathbb{U}_i$ car celle-ci correspond au pire des cas, celui où $v_p(E(\mathbb{L}_1))=1$ et $v_p(E(\mathbb{F}_q))=0$.
...
On va donc voir dans cette section comment à partir d'un polynôme d'interpolation défini uniquement à l'aide de représentants de l'orbite selon l'action du Frobenius on calcule efficacement le polynôme d'interpolation sur toutes les orbites. C'est d'ailleurs l'une des raisons pour travailler uniquement avec des points primitifs de $p^k$-torsion puisque que eux seuls sont définis dans $\mathbb{U}_k$ (on s'est placé dans le cas où $\mathbb{L}_k = \mathbb{U}_k$). Bien évidemment on peut généraliser cette méthode à tous les points de $p^k$-torsion en utilisant le théorème des restes chinois, mais d'un point de vue pratique prendre $k$ tel que $p^{k-1}(p-1) > 4 \ell +1$ est suffisant, asymptotiquement le résultat reste le même.

Soient $P$ et $P'$ deux points primitifs de $p^k$-torsion de $E$ et $E'$. On veut donc calculer le polynôme d'interpolation de degré minimal $A \in \mathbb{F}_q[X]$ tel que:
\begin{equation}
A(x([n]P))=x([n]P') \bmod T(x) \quad \text{pour } n \in \left( \mathbb{Z}/p^k\mathbb{Z} \right)^*
\end{equation} 

avec $T(X)= \prod_{n \in \left( \mathbb{Z}/p^k\mathbb{Z} \right)^*} (X-x([n]P))$. Il est à noter que le souhait d'avoir $A$ minimal signifie qu'il faut aussi prendre en compte le fait que pour $n \in \left( \mathbb{Z}/p^k \mathbb{Z} \right)^*$ on a $x([n]P)=x([-n]P)$ et $x([n]P')=x([-n]P')$. 


Comme dit précédemment comme on cherche à calculer une fraction rationnelle définie sur $\mathbb{F}_q$, ainsi le polynôme d'interpolation que l'on veut calculer est défini sur $\mathbb{F}_q$. 
...
\begin{equation*}
T(X)=\prod_{i=1}^rT^{(i)}(X)
\end{equation*}
avec $\deg(T_i)=f=[\mathbb{U}_k:\mathbb{F}_q]$ (ou $f=[\mathbb{U}_k:\mathbb{F}_q]/2$ voir corollaire \ref{cor:orb:fro}) et $rf=p^{k-1}(p-1)/2$.

Ainsi on travaille modulo chacun de ces $T^{(i)}$ et l'on définit:
...
\begin{enumerate}
\item le polynôme minimal $T$ de $x(P)$ défini sur $\mathbb{F}_q$ de degré égal à $f$,
\item le polynôme d'interpolation $L$ défini sur $\mathbb{F}_q$ de degré strictement inférieur à $f$ tel que $L(x(P))=x(P')$.
\end{enumerate}
...
\end{equation*}
L'information de l'image de $x(P) \in \mathbb{U}_k$ entraîne donc la connaissance des images des points de l'orbite de $x(P)$ sous l'action de n'importe quel élément de $\mathsf{Gal}(\mathbb{U}_k:\mathbb{F}_q)$. On va donc se servir de l'action du Frobenius sur le polynôme d'interpolation défini uniquement pour un représentant de l'orbite (en l'occurence $L(x(P))=x(P')$) pour déterminer $L$ alors que les méthodes classiques \cite[10.1-2]{vzGJG03} préconisent un arbre de produit binaire avec la connaissance des $x([n]P)$ avec $n \in (\mathbb{Z}/p^k\mathbb{Z})*$. 

Montrons donc comment l'on tire avantage de l'action du Frobenius pour la construction d'un tel polynôme d'interpolation, cette idée a donc été utilisé dans \cite{DeFeo11}, et auparavant dans \cite{enge2008}.
