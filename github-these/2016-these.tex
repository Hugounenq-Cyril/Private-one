...
Comme on souhaite travailler distinctement sur les différentes orbites selon l'action du Frobenius, on regarde donc la factorisation de $T(X)$ sur $\mathbb{F}_q$ comme préconisé dans \cite{Couveignes96}:

\begin{equation*}
T(X)=\prod_{i=1}^fT^{(i)}(X)
\end{equation*}
avec $\deg(T_i)=e=[\mathbb{U}_k:\mathbb{F}_q]$ (ou $e=[\mathbb{U}_k:\mathbb{F}_q]/2$ voir corollaire \ref{cor:orb:fro}) et $ef=p^{k-1}(p-1)/2$.

Ainsi à l'aide du théorème des restes chinois on réduit le problème au calcul de chacun des $T^{(i)}$ et l'on définit:

\begin{equation}
A^{(i)}=A \bmod{T^{(i)}}.
\end{equation}

Une fois chacun des $A^{(i)}$ déterminés on calcule $A$ avec le théorème des restes chinois, dés lors on va se concentrer sur le calcul de $A^{(0)}$.
...
\begin{enumerate}
\item le polynôme minimal $T$ de $x(P)$ défini sur $\mathbb{F}_q$ de degré égal à $e$,
\item le polynôme d'interpolation $L$ défini sur $\mathbb{F}_q$ de degré strictement inférieur à $e$ tel que $L(x(P))=x(P')$.
\end{enumerate}

On ne va ici que donner les idées directrices pour calculer $T$, les idées développées par \cite{DeFeo11} étant reprises très largement dans la section \textcolor{red}{ref à mettre plus tard} elles seront plus abordées dans cette section.
...
\end{equation*}
L'information de l'image de $x(P) \in \mathbb{U}_k$ entraîne bien la connaissance des images des éléments de l'orbite de $x(P)$ sous l'action de n'importe quel élément de $\mathsf{Gal}[\mathbb{U}_k:\mathbb{F}_q]$. 
...
Montrons donc comment l'on tire avantage de l'action du Frobenius pour la construction d'un tel polynôme d'interpolation, cette idée a été utilisé dans \cite{DeFeo11}, et auparavant dans \cite{enge2008}.

Soit $P$ un point d'ordre $p^k$ alors $x(P) \in \mathbb{U}_k \setminus \mathbb{U}_{k-1}$ par définition de $\mathbb{U}_k$. On a alors pour tout $n \in \left( \mathbb{Z}/p^k\mathbb{Z} \right)^*$ $x([n]P) \in \mathsf{orb}(x(P))$. On a donc:
\begin{equation*}
T^{(0)}(x)=\prod_{x([n]P) \in \mathsf{orb}(x(P))}(x-x([n]P))=\prod_{i=0}^{d_1p^{k-1}-1}(x-\pi^i(x((P))))=\prod_{i=0}^{d_1p^{k-1}-1}\pi^i(x-x(P))
\end{equation*}
On introduit quelques notations pour permettre un calcul efficace de $T$ à partir de $x-x(P)$.
...
\[
T_0^{(i+1)}=\prod_{j=0}^{b} \sigma_i^j \quad  b = \begin{cases}
    p-1 &\text{if $i<k-1$,}\\
    d_1 &\text{otherwise.}
  \end{cases}
\]
 On voit alors que $T_0^{k}=T_0$ le polynôme minimal de $x(P)$ sur $\mathbb{F}_q$.

Ainsi si l'on représente à l'aide d'un arbre binaire le calcul de $T_0$, on a sur la figure en rouge, les seuls éléments utilisés lors du calcul de $T_0$.
