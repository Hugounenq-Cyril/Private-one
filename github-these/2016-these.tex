...
$O(n p_{n}\mathsf{M}(p_{n}\ell_{n})\max_i(\log(\ell_{i+1})\log(\ell_{i+1}q)))$ 
opérations sur $\mathbb{F}_q$.
\end{prop}
...
de généralités que l'on a $i_0=1$. On décompose cette étape en $n-1$ 
...
récurrence $P^{(i)}_{j+1}=\mathsf{divise}(P^{(i)}_{j},\ell_{i+1})$, ce qui nous
 ...
 $\mathsf{divise}(P^{(i)}_{j},\ell_{i+1})$ est 
 $R(\ell_{i+1},p_id_{i+1}\ell_{i+1}^{i+1})$ c'est à dire 
 $O(p_i d_{i+1} \ell_{i+1}^{j+1}\mathsf{M}(p_i d_{i+1} \ell_{i+1}^{j+2})\log(\ell_{i+1})\log(\ell_{i+1}q))$
 par la proposition~\ref{pro:rac:com}. Ainsi par superlinéarité de 
 $\mathsf{M}$ le coût de calcul de $P^{(i+1)}$ est de 
 $O(p_{i+1} \mathsf{M}(p_{i+1} \ell_{i+1})\log(\ell_{i+1})\log(\ell_{i+1}q))$.
 On majore ce coût par $O(p_{n}\mathsf{M}(p_{n}\ell_{n})\max_i(\log(\ell_{i+1})\log(\ell_{i+1}q)))$
...
$O(n^2 p_{n}\mathsf{M}(p_{n}\ell_{n})\max_i(\log(\ell_{i+1})\log(\ell_{i+1}q)))$ 
opérations sur $\mathbb{F}_q$.
\end{prop}
...
$\ell_i$-adiques est majoré par les coûts données par la 
...
$O((\sum_{i=1}^n k_i\ell_i^{k_i-\beta_i})n p_{n}\mathsf{M}(p_{n}\ell_{n})\max_i(\log(\ell_{i+1})\log(\ell_{i+1}q)))$ 
