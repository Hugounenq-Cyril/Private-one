...
niveau $h-e$ alors la première étape de l'algorithme, celle où l'action du 
...
Ensuite on passe à la partie où l'on calcule à chaque boucle une base de la $\ell^i$ torsion dans laquelle l'action du Frobenius est triangulaire, et l'on remonte d'un niveau dans le volcan et l'on trigonalise à nouveau la $\ell^i$ torsion. Cette étape est répétée du niveau $h-e$ où l'on exprime l'action du Frobenius dans une base de la $\ell^{h-e+1}$ torsion au niveau $h$ où l'on exprime l'action du Frobenius dans une base de la $\ell^{h+1}$ torsion. Il est important de noter qu'à chacune des itérations de cette boucle les points avec lesquels on travaille sont toujours définis dans le même corps $\mathsf{F}_{h+1-\beta}$ \todo{faire un lemme sur cela ????}.
Lors de la boucle $h-e+i$, les étapes \ref{alg:ult:trigonal:solve1} et \ref{alg:ult:rem:solve1} sont résolues à l'aide d'une recherche exhaustive et ont un coût négligeable par rapport aux autres opérations. Lors de la boucle $h-e+i$ les points $P_{h-e+i+1}^{(h-e+i)}$ et $Q_{h-e+i+1}^{(h-e+i)}$ situés sur une courbe au niveau $h-(e-i)$ sont d'ordre $\ell^{h-e+i+1}$ et définis dans $\mathsf{F}_{h-(e-i)+1-\beta+(e-i)}=\mathsf{F}_{h+1-\beta}$. En particulier par la structure des volcans réguliers (proposition \ref{pro:niv:str}) les antécédents de $P_{h-e+i+1}^{(h-e+i)}$ d'ordre $\ell^{h+1}$ sont définis dans  $ \mathsf{F}_{h+1-\beta}$ or comme cette boucle s'arrête lorsque l'on atteint la $\ell^{h+1}$ torsion pour la courbe au niveau du cratère, les opérations $\mathsf{divise}, \pi(P')$ et $\pi(P')=[u]P'+[v]Q'$ sont faites dans $ \mathsf{F}_{h+1-\beta}$. De même pour les points $Q,Q'$ car à l'étape \ref{alg:ult:rem:divide} on transporte la base sur une courbe située un niveau au-dessu dans le cratère et dès lors les points de $\ell$ divions de $\widehat{\varphi}(Q_{h-e+i+1}^{(h-e+i)})$ sont aussi définis dans $ \mathsf{F}_{h+1-\beta}$. Ainsi le coût des étapes \ref{alg:ult:rem:divide} et \ref{alg:ult:trigonal:divide} est de $R(h+1-\beta)$, le coût de calcul de $\pi(P)$ est de $O(\ell^{h-\beta}M(\ell))$. L'écriture de $\pi(P)=[u]P+[v]Q$ aux étapes \ref{alg:ult:rem:frobenius} \ref{alg:ult:trigonal:frob} a un coût de $O(\ell^2 M(\ell^{h+1-\beta}))$. Le coût étant le même pour chaque boucle on a un coût total pour cette boucle de trigonalisation de $O(e(R(h+1-\beta)+\ell^{h-\beta}M(\ell)+\ell^2 M(\ell^{h+1-\beta})))$.

Pour la dernière boucle par une analyse similaire à celle faite dans la preuve de la proposition \ref{pro:alg:diagonal}, cette boucle a un coût de $O(\mathsf{R}(k-\beta) + \ell^2\mathsf{M}(\ell^{k-\beta}) )$ opérations sur $\mathbb{F}_q$.
...
Ce chapitre reprend les travaux effectués en collaboration avec Luca De Feo,
...
Dans ce chapitre on s'intéresse au calcul d'un polynôme d'interpolation défini
 sur $\mathbb{F}_q$ entre deux éléments $u,v \in \mathsf{F}_i$ avec 
 $\mathsf{F}_i$ un corps de la tour d'extension présentée dans le  chapitre 
 \ref{cha:tour} et définies dans la définition \ref{def:tour-ell}.
...
Le contexte de notre travail est le suivant, on travaille sur une tour 
d'extensions $\ell$-adiques telle que $\mathbb{F}_q=F_0 \subset F_1 \subset 
\cdots \subset F_n$, avec $[F_1:F_0]\mid(\ell-1)$, et $[F_{i+1}:F_i]=\ell$ pour
 tout $i>0$. Soit alors deux éléments $v,w \in F_n\setminus F_{n-1}$ on veut 
 alors calculer les polynômes $T$ et $L$:
 \begin{itemize}
\item $T \in \mathbb{F}_q[x]$ est le polynôme minimal de $v$, de degré
  $d=\deg T<\ell^n$;
\item $L \in \mathbb{F}_q[x]$, de degré inférieur à $d$, tel que $L(v)=w$.
\end{itemize}
Comme $v,w \not \in F_{n-1}$, on a forcément $v_\ell(d)=n-1$,ainsi $\ell^{n-1}
 \leqslant d<\ell^n$. L'utilisation d'un algorithme d'interpolation rapide 
 tel que  ceux présentés dans \cite[Chapter~10.2]{vzGG} nous donnerait un temps
 de calcul de  $O\bigl(n\mathsf{M}(\ell^{2n})\log(\ell)\bigr)$ opérations sur 
 $\mathbb{F}_q$. Nous pouvons obtenir de meilleurs résultats en utilisant la 
 forme de la tour d'extensions $\ell$-adiques, et en particulier l'algorithme 
 de calcul du Frobenius donné dans le théorème~\ref{thm:fro-ell}.
...
\begin{lem}\label{lemma:interpolation:minpoly}
  The cost of computing $T$ is $O(n\mathsf{M}(\ell^{n+1})\log(\ell))$
...
\begin{proof}
  ...
  polynomial $T^{(i,j)}$ is of $O(\ell^i\ell^{n-i-1}\mathsf{M}(\ell))$ operations,
  \emph{i.e.} $O(\ell^n\mathsf{M}(\ell))$ for all $O(\ell)$ of them.
  From the $T^{(i,j)}$'s we compute $T^{(i+1)}$ using a subproduct
  tree, as in~\cite[Lemma~10.4]{vzGG}. The result has degree
  $O(\ell^{i+1})$ and coefficients in $F_{n-i}$, thus the overall cost is
  $O(\mathsf{M}(\ell^{n+1})\log(\ell))$. After $T^{(i+1)}$ is computed this way, we
...
\begin{prop}
  Given $v,w\in F_n\setminus F_{n-1}$, the cost of computing the
  minimal polynomial $T\in \mathbb{F}_q[x]$ of $v$ and the interpolating
  polynomial $L\in \mathbb{F}_q[x]$ such that $L(v)=w$ is $O(n\mathsf{M}(\ell^{n+1})\log(\ell))$
  operations in $\mathbb{F}_q$.
\end{prop}
\begin{proof}
  ...
  polynomial of degree $O(\ell^{n-i+1})$ by one of degree $O(\ell^{n-i})$ in
  $F_i[x]$. Using the complexities from Section~\ref{sub:towers} we
  deduce that each division can be done in time $O(\mathsf{M}(\ell^{n+1}))$, for
  a total of $O(n\mathsf{M}(\ell^{n+1}))$ operations. Then, computing
  $w' = w/T'(v)$ takes $O(\mathsf{M}(\ell^n)\log(\ell^n))$ operations.

  Finally, at each step $i$, the polynomials $L^{(i,j)}$ are computed
  at a cost of $O(\ell^n\mathsf{M}(\ell))$, as in the proof of
  Lemma~\ref{lemma:interpolation:minpoly}.  The computation of
  $L^{(i+1)}$ uses the same subproduct tree as for the computation of
  $T^{(i)}$, requiring $O(\log \ell)$ additions, multiplications and
  divisions of polynomials of degree $O(\ell^{i+1})$ with coefficients in
  $F_{n-i}$, for a total of $O(\mathsf{M}(\ell^{n+1})\log(\ell))$. Summing over all
  $i$, the complexity statement follows readily.
\end{proof}

We end with the general problem of interpolating a polynomial in
$\mathbb{F}_q[x]$ at points of $F_n$.
...
\begin{prop}\label{prop:interpol}
  ...
  \begin{itemize}
  \item $T\in \mathbb{F}_q[x]$ of degree $t$ such that $T(v_i)=0$ for all $i$,
    and
  \item $L\in \mathbb{F}_q[x]$ of degree less than $t$ such that $L(v_i)=w_i$ for
    all $i$
  \end{itemize}
  can be computed using
  $O\bigl(\mathsf{M}(t)\log(s) + n\mathsf{M}(\ell^2t)\log(\ell)\bigr)$ operations in $\mathbb{F}_q$.
\end{prop}
\begin{proof}
  The polynomial $T$ is simply the product of all the minimal
  polynomials $T_i$. Let $n_i=v_\ell(t_i)$, so that
  $v_i,w_i\in F_{n_i+1}\setminus F_{n_i}$, and $\ell^{n_i}\leqslant t_i<\ell^{n_i+1}$.
  ...
  cost of $O(n\mathsf{M}(\ell^{n_i+2})\log(\ell))$ operations. Bounding $\ell^{n_i}$ by
  $t_i$, summing over all $i$, and using the superlinearity of $\mathsf{M}$,
  we obtain a total cost of $O(n\mathsf{M}(\ell^2t)\log(\ell))$ operations.
  ...
  $O(\log (s))$, and requires $O(\mathsf{M}(t))$ operations per level. Thus
  we can bound the cost of computing $T$ by $O(\mathsf{M}(t)\log(s))$.
  ...
  the polynomial $L$ at the same cost $O(\mathsf{M}(t)\log(s))$.
\end{proof}
