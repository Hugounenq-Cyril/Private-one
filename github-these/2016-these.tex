...
Cette partie s'appuie sur la construction de composita de 
\cite{DeFeoDoliskaniSchost14} et montre quelques résultats sur une telle 
construction appliquée à des tours d'extensions $\ell_i$-adiques.
...
d'extensions $\ell_i$-adiques définies comme dans le chapitre~\ref{cha:tour} 
dont on note les éléments $\mathbb{F}_q \subset \mathsf{F}_{(i,0)} \subset \dots \subset  \mathsf{F}_{(i,j)}$ ont chacune des degrés $d_i=[\mathsf{F}_{(i,0)}:\mathbb{F}_q]$ distincts.
...
$\mathsf{F}_{(1,n_1)}, \dots \mathsf{F}_{(n,n_n)}$.
...
\todo{dire ici comment on contourne cette difficulté}
\begin{defi}
\label{def:con:com}
On note $\mathbb{K}_1$ le corps isomorphe à $\mathsf{F}_{1,n_1}$. Soit 
$i \in [1,n]$ supposons que $\mathbb{K}_{i}$ est défini alors par récurrence à 
l'aide de la proposition~\ref{pro:init:com} $\mathbb{K}_{i+1}$ le 
corps défini comme le composita de $\mathbb{K}_i$ et $\mathsf{F}_{(i+1,n_{i+1})}$.
\end{defi}
\begin{rem}
Ici le cas où l'on aurait $d_i=d_j$ pour $i<j$ entraîne dés lors que l'on a pas
les conditions pour appliquer la proposition~\ref{pro:init:com}. \todo{fixer cela 
regarder autre article de Luca} 
En pratique il est raisonnable de penser que l'on a pas $d_i=d_j=1$ car cela 
voudrait dire que l'on a $E[\ell_i \ell_j] \subset E(\mathbb{F}_q)$ \todo{reflechir a cela}
\end{rem}

(voir par exemple 
\cite{Schost15} pour un contexte) \todo{surement plus utile}
\begin{nota} Afin de simplifier les notations on va 
 noter $m_i=\prod_{i=j}^n\ell_j^{n_j}/\ell_i^{n_i}$. \todo{encore utile ?}
...
qui définit l'extension de degré $\prod_{i=1}^{n-1}d_i\ell_i^{k_i}$ de 
...
définition~\ref{def:con:com}, on doit donc calculer $n$ constructions de 
composita. 
