...
\begin{proof}
Le cout de calcul de la $p$-torsion nécessite tout d'abord, d'après les formules de Gnuji de calculer $c$ et $\tilde{c}$ racines $p-1$ ème de $H_{E}$ et $H_{E'}$. On doit ensuite construire les corps $\mathbb{f}_q[c]$ et $\mathbb{F}_q[\tilde{c}]$, dans le pire des cas $\mathbb{F}_q[c]$ et $\mathbb{F}_q[\tilde{c}]$ sont des extensions de degré $p-1$ ainsi on aura besoin jusqu'à $\theta(p-1)$ éléments de $\mathbb{F}_q$ pour représenter de éléments de $\mathbb{F}_q[c]$ et $\mathbb{F}_q[\tilde{c}]$. Le couût le plus important des formules de Gunji étant le calcul des déterminants d'une matrice $\frac{p-1}{2} \times \frac{p-1}{2}$ avec quatres diagonales non nulles cela a donc un coût de $\theta(p^2)$ opérations sur $\mathbb{F}_q[c]$ par élimination de Gauss ce qui représente plus de $\Omega(p^3)$ opérations sur $\mathbb{F}_q$ .
\end{proof}
...
\begin{proof}
Une preuve peut être trouvée dans
\end{proof}
