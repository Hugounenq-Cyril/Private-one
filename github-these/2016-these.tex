...
\begin{rem}
Le calcul de point horizontal d'ordre $\ell^k$ aurait pu être fait en calculant une base diagonale de la base de la $\ell^{k+h}$ , mais alors on aurait été obligé de travailler sur une extension $\ell$-adique plus grande et l'on aurait alors obtenu une complexité trop élevée: . \todo{compléter complexité} 
\end{rem}
...
\end{matrix} \right).
\]
Comme la $r$-isogénie $\phi$ commute avec le Frobenius on devrait avoir $\phi(P) \in \langle P' \rangle $ et $\phi(Q) \in \langle Q' \rangle $, or la matrice $\pi$ est scalaire modulo la $\ell^{h}$ torsion ainsi la représentation de $\phi$ dans les bases $(P,Q)$ et $(P',Q')$ n'est pas diagonale:
\[
\left(
\begin{matrix}
P' \\
...
\todo{faire un résultat uniquement sur $\ell \in \log(q)$)}
\begin{prop}
...
Soit une courbe $E$, on cherche le plus petit nombre premier de Elkies qui 
  ...
  En appliquant le résultat de~\cite[Theorem~1]{shparlinski2014distribution},
...
\begin{prop}
Le coût de trouver un nombre premier $\ell$ de Elkies dans l'intervalle  
$[1,L]$ tel que $\ell^h < \sqrt{r}$ pour $L \in O(\log(q))$ est majoré par:
$O(\log(q)^6)$.
\end{prop}
...
sommet du volcan de $\ell$-isogénies à l'aide de $\ell h^2$ 
  factorisations du $\ell$-ème polynôme moduaire. Or $h$ est plus petite que $\log(q)$. Ces algorithmes 
utilisent $\ell h^2$ 
  factorisations du $\ell$-ème polynôme moduaire, le coût de calcul de celui-ci
  est de $O(\ell^3\log(\ell))$ sa factorisation de 
  $O(\mathsf{M}(\ell)\log(\ell)\log(\ell q))$ par l'algorithme de 
  Cantor-Zassenhaus décrit dans \cite[Chapter~14.5]{vzGJG03}. Le coût de test 
  des différents $\ell$ avant d'en trouver un permettant d'utiliser 
   l'algorithme~\ref{alg:cou:ell-adique} est donc borné par $O(\log(q)^6)$. Ces 
   algorithmes sont moins efficace que ceux de
  \cite{Ionica-Joux10} et \cite{MiretMRV05} mais ils ont l'avantage de 
  fonctionner sur tous types de volcans non réguliers compris. 
\end{proof}

\todo{faire un résultat sur le coût de trouver un bon $\ell$}
...
\begin{proof}
  ...
  de Elkies dépasse $\Omega(\sqrt{q})$. On enlève alors les nombres premiers 
  de Elkies $\ell \leqslant L$ tels que $\ell^{h} > \sqrt{r}$, cette condition
  empéchant d'utiliser notre algorithme~\ref{alg:cou:ell-adique}, 
  pour de tels nombres premiers ceux-ci sont des diviseurs de $d_{K}$ et leur 
  produit est donc borné par $O(\sqrt{q})$. Ainsi il reste suffisamment de 
  nombres premiers de Elkies satisfaisant $\ell^{h} < \sqrt{r}$ dans 
  l'intervalle  $[1,L]$, dans le pire des cas on a donc un nombre premier de
  ...
  Il reste encore à prendre en compte le coût de calcul d'identification d'un
  tel nombre de Elkies satisfaisant $\ell^{h}< \sqrt{r}$, ainsi à l'aide des
  algorithmes décrits dans \cite{FouquetMorain02} et abordés dans la 
  sous-section~\ref{sub:alg:FM} on peut faire cela. Ces 
  algorithmes utilisent $\ell h^2$ 
  factorisations du $\ell$-ème polynôme moduaire, le coût de calcul de celui-ci
  est de $O(\ell^3\log(\ell))$ sa factorisation de 
  $O(\mathsf{M}(\ell)\log(\ell)\log(\ell q))$ par l'algorithme de 
  Cantor-Zassenhaus décrit dans \cite[Chapter~14.5]{vzGJG03}. Le coût de test 
  ...
  \cite{Ionica-Joux10} et \cite{MiretMRV05} mais ils ont l'avantage de 
  fonctionner sur tous types de volcans non réguliers compris. 
  On a alors la complexité annoncée en 
