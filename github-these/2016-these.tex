...
\STATE Calcul de $(P,Q),(P',Q')$ bases diagonales de $E_s[\ell^k],E'_s[\ell^k]$ à l'aide l'algorithme \ref{alg:ulti:fro};
...
\ENDIF
\STATE Calcul des liste de représentants $L,L'$ des orbites des points d'ordre $\ell^k$ sous l'action de $\pi$;
\FOR{$M \in \mathsf{diag}(a,b)$  avec $a,b \in \left( \mathbb{Z}/\ell^{k}\mathbb{Z} \right)^*$}
\STATE Calcul des des polynômes $A$ et $T$ par la méthode décrite dans la proposition~\ref{prop:interpol}section \ref{sec:interpolation});
\STATE Calcul de la fraction rationnelle $F=A \bmod T$ à l'aide d'une interpolation de Cauchy;
\IF{$\mathsf{Test}(F)==Faux$}
\STATE $A$
\ENDIF
\STATE{$\lambda \leftarrow \lambda + \ell^{i} (a + bx)$; $\mu \leftarrow \mu + \ell^{i} (d + cy)$.}
\RETURN $(P_{k},Q_{k},\lambda,\mu).$
