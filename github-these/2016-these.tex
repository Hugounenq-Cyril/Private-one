...
\begin{prop}
Soit $K$ un corps quadratique imaginaire soit $\mathcal{O}_K$ l'anneau des entiers de $K$ alors $\mathcal{O}_K$ est l'ordre maximal de $K$.
\end{prop}

\begin{proof}
En effet soit $\mathcal{O}$ un ordre de $K$ et soit $\alpha \in \mathcal{O}$. Comme $\mathcal{O}$ est sous-anneau de $K$, on a donc $\alpha \in K$. Comme $\mathcal{O}$ est un $\mathbb{Z}$-module libre de rang 2 alors c'est un entier algébrique. On a donc montré que tout ordre $\mathcal{O}$ de $K$ était inclus dans $\mathcal{O}_K$.
\end{proof}
...
\begin{lem}
Soit $\mathcal{O}$ un ordre dans un corps quadratique $K$ de discriminant $d_K$. Alors $\mathcal{O}$ a un indice fini dans $\mathcal{O}_K$. Ainsi si l'on note: $f= [\mathcal{O}_K : \mathcal{O}]$, alors on a 
\[ \mathcal{O} = \mathbb{Z} + f\mathcal{O}_K = [1, f\omega_K]  \]
\end{lem}

\begin{proof}
Dans le livre de Cox \cite{Cox89} lemme 7.7.2
\end{proof}

On peut alors définir une relation d'ordre sur les ordres:
\begin{defi}
Soit $\mathcal{O}$ un ordre de $K$, l'indice $f=[\mathcal{O}_K : \mathcal{O}]$ est appelé le conducteur de $\mathcal{O}$.
\end{defi}

\begin{rem}
On peut donc associer à tout ordre un entier: son conducteur. On peut aussi par conséquent définir une relation d'ordre sur les conducteurs. En effet si on a $m \mid n$ alors on a l'ordre $n\mathcal{O}_K \subset m\mathcal{O}_K$.
\end{rem}

C'est cette idée que l'on va retrouver dans les volcans de $l$-isogénies en essayant de former une "chaîne" d'anneaux d'endomorphismes $\mathcal{O}_1 \rightarrow \mathcal{O}_2 \rightarrow ... \rightarrow \mathcal{O}_s$ avec $l || [\mathcal{O}_i : \mathcal{O}_{i+1}] $ 
\\
\\
...
\end{array} 
\right) \right)^2 \] 
Le discriminant ne dépendant pas de la base, on a alors avec la base de $\mathcal{O}$ qui est  $[1, f \omega_K]$:
\[D=f^2d_K\]
\end{defi}



\section{Détection de courbes sur le cratère}
