...
\nomenclature{$\mathbb{K}_{j}$}{Le compositum isomorphe à $\otimes_{i=1}^{j}\mathsf{F}_{(i,n_i)}$}
...
calculé en $\tilde{O}(r^3)$ opérations sur $\mathbb{F}_q$ d'après \cite{Enge09} \cite[Algorithm 6.1, Theorem 1]{BLS12}. \todo{verifier que tilde est défini + se rappeler cette seconde source}
...
$E[p^k]$.
\todo{Regarder si reconstruction rationnelle est définie si ce n'est pas le cas le faire dans BMSS}
...
Sur la figure \ref{fig:p-tor:fro} on peut voir une répartition des abscisses des points d'ordre $p^{i+v_p(E(\mathbb{L}_1))}$ selon l'action du Frobenius et les différentes orbites. $f$ le nombre de représentants des orbites sous l'action de $\mathsf{Gal}(\mathbb{L}_k:\mathbb{L}_1)$ est égal aux nombres d'abscisses de points d'ordre $p^{v_p(E(\mathbb{L}_1))}:\frac{p^{v_p(E(\mathbb{L}_1))-1}(p-1)}{2}$, et est généralement égal à $\frac{p^{v_p(E(\mathbb{L}_1))-1}(p-1)}{2d_1}$ si l'on a pas $2 | [\mathbb{L}_1:\mathbb{F}_q]$. \todo{?!?!?!}
...
\[
E(\mathbb{F}_{q^r}) \cong \frac{\mathrm{End}_{\mathbb{F}_{q^r}}(E)}{\pi^r-1}
\] 
\todo{Dans le cas ordinaire $\pi$ n'est jamais dans $\mathbb{Z}$, et
  $End(E)=End_{\mathbb{F}_{q^r}}(E)$. Tu peux largement simplifier
  l'énoncé.}
\end{prop}
...
\label{cha:act:fro}
Dans cette partie nous allons étudier l'action du 
Frobenius sur le module de Tate $\mathsf{T}_{\ell}(E)$, et voir comment cette action 
détermine des directions dans le volcan de $\ell$-isogénies. \todo{Justifier 
l'intérêt de cela} Ceci peut permettre, par exemple dans le cas Elkies (défini 
plus bas), de savoir rester sur le cratère et, par exemple, de calculer
le nombre de classes \todo{classes de qui?} en parcourant le cratère. Cela peut permettre aussi d'atteindre et 
...
\todo{cas du ordinaire à mettre partout ? Non. Il suffit de le dire une fois au début.}
\begin{defi}
\label{def:cas:elk}
...
\todo{je ne trouve pas la définition de $d_\pi$ dans le document. Rappeler?}
$d_{\pi}$ est un résidu quadratique si et seulement si $\left( 
...
\begin{prop}
\label{pro:hau:vol}
Soit $E$ une courbe elliptique, $\ell$ un nombre premier de Elkies, alors 
...
tel que \todo{\textbackslash{}mathsf\{T\} ou T?} $\pi|T_{\ell}(E)$~est conjugée, sur~$\mathbb{Z}_{\ell}$,
à la matrice 
\begin{equation*}
...
l'intérieur d'un volcan de $\ell$-isogénies en étudiant l'action du Frobenius 
...
\caption{\label{fig:elk:dif:niv:mat} Différentes formes de matrices du Frobenius à conjugaison près selon le niveau dans le volcan d'isogénies.}
\end{figure}
...
\todo{Comment R engendre-t-il un réseau???}
engendré par $R$, soit $\Pi$ la matrice du Frobenius sur $T_{\ell}(E)$, alors la 
...
\todo{Pourquoi donner un nom ($\Pi'$) à cette matrice?}
...
E_s$ d'ordre $2^3$ tels que $\pi(P)=[\color{blue}{\mu}\color{black}]P$, ici sont représentées les différentes isogénies de noyau $\langle P \rangle$.}

%\todo{mettre des mu et des lambda sur les dessins..}

\end{figure}
Sur la Figure~\ref{fig:poi:dia} sont représentées les isogénies qui ont pour
noyau différents points $P \in E_s $ diagonaux d'ordre $2^3$, tels que
\todo{Pas de couleurs dans le texte d'une thèse! (même dans les figures, c'est limite).
  Et puis, pourquoi préciser $\mu$?}
$\pi(P)=
...
cela on peut être amené à travailler dans des extensions de corps, 
...
\rightarrow E'_s$ de direction $\mu$
\todo{Qu'est-ce qui est différent?}
différente de celle de $[2^2]P$, $\psi(P)$
...
\end{cor}
\todo{Je ne comprend pas quel est le critère restrictif.}
...
\todo{``partiellement horizontal'', c'est moche. As-tu besoin du qualificatif? Si tu l'appelles simplement ``horizontal'', il n'y a pas d'ambiguïté (tu remarques juste après que horizontal = p. horizontal + sur le cratère). Mais peut-être cela complique la narration.}
\begin{defi}
\label{def:par:hor}
...
\begin{enumerate}
\item\label{sers:toi:des:labels}  $\langle [\ell^{k-i}]P' \rangle$ est le noyau d'une isogénie montante de
...
\begin{proof}
Pour le point~\eqref{sers:toi:des:labels} montrons que l'image de $P$ par $\varphi$ est encore d'ordre 
...
$R=[a]P+[b]Q$ et
\todo{notation non définie}
$\pi(P,R)= \left( \begin{matrix}\lambda & \beta 
...
existe une
\todo{un changement de base? une conjugaison?}
...
\todo{Toute cette preuve ne se réduit-elle pas à une deux petites lignes d'égalités matricielles?}
\begin{proof}
...
$y$ et $*$  tels que
\todo{$*$ est un mauvais choix si tu dois l'utiliser comme une variable: dans cette expression on ne comprend plus s'il s'agit d'un opérateur ou d'un élément.}
$\ell^{h-e}\gamma=\ell^{h-e}*(by)^{-1} \bmod 
\ell^{h-e+i}$, avec $\ell^{h-e}\gamma \bmod \ell^{h-e+i}$ qui
\todo{Qu'est-ce que cela veut dire pour un entier de ``représenter une isogénie''?}
représente une 
$\ell^i$-isogénie descendante spécifique tout en ayant $\pi(P,R)=\pi(P,R')$.
On pose $\ell^{h} \alpha = \lambda - \mu$ avec $\alpha \wedge \ell=1$, l'égalité $\pi(P,R)=\pi(P,R')$ se traduit donc par:
...
Dans cette sous-section on s'intéresse à la partie algorithmique du calcul des objets présentés dans la sous-section~\ref{subs:elk:dir}. Sont aussi utilisés les résultats et définitions du chapitre \ref{cha:tour}, dont notamment les extensions de corps de la définition \ref{def:tour-ell}.
...
valeurs car, pour n'importe quel corps fini $\mathbb{F}_q$, on a $E[\ell] 
...
\todo{!!! problème notation avec chapitre 2, celles-ci sont meilleures !!!}
...
en dessous elle augmente de $1$ à chaque niveau.
\end{proof}
...
les autres, c'est le calcul d'un antécédent à la 
multiplication par $\ell$ d'un point $P$, on note cette application 
$\mathsf{divise}(\ell,P)$. Ce résultat sera aussi utilisé dans le cas Atkin (voir Section...).

\begin{lem}
\label{lem:div:cou}
Soit $P$ un point de $E$ tel que $P \in E(\mathsf{F}_{n})$, alors le coût du calcul de $E(\mathsf{F}_1)[\ell]$ est de $O(\ell \mathsf{M}(\ell^2)\log(\ell)\log(\ell q))$. Le coût du calcul de $\mathsf{divise}(\ell,P)$ est de $O(\mathsf{R}(n+1))$ lorsque $E[\ell](\mathsf{F}_1)$ a déjà été calculé.
\end{lem}
\todo{dire coût moyen}
...
décrite dans ~\cite[Chapter~14.5]{vzGJG03}.
  ...
  multiplication par $\ell$ par un produit de deux $\ell$-isogénies. Ainsi on
   ...
   degré $\ell$, ainsi le coût de $\mathsf{divise}(\ell,P)$ est de $O(\mathsf{R}(n))$, d'après la définition de 
   $\mathsf{R}(i)$ (voir chapitre~\ref{cha:tour}).  
\end{proof}


Avant de présenter l'algorithme~\ref{alg:ulti:fro} qui, pour une courbe $E$ 
située au niveau $h-e$, calcule une base diagonale de la
\todo{Qui est $E_s$?}
$E_s[\ell^k]$-torsion, nous 
...
\todo{hauteur de qui? Les volcans réguliers sont-ils plus hauts que les non-réguliers?}
...
rationnelle augmente de $1$. Ainsi pour travailler avec
...
\begin{nota}
Soit $P \in E$ avec $E$ située au niveau $h-e+i$ d'un volcan de $\ell$-isogénies, alors on notera $P^{(h-e+i)}$ et $E^{(h-e+i)}$ pour signifier que le point et la courbe sont définis au niveau $h-e+i$ du volcan de $\ell$-isogénies.
\end{nota}
...
\begin{lem}
\label{lem:alg:rat}
Dans l'algorithme~\ref{alg:rem:tri}, avec comme entrée une courbe située
en dessous du cratère d'un 
volcan de $\ell$-isogénies,  les points utilisés et 
...
(on se limite à ce cas là car on 
est dans le cas où la matrice représentant l'action du Frobenius est scalaire). Comme
le coût de 
chaque boucle grandit géométriquement, c'est la dernière qui domine et donne la 
...
\max(0,e-(h-\beta)}))$, auquel il faut ajouter le coût du calcul de la base de la 
...
Une fois que l'on a calculé une base diagonale $(P,Q)$ de la $\ell^k$ torsion pour $k>h$, on connaît (par la proposition \ref{pro:dia:hor}) deux points horizontaux  de directions opposées et d'ordre $\ell^{k-h}$, à savoir $[\ell^{h}]P, [\ell^{h}]Q$. On va donc maintenant se servir de la proposition \ref{pro:pus:hor} pour concevoir un algorithme qui nous donne des points horizontaux sur le cratère.
...
\todo{Tu as déjà fait une remarque similaire. Il faut peut-être factoriser.}
\begin{rem}
Le calcul de point horizontal d'ordre $\ell^k$ aurait pu être fait en calculant une base diagonale de la $\ell^{k+h}$ torsion, mais alors on aurait été obligé de travailler sur une extension de corps plus grande, et l'on aurait alors obtenu une complexité trop élevée en fonction de $h$. Par exemple, pour une courbe située sur le cratère, à l'aide de l'algorithme~\ref{alg:bas:diag} on obtiendrait une complexité moyenne de $O(\mathsf{R}((k+h)-\beta)+\ell^{2}\mathsf{M}(\ell^{k+h-\beta})+\ell\mathsf{M}(\ell^2)\log(\ell)\log(\ell q)$.
\end{rem}
...
la matrice
\todo{Comme plus tôt, $\mod\mathbb{Z}_\ell$ n'est pas une notation usuelle.}
\begin{equation*}
...
de~$E$ dans le volcan.
\todo{Cette dernière phrase est touffue. À conjugaison/changement de base près, tu peux supposer que $c$ est premier avec $\ell$ et $v_\ell(b)\le 2e$, par exemple. Tu peux sans doutes poser encore plus de contraintes à conjugaison près, ça permettrait peut-être de normaliser $a$ et $d$.}
...
égal à $\ell^{h+e}$,
\todo{Pourquoi? J'aime beaucoup la technique de preuve, mais ce passage n'est pas assez évident.}
on a alors le résultat.
Comme vu précédemment on doit avoir 
...
courbe. Pour une courbe située sur
le cratère (ici le $2$ème niveau), les coefficients $b_2$ et $c_2$ sont
premiers avec $\ell$, et la valuation $\ell$-adique des coefficients 
anti-diagonaux vaut $h$. Pour les niveaux en dessous du cratère, au 
...
égale au niveau $h-e$ de la courbe, l'autre coefficient peut avoir une valuation 
...
\begin{prop}
\label{pro:atk:equi}
Soient $P,R,R'$ trois points d'une courbe $E$, située au niveau $h-e$ du volcan de 
$\ell$-isogénies, tels que
\todo{Où est la définition de $i$?}
$\langle P,R \rangle = \langle P,R' \rangle= 
...
cette égalité, et de $c \wedge \ell = 1$, on déduit que $[\ell^{h+e}]R=[\ell^{h+e}]R'$,
ce qui permet de conclure pour ce sens de l'équivalence.

\todo{À nouveau: qui est $i$?}
...
%du volcan car pour les niveaux inférieurs du volcan de $\ell$-isogénies on a la
...
\right)^{\times}$.
\todo{La dernière phrase est triviale. Je ne comprends pas comment la preuve peut être aussi longue.}
\end{prop}
...
x'y'^{-1}\bmod \ell^{h+i}$. On voit donc que seules comptent les valeurs de 
...
\end{tikzpicture}
\end{center}
\caption{\label{fig:atk:dif:niv:iso} Exemples des isogénies correspondant aux différentes matrices du Frobenius, calculées sur la courbe $E$ au sommet du volcan de $\ell$-isogénies. Dans cet exemple, $E$ est la courbe de $j$-invariant $56$ définie sur $\mathbb{F}_{149}$, et $\ell=2$.}
\end{figure}
...
calculée l'action du Frobenius.
\todo{Et le premier vecteur, comment tu l'as fixé?}
Ainsi les courbes dans le volcan atteintes
...
\begin{lem}
\label{lem:fro:atk}
Soient $P,P',R$ trois points d'une courbe elliptique $E$ située au 
...
$\pi(P',R)=\pi(P,R)$.
\todo{En quoi cet énoncé est différent de 5.33? Je continue à ne pas savoir qui est $i$.}
\end{lem}
...
\begin{prop}
Soient $P,P'$ deux points d'ordre $\ell^{h+i}$ d'une courbe elliptique $E$ 
située sur le cratère du volcan de $\ell$-isogénies, tels que $[\ell^{h-e}]P=
[\ell^{h-e}]P'$. Soient $R,R'$ deux points tels que $\langle P, R \rangle = 
\langle P', R' \rangle=E[\ell^{h+i}]$ et $\pi(P,R)=\pi(P',R')$, alors
$[\ell^{h+e}]R=[\ell^{h+e}]R'$.
\end{prop}
...
et $d_1$ divise~$(\ell-1)$.
\todo{Ça m'a l'air carrément faux. Le polynôme de division peut être irréductible. Tu dois avoir oublié une hypothèse.}
\end{prop}
...
\todo{C'est le déterminant qui vaut $q$, pas $ad$.}
\end{proof}
...
de degré $\ell^{\alpha}$,
\todo{Pourquoi?}
et cela contredirait la structure du volcan de 
$\ell$-isogénies $F$-rationnelles. Dans le cas où le volcan serait 
\emph{non-régulier}, \cite[§5.3.1]{Ionica2010} prouve directement que $\alpha=\beta$.
Pour le lien entre $h$ et
...
\todo{Ah bon? Comment? Tu n'as pas les valeurs propres, ici.}
...
$1$ la hauteur du volcan en travaillant sur $\mathsf{F}_{\ell}$, l'extension de
...
$\ell^{j}$ définis dans dans $\mathsf{F}_{j-\beta+\max(0,e-(h-\beta))}$, lorsque $j-\beta+\max(0,e-(h-\beta))>1$, dans
...
\label{cha:alg:fin}
On rappelle que l'on se place dans le contexte suivant: \todo{est-ce un rappel?}
...
\todo{faire un beau dessin de correspondance avec les points partiellement horizontaux}
\begin{prop}
...
\end{figure}
\todo{mettre les figures 6.2 et 6.3 cote à cote}
...
sur le cratère. \todo{est-ce utile ?}
...
point horizontal au lieu de travailler avec une base horizontale.\todo{Dire ça ailleurs}
...
abscisses lors de l'étape d'interpolation.  \todo{A nuancer avec la taille des orbites}
...
sert juste à simplifier les calculs. \todo{reflechir notation $n_i$, $k_i$}
...
\begin{prop}
\label{pro:rep:com}
Soit $P$ un point d'ordre $\upsilon_n$ \todo{mettre dans nomenclature} de $E(\mathbb{K}_n)$ obtenu à l'aide 
...
multiplication n'est que de $\tilde{O}(\mathsf{M}(p_n))$. Ce coût de 
$\tilde{O}(\mathsf{M}(p_n))$ en comparant 
