...
En pratique on travaillera donc sur une extension $F_1$ de $\mathbb{F}_q$ tel que l'on ait $E[\ell] \subset E(F_1)$, cela a pour conséquence d'après le théorème sur la structure de groupe des points d'une courbe elliptique que l'on ait $\ell \mid |F_1|-1$. On utilisera donc en pratique la construction de tous $\ell$-adique développé ici , et on devra ajouter un facteur pour avoir le coût des opérations dans $\mathbb{F}_q$.
