...
Soient $P_1$ un point d'ordre $\ell_1^{k_1}$ de $E$ et $P_2$ un point d'ordre $\ell_2^{k_2}$ de $E$ alors ces deux points associés isomorphiquement à un point $P$ d'ordre $\ell_1^{k_1}\ell_2^{k_2}$ tel que $[\ell_1^{k_1}]P=P_2$ et $[\ell_1^{k_2}]P=P_1$.
\end{defi}
...
\begin{algorithm}
\caption{\label{alg:TRC:init} Théorème des restes chinois sur une courbe elliptique}
\begin{algorithmic}[1]
...
\begin{rem}
La sortie de l'algorithme \ref{alg:TRC:init} n'est pas unique car le choix 
d'un antécédent de $P_1$ par la mutiplication par $\ell_2^{k_2}$ n'est pas 
unique mais défini modulo la $\ell_2^{k_2}$ torsion.
\end{rem}
