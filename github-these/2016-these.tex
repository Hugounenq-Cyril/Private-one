...
%La cryptographie est la science de coder les informations afin de ne pas les
%rendre intelligible par toute personne non destinataire du message, tout en 
%laissant la possibilité au destinataire de pouvoir déchiffrer cette 
%information. De nos jours, de par les nombreux échanges sur internet et 
%l'utilisation de plus en plus présente de l'informatique, cette science est 
%devenue cruciale que cela soit pour des enjeux économiques ou politiques. Ainsi
%la sécurité des données chiffrées est un enjeu majeur et pour prouver celle-ci
%des algorithmes de chiffrement repose sur des problèmes mathématiques jugés 
%difficiles. Tout particulièrement il y a le problème du logarithme discret dans
%un groupe $(G,\cdot)$. Ce problème est de retrouver d'après la connaissance de 
%$P \in G$, $P^{x} \in G$ l'entier $x$. Ce problème dépend notamment du choix du
%groupe. 
%
%De nombreuses attaques génériques sur le logarithme discret ont été 
%faites \cite{Shanks71,Pollard78} en un temps $O(\sqrt{r})$ avec $r$ la taille 
%du plus grand facteur de l'ordre du groupe. Ainsi il a été proposé d'utiliser 
%des groupes définis sur les courbes (hyper-)elliptiques dans lesquels de telles 
%attaques seraient plus difficiles \cite{Miller85,Koblitz87,Koblitz89}. 
%Cependant des méthodes à l'aide de couplage sur les courbes elliptiques 
%\cite{Menezes93,FreyRuck94}  ont permis de réduire ce problème à un problème de 
%logarithme discret sur des corps finis. En dépit de nombreux progrès dans la 
%résolution du problème du logratihme discret sur les corps finis à l'aide de 
%calculs d'indices 
%\cite{JouxLercier06,JouxLercierSmartVercauteren06,BarbulescuGaudryJouxThome14,BarbulescuPierrot14} 
%ce problème reste toujours non résoluble en algorithme en temps 
%polynomial. Ainsi la cryptographie à l'aide de courbes elliptiques est toujours
%utilisée malgré l'existence d'algorithmes quantiques \cite{Shor94} permettant 
...
cryptographie \cite{Miller85,Koblitz87,Koblitz89}.
On s'intéresse tout particulièrement aux courbes elliptiques sur les corps 
finis
\todo{Doucement. Parle d'abord de comptage de points et de SEA.
  Ensuite tu peux introduire les isogénies, dans quelques paragraphes.}
et aux algorithmes qui permettent de calculer les 
isogénies, morphismes de groupe de noyau fini, entre deux courbes elliptiques. 
En particulier deux courbes elliptiques reliées par une isogénie ont la même 
cardinalité.

L'intérêt de tels algorithmes est motivé par leur utilisation dans l'algorithme
de Schoof \cite{Schoof85}, qui est un algorithme primodial car il sert à
\todo{Trois fois ``algorithme'' dans la même phrase.
  Quatre niveaux de subordination... il n'y a que Proust ou Hugo qui ont droit à autant.}
calculer la cardinalité de courbes elliptiques. Bien que la complexité des 
algorithmes de calcul d'isogénies ne domine pas dans l'algorithme de Schoof et
\todo{Ne te tire pas une balle dans le pied: le calcul d'isogénies est intéressant
  puisqu'il apparaît dans SEA. Point.}
son amélioration (SEA) \cite{Schoof95}, due aux travaux de Atkin \cite{Atkin88} et
Elkies \cite{Elkies91}, des travaux ont été fait sur les algorithmes de calcul
\todo{Quel travaux? Pourquoi?}
d'isogénies.

\todo{Une étape fondamentale de SEA consiste en la résolution du problème suivant:}
\paragraph{Problème du calcul explicite d'isogénie}
\todo{Je pense qu'on peut traduire Elkies par ``problème de l'isogénie explicite''.}

Soient deux $j$-invariants $j$ et $j'$ et un entier positif $r$. En supposant que les 
...
$j(E_a)=j$ et $j(E_b)=j'$,
calculer les courbes $E_a$, $E_b$ et 
l'isogénie $\phi:E_a \rightarrow E_b$ de degré $r$ .

Les toutes premières méthodes de calcul 
d'isogénies dans ce contexte \cite{Atkin91} \cite{CharlapColeyRobbins91} sont
dominées par d'autres étapes dans l'algorithme SEA.
\todo{Cette phrase ne fait pas de sens: est-ce que Elkies 98 ne serait pas dominé aussi?
  Laisse tomber cette histoire de savoir qui domine.
  À ce stade, on n'a pas les éléments pour comprendre.}
Le premier algorithme à
traiter du calcul d'isogénies sur les corps finis est celui de Elkies 
\cite{Elkies1998}; celui-ci se limite au cas de corps finis de 
caractéristique suffisamment grande.
\todo{À nouveau, tu ne fais pas justice à Couveignes 94, 96 et Lercier 96.
  C'est le moment de les mentionner, puisque tu parles de caractéristique}
Ce travail a été ensuite amélioré par 
les travaux de Bostan, Morain, Salvy et Schost \cite{BMSS08}, puis par ceux de 
Lercier et Sirvent \cite{Lercier-Sirvent2008} qui ont permis d'utiliser la méthode sans 
contrainte de caractéristique. Ce dernier a été récemment amélioré par un 
résultat de Lairez et Vaccon \cite{LairezVaccon16} sur la précision $p$-adique. 
...
celles-ci peuvent être représentées à isomorphisme près à l'aide de leur 
$j$-invariant.
\todo{Paragraphe incompréhensible. Ce que tu veux dire:
  1) Les algos à la Elkies ou à la Couviegnes résolvent le problème de l'isogénie explicite;
  2) Elkies a une moins bonne complexité, Couveignes est moins efficace en pratique;
  3) Mais de toutes façons c'est absorbé dans les autres coûts de SEA, donc osef et on utilise Elkies en crypto.
  4) Mais il y a des cas (carac moyenne), ou c'est moins clair entre L-S et Couveignes;
  5) Et puis le problème apparaît aussi dans d'autres contextes, où Couveignes gagne (voir paragraphe suivant).}

En effet le calcul d'isogénies peut avoir un intérêt 
en dehors de l'algorithme SEA. comme le montrent les travaux de Teske 
\cite{Teske06} sur un crypto-système à trappes, les travaux de cryptanalyse de 
\cite{MaurerMenezesTeske01}, 
les fonctions de hachage \cite{CGL09}, l'accélération de la multiplication 
scalaire \cite{GallantLambertVanstone01}, \cite{LongaSica14} ou les 
crypto-systèmes post-quantiques à base de graphes d'isogénies 
\cite{RostovtsevStolbunov06} \cite{DeFeoJaoPlut14}.

\\
%parler dej-invariants

%L'une des étapes les plus couteuses de l'algorithme S.E.A. est le calcul

%L'une des permières étapes à faire lorsque l'on veut utiliser une courbe
%en cryptographie est de calculer sa cardinalité pour faire ceci l'algorithme de
%Schoof \cite{Schoof85} propose une solution en temps polynomial en la 
%caractéristique du corps $\mathbb{F}_q$ sur lequel est définie la courbe 
%elliptique. Cette méthode repose principalement sur la détermination de la 
%trace du Frobenius sur un groupe de $r$-torsion. Ceci est fait en déterminant 
%$t_{\pi}$ dans l'équation suivante obtenue à l'aide du polynôme 
%caractéristique du Frobenius:
%\begin{equation}
%x^2-t_{\pi}x+q=0 \bmod r.
%\end{equation} 
%Des améliorations ont été apportées à cet algorithme notamment par Elkies 
%\cite{Elkies91}  et Atkin \cite{Atkin88}, celles-ci permettent de travailler 
%avec des sous-groupes de la $\ell$-torsion lorsque le Frobenius admet deux 
%espaces propres (cas Elkies).

%Ainsi au cours de l'algorithme de S.E.A. (Schoof Elkies Atkin) \cite{Schoof95} 
%on est amené à calculer le $r$-ème polynôme modulaire $\Phi_r$ avec un coût de 
%$\tilde{O}(r^3)$ (par \cite[Algorithm 6.1]{BLS12}), ainsi qu'une isogénie de 
%degré $r$ entre deux courbes elliptiques données par leur $j$-invariants (ces 
%$j$-invariants annulent $\Phi_r$). Les toutes premières méthodes de calcul 
%d'isogénies dans ce contexte \cite{Atkin91} \cite{CharlapColeyRobbins91} sont
%dominées par le coût de calcul du polynôme modulaire. Le premier algorithme à
%traiter du calcul d'isogénies sur les corps finis est celui de Elkies 
%\cite{Elkies1998} celui-ci se limitait au cas de corps de caractéristique 
%suffisamment grande. Ce travail a été ensuite amélioré par les travaux de 
%Bostan Morain Salvy et Schost \cite{BMSS08} puis par ceux de Lercier Sirvent
%\cite{Lercier-Sirvent2008}  qui a permis d'utiliser cette méthode sans 
%contrainte de caractéristique, celui-ci a été récemment amélioré par un 
%résultat de Lairez-Vaccon \cite{LairezVaccon16} sur la précision $p$-adique. 

%Le désavantage avec toutes ces méthodes c'est que celles-ci supposaient la 
%connaissance de $\Phi_r$. En effet le calcul d'isogénies peut avoir un intérêt 
%en dehors de l'algorithme S.E.A. comme le montrent les travaux de Teske 
%\cite{Teske06} sur un schéma à trappes, les travaux de cryptanalyse de \cite{MaurerMenezesTeske01}, 
%les fonctions de hachage \cite{CGL09}, l'accélération de la multiplication 
%scalaire \cite{GallantLambertVanstone01}, \cite{LongaSica14} ou les 
%crypto-systèmes post-quantiques à base de graphes d'isogénies 
%\cite{RostovtsevStolbunov06} voire \cite{DeFeoJaoPlut14}.

L'algorithme de Couveignes 
interpole l'action de l'isogénie sur les points d'ordre $p^k$,
pour $k$ assez grand, où $p$ est la caractéristique du corps de base.
Ce travail de Couveignes fût amélioré
\todo{Seul passé simple de toute l'intro. Ça fait bizarre.}
par la suite par De Feo \cite{DeFeo11}, qui se sert notamment d'une
\todo{Cite aussi Couveignes 2000, les tours d'AS, c'est d'abord son idée.}
construction efficace de tours d'extensions de degré $p$ de corps finis, développée
conjointement avec Schost \cite{DeFeo-Shost'12}. L'algorithme de Couveignes
a toutefois le désavantage d'avoir une dépendance polynomiale en la caractéristique $p$.

Cette thèse s'inscrit dans la continuité des travaux de \cite{Couveignes96} 
et \cite{DeFeo11}, et cherche notamment à les généraliser à l'étude des 
points d'ordre $\ell^k$, avec $\ell$ quelconque,
afin notamment de s'affranchir de la dépendance polynomiale en $p$. Pour 
cela, on doit notamment déterminer des ensembles de points préservés par 
l'action de l'isogénie que l'on veut calculer, tout en se servant d'une construction 
efficace de tours d'extensions de degré $\ell$ de corps finis. 

Pour une étude fine de l'action de l'isogénie sur les
points d'ordre $\ell^k$, on se sert de \emph{graphes 
  d'isogénies}.
\todo{C'est le bon moment pour ajouter un dessin.
  Dis au moins ce que sons les sommets et les arêtes.}
Ceux-ci ont été étudiés pour 
la première fois par \cite{Kohel96} afin de calculer l'anneau des 
endomorphismes d'une courbe elliptique. Fouquet et Morain ont par la suite 
utilisé ces graphes, appelés aussi \emph{volcans d'isogénies}, afin 
d'améliorer SEA. Enfin les travaux de Miret et al. \cite{MiretMRV05} 
\cite{MiretMSTV06} \cite{MiretMSTV08} ont permis d'expliciter un lien entre le 
niveau d'une courbe dans le volcan de $\ell$-isogénies et
la structure des $\ell$ sous-groupes de Sylow.
Les travaux de \cite{Ionica-Joux10} ont permis, eux, à l'aide de couplages,
de se diriger dans le volcan d'isogénies sous certaines conditions.
\todo{Je crois que tu peux vendre aussi bien Miret et cie que Ionica-Joux comme
  des améliorations de Fouquet-Morain, avec SEA comme finalité principale.}

Concernant la construction de tours d'extensions, nos travaux se placent dans la suite de 
ceux de \cite{Doliskani-Schost15}, et en donnent quelques généralisations utiles.
\todo{Tu oublies De Feo-Doloskani-Schost.}

Ainsi notre travail a abouti au théorème suivant:
\todo{Attention à ne pas sous-vendre.
  C'est le résultat principal, mais il y a plus dans ta thèse.}
\begin{thm}
...
l'isogénie explicite" en un temps quasi-quadratique en le degré de l'isogénie, 
avec une dépendance logarithmique en la caractéristique du corps.
\end{thm}
\todo{Dis dans quelle section ce théorème est prouvé.
  Pointe vers l'énoncé exact (il me semble qu'il est plus précis dans le texte).}

\paragraph{Organisation de la thèse}
On présente donc dans le premier chapitre des notions de bases, notamment sur les
courbes elliptiques et leurs endomorphismes, et tout 
particulièrement l'endomorphisme de Frobenius.

Dans le 
deuxième chapitre nous abordons les différents algorithmes de calculs
\todo{Dans l'espace de deux phrases, tu passes de ``on'' à ``nous''.
  Homogénéise sur toute l'intro!}
...
Dans le chapitre 3, nous nous plaçons dans la continuité des travaux de 
\cite{Doliskani-Schost15}, et nous présentons les tours d'extensions $\ell$-adiques.
Nous établissons notamment un 
résultat similaire à celui de \cite{DeFeo11} pour le calcul de polynômes 
d'interpolation.

Dans le chapitre 4 nous rappelons les résultats principaux 
sur les volcans d'isogénies, dont notamment le lien entre les niveaux dans le
...
\cite{MiretMSTV08}, et Ionica Joux \cite{Ionica-Joux10}. 

Ensuite nous étudions dans le chapitre 5 l'action de l'endomorphisme de Frobenius sur 
la $\ell^k$-torsion d'une courbe elliptique, et ses liens avec les directions dans le volcan de
\todo{On ne sait toujours pas ce que c'est qu'une direction.}
$\ell$-isogénies, dans le cas Elkies puis Atkin. Nous présentons aussi dans ce
chapitre des algorithmes permettant de calculer des points qui engendrent le 
noyau de certaines isogénies, spécifiées par leurs directions, dans le volcan. 

Ensuite, nous présentons dans le chapitre 6 comment ces isogénies spécifiées 
par leur directions peuvent être
utilisées dans un algorithme de Couveignes $\ell$-adique.
\todo{Tu n'as jamais dit ce que c'est qu'un algo de Couveignes ``ell-adique''.
  Tu forces le lecteur à deviner. Même si deviner n'est pas dur, dis plus tôt que
  tu appelles ell-adiques tes variantes qui interpolent sur la ell-torsion.}
Nous présentons et analysons donc un algorithme $\ell$-adique de
Couveignes
\todo{Répété ``algorithme ell-adique de Couveignes'' dans l'espace de 2 lignes.}
dans un cas favorable (à savoir, lorsque un entier $\ell$ avec des bonnes propriétés nous
est fourni d'avance), puis dans le cas général, où l'on utilise notamment un résultat de 
Shparlinski et Sutherland \cite{ShparlinskiSutherland14} afin de donner une borne sur
l'entier $\ell$. On fait aussi une présentation d'une variante qui n'impose pas de
contraintes sur $\ell$, mais nous la détaillerons moins
car elle est moins performante.

Enfin nous présentons dans le chapitre 7 des variantes d'algorithmes de 
Couveignes $\ell$-adique et étudions leur intérêt.
\todo{Dis-en un peu plus, sinon on a l'impression qu'il sert à rien ce chapitre.}
...
\]
On définit également la notation $f \in \Omega(g)$ de la façon suivante à l'aide de deux constantes $x_0$ et $M$ telles que: 
\[
f(x)>Mg(x) \quad \text{pour} \quad x>x_0.
\]
On défnit la notation $\Theta$ pour laquelle que $f \in \Theta(g)$ si et seulement si $f \in O(g)$ et $f \in \Omega(g)$.

Dans ce document, de nombreux algorithmes vont utiliser la multiplication rapide, on définit donc celle-ci: $\mathsf{M}:\mathbb{N} \rightarrow \mathbb{N}$ la \emph{fonction de multiplication} tel que le coût de la multiplication de deux polynômes de de degré $n$ définis dans $\mathbb{F}_q[x]$ coûte $M(n)$ opérations arithmétiques dans $\mathbb{F}_q$. En utilisant les résultats de \cite[§8.3]{vzGJG03} on peut supposer que l'on a:
\begin{itemize}
...
Enfin des algorithmes de Las Vegas vont être utilisés dans ce document. Ces algorithmes sont des algorithmes probabilistes qui produisent un résultat correct, dès lors qu'on utilise un algorithme de Las Vegas on parlera de coût moyen. 
...
calculé en $\tilde{O}(r^3)$ opérations sur $\mathbb{F}_q$ d'après \cite{Enge09} \cite[Algorithm 6.1, Theorem 1]{BLS12}. \todo{se rappeler cette seconde source}
...
E_s$ d'ordre $2^3$ tels que $\pi(P)=[ \mu ]P$, ici sont représentées les 
...
\end{tikzpicture}
\end{center}
\caption{\label{fig:atk:dif:niv:iso} Exemples des isogénies correspondant aux différentes matrices du Frobenius, calculées sur la courbe $E$ au sommet du volcan des $\ell$-isogénies. Dans cet exemple, $E$ est la courbe de $j$-invariant $56$ définie sur $\mathbb{F}_{149}$, et $\ell=2$.}
\end{figure}
...
courbe au sommet du volcan. On a donc représenté dans le volcan la 
...
Q_{\mu}')))$ bases partiellement horizontales de $E[\ell^k],E'[\ell^k]$ avec les $\ell^e$-isogénies descendantes $\varphi: E_s \rightarrow E$, $\varphi': E_s \rightarrow E'$;
\ENDIF
...
abscisses lors de l'étape d'interpolation.  \todo{A nuancer avec la taille des orbites qui sont de $\ell^{k-\alpha}$ à $\ell^{k-\beta}$}
...
Couveignes au cas $\ell$-adique. En particulier, en l'état actuel, nous avons 
vu que l'on était capable d'obtenir un algorithme quasi-quadratique uniquement
dans le cas Elkies, et un algorithme de complexité inférieure à cubique dans le
cas Atkin. Cependant l'algorithme $\ell$-adique de Couveignes dans le cas Atkin
a une complexité trop grande, et si on voulait améliorer cet algorithme, avec 
la même méthode que pour le cas Elkies, il faudrait pouvoir distinguer un 
sous-groupe cyclique de la $\ell^k$-torsion, ou trouver un autre moyen pour 
distinguer des couples de points formant une base de $E[\ell^k]$. Nous avons 
réfléchi à travailler avec un groupes de points isomorphes à 
...
Frobenius sur un tel ensemble. Enfin parmi les pistes proposées mais non 
satisfaisante dans le chapitre~\ref{cha:var:cou} seule celle présentée dans la 
section~\ref{sec:cou:con} sur une approche avec un nombre composé de Elkies 
pourrait aboutir quand on aura des meilleurs représentations de compositum 
permettant des changements de représentation en un temps linéaire.

Enfin les propositions~\ref{pro:mat:fro} et \ref{pro:mat:fro:atk} nous 
permettent de savoir exactement à quel niveau se trouve une courbe dans le 
volcan. Ainsi en se plaçant dans le même contexte que \cite{BissonSutherland11},
où l'on connaît déjà une factorisation de $d_{\pi}$, avec les 
propositions ~\ref{pro:mat:fro} et \ref{pro:mat:fro:atk} dans un volcan de 
$\ell_i$-isogénies on connaîtrait la valuation $\ell_i$ adique du 
conducteur de la courbe par rapport à $\mathbb{Z}[\pi]$ et donc on pourrait
déterminer l'anneau d'endomorphisme de la courbe.
...
rapide du Frobenius, il serait intéressant de voir si on ne peut pas se servir
...
Enfin on pourrait utiliser le résultat de la proposition ~\ref{pro:cla:fro:mat}
pour faire un crypto système dans lequel le point P fixe serait la clé maitre 
secrète partagée entre deux utilisateurs et on utiliserait alors les formes des
matrices pour communiquer la clé correspondant au point que l'on doit utiliser.
...
Comme abordé dans la thèse de De Feo, l'algorithme de Couveignes pourrait être 
utilisé sans connaître le degré de l'isogénie.

Enfin on pourrait étudier l'intérêt de l'algorithme de Couveignes $\ell$-adique
dans la cryptanalyse de systèmes à base d'isogénies.
...
\IF{$(E_s,E'_s) \neq (E,E')$}
\STATE 
Calcul de $P_{\lambda},P'_{\lambda}$ points partiellement 
...
\label{cha:ann:comp}
\todo{les todos suivants sont dans le rebus}
...
\begin{proof} \todo{mettre en appendice ?}
...
\begin{cor} \todo{remplacer tout simplement par la proposition se généralise pour $n$}
...
avec $P_i$ des points (ascendants) horizontaux direction $\lambda_i$ et 
...
des points (ascendants) horizontaux direction $\lambda_i$ et profondeur 
...
\begin{proof}
Comme on a $\langle \ell^{h}R' \rangle= \langle [ell^{h}]([*(by)^{-1}]P+Q) 
\rangle$ alors l'isogénie de noyau $\langle \ell^{h}R' \rangle$ est déterminée 
par la valeur de $\ell^{h}b^{-1}y^{-1}* \bmod \ell^{h+i}$, on rappelle de plus 
que l'on est obligé d'avoir $*y \wedge \ell =1$ pour avoir une 
$\ell^{h+i}$-isogénie descendante. On a alors l'équation suivante qui doit être
vérifiée: 
\[ \beta \ell^h (1-y) = (\mu - \lambda) (ay-*)  \bmod \ell^{h+i} \]
...
\[\beta \ell^{h-e} = y\beta \ell^{h-e}  \bmod \ell^{h-e+i} \]
et donc la valeur de la matrice ne dépend que que de $y$. Or la
...
\todo{utilité des deux propositions suivantes ?, elles ne sont pas utilisées ...}
...
\end{algorithmic}
\end{algorithm}
\todo{remplacer les complexités espérées par ... cout moyen ?+ unifier tout cela..}
\todo{cet algorithme est-il vraiment utile ??? je ne pense pas}
\begin{prop}
...
%$O(\mathsf{R}(k-\beta+e) + \ell^2\mathsf{M}(\ell^{k-\beta+e})+ \ell\mathsf{M}(\ell^2)\log(\ell)\log(\ell q))$ opérations sur $\mathbb{F}_q$ et de $O(\mathsf{R}(k-\beta) + \ell^2\mathsf{M}(\ell^{k-\beta}) + \ell\mathsf{M}(\ell^2)\log(\ell)\log(\ell q))$ opérations sur $\mathbb{F}_q$ lorsque $E$ est au dessus du niveau de stabilité.
\end{prop}
...
travaillera dans $\mathsf{F}_{1}$. \todo{Il devient peut être nécessaire d'introduire une notation ici...}
...
Calcul de $(P_{\lambda,i},Q_{\mu,i}),(P'_{\lambda,i},Q'_{\mu,i})$ bases horizontales de $E_s[\ell_i^{k_i}],E'_s[\ell_i^{k_i}]$ à l'aide de l'algorithme \ref{alg:hor:poi};
\IF{$(E_s,E'_s) \neq (E,E')$}
\STATE Calcul de 
