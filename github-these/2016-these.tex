...
MOntrons comment construire un polynôme irréductible $f(x) \in \mathbb{F}_q[x]$ de degré $d=\ell^{\delta}$. Soient deux entiers $\lambda_{e+\delta}, \mu_{e+\delta}$ tels que 
\begin{align*}
\lambda_{e+\delta}=1 \bmod \ell^{e} \quad , \quad \mu_{e+\delta}=q \bmod \ell^e, \\
X^2-tX+q = (X-\lambda_{e+\delta})(X-\mu_{e+\delta}) \bmod \ell^{e+\delta} 
\end{align*}
On note $\lambda_{e+\delta}=1+\ell^el'$ avec $l' \wedge \ell = 1$. Dans la suite on notera tout simplement $\lambda=\lambda_{e+\delta}$ et $\mu = \mu_{e+\delta}$. On pose maintenant:
\begin{equation*}
\mathfrak{d}=(d,\pi-\lambda)=(\ell,\pi-\lambda)^{\delta}=\mathfrak{l}^{\delta}.
\end{equation*}
C'est un idéal inversible, noyau de l'isogénie $i_{\delta}:E_0 \to E_{\delta}$. Le $\ell$ sous-groupe de Sylow de $E_{i+1}(\mathbb{F}_q)$ est le noyau de $\mathfrak{l}^e=(\ell^e,\pi - 1)$, et il est cyclique. Soit $A$ un générateur de $\mathfrak{l}^e$ avec $\ell^e \neq 2$. On écrit l'isogénie $\phi_i$ comme une fration rationnelle : 
\begin{align*}
\phi_{i}:E_i \to E_{i+1}, \\
(x,y) \mapsto \left( \frac{f_i(x)}{g_i(x)},y\left(\frac{f_i(x)}{g_i(x)} \right)^' \right)
\end{align*}
avec $g_i$ le carré du polynôme de degré $\frac{ell-1}{2}$  qui s'annule sur l'abscisse des points du noyau de $\phi_{i}$. On note alors $ T_{1}(X)=f_{1}(x)-g_{1}(x)x(A)$. Soit $B \in E_i(\overline{\mathbb{F}_q})$ tel que $\phi_i{\delta}(B)=A$, alors  $B$ est un générateur de $\mathfrak{l}^{e+\delta}=(\ell^{e+\delta},\pi-\lambda)$. En particulier on a:
\begin{equation*}
\pi(B)=\lambda B
\end{equation*}
avec l'ordre de $\lambda=1+\ell^el'$ dans $(\mathbb{Z}/\ell^{e+\delta}\mathbb{Z})^*$ qui est $d=\ell^{\delta}$. Ainsi l'orbite de Galois de $B$ est de taille $d$ et le polynôme $T_{1}(X)$,dont les racines appartiennent au fibré $\phi_i^{-1}(A)$ est irréductible. 

Pour définir alors des extensions successives on prend alors les pré images par les isogénies $\phi_{i_1},\phi_{i_2}, \cdots$.
