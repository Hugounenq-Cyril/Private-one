...
$Es[\ell^{h-e+1}]$ pour $e>0$. Mais avant cela nous rappelons un résultat sur
la rationalité des points et leur niveau dans le volcan de $\ell$-isogénies.
%\begin{prop}
...
en dessous dans la tour d'extensions $\ell$-adiques à torsion identique, le 
lemme~\ref{lem:alg:rat} en est une illustration.
\end{rem}
...
supplémentaires. On présente donc tout d'abord l'algorithme~\ref{alg:rem:tri} 
...
dont on connaît une base diagonale de la $\ell^j$-torsion pour $j<k$, 
l'algorithme~\ref{alg:bas:diag} calcule une  base diagonale de~$E[\ell^k]$ avec
une complexité espérée de
