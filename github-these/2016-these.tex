...
cryptographie \cite{Miller85,Koblitz87,Koblitz89}. Nous nous intéressons tout 
particulièrement aux courbes elliptiques sur les corps finis $\mathbb{F}_q$ de 
caractéristique $p$. Les algorithmes de calculs de cardinalité de courbes 
elliptiques sont donc primordiaux. L'algorithme de Schoof \cite{Schoof85}, est 
le premier algorithme à calculer la cardinalité de courbes elliptiques. Il a 
...
Le calcul d'isogénies, morphismes de groupes de noyau fini entre deux courbes
elliptiques apparaît dans SEA et a donc un intérêt primordial. En effet deux 
courbes elliptiques reliées par une isogénie ont la même cardinalité. Dans SEA
les courbes elliptiques sont représentées, entre autres, à l'aide de leur 
$j$-invariant qui les définit à isomorphisme près. On formule donc le problème
...
algorithmes sur les corps finis ont été développé \cite{Couveignes94}, 
\cite{Couveignes96}, \cite{Lercier96}, \cite{Elkies1998}.
Les trois premiers algorithmes sus-cités de Couveignes \cite{Couveignes94}, 
\cite{Couveignes96} et Lercier \cite{Lercier96} ne sont utilisables que dans le
cas de petite caractéristique. L'algorithme de Elkies\cite{Elkies1998} qui se 
place dans le contexte SEA, et nécessite le calcul de représentations 
spécifiques des courbes (sous équation de Weierstrass normalisée), est 
utilisable en plus grande caractéristique. Ce travail a
été ensuite amélioré par les travaux de Bostan, Morain, Salvy et Schost 
\cite{BMSS08} puis par ceux de Lercier et Sirvent \cite{Lercier-Sirvent2008} qui a
...
complexité que l'algorithme de Lercier et Sirvent\cite{Lercier-Sirvent2008}, 
celui-ci n'est pas utilisé en pratique à cause de sa trop grande dépendance
en la caractéristique qui le rend trop couteux dés lors que la 
caractéristique augmente trop. Ces deux algorithmes n'étant pas limités par la 
caractéristique ils sont pertinents pour le calcul de points en caractéristique
moyenne. De plus comme dans SEA d'autres opérations, notamment le calcul 
d'équations de Weierstrass normalisées, ont un coût plus élevé que l'algorithme
de Lercier et Sirvent \cite{Lercier-Sirvent2008}, la question d'un algorithme 
plus efficace n'est pas motivée dans ce contexte. 
...
par la suite par \cite{Couveignes00} De Feo \cite{DeFeo11} , qui se sert notamment d'une 
...
dans le cas de courbes ordinaires Pour le calcul d'ensemble de points d'ordre $\ell^k$, préservés par l'action de 
la $r$-isogénie, on se sert de graphes 
d'isogénies . Ceux-ci ont été étudiés pour 
...
Couveignes $\ell$-adique afin de voir notamment si l'on ne peut pas travailler 
avec un seul groupe cyclique ou avec différents nombres de Elkies et étudions leur intérêt. 
