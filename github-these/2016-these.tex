...
\end{itemize}
	...
	Dans le cas Atkin les valeurs propres ne sont définies sur définie sur $\mathbb{F}_\ell$ mais sur $\mathbb{F}_{\ell^2}$, les espaces propres sont alors définis sur $\mathbb{F}_{q^r}$ avec $r \mid \ell+1$.
...
\begin{enumerate}
\item $(1,\ell)$ ou $(1,1, \cdots, 1)$. Dans les deux cas $t_{\pi}^2-4q=0 \bmod \mathbb{Z}/\ell \mathbb{Z}$, dans le premier cas on pose $r=\ell$ et dans le second $r=1$;
...
Maintenant que la courbe isogène est entièrement déterminée, il reste à déterminer le noyau de l'isogénie de celle-ci des algorithmes tels que \cite{BMSS08}  puis \cite{Lercier-Sirvent2008}  cherchent à résoudre ce problème dansun tel contexte. Un présentation de ces algorithmes est faite \textcolor{red}{ref à mettre plus tard}.
