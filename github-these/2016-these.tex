...
L'algorithme de Schoof \cite{Schoof85} consiste à calculer le cardinal d'une courbe elliptique $E$ définie sur $\mathbb{F}_q$ à l'aide du polynôme charactéristique du Frobenius $\pi$:
\begin{equation*}
X^2-t_{\pi}X+q=0
\end{equation*}
Une fois la valeur de la trace du Frobenius $t_{\pi}$ déterminée, on a alors: $|E(\mathbb{F}_q)|=q+1-t_{\pi}$. En pratique cette équation est résolue modulo la $\ell_i$-torsion, avec $\ell_i$ un nombre premier. $t_{\pi}$ sera donc déterminée dans ce cas modulo $\ell_i$. Cette résolution est répétée pour un ensemble de nombres premiers $L$ tel que: $\Pi_{\ell_i \in L}\ell_i>4 \sqrt{q}$, le théorème des restes chinois est alors utilisé pour déterminer $t_{\pi}$ modulo $\Pi_{\ell_i \in L}\ell_i$. Ainsi avec le théorème de Hasse on peut déterminer la cardinalité de la courbe sur $\mathbb{F}_q$.

Le principal problème avec cette approche étant que l'on travaille avec la $\ell$ torsion qui lorsque l'on a $\ell \neq p$ est de la forme $E[\ell] \simeq \mathbb{Z}/\ell \mathbb{Z} \times \mathbb{Z}/\ell \mathbb{Z}$ et l'on doit alors travailler modulo $f_{\ell}$ le polynôme de $\ell$ division (qui s'annule sur les abscisses des points de $\ell$ torsion) qui est de degré $(\ell^2-1)/2$. L'objectif est alors de considérer des ensembles plus restrictifs de la $\ell$-torsion afin de diminuer le coût de l'algorithme.
...
\end{equation*} 
on veut voir quand il est possible de restreindre l'action du Frobenius à des sous-groupes de la $\ell$-torsion.
On a plusieurs cas possibles selon les propriétés du discriminant $d_{\pi}$.
...
\section[Doliskani-Schost]{$\ell= 2$ }
...
\subsection{Construction d'une tour d'extension}
...
Le coût de calcul d'un non résidu quadratique est négligé car un tel résidu peut être trouvé après le test de $O(1)$ éléments au hasard, chaque test ayant un coût de $O(\log(q))$ opérations sur $\mathbb{F}_q$.
...
A partir de maintenant on se placera dans le cas favorable où l'on a $q=1 \bmod 4$ ou $q = 1 \bmod \ell$, selon que l'on considère spécifiquement une tour $2$-adique ou $\ell$-adique, une approche unifiée de ces cas est privilégiée et sera spécifiée à chaque fois.



\subsection{Représentation des éléments de la tour $\ell$-adique}%titre a retravailler
Dans cette sous-section les cas $\ell=2$ et $\ell \neq 2$ vont être traités de façon unifiée. On rappelle que la tour $\ell$-adique est dénotée: $\mathbb{F}_q, \mathsf{F}_0, \mathsf{F}_1, \mathsf{F}_2, \cdots $ avec $\mathsf{F}_0$ une extension de degré $r$ de $\mathbb{F}_q$ et que pour tout $i \geqslant 1$ $\mathsf{F}_i$ est une extension de degré $\ell$ de $\mathsf{F}_{i-1}$. Comme dit dans la sous section précédente on se place dans le cas simple où l'on a $r=1$ et donc $\mathsf{F}_0=\mathbb{F}_q$, cependant afin de permettre un traitement plus général des cas on écrira $F_0$ et non $\mathbb{F}_q$.
...
Cependant lors des représentations d'éléments de $\mathsf{F}_k$ il ne va pas être fait des changements de base univariée à base multivariée mais seulement à une base bivarié pour permettre des passages un niveau plus bas dans la tour $\ell$-adique. On représentera par exemple alors $\mathsf{F}_k$ pour $k\geqslant1$ des deux façons suivantes:
\begin{equation*}
...
Les résultats de cette problématique sont issus d'idées développées dans \cite{Doliskani-Schost14} et sont adaptés à cette construction de tour d'extension $\ell$-adique où le coût du Frobenius est très faible. Le résultat à retenir est celui pour les extensions dyadiques du fait de son usage pratique même si une généralisation est présentée ici. 
...
Le coût de calcul de $\gamma$ et $\beta$ étant négligeables par rapport à celui de $\eta$, on a alors un coût total de $O(M(n)\log(nq))$ opérations sur $\mathbb{F}_q$ si $\mathsf{F}_0 \cong \mathbb{F}_q$ et de $O(M(\ell^k)\log(\ell)\log(\ell^kq^{\ell}))$ opérations sur $\mathbb{F}_q$ si $\mathsf{F}_0 \ncong \mathbb{F}_q$.
