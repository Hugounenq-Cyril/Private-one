...
logiciel SageMath~\ref{steinsagemath}, le code est disponible sur le projet 
github:\href{https://github.com/Hugounenq-Cyril/Two_curves_on_a_volcano}. La 
partie expérimentale a été faite uniquement pour le cas $\ell=2$ et des courbes
en entrée se situant sur le cratère.

La construction des tours d'extensions $2$-adiques a été faite selon la méthode
...
à partir d'une courbe et il est disponible \todo{trouver référence}.

Pour l'analyse des temps de calcul de notre algorithmes de Couveignes 
$\ell$-adique, on a décomposé notre algorithme en 3 parties:
\begin{enumerate}
\item[Tate Module] est la première partie qui calcule une base diagonale de la 
$\ell^k$ torsion avec $k$ le plus petit entier tel que $2^{2k-2}(2+1)> 2r$,
cela représente donc les étapes à de l'algorithme~\ref{alg:cou:ell-adique};
...
l'algorithme sont aussi effectués, cela représente donc les étapes à 
de l'algorithme~\ref{alg:cou:ell-adique};
\item[Calcul Isogenie Step] est la troisième partie qui calcule uniquement 
les différentes polynômes d'interpolation $A_{i,j}$ puis l'interpolation de 
Cauchy, cela représente donc les étapes à de 
l'algorithme~\ref{alg:cou:ell-adique}.
\end{enumerate}
