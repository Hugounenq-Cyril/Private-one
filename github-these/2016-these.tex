...
Le calcul de la $p^k$-torsion peut donc être effectué itérativement à l'aide de la $p$-descente, pour celle-ci on se sert de la décomposition de la multiplication par $p$: 
\begin{equation}
[p]=\pi \circ V
\end{equation}
...
La preuve suivante n'est pas différente de celle dans \cite{DeFeo10} mais celle-ci est tout de même intéressante par l'approche qu'elle a et notamment l'étude de l'action du Frobenius sur le $p$ module de Tate d'où sa retranscription ici.
...
X^2 - t_{\pi}X + q = 0
\end{equation}
Maintenant si l'on regarde cette équation restreiente à la $p$-torsion, on a alors l'équation qui a deux solutions $0 \bmod p$ et $t_{\pi} \bmod p$. La solution $0 \bmod p$ pouvant être écartée car le Frobenius ne peut avoir de noyau non trivial. Ainsi l'action du Frobenius sur la $p$-torsion revient à une multiplication par $t_{\pi} \bmod p$. Dés lors si l'on regarde cette équation sur $T_{p}(E)$ on voit que l'action du Frobenius peut être représentée par une multiplication par $\mathfrak{t}_{\pi} \in \mathbb{Z}_{p} $. 
...
Dés lors pour tout $k\geqslant 1$ en appliquant le résultat du lemme à $t_{k+1}=\mathfrak{t}_{\pi} \bmod p^{k+1}$ on obtient que $\mathrm{ord}(t_{k+1})=\mathrm{ord}(t_{k})$ implique que $\mathrm{ord}(t_{k})=\mathrm{ord}(t_{k-1})$ ce qui termine la preuve.
\end{proof}
...
\begin{lem}
Le coût de calcul de la $p^k$-torsion est d'au moins $\Omega(\ell^2)$ opérations sur $\mathbb{F}_q$.
\end{lem}

\begin{proof}
On utilise donc les méthodes préconisées par les travaux de Gunji et Voloch. Pour la dernière itération de la $p$-descente on a tout d'abord besoin de résoudre une équation d'Artin Schreier \eqref{eq:vol:cov}, pour cela \cite{Couveignes96} utilise un précalcul de l'inverse de l'application $\mathbb{F}_q$-linéaire  $x^q-x$ qu'il applique ensuite. Il est fait de même avec l'application $\mathbb{F}_q$ linéaire $x \rightarrow x^p - x$. Le précalcul a un cout de $O(p^{wk})=O(\ell^{w})$ opérations sur $\mathbb{F}_q$ avec un stockage de $O(\ell^2)$ éléments de $\mathbb{F}_q$.
...
\begin{equation}
A(x([n]P))=x([n]P') \bmod T(x) \quad \text{pour } \n \in \left( \mathbb{Z}/p^k\mathbb{Z} \right)^*
\end{equation} 

avec $T(X)= \prod_{\n \in \left( \mathbb{Z}/p^k\mathbb{Z} \right)^*} (X-x([n]P))$
