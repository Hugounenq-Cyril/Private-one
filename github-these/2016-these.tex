...
\todo{Mettre référence à $\mathsf{R}$ ici! + a l'algorithme de division d'un point}
$\mathsf{R}(i)$ is a bound on the expected cost of finding a root of a polynomial of degree
  $\ell$ in $F_i[x]$.
  For general $\ell$, we have
$\mathsf{R}(i)=O(\ell^i\mathsf{M}(\ell^{i+1})\log(\ell)\log(\ell q))$ using the variant of the
Cantor-Zassenhaus algorithm described in~\cite[Chapter~14.5]{vzGJG03}
...
\STATE\label{alg:point:divide}
  $P' \leftarrow \mathsf{div}(\ell, P_{i})$; $Q' \leftarrow \mathsf{div} (\ell, Q_{i})$.
...
opérations en utilisant l'algorithme de Cantor-Zassenhaus.
  ...
  multiplication-par-$\ell$ par un product de deux $\ell$-isogenies. Pour tout
   $P$ defini in $E(F_{i-\beta})$, le calcul de
  $\mathsf{div}(\ell, P)$ à l'étape ~\ref{alg:point:divide} coute $O(\mathsf{R}(i-\beta+1))$
  opérations.
...
\STATE   $P' \leftarrow \mathsf{div}(\ell, P_{i})$; $Q' \leftarrow \mathsf{div} (\ell, Q_{i})$.
\STATE compute $\pi|(P',Q')=\left( \begin{smallmatrix}
\lambda + a\ell^{i} & b\ell^{i}\\
c\ell^{i} & \mu + d\ell^{i} \end{smallmatrix} \right) \pmod {\ell^{i+1}}.$
\ENDWHILE 
...
\STATE\label{alg:diagonal:divide}
  $P' \leftarrow \mathsf{div}(\ell, \varphi(P))$; $Q' \leftarrow \mathsf{div} (\ell, \varphi(Q))$.
\STATE\label{alg:diagonal:frobenius}
  compute $\pi|(P',Q')=\left( \begin{smallmatrix}
\lambda  & \ell \delta \\
...
\STATE\label{alg:diagonal:solve2}
  \textbf{else} solve equation
 $(-\ell \delta ) y^2 + (c\ell^{k-1}+\lambda )y - \mu $; \textbf{end if}.
\STATE\label{alg:diagonal:upd-P}
  $P \leftarrow P' + y Q'$.
...
%\STATE\label{alg:diagonal:divide}
%  $P' \leftarrow \mathsf{div}(\ell,\widehat{\varphi} (P_{i}))$; $Q' \leftarrow  \widehat{\varphi}(Q_{i})$.
%\STATE\label{alg:diagonal:frobenius}
%  compute $\pi|(P',Q')=\left( \begin{smallmatrix}
%\lambda & b\ell^{k-1}\\
...
\STATE\label{alg:horizontal:divide} $P' \gets \mathsf{div}(\ell, \psi_i(P_{i-1}))$.
