...
\end{matrix}\right ) \quad \text{avec }a,b,d \text{ premiers avec }\ell, \ell^{h} |a-d.
\end{equation*}
% where $a \in \mathbb{Z}$, $0 \leq a \leq \ell^{h} - 1$,
De plus $e = 0$ si~$E$ se situe sur le cratère,
sinon $e$~est la profondeur de~$E$ dans le volcan.
Lorsque $e \neq 0$ on a $c=0$, sinon $c$ est premier avec $\ell$.
...
le discriminant on montre que $a \neq  d \bmod \ell^{h+1}$. \todo{doit-on expliciter que $\mathsf{discriminant}=\det/\ell^{2h}$}
...
$E[\ell^{h+i}]=\langle P,Q \rangle$.
Alors la matrice 
\[
\pi(P,[x]P+[y]Q)=\left ( \begin{matrix}a & b\ell^{h} \\ c\ell^{h} & d
\end{matrix}\right ) 
\] représentant l'action du Frobenius dans la base $\langle P,Q \rangle$ est 
conjuguée aux matrices de la forme:
\[
\pi(P,Q')=\left ( \begin{matrix}a & b\ell^{h}\beta \\ c\ell^{h}\beta & d
\end{matrix}\right ) 
\]
avec $Q'=[x']P+[y']Q$, $\beta \in \left( \mathbb{Z}/\ell^{i} \mathbb{Z} \right)^{\times}$. De plus tous les points qui forment des matrices 
conjuguées à celle de $\pi(P,[x]P+[y]Q)$ engendrent tous la même $\ell^i$-isogénie.
\end{prop}

\begin{proof}
Deux points $Q,Q'$ engendrent la même $\ell^i$-isogénie si ils vérifient $x'y'^{-1}=xy^{-1} \bmod \ell^{i}$
On va distinguer les cas selon que l'on ait $y \wedge \ell=1$ ou $y \wedge \ell=\ell$.
\begin{itemize}
\item[$y \wedge \ell=1$] soit le point de $\ell^i$ torsion $[\ell^h](xP+yQ)$, 
le groupe $\langle [\ell^h]([x]P+[y]Q) \rangle$ engendré par ce point peut aussi s'écrire:$\langle [x]([\ell^h]P)+ [y]([\ell^h]Q) \rangle=\langle [xy^{-1}]([\ell^h]P)+ ([\ell^h]Q) \rangle$. De même on peut écrire le groupe engendré par $[x']P+[y']Q$ comme $\langle [x'y'^{-1}]([\ell^h]P)+ ([\ell^h]Q) \rangle$, ainsi on a égalité des $\ell^i$-isogénies engendrées par $[x']P+[y']Q$ et $[x]P+[y]Q$ si et seulement si $xy^{-1}=x'y'^{-1}\bmod \ell^{i}$. Ainsi on voit que seul compte les valeurs de $x$ et $y$ modulo $\ell^i$.
On peut vérifier cela en voyant que la conjugaison de la matrice $\left ( \begin{smallmatrix}a & b\ell^{h}\\c\ell^{h} & d
\end{smallmatrix}\right )$ par~$\left ( \begin{smallmatrix}1 & 0 \\0 & 1 + k \ell^i
\end{smallmatrix} \right )$   laisse la matrice invariante modulo $\ell^{h+i}$. De même la conjugaison de la matrice $\left ( \begin{smallmatrix}a & b\ell^{h}\\c\ell^{h} & d
\end{smallmatrix}\right )$ par~$\left ( \begin{smallmatrix}1 & k \ell^i \\0 & 1
\end{smallmatrix} \right )$   laisse la matrice invariante modulo $\ell^{h+i}$.
Par la discussion précédente les seuls changements (modulo blabla...) qui 
préservent l'isogénie générée par $[x]P+[y]Q$ sont les changements de la forme 
$[x]P+[y]Q \rightarrow [\beta x]P+[\beta y]Q$ avec 
$\beta \in \left( \mathbb{Z}/\ell^i\mathbb{Z}\right)^{\times}$. On voit alors 
que matriciellement cela nous donne la matrice 
$\left( \begin{smallmatrix}a & b \ell^h \beta \\c \ell^{h} \beta^{-1} & d
\end{smallmatrix} \right)$  par conjugaison de $\left( \begin{smallmatrix}a & b
 \ell^h \\c \ell^{h} & d \end{smallmatrix} \right)$ par $\left( 
 \begin{smallmatrix}1 & 0 \\0  & \beta \end{smallmatrix} \right)$.
Ainsi les seuls changement qui respectent cela sont les changements de la forme, ce qui nous donne les matrices de l'énoncé.
\end{itemize}
\end{proof}

\begin{prop}
Soient $P $ et $R=[x]P+[y]Q$ tels que $\langle P ,R \rangle=E[\ell^{h+i}]$ 
alors la forme de la matrice $\pi(P,R)$ ne dépend que des valeurs de 
$x\ell^h \bmod \ell^i$ et $y\ell^h \bmod \ell^i$
\end{prop}

\begin{proof}
Soient deux points $[x]P+[y]Q$ et $[x']P+[y']Q$ tels que 
$\pi([x]P+[y]Q)=[b\ell^h]P+[d]([x]P+[y]Q)$ et 
$\pi([x']P+[y']Q)=[b\ell^h]P+[d]([x']P+[y']Q)$. On a alors 
$\pi([x-x']P+[y-y']Q)=[d]([x-x']P+[y-y']Q)$ ainsi $[x-x']P+[y-y']Q$ est un 
espace propre pour le Frobenius c'est donc par la proposition \todo{mettre référence}
un point de $\ell^h$ torsion. On a donc le résultat.
\end{proof}

\begin{prop}
Soient $P,R,R'$ trois points de $E$ définie au niveau du cratère du volcan de 
$\ell$-isogénies tels que $\langle P, R \rangle = \langle P,R' \rangle= 
E[\ell^{h+i}]$ et $\pi(P,R)=\pi(P,R')$ alors $R$ et $R'$ engendrent la même 
$\ell^i$-isogénie.
\end{prop}

\begin{proof}
Comme énoncé dans la proposition \ref{pro:cla:fro:mat} on a jusqu'à 
$\ell^{h+i-1} \times \ell^{h+i}$ choix possible de second point pour compléter 
une base de la $\ell^{h+i}$-torsion de $E$ or pour ces choix de matrice seul 
compte la valeur de $x\ell^h$ et $y\ell^h$ par la proposition \todo{mettre référence}.
On a donc $\ell^{i-1} \times \ell^i$ matrices possibles, or chaque 
$\ell^i$-isogénie est associée à $\ell^{i-1}$ matrices distinctes par la 
proposition \todo{mettre référence}. Ainsi par la cardinalité on a donc chaque 
matrice à classe de conjugaison près par $\left( 
 \begin{smallmatrix}1 & 0 \\0  & \beta \end{smallmatrix} \right)$ qui 
représente une $\ell^i$-isogénie différente.
\end{proof}

\begin{lem}
$\pi(P+\gamma \ell^{h+i-1}R,R)=\pi(P,R)$ 
\todo{pas faux mais à fixer...}
\end{lem}

\begin{proof}
La matrice $\left( \begin{smallmatrix}a & b\ell^h \\c\ell^h  & d \end{smallmatrix} 
\right)$ est invariante par action de la matrice $\left( 
\begin{smallmatrix}1 & 0 \\-\gamma\ell^{h+i-1}  & \beta \end{smallmatrix} 
\right)$
\end{proof}
