...
%\newglossaryentry{ScalarMult}{
%		name=$[n]P$,
%		description={Multiplication scalaire par $n$ d'un point d'une courbe elliptique}% 
%		}
%\newglossaryentry{Torsion}{
%		name=$E[n]$,
%		description={Ensemble des points de $n$-torsion d'une courbe elliptique}% 
%}
%\newglossaryentry{TowerElement}{
%		name=$\mathsf{F}_i$,
%		description={$i$-ème corps de la tour d'extensions $\ell$-adiques}%
%}
%\newglossaryentry{IsoDes}{
%		name=$\varphi$,
%		description={isogénie descendante}%
%}
%\newglossaryentry{IsoMon}{
%		name=$\widehat{\varphi}$,
%		description={isogénie montante/ ascendante}
%}
%\newglossaryentry{Riso}{
%		name=$\phi$,
%		description={une $r$-isogénie}
%}
%\newglossaryentry{Hiso}{
%		name=$\psi$,
%		description={isogénie horizontale}
%}
%\newglossaryentry{Diso}{
%		name=$\widehat{\psi}$,
%		description={isognie duale de l'isogénie $\psi$}
%}
%\newglossaryentry{PolMod}{
%		name=$\Phi_{\ell}$,
%		description={$\ell$-ème polynôme modulaire}
%}
...
\section{Notations} \todo{A enrichir}
...
\todo{Faire une liste de notations}
...
Reflechir a la variante proposée par François disant que l'on peut se contenter de calculer une vecteur propre pour le Frobenius, déjà pour calculer des points horizontaux on est obligé de calculer une base diagonale de la $\ell^{h+1}$ torsion.
...
$\ell^k:$ $\frac{ell^{k-1}}{2}$ est strictement plus grand que $2r$.

Pour le calcul du point horizontal d'ordre $\ell^k$ on est tout de même obligé 
de calculer une base horizontale de la $\ell^{h+1}$-torsion et de travailler 
dans l'élément $\mathsf{F}_{h+1-\beta}$ de la tour d'extensions $\ell$-adiques.
En rappelant que l'on travaille avec la convetion $\alpha \geqslant \beta$, 
avec $\alpha$ et $\beta$ définis comme dans la proposition~\ref{}
Donc le coût de calcul de la base horizontale est de:.

Pour le calcul du polynôme d'interpolation on utilise donc des méthodes 
similaires à celles de la section~\ref{sec:interpolation} mais plus proches de 
celles de \cite{DeFeo11} vu qu'ici on travaille uniquement avec un sous-groupe 
cyclique. Ainsi on travaille avec le sous-groupe cyclique d'ordre $\ell^k$ 
défini dans la plus petite extension $\ell$-adique: $\mathsf{F}_{k-\alpha}$. 
On a alors $\frac{\ell^{k-1}}{d_0\ell^{k-\alpha}}=O(\ell^{\alpha-1})$ représentants 
des orbites selon l'action du Frobenius à calculer dans $\mathsf{F}_{k-\alpha}$.
Le calcul de ces représentants a un coût de $O()$.

Ensuite on doit calculer le polynôme d'interpolation avec $t=$, $s=$...

Ainsi pour le calcul du polynôme 
