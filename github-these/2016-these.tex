...
l'on $A(v)+T(v)=w$ on va donc chercher à calculer le représentant minimal de 
$A$ dans la classe de polynômes $\mathsf{F}_n[x]/ \langle T \rangle$.
...
Ainsi on note $T^{(0)}=(x-v)$, $\sigma_i$ l'application qui applique à tous les
 ...
 $T^{(i,j)}=\sigma_j(T^{(i)})$, et l'on pose:  
\begin{equation*}
...
si l'on note $\sigma$ l'action de $\pi \in 
\mathsf{Gal}(\mathsf{F_n}:\mathbb{F}_q)$ sur un polynôme représentant l'action 
de $\pi$ sur chacun des coefficients du polynôme on doit avoir:
\begin{equation*}
\sigma(A(v))=\pi(w)
\end{equation*} 
