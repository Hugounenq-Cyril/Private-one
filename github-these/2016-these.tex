...
\todo{Faire une notation cohérente avec $d_K$ au lieu de $D$...}
\begin{cor}
\label{cor:tab:vol} 
...
sur les points de la courbe qui génèrent leur noyau. C'est d'ailleurs 
l'approche que l'on aura tout au long de cette section, pour déterminer les 
directions des isogénies on étudiera l'action du Frobenius sur un point 
...
\subsubsection{Calcul des directions dans le volcan de $\ell$-isogénies}

L'objectif de cette sous-section est de calculer pour une courbe se trouvant 
sur un volcan de $\ell$-isogénies une base 
horizontale (ou partiellement horizontale) de la $\ell^k$-torsion. On a vu à la 
proposition~\ref{pro:dia:hor} que l'on devait avoir $k \geqslant h+1$ avec $h$
la hauteur du volcan, on rappelle aussi que $\beta$ représente la plus grande 
valuation $\ell$-adique de la $\ell$-torsion rationnelle pour une courbe située
sur le cratère du volcan de $\ell$-isogénies(voir définition~\ref{def:alp:bet}).
...
\end{lem}
\todo{dire coût moyen ou coût espéré}
...
\end{rem}
\todo{remarque à revoir plus tard}
\begin{rem}
...
\begin{algorithmic}[1]
\REQUIRE $E_s$: une courbe elliptique ordinaire située sur le cratère d'un volcan de $\ell$-isogénies avec cratère cyclique; $k$: un entier positif; $j<k$; $\lambda, \mu \in \mathbb{Z}/\ell^{j}\mathbb{Z}$; $P_j,Q_j$ tels que $E_s[\ell^j]=\langle P_j,Q_j \rangle$ $\pi(P_j)=[\lambda] P_j, \pi(Q_j)=[\mu]Q_j$;
...
\todo{Algorithme utile pour décomposer le très gros algorithme}
\begin{prop}\label{pro:alg:diagonal}
  Soit $E$ une courbe située au niveau du cratère du volcan de $\ell$-isogénies
  dont on connaît une base diagonale de la $\ell^j$-torsion, 
  l'algorithme~\ref{alg:bas:diag} calcule une  base diagonale
...
\ref{alg:rem:tri:trigonal:fin} sont définis dans $\mathsf{F}_{h+1-\beta}$.
\end{lem}
...
\max(0,e-i-(h-\beta))}$ \emph{i.e.} $\mathsf{F}_{h-e+1-\beta+\max(0,e-(h-\beta
))}$ par la proposition~\ref{pro:niv:str}. En particulier les antécédents par
$[\ell]$ de $P_{h-e+i+1}^{(h-e+i)}$ d'ordre $\ell^{h+1}$ sont définis dans 
$ \mathsf{F}_{h-e+1-\beta+\max(0,e-i-(h-\beta))}$ or cette boucle s'arrête 
lorsque l'on atteint la $\ell^{h+1}$ torsion pour la courbe au niveau du cratère, les opérations 
 ...
 \ref{alg:rem:tri:rem:divide} on transporte la base sur une courbe située un niveau
...
tels que $\pi(P_h)= [\lambda] P_h$, $ \pi(Q_h)= [\mu] Q_h$; $L$ une liste de 
...
\STATE\label{alg:rem:tri:rem:iso}  $\widehat{\varphi} \leftarrow $ isogénie de noyau $\langle [\ell^{h-e+i}]P_{h-e+i+1}^{(h-e+i)} \rangle$
...
\STATE\label{alg:rem:tri:rem:solve1}
  ...
  $ y^2 \delta \ell + y (\ell^{h-e+i}(a-d)+\lambda - \mu) - c\ell^{h-e+i} $; \textbf{fin si}.
\STATE
  $P' \leftarrow P' + [y \ell^{h-e+i}] Q'$; $\lambda \leftarrow \lambda + a \ell^{h-e+i} + \delta y, \mu \leftarrow \mu + d \ell^{h-e+i} -\delta y$.
...
\RETURN $P_i,Q_i, \lambda, \mu, L$
\end{algorithmic}
\end{algorithm}
\begin{prop}
L'algorithme~\ref{alg:rem:tri} a une complexité de $O()$ opérations sur $\mathbb{F}_q$
\end{prop}

\begin{proof}
\begin{itemize}
\item[\'Etapes \ref{alg:rem:tri:trigonal:debut} à \ref{alg:rem:tri:trigonal:fin}] Ensuite on passe à la partie entre les étapes \ref{alg:ult:trigonal:debut} et \ref{alg:rem:tri:trigonal:fin} où l'on calcule à chaque boucle une base de la $\ell^{h-e+i}$ torsion dans laquelle l'action du Frobenius est triangulaire, et l'on remonte d'un niveau dans le volcan et l'on trigonalise à nouveau la $\ell^{h-e+i}$ torsion. Cette étape est répétée du niveau $h-e$ où l'on exprime l'action du Frobenius dans une base de la $\ell^{h-e+1}$ torsion au niveau $h$ où l'on exprime l'action du Frobenius dans une base de la $\ell^{h+1}$ torsion. Par le lemme~\ref{lem:alg:rat} les points avec lesquels on travaille sont toujours définis dans le même corps $\mathsf{F}_{h+1-\beta}$.
Ainsi le coût des étapes \ref{alg:rem:tri:rem:divide} et \ref{alg:rem:tri:trigonal:divide} est de $\mathsf{R}(h+1-\beta)$, voire discussion dans la preuve de la proposition~\ref{pro:alg:rem:tri} pour le calcul de $E^{(h-e+i)}[\ell]$, le coût de calcul de $\pi(P)$ est de $O(\ell^{h+1-\beta}\mathsf{M}(\ell))$. L'écriture de $\pi(P)=[u]P+[v]Q$ aux étapes \ref{alg:rem:tri:rem:frobenius} et \ref{alg:rem:tri:trigonal:frob} a un coût de $O(\ell^2 \mathsf{M}(\ell^{h+1-\beta}))$. Lors de la boucle $h-e+i$, les étapes \ref{alg:rem:tri:trigonal:solve1} et \ref{alg:rem:tri:rem:solve1} sont résolues à l'aide d'une recherche exhaustive et ont un coût négligeable par rapport aux autres opérations. 
...
\begin{algorithm}
\caption{\label{alg:ulti:fro}Calcul d'une base diagonale de $E_s[\ell^k]$ à partir d'une courbe $E$}
\begin{algorithmic}[1]
...
\STATE 
$Q_{i} \leftarrow Q_{i}+[x \ell^h]P_{i}$
\STATE \text{Calcul d'une base diagonale de $E_s[\ell^k]$ à l'aide de l'algorithme~\ref{alg:bas:diag} avec entrées $P_h,Q_h,\lambda,\mu$} \label{alg:ult:diagonal:debut}
...
\end{defi}
\todo{Faire explicitement le lien avec le chapitre 4}
...
\todo{faire un dessin avec un exemple de volcan avec les matrices correspondantes au niveau}
...
\todo{Faire un dessin avec un volcan qui représente le lien entre les isogénies et la forme de la matrice}
\begin{prop}
\label{pro:atk:equi}
...
à étudier la $\ell^{h+i+e}$-torsion.


Le résultat suivant nous permet de distinguer les 
différents générateurs d'une même $\ell^{i-e}$-isogénie.
...
\end{tikzpicture}
\end{center}
\caption{\label{fig:atk:dif:niv:iso} Exemples des isogénies obtenues à partir de différentes matrices du Frobenius calculées sur la courbe située au niveau du cratère du volcan de $\ell$-isogénies. Pour cet exemple on a travaillé avec une courbe de $j$-invariant $56$ définie sur $\mathbb{F}_{149}$.}
\end{figure}
...
proposition~\ref{pro:atk:equi}) a été représenté.
...
\todo{On peut faire un dessin pour expliquer en quoi c'est pas bien! Ce sera très parlant}
\begin{figure}
\begin{center}
...
$\color{blue} \mu$ pour les correspondances induites par la $r$-isogénie.  \todo{préciser et faire référence aux isogénies descendantes pour lesquelles le Frobenius ne peut rien, cela est vu en annexe}
