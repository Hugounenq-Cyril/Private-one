...
Dans cette section nous présentons tout d'abord l'algorithme de Couveignes~\cite{Couveignes96}, puis ensuite les différentes améliorations apportées par De Feo dans \cite{DeFeo11}.
...
\begin{proof}
Nous procèdons de manière similaire à la preuve de la proposition~\ref{pro:clas:fro}.
...
\end{nota}
\todo{conflit de notation de $d_i$}
...
précalcul qui a un coût booléen de $O(\log(p_{i})\log(\ell_i^{k_i}qd_{(i,1)}))$ et  
...
anneau d'endomorphismes spécifique à partir d'une courbe choisie au hasard dont 
