...
\begin{algorithm}
\caption{\label{alg:ulti:fro:new}Calcul d'une base diagonale de $E_s[\ell^{h+1}]$ à 
...
\ell^{h-e+1}\mathbb{Z}$ tels que $E[\ell^{h-e+1}]=\langle P,Q \rangle$, $\pi(P)
=[\lambda]P, \pi(Q)=[b \ell^{h-e}]P +[\mu]Q$;
\ENSURE $(P_h, Q_h)$: une base de $E_s[\ell^h]$;
$\lambda, \mu \in \mathbb{Z}/\ell^h \mathbb{Z}$
tels que $\pi(P_h)= [\lambda] P_h$, $ \pi(Q_h)= [\mu] Q_h$.
