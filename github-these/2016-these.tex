...
\end{equation}
est un recouvrement étale de degré $p$ et est isomorphe à $E$ sur $\mathbb{F}_q[c]$ à l'aide l'isomorphisme suivant:
\begin{equation}
\label{eq:vol:iso}
(\tilde{x},\tilde{y})=V(x,y) \\
...
Comme le travail de \cite{Defeo11} repose sur les constructions de tour d'Artin Schreier, dont une brève description sera faite pour plus de détails voir \cite{DeFeo-Shost'12}, et que celles-ci sont construites comme des extensions de $\mathbb{F}_q$ alors les complexités seront exprimées en terme d'opérations sur $\mathbb{F}_p$ en utilisant la convention que l'on a $q=p^d$.
...
Pour la calcul de la $p^k$-torsion on a tout d'abord besoin de savoir la taille du corps dans lequel on est ammené à effectuer les calculs. Pour cela on se sert du résultat suivant issu de \cite{DeFeo10} qui est une généralisation de \cite[Proposition 26]{Ler97a} 
...
\begin{prop}
\label{pro:ord:fro}
Pour une tour d'extension $p$-adique de $\mathbb{F}_q$ défini comme dans \ref{def:p-adic} on a:
\begin{itemize}
...
\begin{proof}
On a $[\mathbb{U}_1:\mathbb{F}_q] \mid p-1$ par \ref{thm:Gunji}. Prouvons maintenant le second point,
on rappelle tout d'abord l'équation caractéristique du Frobenius:
\begin{equation}
X^2 - t_{\pi}X + q = 0
\end{equation}
Maintenant si l'on regarde cette équation restreinte à la $p$-torsion, on a alors l'équation qui a deux solutions: $0 \bmod p$ et $t_{\pi} \bmod p$. La solution $0 \bmod p$ pouvant être écartée car le Frobenius ne peut avoir de noyau non trivial. Ainsi l'action du Frobenius sur la $p$-torsion revient à une multiplication par $t_{\pi} \bmod p$. Dés lors si l'on regarde cette équation sur $T_{p}(E)$ on voit que l'action du Frobenius peut être représentée par une multiplication par $\mathfrak{t}_{\pi} \in \mathbb{Z}_{p} $. 
...
Enfin à l'aide de ces pré-calculs on effectue la résolution de l'équation d'Artin Schreier à l'aide de $O(\ell^2 )$ opérations sur $\mathbb{F}_q$, le coût de l'inversion de l'application $\mathbb{F}_q$ linéaire $x \mapsto x^p - x$ étant négligeable, pour plus de détails le lecteur peut voir \cite[§2.4]{Couveignes96}.
\end{proof}
