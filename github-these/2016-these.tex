...
nous permet de savoir dans quelle mesure on peut trouver des nompres premiers 
$l$ de Elkies pour une courbe quelconque. Plus précisément pour presque 
tous les nombres premiers $q$ et courbes $E/\mathbb{F}_q$, pour 
$L\geqslant\log(q)^\epsilon$ pour n'importe quel $\epsilon>0$,
on obtient asymptotiquement que la moitié des nombres premiers $\ell \leqslant 
L$ sont des nombres premiers de Elkies.
Ainsi on espère avoir des nombres de Elkies suffisamment petits pour les 
utiliser avec notre algorithme.
Le théorème suivant donne une borne sur le pire des cas en fonction uniquement
 de $r$ et $q$.
\begin{thm}
