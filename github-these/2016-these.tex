...
$O((h+1)\mathsf{R}(h+1-\beta)+(h+1)\ell^2 \mathsf{M}(\ell^{h+1-\beta})+\ell \mathsf{M}(\ell^2)\log(\ell)\log(\ell q))$, 
et donc en bornant $h$ par $k$ on a $\ell^h \in O(r)$ ce qui en utilisant cette
borne et le coût de $\mathsf{R}(i)$ donné par~\cite[chapter 14.5]{vzGJG03} nous 
donne une complexité de :
$O(\mathsf{M}(r\ell^2)r\ell^2\log(\ell)\log(\ell q) \log(r))$.
\end{proof}
Ce qui comparé au résultat dans la preuve de la 
proposition~\ref{pro:full-complexity} nous donne un résultat quadratique en $r$
contre un résultat quasi linéaire auparavant.
...
Ensuite on doit calculer le polynôme d'interpolation en appliquant le résultat
 de la proposition~\ref{prop:interpol} avec $t \in O(r)$, $s=\ell^{\alpha-1}$, 
...
$O(\mathsf{M}(\ell^2r)\log(\ell)\log(r))$ opérations sur $\mathbb{F}_q$.
