...
is to consider a \emph{multiplicative lattice} in~$(\mathbb{Q}_{\ell}^×)^2$.

Let~$\varepsilon$ be a primitive $(\ell-1)$-th root of unity in~$\mathbb{Z}_{\ell}$.
Then by~\cite[Théorème II.3.2]{Serre.Arith},
the map~$f(x, y, z) = \ell^x· \varepsilon^y· \exp (\ell z)$
is a group isomorphism between~$\mathbb{Z} × (\mathbb{Z}/(\ell-1) \mathbb{Z}) × \mathbb{Z}_{\ell}$ and~$ℚ_{\ell}^{×}$.
For~$i ∈ \bcro{0,k-1}$ and~$c ∈ \mathbb{Z}/(\ell-1)\mathbb{Z}$,
...
We also define~$W(i,j,c,d) = V(i, c) · P \,+\, V(j, d) · Q ⊂ E[\ell^k]$.

Since~$\lambda ≡ 1 \pmod{\ell^{\alpha}}$, we may write~$\lambda = f(0,0,u\, \ell^{\alpha-1})$
and~$\mu = f(0,0, v\, \ell^{\beta-1})$
for some~$u, v ∈ \mathbb{Z}_{\ell}^{×}$.
...
the columns of~$\smat{\ell^i & 0 & u \ell^{\alpha-1}\\ 0 & \ell^j & v \ell^{\beta-1}}$,
whereas the size of each orbit is~$[(\mathbb{Z}/\ell^i \mathbb{Z})×(\mathbb{Z}/\ell^j \mathbb{Z})\::\: \Lambda_{i,j}]$.
