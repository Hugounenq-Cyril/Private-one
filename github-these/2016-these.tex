...
$\ell_1, \ell_2, \ell_3, \dots$, pour une isogénie de degré $r$ on 
calcule des bases horizontales de la $\ell_i^{k_i}$ torsion avec $k_i>h_i$ et 
$k_i$ tel que $\frac{\ell_i^{2k_{i}-2}(\ell_i+1)}{4}<r$ mais dès lors il faudrait 
trouver un autre moyen que celui déjà utilisé pour valider une correspondance 
entre deux bases horizontales de la $\ell_i^{h_i}$ torsion.
