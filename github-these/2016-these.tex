...
\begin{lem}
Soient $H(t)=1+\tilde{A}t^4+\tilde{B}t^6$, $G(x)=\frac{1}{1+Ax^4+Bx^6}$ alors à partir de $S$ une solution de l'équation différentielle $(S')^2=(H \circ S) G$ telle que $G(0)H(\alpha)=\beta^2$ définie modulo $x^{d+1}$, une solution de cette même équation définie modulo $x^{2d}$ peut être calculée en utilisant $O(\mathsf{M}(2^d))$ opérations.
\end{lem}
