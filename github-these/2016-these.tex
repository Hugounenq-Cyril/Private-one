...
tels que $\pi(P_k)= [\lambda] P_k$, $ \pi(Q_k)= [\mu] Q_k$.
% \STATE Calcul $(P_1,Q_1)$, a basis of $E[\ell]$
\STATE$(\lambda, \mu, i) \leftarrow (0,0,0)$; $\mathsf{trigonal} \leftarrow \mathsf{Vrai} $; $\mathsf{scalaire} \leftarrow \mathsf{Vrai}$ ;$P', Q' \leftarrow $ tels que~$\langle P',Q' \rangle =  E[\ell]$. %; $\mu \leftarrow 0$;$i \leftarrow 0$
...
Soient $E$, $E'$ deux courbes elliptiques de $j$-invariants $j$ et $j'$ reliées par une isogénie $\phi$ de degré $r$, alors pour deux bases diagonales $(P,Q),(P',Q')$ de $E$ de $E[\ell^k]$ et $E'[\ell^k]$ telles que 
\[
...
\end{matrix} \right) 
\]
alors comme l'isogénie $\phi$ commute avec le Frobenius on a $\phi(P) \in \langle P' \rangle $ et $\phi(Q) \in \langle Q' \rangle $, cependant comme la matrice $\pi$ est scalaire modulo la $\ell^{h-1}$ torsion alors on a $\phi(P) \in \langle P'+[\ell^{k-h}]Q' \rangle $ et $\phi(Q) \in \langle Q'+[\ell^{k-h}]P' \rangle $,
on pourrait croire que cela est suffisant cependant comme l'action du Frobenius sur la $\ell^h$ torsion est égal à l'action d'une matrice scalaire alors l'information
...
Par définition de ~$k$, on a ~$\ell^{2k+1} \in O(r\ell^2)$.
  ...
  celui du calcul de~$A_{a,b}$~\cite[§3.3]{DeFeo10}.  Enfin, en moyenne 
