...
\begin{prop}
Soit une courbe elliptique $E$ définie sur $\mathbb{F}_q$ sur un volcan de $\ell$-isogénies à cratère cyclique ($\ell$ est donc un nombre de Elkies). Soit $(P,Q)$ une base diagonale de $E[\ell^{h+i}]$, soit $R=[a]P+[b]Q$ un point d'ordre $\ell^{h+i}$, alors pour toute $\ell^i$-isogénie descendante il existe une transformation \begin{equation*}
R=[a]P+[b]Q \rightarrow R'=[*]P+[yb]Q
\end{equation*}
telle que $\pi(P,R)=\pi(P,R')$
\end{prop}
...
$\ell^{h}b^{-1}y^{-1}*$. Fixons par exemple $\gamma = \ell^{h}b^{-1}y^{-1}*$
