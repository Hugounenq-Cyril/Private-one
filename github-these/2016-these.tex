...
section~\ref{sec:interpolation}, le calcul de $T$ à partir des $T_j$ a un même
coût que dans la proposition~\ref{prop:interpol} de $O(\mathsf{M}(t)\log(s))$. 
Cependant comme on a vu précédemment le coût de calcul des $A_j$ est de 
$O(n\mathsf{M}(t_j\max_i(d_i\ell_i^{k_i}))(p_{n-1})^{(\omega-1)/2}
\max_i(\ell_i(k_i+1)))$ on majore donc le cout total par $O(n\mathsf{M}(t\max_i(d_i
...
Couveignes à l'aide du coût de calcul de la partie de l'interpolation. 
L'interpolation étant répétée $O(r \prod_{i=1}^n\ell_i)$ en moyenne, on peut 
donc estimer le coût de cet algorithme par $O(r (\prod_{i=1}^n\ell_i)n\mathsf{M}(r (\prod_{i=1}^n \ell_i^2)
\max_i(d_i\ell_i^{k_i}))(p_{n-1})^{(\omega-1)/2}\max_i(\ell_i (k_i +1)) )$. 
Avec $p_n \in O(r(\prod_{i=1}^n\ell_i)^{3,5})$ cela nous donne une estimation 
de 
\[ 
