...
Comme dit précédemment on prend avantage du faible coût du Frobenius dans une tour d'Artin-Schreier à l'aide de l'algorithme \cite[Iter Frobenius,Theorem 17 et 18]{DeFeo-Shost'12} qui a une complexité de $O((i-j)L(i))$ pour le calcul d'une puissance $p^{p^{jd}}$ d'un élément de $\mathbb{U}_i$.
...
On présente uniquement les idées directrices pour calculer $T$, les idées développées par \cite{DeFeo11} et auparavant par \cite{enge2008} étant reprises très largement dans la section \textcolor{red}{ref à mettre plus tard} elles seront plus détaillées à cette occasion.
...
\begin{tikzpicture}[scale=0.3]
		...
		\draw (ABAa) node[fill=white][left]{$\color{red}{T_0^{(0)}}$};%ancien \pi(T)
		...
		\draw (ACA)--(ACAa);
		\draw (ACAa) node[fill=white]{$\pi^2(T_0^{(0)})$};%ancien \pi^2(T)
		...
		\draw (ACA)--(ACAb);
		\draw (ACAb) node[fill=white]{$\pi^6(T_0^{(0)})$};%ancien \pi^3(T)
		\draw (ACA) node[fill=white]{$\color{red}{\pi^2(T_0^{(1)})}$};% ancien \pi^2(T_2)
		...
		\draw (ABA)--(ABAb);
		\draw (ABAb) node[fill=white]{$\pi(T_0^{(0)})$};%ancien \pi^4(T)
		...
		\draw (ABA)--(ABAa);
		\draw (ABAa) node[fill=white,left]{$\pi^5(T_0^{(0)})$};%ancien \pi^5(T)
		\draw (ABA) node[fill=white]{$\pi^4(T_0^{(1)})$};%ancien \pi^4(T_2)
		...
		\draw (AC)--(ACA);
		\draw (AC) node[fill=white]{$\color{red}{\pi^(T_0^{(2)})}$};%ancien \pi^4(T_3)
		...
		\draw (ACA)--(ACAa);
		\draw (ACAa) node[fill=white]{$\pi^3(T_0^{(0)})$};%ancien \pi^6(T)
		...
		\draw (ACA)--(ACAb);
		\draw (ACAb) node[fill=white]{$\pi^7(T_0^{(0)})$};%ancien \pi^7(T)	
		\draw (ACA) node[fill=white]{$\pi^6(T_0^{(1)})$};%ancien \pi^6(T_2)	
		\end{scope}
...
\end{tikzpicture}
\end{center}
\caption{Arbre de sous produits pour le calcul de $T_0^{(0)}$ dans le cas d'une tour d'extension quadratique. Seul les éléments en rouge de l'arbre sont pris en compte pour le calcul, $\pi$ peut être n'importe quel élément générateur de $\mathsf{Gal}(\mathbb{U}_3:\mathbb{U}_0)$.}
\label{fig:orb:T}
\end{figure}
...
Le calcul de $A_0$ peut lui aussi être effectué efficacement à l'aide du Frobenius, une présentation détaillée sera faite en \textcolor{red}{ref à mettre}. Ainsi avec l'utilisation du Frobenius on obtient le résultat suivant pour le calcul du polynome d'interpolation sur les points d'ordre $p^k$:
