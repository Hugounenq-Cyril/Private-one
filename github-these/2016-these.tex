...
\section{Etude sur le module de Tate}
\label{cha:act:fro}
...
Enfin on verra si l'on peut tirer des 
informations sur les directions dans le volcan pour des courbes situées 
en-dessous du cratère excepté le résultat de la proposition.
...
le corollaire~\ref{cor:elk:bet:reg} permet d'utiliser notre résultat aux autres
courbes. 
...
\subsubsection{Coupure Algorithme}

On travaille sur une courbe elliptique se trouvant sur un volcan de 
$\ell$-isogénies à cratère cyclique (cas Elkies),  on veut calculer une base 
horizontale (ou partiellement horizontale) de la $\ell^k$-torsion, on suppose 
de plus que l'on ne connaît pas la cardinalité de la courbe. On rappelle que 
l'on note $h$ la valuation $\ell$-adique de la différence des valeurs propres 
du polynôme caractéristique de $\pi$ et que $h$ représente la hauteur du volcan
de $\ell$-isogénies (voir proposition~\ref{pro:hau:vol}). On a vu à la 
proposition~\ref{pro:dia:hor} que l'on devait avoir $k \geqslant h+1$, on 
rappelle aussi que $\beta$ représente la plus grande valuation de la 
$\ell$-torsion rationnelle pour une courbe située sur le cratère du volcan de 
$\ell$-isogénies(voir définition~\ref{def:alp:bet}).

On commence tout d'abord par énoncer un algorithme qui sera utilisée dans tous 
...
$\mathsf{divise}(\ell,P)$. Ce résultat sera bien évidemment aussi utilisé dans
le cas Atkin.
...
\end{algorithmic}
\end{algorithm}
\todo{remplacer les complexités espérées par ... cout moyen ?}
\begin{prop}
...
\begin{algorithm}
\caption{\label{alg:rem:tri}Calcul d'une base diagonale de $E_s[\ell^k]$ à partir d'une base triangulaire de $E[\ell^k]$}
\begin{algorithmic}[1]
...
\begin{algorithmic}[1]
\REQUIRE $E_s$: une courbe elliptique ordinaire située sur le cratère d'un volcan de $\ell$-isogénies avec cratère cyclique;
$k$: un entier positif;
\ENSURE $(P_k, Q_k )$: une base de $E[\ell^k]$;
$\lambda, \mu \in \mathbb{Z}/\ell^k \mathbb{Z}$
...
remplaçant les points de $p$-torsion par ceux de $\ell$-torsion. Avec une telle
démarche on 
\begin{itemize}
\item calcule une base de la $\ell^k$-torsion telle que $\ell^{2k} \in \tilde 
O(r)$ sur chacune des $2$ courbes;
\item teste toutes les matrices $M \in \mathsf{GL}_2(\mathbb{Z}/\ell^k
...
$O(r^2)$ de matrices à tester, on doit donc travailler avec un ensemble de 
points plus restrictifs si l'on veut obtenir une complexité quadratique comme 
