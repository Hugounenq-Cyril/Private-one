...
une isogénie de degré $r$ on calcule des bases horizontales de la 
$\ell_i^{k_i}$ torsion, on peut alors travailler avec les deux groupes de 
points isomorphes:
\begin{itemize}
...
L'avantage de cette approche c'est que bien que l'on soit obligé de travailler
...
Détaillons comment on procède avec un tel algorithme:
...
\STATE Calcul de $((\color{red}P_{\lambda,i}\color{black},\color{blue}Q_{\mu,i}\color{black}),(\color{red}P'_{\lambda,i}\color{black},\color{blue}Q_{\mu,i}'\color{black}))$ bases (partiellement) horizontales de $E[\ell_i^{k_i}],E'[\ell_i^{k_i}]$ 
...
\STATE \label{alg:cou-ell:int} Calcul des des polynômes $A_{a,b,i}$ et $T_i$ par les méthodes décrites dans la section~\ref{sec:interpolation};
...
Les étapes \ref{alg:cou-ell:bdiag} à \ref{alg:cou-ell:rep} de 
l'algorithme~\ref{alg:cou:ell-adique} qui calculent une base (partiellement) 
...
parallèle sur chacune des tours d'extension $\ell_i$-adiques.
 Ici pour le calcul du polynôme d'interpolation
on définit à l'aide du théorème des restes chinois deux coefficients $a,b \in 
\mathbb{Z}/\prod_{i=1}^n\ell_i^{k_i} \mathbb{Z}$ tels que 
$a=a_i \bmod \ell_i^{k_i}$ et $b=b_i \bmod \ell_i^{k_i}$ pour $i\in [1,n]$, 
avec $a_i,b_i$ choisis comme dans l'algorithme~\ref{alg:cou:ell-adique} c'est 
à dire inversibles dans $\mathbb{Z}/\ell_i^{k_i}\mathbb{Z}$.
On calcule ensuite à l'aide des méthodes de la section~\ref{sec:interpolation} 
sur chacune des tours d'extensions $\ell_i$-adiques les polynômes 
d'interpolation $A_{a,b,i}$ et $T_i$ définis dans $\mathbb{F}_q$.
 ...
 les polynômes $A_{a,b},T \in \mathbb{F}_q$.
On fait le calcul de la fraction rationnelle $F=A_{a,b} \bmod T$ à l'aide d'une
interpolation de Cauchy. Et l'on teste si celle-ci est bien une $r$-isogénie,
...
inférieur à l'étape~\ref{alg:cou-ell:int} de 
l'algorithme~\ref{alg:cou:ell-adique} mais toutefois on a le calcul de 
l'interpolation de Cauchy qui lui est au moins de $O(\mathsf{M}(r)\log(r))$ par
...
boucle pour tous les $a,b$ possibles appartenant à $(\mathbb{Z}/\prod_{i=1}^n\ell_i^{k_i} \mathbb{Z})^*$.
 le coût d'un tel algorithme serait de 
$\Omega(r\mathsf{M}(r)\log(r))$. 

Le principal inconvénient de cette approche 
c'est que l'on fixe comme condition sur les $k_i: \sum_{i=1}^n\frac{\ell_i^{2k_{i}-2}(\ell_i+1)}{4}>r$
...
condition nécessaire: $\prod_{i=1}^n\frac{\ell_i^{2k_{i}-2}(\ell_i+1)}{4}>r$
c'est ce que l'on va aborder dans la seconde approche.

L'algorithme décrit dans cette sous-section est disponible en appendice, c'est 
l'algorithme~\ref{alg:cou:mult-adique}.
...
aurait à tester $\sum_{i=1}^n \ell_i^{2k_i-2} \in \Theta(r)$ choix possibles pour le polynôme 
