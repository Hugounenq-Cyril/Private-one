...
\RETURN $P_{h+1}^{(h)},Q_{h+1}^{(h)}, \lambda \bmod \ell^{h}, \mu \bmod \ell^{h}, L$
\end{algorithmic}
\end{algorithm}
\begin{prop}
L'algorithme~\ref{alg:rem:tri} a une complexité espérée de 
\[O(e(\mathsf{R}(\bigstar)+
\ell^{\bigstar}\mathsf{M}(\ell)+\ell^2 \mathsf{M}
(\ell^{\bigstar})))\] 
opérations sur $\mathbb{F}_q$ avec $\bigstar=h-\beta-(e-i)+1+\max(0,e-i-(h-\beta))$. 
En particulier lorsque $e-i<=h-\beta$, c'est à dire lorsque la courbe en entrée
de l'algorithme se trouve au dessus du premier niveau de stabilité l'aglorithme
a une complexité espérée
de:
\[
O(e(\mathsf{R}(h-\beta-(e-i)+1)+
\ell^{h-\beta-(e-i)+1}\mathsf{M}(\ell)+\ell^2 \mathsf{M}
(\ell^{h-\beta-(e-i)+1}))).
\]
Enfin lorsque $e-i > h-\beta$, c'est à dire lorsque la courbe en entrée se 
trouve à un niveau inférieur au premier niveau de stabilité l'algorithme a une 
complexité espérée de:
\[
O(e(\mathsf{R}(1)+
...
\begin{proof}
\begin{itemize}
\item[\'Etapes \ref{alg:rem:tri:trigonal:debut} à \ref{alg:rem:tri:trigonal:fin}]
Ensuite on passe à la partie entre les étapes \ref{alg:rem:tri:trigonal:debut} et
\ref{alg:rem:tri:trigonal:fin} où l'on calcule à chaque boucle une base de la
$\ell^{h-e+i}$ torsion dans laquelle l'action du Frobenius est triangulaire, et
l'on remonte d'un niveau dans le volcan et l'on trigonalise à nouveau la 
$\ell^{h-e+i}$ torsion. Cette étape est répétée du niveau $h-e$ où l'on exprime
...
-(h-\beta)))$.  Pour le calcul de $E^{(h-e+i)}[\ell]$ ll est à noter qu'il est 
tout à fait vraisemblable de supposer que l'on connaît déjà une base de 
$E^{(h-e+i-1)}[\ell]$ car à chaque étape le coût de ********* \todo{ça!}
 est au maximum le coût d'évaluation la $\ell$-isogénie montante 
 $\widehat{\varphi}$ sur $\ell+1$ points d'ordre $\ell$ définis sur 
 $E^{(h-e+i-2)}[\ell]$. Le coût de calcul de $\pi(P)$ est de $O(\ell^{h-(e-i)+1
-\beta +\max(0,e-i-(h-\beta))}\mathsf{M}(\ell))$. L'écriture de $\pi(P)=[u]P+[v]Q$ 
...
et ont un coût négligeable par rapport aux autres opérations. 
 Le coût étant le même pour chaque boucle on a un coût total pour cette boucle 
...
tels que $\pi(P_k)= [\lambda] P_k$, $ \pi(Q_k)= [\mu] Q_k$.
% \STATE Calcul $(P_1,Q_1)$, a basis of $E[\ell]$
\STATE$(\lambda, \mu, i) \leftarrow (0,0,0)$; $\mathsf{scalaire} \leftarrow \TRUE$; $L \leftarrow \emptyset $ ;$P', Q' \leftarrow $ tels que~$\langle P',Q' \rangle =  E[\ell]$. %; $\mu \leftarrow 0$;$i \leftarrow 0$
...
\STATE \text{Calcul d'une base diagonale $(P_{h+1},Q_{h+1})$ de $E_s[\ell^{h+1}]$, $\lambda, \mu \in \mathbb{Z}/\ell^{h+1}\mathbb{Z}$} \text{ tels que $\pi(P_{h+1}, Q_{h+1})= \left( \begin{smallmatrix}
\lambda & 0\\
