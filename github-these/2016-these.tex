...
multiplication and Euclidean division of polynomials of degree
  at most $d$ in $\mathsf{F}_i[x]$ using $O(\mathsf{M}(d\ell^i))$ operations, via
  Kronecker's
...
\begin{lem}\label{lemma:interpolation:minpoly}
  ...
  operations dans $\mathbb{F}_q$.
\end{lem}

\begin{proof}
  \`A l'étape $i$, à partir de la connaissance de $T^{(i)}$ on  calcule toutes 
  les $T^{(i,j)}$ en utilisant le théorème~\ref{thm:frob-ell}. Le cout du 
  ...
  On additionne ces résultats pour tous les $i$ pour obtenir le résultat 
  énoncé. 
\end{proof}
...
$\mathsf{F}_{n-i}$. On calcule alors le polynôme $L \in \mathsf{F}_{n}[x]/ 
\langle T^{(i)} \rangle$ tel que 
\begin{equation}
\label{eq:crt:int}
L= \sigma(L^{(i)}) \bmod \sigma(T^{(i)}) \quad \text{pour tout} \sigma \in 
\mathsf{Gal}(\mathsf{F}_{n-i}/ \mathsf{F}_{n-i-1}) 
\end{equation} 
ce polynôme $P$ étant invariant sous l'action du groupe de Galois: 
$\mathsf{Gal}(\mathsf{F}_{n-i}/ \mathsf{F}_{n-i-1})$, il est alors défini dans
 ...
 coefficients dans $\mathsf{F}_{n-i}$ alors pour $j \in [0, \ell-1]$ on définit
 $A^{(i,j)}=\sigma_j(A^{(i)})$, et l'on pose:  
\begin{equation*}
...
\begin{proof}
  ...
  \`A l'étape $i$, on calcule la division Euclidienne d'un polynôme de degré
   ...
   On a alors le coût de chaque division qui est de $O(\mathsf{M}(\ell^{n+1}))$ par \todo{Rediger référence !!!}, 
...
On énonce alors un porblème plus général de calcul de polynôme d'interpolation 
...
\begin{proof}
  ...
  elle a un coût algébrique nul. On calcule les $T_i$ utilisant à l'aide 
   $O(n\mathsf{M}(\ell^{n_i+2})\log(\ell))$ opérations par 
   \ref{lemma:interpolation:minpoly} . En bornant 
  ...
  a une profondeur de $O(\log (s))$ et nécessite $O(\mathsf{M}(t))$ opérations
   par niveau. Ainsi le coût de calcul de $T$ ests borné par 
...
Comme $v,w \not \in F_{n-1}$, on a forcément $v_\ell(d)=n-1$,ainsi $\ell^{n-1}
 ...
 tel que  ceux présentés dans \cite[Chapter~10.2]{vzGJG03} nous donnerait un temps
