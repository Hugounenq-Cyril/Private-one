...
(\ell^{k+1})\log(\ell)\log(\ell q))=O(\sqrt{r}\mathsf{M}(\sqrt{r})\log(\ell)\log(\ell q))$ 
...
$O(\mathsf{M}(\ell^k)\log(\ell^k)\ell^k)=
O(\mathsf{M}(\sqrt{r})\sqrt{r}\log(r)\log(\ell))$ ce coût est considéré comme 
un pré-calcul.
...
$O(\ell^{k-h}\mathsf{M}(r\ell^4)\log(r)\log(\ell))$ à $S'$ fixé.

Enfin l'étape à et la boucle étant répétées $\ell^k$ fois cela donne la complexité totale.
\end{proof}
