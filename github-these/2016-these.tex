...
une complexité de 
$O(\ell^4(\mathsf{M}(\ell^{h-\beta-e+i+1})+(\beta)\mathsf{M}(\ell)))$
...
$r$-isogènes, avec $r \wedge \ell=1$ qui se situent au cratère d'un volcan de 
$\ell$-isogénies de hauteur $h$, réduit à une courbe car on se place dans le 
cas Atkin. On verra par la suite comment généraliser cela
à des courbes non situées sur le cratère. \todo{Le faire!}

Soient $E$, $E'$ deux courbes elliptiques de $j$-invariants $j$ et $j'$ reliées
par une $r$-isogénie $\phi$ alors pour $k$  plus grand que $h$, $P \in E$ et 
$P' \in E'$ deux points d'ordre $\ell^{k}$ tels que $\phi(P)=P'$. Pour deux 
bases $(P,Q),(P',Q')$ de $E[\ell^k]$ et $E'[\ell^k]$ telles que $\pi(P,Q)=
\pi(P',Q')$ on détermine de façon unique $\phi([\ell^h]Q)=[\ell^h]Q'$ par le 
corollaire~\ref{cor:atk:ess}. On détermine alors le point 
$\phi(Q)$ en testant pour tout point $R$ antécédent par $[\ell^h]$ de 
$[\ell^h]Q'$ si la correspondance $(P,Q) \rightarrow (P',R')$ premet de définir
une $r$-isogénie. Ce test consiste comme dans le cas Elkies à calculer le 
polynôme d'interpolation $A_{R'}$ tel que 
\[
A_{R'}(x([u]P+[v]Q))=x([u]P'+[v]R') \quad \text{pour tout $[u]P+[v]Q$ d'ordre $\ell^k$}.
\]
Ensuite à l'aide du polynôme $A_{R'}$ on calcule la fraction rationnelle $F$ telle que:
\[
F=A_{R'} \bmod T
\]
...
choisi $R'$ doit représenter l'action de la $r$-isogénie $\phi$, on cherche 
...
\FOR{$S' \in E'$ d'ordre $\ell^k$}
\STATE \label{alg:cou:atk:matc:fro} Calcul de $R' \in E'$ tel que $\pi(S',R')=\pi(P,Q)$ à l'aide de l'algorithme~\ref{alg:atk:bas:mat}
\FOR{$U' \in E'$  tel que $[\ell^{h}]U'=[\ell^{h}]R'$}

\STATE Calcul des listes de représentants pour les orbites selon l'action du Frobenius $L$ et $L'_{U',R'}$ afin de respecter la correspondance induite par le choix de $U'$ et $R'$;
\STATE \label{alg:cou:atk:int} Calcul des des polynômes $A_{U',R'}$ et $T$ par les méthodes décrites dans la section~\ref{sec:interpolation};
\STATE \label{alg:cou:atk:Cauchy} Calcul de la fraction rationnelle $F=A_{U',R'} \bmod T$ à l'aide d'une interpolation de Cauchy;
\IF{$\mathsf{Test}(F)$}
\RETURN $F$
\ENDIF
\ENDFOR
...
calcule itérativement l'action du Frobenius sur $\ell^j$-torsion pour $j$ 
...
$\mathbb{F}_q$ d'après le théorème~\ref{thm:frob-ell}, on effectue alors 
...
$O(\ell^2(\beta \mathsf{M}(\ell)+\mathsf{M}(\ell^{i-\beta+1})))$ 
opérations sur $\mathbb{F}_q$.

L'étape~\ref{alg:cou:atk:matc:fro} a un coût de 
$O(\ell^4(\mathsf{M}(\ell^{h-\beta+i+1})+(\beta-e)\mathsf{M}(\ell)))$ 
opérations sur $\mathbb{F}_q$ d'après la proposition~\ref{pro:atk:comp:alg}.

Par le corollaire~\ref{cor:atk:orb:rep} on a $\ell^{k+\beta-2}(\ell+1)$ 
...
$O(\mathsf{M}(\ell^{2k})\log(\ell^k))$ opérations sur $\mathbb{F}_q$. 

Pour l'étape~\ref{alg:cou:atk:int} par la proposition~\ref{prop:interpol}
...
\section{Variante de Elkies avec un seul sous-groupe cyclique}
