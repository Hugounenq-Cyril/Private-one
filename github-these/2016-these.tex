...
En effet l'ensemble des abscisses des point primitifs de $p^k$-torsion est de taille $p^k(p-1)/2$. La taille d'une orbite sous l'action du Frobenius est quant à elle égale à $[\mathbb{U}_k:\mathbb{F}_q]$ (ou $\frac{[\mathbb{U}_k:\mathbb{F}_q]}{2}$ voire remarque ). Ainsi si l'on note $f$ la taille de ces orbites et $d$ le nombre de représentants on a $d \times f=p^k(p-1)/2$.

Détaillons cela, on a vu précédemment dans la proposition \ref{pro:ord:fro} que pour tout $i\geqslant 1$ l'extension $U_{i+1}$, dans laquelle est définie $E[p^{i+1}]$, est soit une extension $p$-adique de $\mathbb{U}_{i}$ soit égale à $\mathbb{U}_{i}$. Ainsi les points d'ordre $p^{v_E(p)}$ de $E$ sont par définition de $v_E(p)$ définis sur $E(\mathbb{F}_q)$, on note $d$ le nombre d'abiscisses différentes de ces éléments. Dès lors les points d'odre $p^{v_E(p)+i}$ avec $i \geqslant 1$ sont définis comme des antécédents par $[p]$ (la multiplication par $p$) d'un point $P_i$ d'ordre $p^{v_E(p)+i}$ et chaque point de cet orbite sous l'action du groupe de Galois $[\mathbb{U}_{v_E(p)+i}:\mathbb{U}_{v_E(p)+i-1}]$ est un antécédent de $P_i$ par $[p]$. Sur la figure \ref{fig:p-tor:fro} on peut voir une répartition des points de $p^i$-torsion selon l'action du Frobenius et les différentes orbites.
...
\begin{tikzpicture}[scale=0.60,domain=-10/3:4.5,samples=100,yscale=1/2]
          ...
          \draw (U0) node{$\mathbb{F}_{q}=\mathbb{U}_{v_E(p)}$};
		  ...
		  \draw (U10) node{$E[p^{v_E(p)}]$};
          \draw (U11) node{$E[p^{v_E(p)+1}]$};
          \draw (U12) node{$E[p^{v_E(p)+i}]$};        
...
\begin{rem}
La taille des orbites des abscisses $f$ selon l'action du groupe de Galois $[\mathbb{U}_k:\mathbb{F}_q]$  est égale à $[\mathbb{U}_k:\mathbb{F}_q]$ si et seulement si $2 \nmid [\mathbb{U}_k:\mathbb{F}_q]$. En effet dans le cas où l'on a $2 \mid [\mathbb{U}_k:\mathbb{F}_q]$ si l'on note $e=[\mathbb{U}_k:\mathbb{F}_q]$ alors on a $\pi^{e/2}(x(P))=x(-P)=x(P)$ et donc $f \mid [\mathbb{U}_k:\mathbb{F}_q]/2$.
\end{rem} 
