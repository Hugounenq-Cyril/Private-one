...
\label{def:tour-ell}
On appelle tour d'extension $\ell$-adique du corps fini $\mathbb{F}_q$ la suite d'extensions: $ \mathsf{F}_{0}, \mathsf{F}_{1}, \mathsf{F}_{2}, \cdots$ pour lesquels:
\begin{itemize}
\item $\mathsf{F}_{1}$ est une extension de $\mathsf{F}_{0}=\mathbb{F}_q$ de degré $d_1$ avec $d_1 =o(q)$ avec $o$ qui représente l'odre multiplicatif dans $\mathbb{Z}/\ell \mathbb{Z}$,
...
Les complexités sont calculées en nombre d'opérations ($+, \times,\div$) à coût identique dans $\mathbb{F}_q$, indépendamment de la construction de $\mathbb{F}_q$. On notera par $\mathsf{M}: \mathbb{N} \rightarrow \mathbb{N}$ l'application telle que la multiplication d'un polynôme de degré $n$ à coefficients dans $R$ coûtera $\mathsf{M}(n)$ opérations dans $R$ et telle que $\mathsf{M}$ est super-linéaire, en utilisant les résultats de \cite{Cantor-Kaltofen91} on a $\mathsf{M}(n)=O(n\log(n) \log \log(n))$.
...
L'idée de base derrière le travail de \cite{Doliskani-Schost15} est de permettre un passage efficace d'une représentation bivariée à une représentation univariée afin de rajouter à la volée des extensions de la tour compatibles avec ce qui a été fait auparavant.
