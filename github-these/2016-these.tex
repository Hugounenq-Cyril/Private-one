...
\[
T_1=X_1-\alpha \quad \textit{et} \quad T_k=X_k^2-X_{k-1} \quad \textit{avec $k$ entier naturel}
\]  de la même manière la suite de polynômes $P_1,P_2, \cdots P_k$ est définie à l'aide de $\alpha$ de la façon suivante:
\[
P_k=X_k^{2^k}-\alpha \quad \textit{avec $k$ entier naturel} 
\]
Il faut alors montrer que les représentations 
On va donc voir comment passer d'une représentation de $L_k$
Ces deux ensembles

Comme abordé dans plus tôt et discuté dans \cite{lang2002algebra}, l'égalité entre les idéaux suivants est vérifiée:
\[
\langle T_1, T_2, \cdots, T_k \rangle = \langle P_k, X_{k_1}-X_k^2, \cdots, X_1-X_{k}^{2^{k-1}} \rangle,
\]
