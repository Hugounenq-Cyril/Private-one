...
On travaille avec une courbe elliptique se trouvant sur un volcan de $\ell$-isogénies à cratère cyclique, on se place dans le contexte de l'algorithme de Couveignes et on veut calculer une base horizontale (ou partiellement horizontale) de la $\ell^k$-torsion pour calculer une isogénie de degré $r$. En particulier on ne connaît pas la cardinalité de la courbe. On verra dans un autre chapitre~\ref{cha:alg:fin} \todo{spécifier ce chapitre} le lien entre $k$ et $r$, dans cette section on se place dans le cas Elkies où le polynôme caractéristique du Frobenius se scinde sur $\mathbb{Z}_{\ell}$, avec $h$ la valuation $\ell$-adique de la différence des valeurs propres. On a vu à la proposition \ref{pro:dia:hor} que l'on devait avoir $k \geqslant h+1$, on rappelle aussi que $\beta$ représente la plus grande valuation de la $\ell$-torsion rationnelle pour une courbe située sur le sommet du cratère du volcan de $\ell$-isogénies(voir sous section \ref{sub:classes}).
...
\begin{lem}
\label{lem:div:cou}
Soit $P$ un point de $E$ tel que $P \in E(\mathsf{F}_{n})$, alors le coût de calcul de $E(\mathsf{F}_1)[\ell]$ est de $O(\ell \mathsf{M}(\ell^2)\log(\ell)\log(\ell q))$. Le coût de calcul de $\mathsf{divise}(\ell,P)$ est de $\mathsf{R}(n)$ lorsque $E[\ell](\mathsf{F}_1)$ a déjà été calculé.
\end{lem}
...
\begin{proof}
L'équation à l'étape \ref{alg:trigonal:solve1} lorsque $i<h-e-1$ on a $a=b$ et $c=d=0$ par \ref{pro:mat:fro} alors le calcul est trivial. Pour $i \geqslant h-e$ l'équation peut être simplifiée par  $\ell^{h-e}$ avant d'être résolue modulo $\ell^{e+1}$. 
...
Pour~$i = 0$, l'étape~\ref{alg:point:divide}
...
\end{nota}
Cette notation étant très lourde elle ne sera utilisée que lorsque le niveau auxquels ces objets sont définis n'est pas évident.
...
régulier on a les points $P$ et $Q$ d'ordre $\ell^{k-1}$ situés au niveau $h-e+i-1$
 ...
 connaît déjà une base de $E^{(h-e+i-1)}[\ell]$ à chaque étape le coût de cela
...
tels que $\pi(P_k)= [\lambda] P_k$, $ \pi(Q_k)= [\mu] Q_k$.
% \STATE Calcul $(P_1,Q_1)$, a basis of $E[\ell]$
\STATE$(\lambda, \mu, i) \leftarrow (0,0,0)$; $\mathsf{trigonal} \leftarrow \TRUE $; $\mathsf{scalaire} \leftarrow \TRUE$ ;$P', Q' \leftarrow $ tels que~$\langle P',Q' \rangle =  E[\ell]$. %; $\mu \leftarrow 0$;$i \leftarrow 0$
...
Soient $E$, $E'$ deux courbes elliptiques de $j$-invariants $j$ et $j'$ reliées par une isogénie $\phi$ de degré $r$, deux bases diagonales $(P,Q),(P',Q')$ de $E$ de $E[\ell^k]$ et $E'[\ell^k]$ telles que 
\[
...
\end{matrix} \right) 
\]
alors comme l'isogénie $\phi$ commute avec le Frobenius on a $\phi(P) \in \langle P' \rangle $ et $\phi(Q) \in \langle Q' \rangle $, cependant comme la matrice $\pi$ est scalaire modulo la $\ell^{h}$ torsion alors la représentation de $\phi$ dans les bases $(P,Q)$ et $(P',Q')$ n'est pas diagonale:
\[
\left(
\begin{matrix}
P' \\
...
\]
 ...
 $E[\ell^k]$ et $E'[\ell^k]$ pour lesquelles l'action de $\phi$ sur la base 
 $(\color{red}P_{\lambda} \color{black},\color{blue}Q_{\mu}\color{black})$ qui 
...
Par définition de ~$k$, on a ~$\ell^{2k+1} \in O(r\ell^2)$.
  ...
  le fait que $k$ strictement plus grand que $h$ par définition de $k$.
 
  On va utiliser ce résultat tout au long de la preuve. Le coût de calcul des bases 
  ...
  paragraphe~\cite{par:val:iso} pour une description détaillée des tests 
  possibles) dont les opérations 
  les plus coûteuses sont regarder si le dénominateur de $F$ est un carré ou 
...
signifie que dans ce cas la hauteur $h$
  du volcan de $\ell$-isogénies est supérieure au $k$ spécifié dans 
...
$\ell$ de Elkies \todo{regarder si nombre de Elkies c'est défini quelque part} pour une courbe quelconque et par conséquent donner une borne sur
...
On utilise donc ce résultat pour démontrer le théorème suivant, notamment en donnat une 
...
$\mathbb{F}_q$, il est possible de résoudre "Calcul de l'isogénie explicite" 
en un temps espéré de $O \bigl( r \mathsf{M}(r\log(q)^6) \log(r) \log \log(q) \bigr)$.
%  For almost all primes $q$ and curves $E,E'$ over $\mathbb{F}_q$, it is
...
\begin{proof}
  ...
  satisfying the conditions of Proposition~\ref{pro:full-complexity}.
  ...
  En appliquant le~\cite[Theorem~1]{shparlinski2014distribution},
  ...
  empêche d'utiliser notre algorithme, pour de tels nombres premiers ceux-ci sont des diviseurs. 
  porte sur la hauteur $h$ du volcan de $\ell$-isogénies)
  $\ell$-isogénies 
  As a special case of~\cite[Theorem~1]{shparlinski2014distribution},
  we can take $L \in O(\log(q))$ such that the product of all
  Elkies primes $\ell \leqslant L$ exceeds $\Omega(\sqrt{q})$.
On the other hand, we discard those primes~$\ell \leqslant L$ for which
the height~$h$ satisfies~$\ell^{h} > \sqrt{r}$;
since those discarded primes are divisors of~$\sqrt{d_K}$,
their product is at most~$O(\sqrt{q})$.
This shows that there remains enough “good” Elkies primes
in~$[ 1,L]$,
so that in the worst case $\ell \in O(\log(q))$.
%   For any such~$\ell$, the
%   corresponding height~$h = h_\ell$
%   is~$h_\ell = \frac 12 v_\ell(d_K)$, so that $\prod \ell^{h_\ell}\leqslant2√q$.
%   Hence, even discarding those primes such that~$\ell^{h_\ell} > √r$,
%   there will be enough good Elkies primes left in the interval $\llbracket 1,L\rrbracket$.
%   Therefore in the worst case~$\ell = O(\log(q))$.
