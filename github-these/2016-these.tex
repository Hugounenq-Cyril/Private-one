...
On va noter par $\tilde{O}$ le coût de la complexité exprimée sans facteurs 
logarithmiques.

Enfin des algorithmes de Las Vegas vont être utilisés dans ce document. Ces algorithmes sont des algorithmes probabilistes qui produisent un résultat correct, dés lors qu'on utilise un algorithme de Las Vegas on parlera de complexitée espérée. \todo{cout moyen ?}
\todo{définir tilde}
...
I(x,y) = \left( x + \sum_{Q \in G \setminus \{0_E\}} \frac{f'(Q)}{x-x(Q)}+\frac{2f(Q)}{(x-x(Q))^2}
, y + \sum_{Q \in G \setminus \{0_E\}} -\frac{yf'(Q)}{(x-x(Q))^2}-\frac{4yf(Q)}{(x-x(Q))^3}  \right)
\end{equation}
\todo{faire un saut à la ligne}
...
Soient $k \geqslant 1$, $P$ un point d'ordre $p^k$ alors $x(P) \in \mathbb{L}_{k} \setminus \mathbb{L}_{k-1}$. On note $\mathsf{orb}(x(P))$ l'ensemble des abscisses des points de l'orbite créée par l'action d'un élément générateur $\pi$ de $\mathsf{Gal}(\mathbb{L}_k:\mathbb{F}_{q})$ sur l'élément $x(P) \in \mathbb{L}_k$. On a donc: \todo{problème de place de l'équation}
...
En particulier pour $\mathcal{O}=\langle 1, \omega_K f \rangle$, le polynôme 
minimal de $f \omega_K$ est:
\[
x^2-fd_Kx+f^2d_K\frac{d_K-1}{4}=0
\]
Le discriminant de ce polynôme et de cet ordre est alors $f^2d_K$.
\begin{rem}
\label{rem:def:dpi}
De par l'isomorphisme de groupes entre le groupe de classes de formes et le groupes de classes d'idéaux (voir Theoreme 7.7 Cox ou Theoreme 5.2.8 Coh96) on peut aussi définir le discriminant de $d_{\pi}$ comme étant le discriminant du polynôme minimal du Frobenius: $x^2-t_{\pi}+4q$ et l'on a alors $d_{\pi} =t_{\pi}^2-4q$ le discriminant de l'ordre $\mathbb{Z}[\pi]$.
...
\begin{rem} \todo{A retravailler}
\label{rem:lie:dp:dk}
Ainsi pour un volcan de $\ell$-isohénies de hauteur $h$, on a 
...
ordinaire. \todo{verifier que c'est un rappel} \todo{Justifier 
...
le nombre de classe d'idéaux \todo{classes de qui?} en parcourant le cratère. Cela peut permettre aussi d'atteindre et 
...
\end{equation*}
Dés lors puisque $d_{\pi}=\ell^{2h}d_{K}$ avec $\ell^2 \nmid d_{K}$ (voir remarque~\ref{rem:lie:dp:dk} \todo{regarder chapitre 4} et définition ) avoir 
...
et dans \cite{FouquetMorain02}, pour obtenir une $\ell^k$-isogénie descendante 
il est suffisant d'avoir la première $\ell$-isogénie engendrée par le noyau de 
la $\ell^k$-isogénie qui soit descendante et que l'on ait pas ensuite de 
$\ell$-isogénie duales le seul critère restricitf pour obtenir ce résultat est d'avoir la $\ell$-isogénie engendrée par $\langle [a \ell^{k-1}]P+ [b \ell^{k-1}]Q \rangle $ de type descendant (pour plus de détails voir sous-section \ref{sub:alg:FM} ou \cite{FouquetMorain02}).
