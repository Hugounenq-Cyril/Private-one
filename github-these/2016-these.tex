...
Ce résultat nous permet d'énoncer la définition suivante:

\begin{defi}
Soit $\mathbb{U}_0, \mathbb{U}_1, \mathbb{U}_2, \cdots, $ une tour d'extension $p$-adique de $\mathbb{F}_q$ définie comme dans \ref{def:p-adic} alors on définit l'indice de valuation rationnelle $p$-adique de la tour comme étant le plus grand $i$ tel que $\mathbb{U}_i \subset \mathbb{F}_q$. On le note $v_E(p)$
\end{defi}

Ainsi on a $[\mathbb{U}_i:\mathbb{U}_1]=p^{i-v_E(p)}$ pour tout $i \geqslant v_E(p)$. Ainsi asymptotiquement on aura $[\mathbb{U}_k:\mathbb{U}_1] \in O(p^k)$
...
Enfin à l'aide de ces pré-calculs on effectue la résolution de l'équation d'Artin Schreier à l'aide de $O(\ell^2 )$ opérations sur $\mathbb{F}_q$, le coût de l'inversion de l'application $\mathbb{F}_q$ linéaire $x \mapsto x^p - x$ est négligeable, pour plus de détails le lecteur peut voir \cite[§2.4]{Couveignes96}.
\end{proof}

\paragraph{Interpolation de Cauchy} L'interpolation de Cauchy se fait en deux étapes: tout d'abord il faut calculer un polynôme d'interpolation $A$ d'après la donnée de valeurs prises en les points primitifs de $p^k$-torsion. Ensuite on doit reconstruire à l'aide de l'algorithme d'Euclide une fraction rationnelle $\frac{R}{V}$ égale à $A$ modulo le polynôme unitaire qui s'annule les abscisses des points de $p^k$ torsion primtifs. 
...
En effet on a l'ensemble des abscisses des point primitifs de $p^k$-torsion qui est de taille $p^k(p-1)/2$. La taille d'une orbite sous l'action du Frobenius est quant à elle égale à $[\mathbb{U}_k:\mathbb{F}_q]$. Ainsi si l'on note $f$ la taille de ses orbites et $d$ le nombre de représentants on a $d \times f=p^k(p-1)/2$.

Détaillons plus en détail cette notion. On a vu précédemment dans la proposition \ref{pro:ord:fro} que pour tout $i\geqslant 1$ l'on avait l'extension $U_{i+1}$ dans laquelle est définie la $p^{i+1}$ torsion de $E$ qui est soit une extension $p$-adique de $\mathbb{U}_{i}$ soit est tout simplement égale à $\mathbb{U}_{i}$. Ainsi si l'on regarde les points d'ordre $p^{v_E(p)}$ de $E$, alors par définition de $v_E(p)$ ils sont définis sur $E(\mathbb{F}_q)$. Si l'on regarde alors les points d'odre $p^{v_E(p)+i}$ avec $i \geqslant 1$ chacun sont définis à partir comme un antécédent de $[p]$ (la multiplication par $p$) d'un point d'ordre $p^{v_E(p)+i}$ et appartiennet ainsi à l'ordite de celui-ci sous l'action du groupe de Galois $[\mathbb{U}_{v_E(p)+i}:\mathbb{U}_{v_E(p)+i-1}]$. Ainsi comme illustré sur la figure on peut voir une répartition des points de $p^i$-torsion selon l'action du Frobenius et leurs orbites.

\begin{figure}
\begin{tikzpicture}[scale=0.60,domain=-10/3:4.5,samples=100,yscale=1/2]
          \begin{scope}[fixed point arithmetic]
          \coordinate (Q) at (0, 2) {$\mathbb{U}_{v_E(p)}$};
          \coordinate (Q) at (0, 4) {$\mathbb{U}_{v_E(p)+1}$};
          \coordinate (Q) at (0, 6) {$\bullet$};
          \coordinate (Q) at (0, 8) {$\mathbb{U}_{v_E(p)+i}$};
          \coordinate (-Q) at (2, 43) {};
          \coordinate (-2Q) at (76, 24);
          \coordinate (3Q) at (61, 91);
...
\begin{defi}
Soient $\mathbb{U}_i$ définis comme dans 
\end{defi}

\begin{rem}
On a $f$ la taille des orbites des abscisses qui est égale à $[\mathbb{U}_k:\mathbb{F}_q]$ si et seulement si $2 \nmid [\mathbb{U}_k:\mathbb{F}_q]$. En effet dans le cas où l'on a $2 \mid [\mathbb{U}_k:\mathbb{F}_q]$ si l'on note $p=[\mathbb{U}_k:\mathbb{F}_q]$ alors on a $\pi^{p/2}(x(P))=x(-P)=x(P)$ et donc $f=[\mathbb{U}_k:\mathbb{F}_q]/2$.
\end{rem} 

Dans l'algorithme original de Couveignes le coût minimal du calcul du polynôme d'interpolation a été sous-estimé. En effet on cherche à calculer un polynôme de degré $p^{k-1}(p-1)=\phi(p^k)$ à coefficients dans $\mathbb{U}_k$, ainsi ne serait ce que pour représenter ce polynôme on aurait besoin de $O(p^{2k})$ éléments de $\mathbb{F}_q$ et donc on aurait à minima un nombre de $O(p^{2k})=O(\ell^2)$ opérations à effectuer sur $\mathbb{F}_q$, il faut donc utiliser des méthodes supplémentaires pour optimiser ce temps de calcul. Les travaux de \cite{DeFeo10} ont donc résolu cette question et seront présentés à \textcolor{red}{ref à mettre}.
...
On note $m_1, m_2, \cdots, m_d$ les différents représentants des orbites selon l'action du Frobenius. Chacune de ces orbites est de taille $[\mathbb{U}_k:\mathbb{U}_1]$ avec $d=[\mathbb{U}_1:\mathbb{F}_q]$.
\begin{lem}
...
\end{equation*}
pour plus de détails sur le calcul de complexité voire \cite{defeo2011}, la démarche étant la même que celle décrite dans la sous section précédente. 
...
A nouveau le détail des calculs de complexité peut être trouvés dans \cite{defeo2011}, la complexité obtenue pour résoudre \eqref{eq:vol:cov} est faite à l'aide de techniques spécifiques à la construction de tours $p$-adique, plus de détails peuvent être trouvés dans \cite{DeFeo-Shost'12}.
