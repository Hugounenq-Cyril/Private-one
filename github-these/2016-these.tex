...
$\mathbb{F}_{i-j}$ à $\mathbb{F}_{1}$ en décomposant la multiplication par 
...
\`A l'étape~\ref{alg:atk:coef:int} le calcul de $R_j$ peut être fait à l'aide 
...
Frobenius sur un point $R_j$ défini dans $\mathsf{F}_{i-j}$ lorsque $i-j>1$ 
$\mathsf{F}_1$ sinon a un coût total de $O(\ell^{i-j}\mathsf{M}(\ell))$ par le 
...
actualise à chaque itération la valeur de $MP_j$ et $M_R_j$. Pour cela on est 
...
$\mathbb{F}_q$.
d'abord $MP$ et $MQ$ cela se fait à l'aide de $O(\ell^{h-e+i})$ additions de 
points définis dans $\mathsf{F}_{i}$, cela a donc un coût de $O(\ell^{h-e+i}
\mathsf{\ell^{i}})$ opérations sur $\mathbb{F}_q$, ensuite on calcule avec le 
même coût que .
Le coût de calcul de $M_jP_j, M_j Q_j$ est lui de 
 , la multiplication par $\ell$ 
d'un point dans Ainsi à l'étape $j$ de la boucle pour,
