...
Dans ce chapitre on s'intéresse à la partie algorithmique du calcul des objets présentés dans le chapitre \ref{cha:act:fro}.

\section{Elkies}

Vu que l'on se place dans le contexte de l'algorithme de Couveignes alors on veut calculer la une base horizontale (ou partiellement horizontale) de la $\ell^k$-torsion pour calculer une isogénie de degré $r$. On verra dans un autre chapitre \todo{spécifier ce chapitre} le lien entre $k$ et $r$, on a aussi vu précédemment que l'on devait avoir $k \geqslant h+1$.
...
\begin{algorithm}
\caption{\label{alg:bas:tri}Calcul d'une base de $E[\ell^k]$ pour $E$ non situéee au niveau du cratère et de l'action de $\pi$ dans celle-ci}
\begin{algorithmic}[1]
...
\FOR{$i = 1$ to~$k-1$}
\STATE $\psi_i \gets $ isogeny with kernel~$\chev{\ell^{h} P_{i-1}}$
