...
\begin{equation}
2f_1'f_2'+f_1'^2=G(x)H(f_1)+G(x)H'(f_1)f'_2 \bmod x^{2d}
\end{equation}
avec la condition initiale $f_2(0)=0$. Cette équation a alors pour solution:
\begin{equation}
...
Si la quantité $p_1$ est connue une solution de l'équation différentielle \eqref{eq:BMSS:ED3} est résolue modulo $x^{2\ell}$ à l'aide de l'algorithme \ref{alg:BMSS:eqdiff} , sinon une solution de l'équation différentielle est calculée modulo $x^{4\ell}$ à l'aide de l'algorithme \ref{alg:BMSS:eqdiff}. Une fois $S$ solution de l'équation $\eqref{eq:BMSS:ED3}$ calculée on définit les valeurs suivantes:
\begin{equation}
S(x)=xT(x^2), \quad R(x)=\frac{1}{T(x)^2} \quad \text{tel que } \frac{g(x)}{h(x)}=xR(1/x)
\end{equation}
que l'on détermine successivement dans l'algorithme \ref{alg:BMSS:total}. La dernière étape de \ref{alg:BMSS:total}, celle de la reconstruction rationnelle, diffère. En effet pour le cas où l'on connaît déjà $p_1$ alors la même méthode que celle préconisée dans Elkies 1998 est utilisée (voir remarques \ref{rem:ELK:h-p}, \ref{rem:pol:rec}), sinon une reconstruction rationnelle est utilisée (\textcolor{red}{mettre référence}).

\begin{rem}
L'algorithme BMSS impose la nécessité d'avoir $p>>\ell$ car à l'étape $7$ de $\ref{alg:BMSS:eqdiff}$ une intégration est nécessaire ce qui entraîne des divisions pas les entiers compris entre $1$ et $2\ell$ lorsque $p_1$ est connu, entre $1$ et $4\ell-1$ sinon. Cela pose donc des limites aux applications de BMSS.
\end{rem}
...
\STATE Calculer $G(x)=1/(1+Ax^4+Bx^6) \bmod x^{2\ell}$ si ${p_1}$ est connu, $\bmod x^{4\ell-1}$ sinon.
\STATE Calculer $S(x)$ $\bmod x^{2\ell}$ (resp. $x^{4\ell-1}$) à l'aide de l'algorithme \ref{alg:BMSS:eqdiff}.
\STATE Soit $T(x)=\sum_{i=0}^{\ell}s_{2i+1}x^i$ (resp. $T(x)=\sum_{i=0}^{2\ell-1}s_{2i+1}x^i$).
\STATE Calculer $R(x)=1/T(x)^2 \bmod(x^{\ell})$ (resp. $\bmod(x^{2\ell-1})$).
\STATE Calculer $\frac{g(x)}{h(x)}$ à l'aide des sommes de Newton (resp. à l'aide de reconstruction rationnelle). 
\end{algorithmic}
\end{algorithm}
