...
Enfin on propose le résultat suivant concernant les opérations sur les 
...
\section{Ouverture 1812}
\subsection{Variante avec un seul sous-groupe cyclique}
...
\subsection{Approche multi modulaire de l'algorithme}
...
\subsubsection{Aprroche avec le groupe de points isomorphe à $\oplus_{i=1}^n \mathbb{Z}/\ell_i^{k_i} \mathbb{Z} \times \mathbb{Z}/\ell_i^{k_i} \mathbb{Z}$}
...
\subsubsection{Approche avec le groupe de points isomorphe à $\prod_{i=1}^n \mathbb{Z}/\ell_i^{k_i} \mathbb{Z} \times \mathbb{Z}/\ell_i^{k_i} \mathbb{Z} $}
...
$(\color{red}P_{\lambda,i}\color{black},\color{blue}Q_{\mu,i}\color{black})$ de 
$E[\ell_i^{k_i}]$, pour faire cela on a besoin de définir quelques notions.
...
\begin{algorithm}
\caption{\label{alg:TRC:init}Calcul de  représentant de $P_1$ dans $E[\mathbb{Z}/\ell_1^{k_1}\ell_2^{k_2}\mathbb{Z}]$}
\begin{algorithmic}[1]
...
$\mathbb{Z}/ \ell_2^{k_2}$ telle que l'action de $\eta \ell_1^{k_1-o_1}$ sur le
 ...
 points d'ordre $\ell_2^{i_0+1}$. Ainsi le coût de calcul de $\eta$ est majoré 
 par le coût de $\ell_2$ fois la multiplication d'un point d'ordre $\ell_2^{k_2}$ par un coefficient dans $\mathbb{Z}/ \ell_2^{k_2} \mathbb{Z}$.
...
\begin{proof}
Soient $P,S$ deux points d'ordre $\ell_1^{k_1}\ell_2^{k_2}$  obtenus par l'algorithme~\ref{alg:TRC:init}  tels que $[\ell_2^{k_2}]P=[\ell_2^{k_2}]S=P_1$ et  $[\ell_1^{k_1}]P=[\ell_1^{k_1}]S=0_E$. Alors on a $[\ell_2^{k_2}](P-S)=0$ et $[\ell_1^{k_1}](P-S)=0$ ce qui veut dire que l'ordre de $P-S$ divise à la fois $\ell_1^{k_1}$ et $\ell_2^{k_2}$ d'où le résultat. On vérifie donc bien le théorème des restes chinois.
\end{proof}

Ainsi une solution du prolbème des restes chinois pour $P \in E[\mathbb{Z}/\ell_1^{k_1} \ell_2^{k_2}\mathbb{Z}]$ tel que $\ell_1^{k_1}P=P_2 \in E[\ell_2^{k_2}]$ et $\ell_2^{k_2}P=P_1 \in E[\ell_1^{k_1}]$ est donné par $P=Q+R$ avec:
\begin{itemize}
...
$\ell_1^{k_1}, \ell_2^{k_2}$ et $P2$ d'ordre $\ell_2^{k_2}$.
\end{itemize}

\'Enonçons une généralisation du thèorème des restes chinois pour une courbe 
elliptique à $n$ nombres premiers distincts.

On peut maintenant énoncer un algorithme qui résoud le problème des restes 
chinois généralisé sur une courbe elliptique.
...
chinois d'une courbe elliptique comme présenté dans l'algorithme~\ref{alg:TRC}.
\begin{algorithm}
...
Ainsi cette approche de théorème des restes chinois a été motivée pour 
l'appliquer à des points horizontaux d'ordre $\ell_i^{k_i}$ voyons si celle-ci
fait sens dans le contexte de l'algorithme de Couveignes.

\begin{prop}
\label{pro:par:TRC}
Soient $\ell_1, \ell_2$ deux nombres premiers distincts, 
$P_1 \in E[\ell_1^{k_1}]$ un point horizontal d'ordre $\ell_1^{k_1}$ et de 
direction $\lambda_1$, $P_2 \in E[\ell_2^{k_2}]$ un point horizontal d'ordre 
$\ell_2^{k_2}$ et de direction $\lambda_2$. Soit 
$P \in E[\mathbb{Z}/\ell_1^{k_1}\ell_2^{k_2}\mathbb{Z}]$ un point d'ordre 
...
\begin{nota}
Soit $P$ un point d'ordre $\prod_{i=1}^n\ell_i^{k_i}$ de $E$ construit à l'aide de 
...
on dit que $P$ est de direction $(\lambda_1, \dots, \lambda_n)$. 
\end{nota}
...
On a donc bien une correspondance entre les points définis à l'aide du Théorème
 des Restes Chinois appliqué à des points (partiellement) horizontaux. 
 Cependant il nous reste à voir comment la représentation de composita avec une
  méthode telle que celle préconisée dans~\cite{DeFeoDoliskaniSchost14} nous 
  permet ou pas d'avoir des résultats similaire sur les optimisations de calcul
  dues à la construction des tours d'extension $\ell$-adiques utilisées pour 
  l'algorithme~\ref{alg:cou:ell-adique}. 
 ...
 chapitre~\ref{cha:con:poi}, on doit alors les exprimer dans la composita afin
 de les utiliser pour le TRC Ellipique, or le plongement dans un composita a un
 coût de $O()$ \todo{trouver complexité}. Le coût de résolutions de polynômes 
 de degré $d$ dans une composita de taille  est de $O()$ \todo{trouver 
 complexité bis}. Enfin pour les étapes concernant les méthodes décrites dans 
 la section~\ref{sec:interpolation}, on a besoin de:
 \begin{itemize}
 \item calculer successivement l'action de 
 $\mathsf{Gal}(\mathsf{F}_{\prod_{j=1}^i\ell_j^{k_j}}:\mathsf{F}_{\prod_{j=1}^{i-1}\ell_j^{k_j}})$ sur un polynôme à coefficients dans $\mathsf{F}_{\prod_{j=1}^i\ell_j^{k_j}}$,
 $\mathbb{F}_q)$,
 \item savoir l'action afin de calculer les représentants de l'action de \todo{verifier cela aussi}
 $\mathsf{Gal}(\mathsf{F}_{\prod_{j=1}^i\ell_j^{k_j}}:\mathsf{F}_{\prod_{j=1}^{i-1}\ell_j^{k_j}})$.
 \end{itemize}
 
 Le contexte de notre travail est le suivant, soit deux éléments primitifs $v,w 
\in \mathsf{F}_{\prod_{i=1}^n\ell_i^{k_i}}$ on veut alors calculer le polynôme $A \in \mathbb{F}_q[x]$ 
tel que:
\begin{equation}
A(v)=w
\end{equation}
...
$A$ dans la classe de polynômes $\mathbb{F}_q[x]/ \langle T \rangle$.
 ...
 d'abord $T$. On définit la suite de polynômes suivante $T_i$ d'ordre $xxx$ \todo{a compléter}
 et à coefficients définis dans $xxxx$\todo{a compléter}. On
 définit $T^{(1)}(x)=(x-u)$. Supposons que l'on ait calculé un élément $T_i$ 
 alors on définit $T^{(i,j)}=\sigma_{\mathsf{Gal}(j)}(T^{(i)})$ avec 
 $\sigma_{\mathsf{Gal}(j)}$ l'application qui à tous
 les coefficients du polynôme $T^{(i)}$ applique $j$ fois l'action d'un 
 élément générateur fixé de $\mathsf{Gal}(\mathsf{F}_{\prod_{r=i}^n\ell_r^{k_r}}:\mathsf{F}_{\prod_{r=i+1}^n\ell_r^{k_r}})$, 
 on définit alors récursivement $T^{(i)}$ de la façon suivante:
 \begin{equation*}
T^{(i+1)}=\prod_{j=0}^{\ell_i^{k_i}-1}T^{(i,j)}    
\end{equation*}
\todo{Faire un dessin à nouveau !}
 On a alors $T^{(n+1)}$ qui est le polynôme minimal de $u$ sur $\mathbb{F}_q$. On peut alors 
 appliquer les méthodes simlaires à celles employées dans \cite{DeFeo10} et 
 \ref{sec:interpolation} pour calculer un polynôme d'interpolation 
 $A \in \mathbb{F}_q[x]$ tel que $A(u)=v$ pour $u,v$ deux éléments primitifs de
 $\mathsf{F}_{\prod_{i=1}^n\ell_i^{k_i}}$. 
...
\end{proof}
 ...
 Déterminons tout d'abord l'action de 
 $\mathsf{Gal}(\mathsf{F}_{\prod_{j=1}^{n}\ell_j^{k_j}}:\mathsf{F}_{\prod_{j=2}^{n}\ell_j^{k_j}})$.
 Soit $P$ un point de direction $(\lambda_1, \dots, \lambda_n)$ calculé à 
 l'aide de l'algorithme~\ref{alg:TRC} ayant pour entrées des points $P_i$
 (partiellement) horizontaux de direction $\lambda_i$ et d'ordre 
 $\ell_i^{k_i}$. Alors $P$ est isomorphe à l'aide du \todo{se mettre d'accord  sur TRC} théorème des 
 restes chinois à $\mathbb{Z}/\prod_{i=1}^{n-1}\ell_i^{k_i}\mathbb{Z} \times 
 \mathbb{Z}/\ell_n^{k_n}\mathbb{Z}$, alors l'action de $\mathsf{Gal}(\mathsf{F}_{\prod_{j=1}^{n}\ell_j^{k_j}}:\mathsf{F}_{\prod_{j=1}^{n-1}\ell_j^{k_j}})$ 
 sur $P$ est donc déterminée par l'action de $\mathsf{Gal}(\mathsf{F}_{\prod_{j=1}^{n}\ell_j^{k_j}}:\mathsf{F}_{\prod_{j=1}^{n-1}\ell_j^{k_j}})$
 sur $Q=[\ell_1^{k_1}]P$ un point d'ordre $\prod_{i=2}^{n}\ell_i^{k_i}$ défini 
 sur $\mathsf{F}_{\prod_{j=2}^{n}\ell_j^{k_j}}$ et sur 
 $P_1=[\prod_{j=2}^{n}\ell_j^{k_j}]P$ un point d'ordre $\ell_1^{k_1}$ défini 
 sur $\mathsf{F}_{\ell_1^{k_1}}$. L'action de 
 $\mathsf{Gal}(\mathsf{F}_{\prod_{j=1}^{n}\ell_j^{k_j}}:\mathsf{F}_{\prod_{j=2}^{n}\ell_j^{k_j}})$ 
 est triviale sur $Q$ car celui-ci est défini sur 
 $\mathsf{F}_{\prod_{j=2}^{n}\ell_j^{k_j}}$.  L'action de 
 $\mathsf{Gal}(\mathsf{F}_{\prod_{j=1}^{n}\ell_j^{k_j}}:\mathsf{F}_{\prod_{j=2}^{n}\ell_j^{k_j}})$
 sur $P_n$ est quant à elle d'ordre $\ell_1^{k_1}$ et équivalente à 
 l'action de $\mathsf{Gal}(\mathsf{F}_{\ell_k^{k_n}}\mathbb{F}_q)$. Ainsi pour
 prendre un représentant de $P$ sous l'action de Galois il faut en construire  
 à l'aide du théorème des restes chinois pour chaque représentant de $P_1$.
 
 \todo{faire une proposition de cela}
...
\STATE Calcul de $P',Q' \in E'$ d'ordres $ \mathbb{Z}/\prod_{i=1}^n \ell_i^{k_i}\mathbb{Z}$ tel que $P'$ est de direction $(\lambda_1, \dots, \lambda_n)$ et $Q'$ de direction $(\mu_1, \dots, \mu_n)$ à l'aide de l'algorithme~\ref{alg:TRC};
\STATE \label{alg:mult-ell:rep} Calcul des listes de représentants $L_i,L_i'$ des orbites de points d'ordre $\ell_i^{k_i}$ sous l'action de $\pi$;
\FOR{$M \in \mathsf{diag}(a,b)$  avec $a,b \in \left( \mathbb{Z}/\prod_{i=1}^n\ell_i^{k_i}\mathbb{Z} \right)^*$}
