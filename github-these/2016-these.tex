...
La première amélioration que l'on peut apporter à l'algorithme de Couveignes c'est la première étape de l'algorithme, le calcul de générateurs de la $p^k$ torsion. Dans l'algorithme original celui-ci était effectué à l'aide d'une $p$-descente et de la décomposition de la multiplication par $[p]$:
\begin{equation}
[p]=\pi \circ V
\end{equation}
avec $\pi$ le Frobenius défini sur $\mathbb{F}_q$ et $V$ le verschebung. L'idée originale est donc d'inverser chacune  de ces isogénies pour calculer itérativement des antécédents de point de $p^i$-torsion par $[p]$. Inverser $V$ revient notamment à factoriser un polynômes de degré $p$ dans une extension. 

Une autre approche a été proposé par De Feo pour prendre avantage notamment des constructions de tour d'Artin Schreier
défini modulo 
