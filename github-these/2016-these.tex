...
finis $\mathbb{F}_q$ et aux algorithmes qui permettent de calculer les 
...
algorithmes de calculs d'isogénies ne domine pas dans l'algorithme de Schoof et
son amélioration (S.E.A) \cite{Schoof95}, due aux travaux de Atkin \cite{Atkin88} et 
...
\cite{Elkies1998}; celui-ci se limitait au cas de corps finis $\mathbb{F}_q$ de 
caractéristique $p$ suffisamment grande. Ce travail a été ensuite amélioré par 
les travaux de Bostan Morain Salvy et Schost \cite{BMSS08} puis par ceux de 
Lercier Sirvent \cite{Lercier-Sirvent2008} qui a permis d'utiliser cette méthode sans 
...
Soient deux $j$-invariants $j$ et $j'$ et un entier positif $r$. Si les 
$j$-invariants sont $r$-isogènes, c'est à dire il existe deux courbes 
elliptiques $E_a$ et $E_b$ reliées par une isogénie de degré $r$ telles que 
$j(E_a)=j$ et $j(E_b)=j'$, alors le problème du calcul explicite d'isogénie est
de calculer les courbes $E$, $E'$ telles que $j(E)=j$ et $j(E')=j'$, et 
l'isogénie de degré $r$ $\phi:E \rightarrow E'$.
\\
...
Ainsi il est pertinent de suivre le travail de Couveignes sur le calcul 
d'isogénies \cite{Couveignes96} car celui-ci ne suppose pas la connaissance de 
courbes sous une représentation spécifique. L'algorithme de Couveignes 
interpole l'action de l'isogénie sur les points d'ordre $p^k$, préservés par 
l'action de l'isogénie de degré $r$ avec $r \wedge p=1$, et se sert de cette 
information pour déterminer l'isogénie. Ce travail de Couveignes fût amélioré 
par la suite par De Feo \cite{DeFeo11}, qui se sert notamment d'une 
construction efficace d'extensions $p$-adiques de corps finis faite 
conjointement avec Schost \cite{DeFeo-Shost'12}. Cet algorithme
a toutefois le désavantage d'avoir une dépendance exponentielle en $p$. Il 
reste intéressant car il a une complexité quadratique meilleure que la 
complexité pour le calcul nécessaire à l'obtention de représentations 
spécifiques (équation de Weierstrass normalisée) des courbes utilisées dans le 
contexte S.E.A.. 

Cette thèse s'inscrit dans la suite des travaux de \cite{Couveignes96} 
et \cite{DeFeo11} et cherche notamment à généraliser celui-ci à l'étude des 
points d'ordre $\ell^k$, dans le cas de courbes ordinaires, avec $\ell \neq p$
afin notamment de s'affranchir de la dépendance exponentielle en $p$. Pour 
cela on doit notamment déterminer des ensembles de points préservés par 
l'action de l'isogénie de degré $r$ tout en se servant d'une construction 
efficace d'extensions $\ell$-adiques de corps finis. 

Pour le calcul d'ensemble de points d'ordre $\ell^k$ préservés par l'action de 
la $r$-isogénie on se sert de graphes 
d'isogénies dans le cas de courbes ordinaires. Ceux-ci ont été étudiés pour 
...
utilisé ces graphes d'isogénies, appelés \emph{volcans d'isogénies}, afin 
d'améliorer S.E.A.. Enfin les travaux de Miret et al. \cite{MiretMRV05} 
...
Les travaux de \cite{Ionica-Joux10} ont permis eux, à l'aide de couplage de 
points, de se diriger dans le volcan d'isogénies sous certaines conditions. 

Pour la construction de tours d'extensions nos travaux sont dans la veine de 
ceux de \cite{Doliskani-Schost15} et généralisent les résultats qui nous 
intéressent.
...
avec une dépendance polynomiale en la caractéristique du corps.
\end{thm}
...
particulièrement l'endomorphisme de Frobenius. Dans le 
...
par De Feo \cite{DeFeo11}. Dans le chapitre 3, nous nous plaçons dans la continuité des travaux de 
...
d'interpolation. Dans le chapitre 4 nous rappelons les résultats principaux 
...
Ensuite nous étudions dans le chapitre 5 l'action du Frobenius sur 
la $\ell^k$-torsion, et ses liens avec les directions dans le volcan de 
...
Couveignes dans un cas favorable (en particulier on connaît déjà un nombre de 
Elkies), puis dans le cas général, où l'on utilise notamment un résultat de 
Shparlinski et Sutherland \cite{ShparlinskiSutherland14} afin de trouver un tel
nombre de Elkies. On fait aussi une présentation moins détaillée du cas Atkin, 
car celle-ci est moins performante.
...
déterminer l'anneau d'endomorphisme de la courbe. Cela pourrait avoir par 
exemple une application dans l'algorithme de Broker utilisé pour calculer
un polynôme de Hilbert. En effet dans l'algorithme, il est nécessaire de 
calculer une courbe avec un anneau d'endomorphismes spécifique à partir d'une 
courbe au hasard dont $\mathbb{Z}[\pi]$ est fixée. On pourrait alors à partir
d'une telle courbe se servir des propositions~\ref{pro:mat:fro} et 
\ref{pro:mat:fro:atk} afin de déterminer l'anneau d'endomorphisme de la courbe.
Ensuite on pourrait se servir de la propositioné\ref{pro:tri:dir} et du 
...
Dans l'algorithme de Sutherland on est obligé, de par la taille du $\ell$-ème 
...
l'algorithme de marche dans le volcan similaire aux travaux de 
\cite{FouquetMorain02} (voir sous-section~\ref{sub:alg:FM}) utilisant
le $\ell$-ème polynôme modulaire, ainsi on pourrait rendre cet algorithme non
dépendant du (pré)calcul de $\ell$-ème polynôme modulaire.
De plus dans l'algorithme BLS (Bröker Lauter Sutherland) \cite{BLS12} pour 
calculer le $\ell$-ème polynôme modulaire les auteurs se placent dans le cas
où ils travaillent sur un volcan de $\ell_i$-isogénies de hauteur $h = 1$
pour lequel on a de $E[\ell_i]\subset E(\mathbb{F}_p)$ pour les courbes situées
