...
Objectif finir le Frobenius avec l'analyse des différentes classes pour l'endomorphisme de Frobenius.
...
\item for~$i = 1, …, k-1$:
$(\ell-1) · \ell^{\min (i, \alpha-1)}$ classes of size~$\ell^{\max (0, i-\alpha+1)}$, and
$(\ell-1) · \ell^{\min (i, \beta-1)}$ classes of size~$\ell^{\max (0, i-\beta+1)}$;
