...
h(x)=\prod_{Q \in G \setminus \{0_E\}}(x-x(Q)),
\end{equation*}
avec $G=\ker(I)$, $c \in \mathbb{F}_q$ et $g$ un polynôme. On rappelle qu'en plus à l'aide de la reformulation d'Elkies on a:
\begin{equation}
\frac{g(x)}{h(x)}= \deg(I) x -p_1 - f'(x) \frac{h'(x)}{h(x)} -2f(x)  \left(\frac{h'(x)}{h(x)}\right)'
\end{equation}
avec $f(x)=....$ $p_1$ la somme des abscisses des points du noyau que l'on peut lire sur le coefficient de $x^{\ell-2}$ dans $h$. Ainsi à partir de $h(x)$ on est capable de calculer l'isogénie en $\mathsf{M}(\deg(I))$ opérations sur $\mathbb{F}_q$. On va donc expliciter des algorithmes dans le reste de ce document qui calculent $h$.


Il est important de noter qu'il exciste des isogénies de la forme:
\begin{equation}
\label{eq:iso:gen}
...
De plus on appellera les équations de 
\end{def}
\section{Elkies 1998}
...
\end{equation}
que l'on injecte dans \ref{eq:Elkies-deriv} pour trouver des relations de récurrence entre les $r_i$ en identifiant les coefficients des $x^{-i}$:
\begin{equation}
...
\begin{equation}
r_1=\frac{A-\tilde{A}}{5} \quad r_2={\frac{B-\tilde{B}}{7}}.
\end{equation}
Ainsi, à l'aide de cette récurrence, le coût de calcul de $h_3, \cdots , h_{\ell-2}$ est de $O(\ell^2)$ opérations sur $\mathbb{F}_q$. 

\begin{rem}
On peut ensuite calculer $h(x)$ sans passer par une reconstruction rationnelle, en effet on peut retrouver les sommes de Newton: $p_0,p_1,\cdots, p_n$ à l'aide de la connaissance de $p_0=\ell-1$ et $p_1(x)$. Dans le cas considéré par Elkies il est tout à fait crédible de considérer la connaissance de $p_1$ car le calcul du $\ell$-ème polynôme modulaire $\phi_{\ell}$ a été effectué (voir pour plus de contexte), à l'aide des dérivées de $\phi_{\ell}$. Ensuite à l'aide de \ref{eq:iso:frac} on établit la récurrence:
\begin{equation*}
h_i=(2i+1)p_{i+1}+(2i-1)Ap_{i-1}+(2i-2)Bp_{i-2}
\end{equation*}
à l'aide de laquelle on calcule tous les $p_i$ en $O(\ell)$ opérations sur $\mathbb{F}_q$. Ensuite à l'aide de la remarque \ref{rem:pol:rec} et des sommes de Newton de $h$ on calcule $h$ en $O(\mathsf{M}(\ell))$ opérations sur $\mathbb{F}_q$ par \citep{BMSS08}.
Ensuite à l'aide de $h$ on peut calculer $g$ en $O(\mathsf{M}(\ell))$ opérations sur $\mathbb{F}_q$.



r les $h_i$ on peut calculer $h(x)$ en $O(\ell^2)$ opérations sur $\mathbb{F}_q$ voire en $O(\mathsf{M}(n))$ opérations sur $\mathbb{F}_q$ voir \cite{BMSS08} pour plus de détails. 
\end{rem}
...
\end{rem}
Il est à noter que la condition $p >> \ell$ est là pour nous assurer que $2, \cdots, 2\ell-1$ sont inversibles dans $\mathbb{F}_q$. 
\section{BMSS}
