...
\section{Elkies 1998}
Ici on va se restreindre au cas où la caractéristique $p$ de $\mathbb{F}_q$ est différente de $3$.
...
\end{equation}
 et $\tilde{E}:$
\begin{equation}
y^2=x^3+\tilde{a_2}x^2+\tilde{a_4}x+\tilde{a_6}
\end{equation}
...
\begin{equation}
(x^3+a_2x^2+a_4x+a_6)\left(\frac{g(x)}{h(x)} \right)'^2=  \left(\frac{g(x)}{h(x)} \right)^3+ \tilde{a_2} \left(\frac{g(x)}{h(x)} \right)^2 + \tilde{a_4} \left(\frac{g(x)}{h(x)} \right) + \tilde{a_6}.
\end{equation}
Cette équation est alors dérivée pour obtenir une équation différentielle du second ordre:
\begin{equation}
\label{eq:Elkies-deriv}
(3x^2+a_2x + a_4)\left(\frac{g(x)}{h(x)} \right)'+2(x^3+a_2x^2+a_4x+a_6)\left(\frac{g(x)}{h(x)} \right)''=  3\left(\frac{g(x)}{h(x)} \right)^2 + 2\tilde{a_2}\left(\frac{g(x)}{h(x)} \right)  + \tilde{a_4}
\end{equation}
...
\end{equation}
que l'on injecte dans \ref{eq:Elkies-deriv} pour trouver des relations de récurrence entre les $h_i$, les termes $h_1,h_2$ étant donnés par des équations sur les coefficients des équations de Weierstrass de $E$ et $\tilde{E}$.
\section{BMSS}
