...
\begin{localblock}{Problème du calcul explicite d'isogénie} \label{prob:isogeny-problem}
...
Soient $E$, $E'$ deux courbes elliptiques de $j$-invariants $j$ et $j'$ reliées par une isogénie $\phi$ de degré $r$, alors pour deux bases diagoanles $(P,Q),(P',Q')$ de $E$ de $E[\ell^k]$ et $E'[\ell^k]$
on pourrait croire que cela est suffisant cependant comme l'action du Frobenius sur la $\ell^h$ torsion est égal à l'action d'une matrice scalaire alors
 alors pour deux bases $(P,Q),(P',Q')$ (partiellement) horizontales de $E[\ell^k]$ et $E'[\ell^k]$ on a l'action de $\phi$ sur la base $(P,Q)$ qui peut être représentée comme une matrice diagonale car par la proposition~\ref{pro:par:iso} et le corollaire~\ref{cor:par:hor} les points de même direction sont mis en correspondance.
 On énumère donc toutes les $\ell^{2k-2}$ matrices possibles $M$. Pour chaque matrice $M$ on interpole l'action de $M$ sur les bases $(P,Q)$, $(P',Q')$ et l'on obient alors un polynôme d'interpolation $A$ dont on se sert pour calculer à l'aide d'une interpolation de Cauchy la fraction rationnelle représentant l'action de l'isogénie, on teste alors si l'on obtient bien une $r$-isogénie. En effet on cherche à calculer une fraction de degré $(r,r-1)$ à l'aide des abscisses des points d'ordre $\ell^k$ (voir discussion sur les points partiellement horizontaux dans la section \ref{sub:ss-gr}) il y en a $\ell^{2k-2}(\ell+1)$ or la fracion rationnelle est déterminée par $2r$ coefficients on a donc besoin d'avoir $k$ tel que $k\geqslant h+1$ et $\ell^{2k-2}(\ell+1)> 2r$.
