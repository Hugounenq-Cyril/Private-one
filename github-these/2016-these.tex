...
\begin{algorithm}
\caption{\label{alg:rem:tri}Remontée d'une base de $E[\ell^k]$ pour $E$ non situéee au niveau du cratère et de l'action de $\pi$ dans celle-ci}
\begin{algorithmic}[1]
\REQUIRE $E$: une courbe elliptique ordinaire non située sur le cratère d'un volcan de $\ell$-isogénies avec cratère cyclique;
$k$: une entier positif;
\ENSURE $(P_k, Q_k )$: une base de $E[\ell^k]$;
...
\STATE\label{alg:diagonal:upd-P}
  $P_{i+1} \leftarrow P' + y Q'$; $Q_{i+1} \leftarrow  Q'$.
\STATE{$\lambda \leftarrow \lambda + \ell^{i} a + y$; $\mu \leftarrow \mu + \ell^{i} d $; $\delta \leftarrow \delta + b \ell^i$.}
\ENDFOR
\RETURN $(P_{k},Q_{k},\lambda,\mu).$
\end{algorithmic}
\end{algorithm}
