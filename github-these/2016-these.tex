...
\begin{lem}
\label{lem:bij:ord}
Soient $P_{k-1}$, $P_k$ deux points d'ordres $p^{k-1} \neq 1$ et $p^{k}$ d'une courbe elliptique $E$ ordinaire définie sur une tour de niveau d'extensions $\ell$-adiques tels que $[p]P_{k}=P_{k-1}$. On suppose de plus que $\mathbb{L}_k \not \cong \mathbb{L}_{k-1}$. Alors il y a une bijection entre les points de $p$-division de $P_{k-1} \in \mathbb{U}_{k-1}$ et les points de l'orbite de $\sigma \in \mathsf{Gal}(\mathbb{U}_k:\mathbb{U}_{k-1})$ appliqué sur $P_{k}$.
\end{lem}

\begin{proof}
L'ensemble des points de $p$-division de $P_{k-1}$ car $P$ appartient à $E$ une courbe elliptique ordinaire. La taille de l'orbite de $\sigma$ est quant à elle égale $[\mathbb{L}_k:\mathbb{L}_{k-1}]$ égale à $p$ car $\mathbb{L}_k \not \cong \mathbb{L}_{k-1}$. Il ne reste donc qu'à montrer une inclusion pour montrer l'égalité. On a vu dans la preuve de la proposition \ref{pro:ord:fro} que l'on pouvait déterminer les points de $p^k$ torsion comme des vecteurs propres pour le Frobenius. Ainsi le point $P_k$ correspond à $t_{k} \bmod p^k$ et ses conjugués correspondent à $t_{k-1}^i P_{k} \bmod p^k$ avec $i \in \{0, \cdots , p-1  \}$ et l'on a $[pt_{k-1}^i]P_{k}=[t_{k-1}^i \bmod p^k]P_{k-1}=[1]P_{k-1}$ car $t_{k-1}^i= \mathfrak{t}_{\pi} \bmod p^{k-1}$.
\end{proof}
...
Soit $k >  0$,
 $P \in E(\mathbb{L}_{k})$ tel que $\langle P \rangle =E[p^k]$ 
 alors l'orbite de $P$ sous l'action d'un élément générateur
  de $\mathsf{Gal}(\mathbb{L}_{k}:\mathbb{F}_q)$:
   $\mathsf{orb}(x(P))=
   \{ x([n]P) , 
   n \wedge p =1 \}$. 
\end{cor}

Ainsi ce cas du corollaire n'est pas intéressant à étudier d'un point de vue théorique, il correspond au pire des cas en pratique. On va donc se placer dans la suite dans un cas du théorème \ref{thm:orb:fro}. 

Soit $P$ un point d'ordre $p^k$ alors $x(P) \in \mathbb{U}_{k-v_p(E(\mathbb{L}_1))} \setminus \mathbb{U}_{k-v_p(E(\mathbb{L}_1))-1}$ par définition de $\mathbb{U}_{k-v_p(E(\mathbb{L}_1))}$, alors pour tout $n \in \left( \mathbb{Z}/p^{k}\mathbb{Z} \right), n=1 \bmod p^{v_p(E(\mathbb{L}_1))}$ $x([n]P) \in \mathsf{orb}(x(P))$ avec $\mathsf{orb}(x(P))$ créé par l'action d'un élément générateur de $\mathsf{Gal}(\mathbb{U}_k:\mathbb{L}_{v_p(E(\mathbb{L}_1))})$. On a donc:
\begin{equation*}
