...
\end{equation*}
avec les $a_i^*$ des éléments de $F_0$. On réécrit $a$ alors sous la forme:
\begin{equation*}
a_O+a_1x_i+ \cdots + a_{d-1}x_i^{d-1}
\end{equation*}
avec les $a_i \in F_1$ en écrivant pour $k$ appartenant à $[1; d_1d-1]$ $k=u_k d + v$ avec $0 \geqslant v< d$ prenant en compte que l'on a $x_i^k=x_1^{}$
\end{proof}
