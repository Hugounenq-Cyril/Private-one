...
primordiaux, afin par exemple de définir les paramètres du cryptosystème. 
L'algorithme de Schoof \cite{Schoof85}, est le premier algorithme de complexité
polynômiale à calculer la cardinalité de courbes elliptiques sur les corps 
...
Sirvent\cite{Lercier-Sirvent2008}, celui-ci n'est pas utilisé en pratique à 
cause d'une dépendance polynomiale en la caractéristique $p$ qui le rend trop 
coûteux dès que la caractéristique augmente. Ainsi l'algorithme de Lercier et 
Sirvent \cite{Lercier-Sirvent2008}, avec une dépendance logarithmique en $p$, 
est préféré pour les plus grandes caractéristiques. Ces deux algorithmes, 
...
Couveignes~\cite{Couveignes96}. De plus comme dans SEA d'autres opérations ont 
...
particulier, toutes les courbes d'un même graphe d'isogénies ont la même 
cardinalité.
\begin{figure}
\begin{center}
...
elliptique. Kohel a permis de définir des niveaux de hauteur dans le graphe 
d'isogénies. Ainsi le graphe de la Figure~\ref{fig:int:vol} a été représenté
selon cette convention, les isogénies allant entre deux courbes située au même
...
en connaissant la cardinalité de celle-ci.
Fouquet et Morain \cite{FouquetMorain02} ont par la suite résolu le même 
problème, sans la connaissance de la cardinalité de la courbe, afin d'améliorer
SEA, et ont nommé les graphes d'isogénies \emph{volcans d'isogénies}. Enfin les 
travaux de Miret et al. \cite{MiretMRV05}  \cite{MiretMSTV06} \todo{Verifier que les 3 citations sont pertinentes}
...
déterminer si une isogénie était montante, descendante ou horizontale pour 
...
la suite de ceux de De Feo, Doliskani et Schost 
\cite{DeFeo-Doliskani-Schost13}, Doliskani et Schost \cite{Doliskani-Schost15},
...
Pour arriver à ce résultat nous avons développé de nouvelles méthodes, sans 
...
$\ell$-adiques. Nous généralisons notamment un résultat similaire à celui de De 
...
sur $\ell$. Celle-ci est moins détaillée car elle est moins performante.
...
dépendance polynômiale en la caractéristique. En particulier, en l'état 
actuel, nous avons vu que nous étions capables d'obtenir un algorithme qui 
résolve le problème de l'isogénie explicite avec une complexité quasi 
quadratique en le degré de l'isogénie uniquement dans le cas Elkies. Dans le 
cas Atkin on a seulement obtenu un algorithme de complexité inférieure à 
cubique. Cet algorithme a une complexité trop grande par rapport à l'algorithme
de Couveignes \cite{Couveignes96}, mais reste meilleur que le coût de calcul 
du $\ell$-ème polynôme modulaire (cubique d'après \cite{BLS12}). 
Nous avons aussi vu au travers de notre étude des moyens pour déterminer des 
directions dans le volcan de $\ell$-isogénies à l'aide de l'étude de l'action
du Frobenius sur le module de Tate $\ell$-adique, excepté pour le cas des 
isogénies descendantes dans le cas Elkies où l'on ne peut rien déduire.
...
quasi-quadratique dans le cas Atkin.
...
Enfin, parmi les pistes proposées mais non satisfaisante dans le 
...
\cite{Kohel96}, \cite{BissonSutherland11}, où l'on connaît déjà une 
factorisation de $d_{\pi}$, avec les propositions ~\ref{pro:mat:fro} ou 
\ref{pro:mat:fro:atk}, selon le cas Elkies ou Atkin, on connaîtrait la 
valuation $\ell_i$ adique du conducteur de la courbe par rapport à 
$\mathbb{Z}[\pi]$ et donc on pourrait déterminer l'anneau des endomorphismes de
...
$\mathbb{Z}[\pi]$ est fixée. On pourrait alors à partir d'une telle courbe se servir des 
