...
\begin{enumerate}
\item 
 $O(\ell\mathsf{M}(\ell^2)\log(\ell)\log(\ell q)+(e+1)(R(h+1-\beta)+\ell^2 \mathsf{M}(\ell^{h+1-\beta}))+\mathsf{R}(k-\beta) + \ell^2\mathsf{M}(\ell^{k-\beta}))$ opérations sur $\mathbb{F}_q$ pour une courbe en entrée située sur un volcan de $\ell$-isogénies régulier,  
...
Soient deux $j$-invariants $j$ et $j'$ et un entier positif $r$. Si les $j$-invariants sont $r$-isogènes, c'est à dire il existe deux courbes elliptiques $E_a$ et $E_b$ $r$-isogènes telles que $j(E_a)=j$ et $j(E_b)=j'$, alors le problème du calcul explicite d'isogénie est de calculer les courbes $E$, $E'$ telles que $j(E)=i$ et $j(E')=j'$, et la fonction rationnelle qui définit la $r$-isogénie $\phi:E \rightarrow E'$.
...
Par définition de ~$k$, on a ~$\ell^{2k+1} \in O(r\ell^2)$.
  ...
  par la proposition~\ref{pro:alg:ulti}. Cette borne est prise pour la cas où 
  ...
  paragraphe dans la sous-section~\cite{sub:ori:cou} pour une description 
...
logiciel SageMath~\cite{Sage}, le code est disponible sur le projet 
github:\url{https://github.com/Hugounenq-Cyril/Two_curves_on_a_volcano}. La 
partie expérimentale a été faite uniquement pour le cas $\ell=2$ et des courbes
en entrée se situant sur le cratère cyclique d'un volcan de $2$-isogénies.
...
dans les algorithmes de l'article~\cite{FouquetMorain02} en résolvant donc les
polynômes modulaires utilisés d'après la base de données "database-kohel" 
implantée par David Kohel. Un module a été fait par Sébastien Besnier sur 
Sagemath celui-ci permet d'afficher directement les volcans de $\ell$-isogénies
à partir d'une courbe et il est disponible à cette adresse 
...
On a donc regroupé les deux premières parties qui sont en fait du précalcul pour la troisième partie. Encore une fois le détail des timings pour les différentes parties est disponible sur le projet github: \url{https://github.com/Hugounenq-Cyril/Two_curves_on_a_volcano}. 

On présente donc une première série de tests qui ont été réalisé sur des 
courbes qui ont toutes leur $\beta=h=2$, ainsi notre algorithme a calculé de la 
$2^3$ jusqu'à la $2^8$ torsion sur des courbes elliptiques définies sur corps 
finis de taille $p$ allant d'une taille de $7$ bits à $252$ bits.
...
on a besoin de travailler avec la même taille de $\ell^{k}$ torsion, ainsi on 
aura en particulier les polynômes $A_{a,b}$ et $T$ de même degré ce qui fait 
que la complexité et le temps de calcul sont les mêmes. Ceci est une raison 
...
La deuxième série de tests a été réalisé sur des courbes qui ont 
toutes leur $\beta=h=3$ définies sur des corps finis de taille $p$ allant de 
$9$ bits à $62$ bits, notre algorithme a calculé de la $2^4$ jusqu'à $2^7$ 
torsion des courbes elliptiques. On voit ici que comme on a $h=3$ alors on a 
pas comme avec les précédents tests un palier où l'on calcule travaille avec la 
$\ell^{3}$ mais la $\ell^4$ torsion, car il est nécessaire d'avoir $k$ 
strictement supérieur à $h$.

\subsection{Développements possibles de l'algorithme}

On pourrait penser à utiliser notre algorithme avec une approche multimodulaire
de façon similaire à un CRT, c'est à dire pour différents nombres de Elkies 
$\ell_1, \ell_2, \ell_3, \dots, \ell_{n}$, pour une isogénie de degré $r$ on 
calcule des bases horizontales de la $\ell_i^{k_i}$ torsion avec $k_i>h_i$ et 
$k_i$ tel que $\frac{\ell_i^{2k_{i}-2}(\ell_i+1)}{4}<r$. 

Il y a alors deux approches possibles:
\subsubsection{Aprroche 1 la somme}
...
définir la fraction rationnelle $F$ censé représenter la $r$-isogénie $\phi$ 
que l'on veut calculer. On aurait alors à travailler sur diverses tours 
d'extensions $\ell_i$-adiques sur chacune d'elles on calculerait des polynômes
d'interpolation $A_{a,b,i}$ et $T_i$ définis dans $\mathbb{F}_q$ à l'aide des 
méthodes du chapitre~\ref{cha:tour} auxquels on 
appliquerait un théorème des restes chinois pour obtenir une fraction 
rationnelle $F$ à tester comme candidate pour être la représentante de la 
$r$-isogénie. L'un des points les plus génants avec cette approche est que l'on
doit considérer en moyenne les $\prod_{i=1}^n \ell_i^{2k_i-2}$ choix possibles 
pour le polynôme d'interpolation et que ceci a une complexité supérieure à 
$O(r)$. Cependant le calcul des polynômes d'interpolations 
$A_{a,b,i}$ et $T_i$ devrait être dans un tel cas inférieur à ce que l'on a 
dans l'algorithme~\ref{alg:cou:ell-adique} car pour tout $i_0$ on a plus 
$\frac{\ell_i^{2k_{i}-2}(\ell_i+1)}{4}>r$ mais $\prod_{i=1}^{n}\frac{\ell_i^{2k_{i}-2}(\ell_i+1)}{4}>r$.

Une telle approche serait meilleure que l'algorithme~\ref{alg:cou:ell-adique} 
si l'on avait un moyen de préciser si un polynôme $A_{a,b,i_0}$ était correct 
sans connaître la valeur des autres $A_{a,b,i}$ dans un cas comme celui-ci on 
aurait à tester $\sum_{i=1}^n \ell_i^{2k_i-2} \in O(r)$ choix possibles pour le polynôme 
d'interpolation, comme dans l'algorithme~\ref{alg:cou:ell-adique}.



\subsubsection{Approche 2 le produit}
