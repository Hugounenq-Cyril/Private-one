...
\begin{prop}
Soit $k$ tel que $\ell^k \in O(r\ell)$, alors pour $h<k$ le coût de calcul d'une 
base diagonale de la $\ell^{h+1}$ torsion à l'aide de 
l'algorithme~\ref{alg:ulti:fro} est de:
$O(\mathsf{M}(r\ell)r\log(\ell)\log(\ell q) \log(r))$.
\end{prop}
...
$O(\ell^{\alpha-1}\mathsf{M}(\ell^{k-\alpha})\log(\ell^k)) \subset O(\mathsf{M}(r)\log(r))$ opérations.
...
On voit bien ici à travers cette variante de 
l'algorithme~\ref{alg:cou:ell-adique} non transposable à 
l'algorithme~\ref{alg:cou:atk} proposé dans le cas Atkin, que l'étude de 
l'action du Frobenius dans le cas Elkies nous permet de spécifier des sous
