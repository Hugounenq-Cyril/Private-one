...
Ainsi les éléments des tours d'extension seront toujours représentés comme des polynômes à coefficients dans $\mathbb{F}_q$, la taille des éléments de $\mathsf{F}_{k}$ sera donc $O(d\ell^k)$ éléments de $\mathbb{F}_q$. 
...
\subsubsection{Titre non defini}

\begin{thm}
Soit $\ell$ un nombre premier, soit $\mathbb{F}_q$ un corps fini, alors une tour d'extensions $\ell$-adiques du corps $\mathbb{F}_q$: $\mathbb{F}_q, \mathsf{F}_0, \mathsf{F}_1, \mathsf{F}_2, \cdots$ est calculée en $O_e(\mathsf{M}(\ell)\log(\ell q))+ O_e(\mathsf{M}(\ell)\log(r)+\mathsf{M}(r)\log(\ell)\log(r)+\mathsf{M}(r)\log(q))+O(r^2)$ opérations sur $\mathbb{F}_q$.
\end{thm}
...
La $\mathsf{F}_0$ base monomiale de $\mathsf{F}_k$ est:
\begin{equation*}
...
La descente correspond donc au passage d'un élément de $\mathsf{F}_k$ écrit sous la forme $G(x_k)$ en élément de la forme $G_1(x_{k-1})$ appartenant à $\mathbb{F}_{k-1}$. Le relèvement est l'opération inverse.

\section{Opérations dans la tour d'extension $\ell$-adique}

\subsubsection*{Inversion d'un élément}
