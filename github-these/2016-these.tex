...
\begin{prop}
  ...
  ${\phi:E \rightarrow E'}$ en un temps espéré de \[
  O(r(\sqrt{r}(\mathsf{M}(r\ell^4)\log(r)\log(\ell)+\mathsf{M}(\sqrt{r}\ell)\sqrt{r}\log(\ell)\log(\ell q))))\]
...
$O(\ell^{k-h}(\mathsf{M}(r\ell^4)\log(r)\log(\ell))+\mathsf{M}(\sqrt{r}\ell)\sqrt{r}\log(\ell)\log(\ell q))$ 
à $S'$ fixé.

Enfin l'étape~\ref{alg:cou:atk:frob:bas} et la boucle commençant à 
l'étape~\ref{alg:cou:atk:parc:poi} étant répétées $\ell^{2k-2}$ fois cela donne
une complexité de $O(r(\sqrt{r}(\mathsf{M}(r\ell^4)\log(r)\log(\ell)+\mathsf{M}(\sqrt{r}\ell)\sqrt{r}\log(\ell)\log(\ell q))))$ à laquelle il faut ajouter les précalculs pour obtenir 
le résultat.
\end{proof}

\begin{rem}
Il est intéressant de noter que la complexité d'un tel algorithme sur un volcan
\emph{régulier} est de:
$O()$
\end{rem}
