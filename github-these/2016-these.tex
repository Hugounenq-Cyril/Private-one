...
$\ell$ et l'autre est exactement divisible par $\ell^{2e}$.
Lorsque $e=0$ on a $\ell^{h} || a-d$.
...
\begin{prop}
Soient $P,R$ deux points de $E$ située au niveau $h-e$ tels que $\langle P,R 
\rangle = E[\ell^{h-e+i}]$ alors $\pi(P,R)$ ne dépend que de $[\ell^{h-e}]R$
\end{prop}

\begin{proof}
Matriciellement on peut vérifier cela en voyant que la conjugaison de la 
...
\begin{prop}
Soient $P,R,R'$ trois points de $E$ située au niveau $h-e$ tels que $\langle 
P,R \rangle = \langle P,R' \rangle$ alors $\pi(P,R)=\pi(P,R') $ si et seulement
si $[\ell^{h-e}]R=[\ell^{h-e}]R'$.
\end{prop}

\begin{proof}
Montrons tout d'abord que pour deux points $R,R'$ tels que $\pi(P,R)=\pi(P,R')$
alors $[\ell^{h-e}]R=[\ell^{h-e}]R'$.
...
\begin{enumerate}
\item $\pi(P)=[a]P+[\beta^{-1}c\ell^{h-e}]R=[a]P+[\beta^{-1}c\ell^{h-e}]R'$, de
cette égalité on déduit  lorsqe $c \wedge \ell = 1$ que $[\ell^{h-e}]R=
[\ell^{h-e}]R'$ et lorsque $\ell^{2e} || c$ que $[\ell^{h+e}]R=[\ell^{h+e}]R'$,
on se place donc dans le second cas pour la suite de la preuve.
\item $[d]P=[b\ell^{h-e}\beta]R-\pi(R)=[b\ell^{h-e}\beta]R-\pi(R')$, on obtient
alors $\pi(R-R')=[b\ell^{h-e}\beta](R-R')$, ainsi $R-R'$ est point de 
$\ell^i$-torsion et l'on a donc $R-R'=0_E$.
\end{enumerate}

Supposons maintenant que $[\ell^{h-e}]R=[\ell^{h-e}]R'$ et prouvons par 
l'absurde que $\pi(P,R) \neq \pi(P,R')$. On suppose donc que $\pi(P,R)= \left ( 
\begin{smallmatrix}a & b\ell^{h-e}\\c\ell^{h-e} & d \end{smallmatrix}\right)$
et que $\pi(P,R')= \left( \begin{smallmatrix}a' & b'\ell^{h-e}\\c'\ell^{h-e} & 
d' \end{smallmatrix}\right)$, alors on a $\pi(P)=[a]P+[c\ell^{h-e}]R=[a']P+[c'\ell^{h-e}]R'$
que l'on peut réécrire $\pi(P)=[a-a']P+[c-c']([\ell^{h-e}]R)$
et donc comme $P$ et $R$ forment une base alors  $a=a' \bmod \ell^{h-e+i}$ et
$c\ell^{h-e} =c'\ell^{h-e} \bmod \ell^{h-e+i}$. Le déterminant et la trace du 
Frobenius étant invariants selon le choix de la base on a alors $d=d' \bmod 
\ell^{h-e+i}$ et $b\ell^{h-e} =b'\ell^{h-e} \bmod \ell^{h-e+i}$ on obtient donc
 une contradiction, on a donc $\pi(P,R)=\pi(P,R')$.
\end{proof}
...
\end{matrix}\right ) 
\] représentant l'action du Frobenius dans la base $\langle P,R \rangle$ ne 
dépend que de la valeur de $[\ell^{h-e}]R$ et est 
conjuguée aux matrices de la forme:
\[
\pi(P,R')=\left ( \begin{matrix}a & b\ell^{h-e}\beta^{-1} \\ c\ell^{h-e}\beta & d
\end{matrix}\right ) 
\]
si et seulement si $\beta \ell^{h-e}R= \ell^{h-e}R'$avec $\beta \in \left( \mathbb{Z}/\ell^{i} \mathbb{Z} 
...
de noyau $\langle [\ell^{h-e}]S \rangle$.
\end{prop}

\begin{proof}
On travaille avec deux points $R=[x]P+[y]Q,R'=[x']P+[y']Q$ avec $y,y' \wedge 
\ell=1$, car seulement dans ce cas $R$ et $R'$ forment avec $P$ une base de la 
$\ell^{h-e+i}$-torsion. Nous allonrs alors montrer que $R$ et $R'$ engendrent 
la même $\ell^i$-isogénie si ils vérifient 
$\ell^{h-e}x'y'^{-1}=\ell^{h-e}xy^{-1} \bmod \ell^{h-e+i}$.
...
x'y'^{-1}\bmod \ell^{h-e+i}$. On voit donc que seul compte les valeurs de 
$\ell^{h-e}x$ et $\ell^{h-e}y$ modulo $\ell^{h-e+i}$ pour la détermination de la 
$\ell^i$-isogénie.
...
$\ell^{h-e}xy^{-1} \bmod \ell^{h-e+i}$ ce sont donc les changements de la forme 
...
Soient $P $ et $R$ deux points de la courbe elliptique $E$ située à une 
profondeur $e$ tels que $\langle P ,R \rangle=E[\ell^{h-e+i}]$ 
alors la forme de la matrice $\pi(P,R)$ à $P$ fixé ne dépend que de $[\ell^{h-e}]R$
\end{cor}
...
Soient $P,R,R'$ trois points de la courbe elliptique $E$ située au niveau $h-e$
du volcan de $\ell$-isogénies tels que $\langle P, R \rangle = \langle 
P,R' \rangle= E[\ell^{h-e+i}]$ et $\pi(P,R)=\pi(P,R')$ %$ \left(  \begin{smallmatrix}1 & 0 
%\\0  & \beta \end{smallmatrix} \right)^{-1} \pi(P,R') \left(  
%\begin{smallmatrix}1 & 0 \\0  & \beta \end{smallmatrix} \right)$ pour $\beta 
%\in (\mathbb{Z}/\ell^i\mathbb{Z})^{\times}$
avec $i>e$ alors $[\ell^{h}]R=[\ell^h]R'$.
\end{prop}

\begin{proof}
Comme énoncé dans la proposition \ref{pro:cla:fro:mat} on a jusqu'à 
$\ell^{h+i-e-1} \times \ell^{h+i-e}$ choix possible de second point pour compléter 
une base de la $\ell^{h+i-e}$-torsion de $E$ or pour ces choix de matrice seul 
compte la valeur de $x\ell^{h}$ et $y\ell^{h} \bmod \ell^{i-e}$ par le 
corollaire~\ref{cor:atk:fro:dis}. On a donc $\ell^{i-e-1} \times \ell^{i-e}$ matrices
possibles, or chaque $\ell^{i-e}$-isogénie est associée à $\ell^{i-e-1}$ matrices 
distinctes par la proposition~\ref{pro:cla:fro:mat}. Comme deux points qui 
engendrent deux $\ell^{i-e}$-isogénies distinctes n'ont pas la même matrice par la 
proposition~\ref{pro:atk:inv:mat} alors par un argument de cardinalité 
chaque matrice à classe de conjugaison près par $\left( 
 \begin{smallmatrix}1 & 0 \\0  & \beta \end{smallmatrix} \right)$
représente une $\ell^{i-e}$-isogénie différente et l'on distingue les 
différents générateurs d'une même $\ell^{i-e}$-isogénie à l'aide des 
coefficents anti-diagonaux.
\end{proof}
...
proposition~\ref{pro:atk:fro:iso}.
\end{proof}

\subsection{Décomposition de la $\ell^{h+i}$ torsion sous l'action du Frobenius}
L'action du Frobenius sur la $\ell^{h+i}$ torsion d'une courbe située au niveau
du cratère peut être représentée à l'aide d'une matrice de la forme $\left( 
\begin{smallmatrix}a & b\ell^{h-e} \\c\ell^{h-e}  & d \end{smallmatrix} \right)
$ avec $a,b,c,d$ tous premiers avec $\ell$ et $\ell^h||a-d$. On cherche alors à
déterminer la taille des orbites pour les points $Q$ et par conséquent leur 
nombre de représentants.  

\todo{ecrire pourquoi on se restreint au cas du cratere...}

On se place dans le cas où la courbe elliptique ordinaire $E$ se situe au 
niveau $h-e>0$ du volcan de $\ell$-isogénies.
 Pour $\ell$ premier impair, on note~$\alpha
= v_{\ell}(a^{\ell-1}-1)$ et~$\beta=v_{\ell}(d^{\ell-1}-1)$; pour $\ell=2$,
on note~$\alpha=v_2(a^2-1)-1$ et~$\beta = v_2(b^2-1)-1$, on suppose 
sans perte de généralité que $\alpha \geqslant \beta$. On s'intéresse à ces valeurs-là car pour n'importe quel corps fini $\mathbb{F}_q$ on a $E[\ell] \subset E(\mathbb{F}_q^{\ell-1})$ pour $\ell \neq 2$ et $E[4] \subset E(\mathbb{F}_q^{2})$ pour $\ell=2$. Comme $\lambda \neq \mu \bmod{\ell^{h+1}}$, il est
impossible que $a \equiv d \equiv 1 \bmod{\ell^h+1}$ par la structure 
d'un volcan de $\ell$-isogénies (voir section \ref{sec:lie:niv}), ainsi au moins une des deux
valuations~$\alpha, \beta$ est~$\leqslant h-e$, par conséquent on va supposer sans perte de généralité que~$\beta \leqslant h-e$. \todo{est-ce vraiment utile cette dernière remarque ? A retravailler celle-ci}

\begin{prop}\label{pro:clas:fro:atk} \todo{nécessaire}
Soit $E$ une courbe elliptique ordinaire située au niveau $h-e>0$ d'un volcan de $\ell$-isogénies.
...
\begin{proof}
De manière similaire à la preuve de la proposition~\ref{pro:clas:fro}.
Le degré ~$d_k$ correspond à l'ordre de la matrice~$\pi|E[\ell^k]$.
%$d_k$ est donc le plus petit multiple commmun des ordres multiplicatifs
%de~$\lambda, \mu$ modulo~$\ell^k$,
On obtient donc~(1) en utilisant le fait que~$a \cdot d = q$.
Pour les points~(2)--(5) on peut supposer que l'on a $d_1 = 1$,
alors pour tout entier~$N$, $v_{\ell}(\lambda^{2N}-1) = \alpha + v_{\ell} (2N)$.
Soit~$(P, Q)$ une base diagonale de~$E[\ell^k]$,
le point $(\pi^N - 1) ([x] P + [y] Q) = [(\lambda^N-1) x] P + [(\mu^N-1) y] Q$
...
\begin{prop}
Soit $P$ un point d'ordre $\ell^{h+i}$ de la courbe elliptique $E$ située au 
niveau du cratère du volcan de $\ell$-isogénies alors la taille de l'orbite de 
$P$ selon l'action du Frobenius est de taille $\ell^i$.
\end{prop}

\begin{proof}
Dire que l'orbite du point $P$ de la courbe elliptique $E$ située au niveau du 
cratère soit de taille $x$ inférieure à $\ell^i$ revient 
à dire que le volcan de $\ell$-isogénies a pour hauteur $h+i$ sur l'extension 
de degré $x$ de $\mathbb{F}_q$, or le seul moyen pour avoir un volcan de 
hauteur $h+i$ est de prendre une extension de degré multiple à $\ell^i$.
\todo{Faire une distinction cas $\ell=2$}
\end{proof}

\begin{prop}
\label{pro:atk:dist:orb}
Soient $R,R'$ deux points d'ordre $\ell^{h-e+i}$ de $E$ située au niveau $h-e$ 
du volcan de $\ell$-isogénies, alors si l'on a $\pi^x(R)=R'$ alors
$[\ell^i]R$ et $[\ell^i]R'$ engendrent la même $\ell^{h-e}$-isogénie.
\end{prop} 

\begin{proof}
A partir de $\pi^x(R)=R'$ on a $[\ell^{i}]\pi^x(R)=\pi^x([\ell^i]R)=[\ell^i]R'$
or l'action du Frobenius sur les points de $\ell^{h-e}$-torsion est diagonale ainsi
il existe $\alpha \in (\mathbb{Z}/\ell^h\mathbb{Z})^{\times}$ tel que 
$\pi([\ell^i]R)=[\alpha]([\ell^i]R)$ on obtient alors l'égalité suivante entre
$R$ et $R'$: $[\alpha^x]([\ell^i]R)=[\ell^i]R'$ et donc on a le résultat.
\end{proof}

Ainsi avec ce résultat on a un critère pour savoir si deux points sont dans des
orbites distinctes.

\begin{prop}
Soit $E$ une courbe située au niveau $h-e$ d'un volcan de $\ell$-isogénies
alors il existe $\ell^{h-e-1}(\ell+1)$ représentants d'orbites distinctes des 
points d'ordre $\ell^{h-e+i}$.
\end{prop}

\begin{proof}
A l'aide de la proposition~\ref{pro:atk:dist:orb} pour trouver différents 
représentants d'orbites selon l'action du Frobenius il suffit de prendre des 
points qui engendrent tous des $\ell^h$-isogénies distinctes. Ainsi en fixant 
une base $(P,Q)$ de la $\ell^{h-e+i}$-torsion, on cherche tous les points de la
forme $[a]P+[b]Q$ tels que les points $[\ell^i]([a]P+[b]Q)$ engendrent des 
$\ell^h-e$-isogénies distinctes. On a alors $\ell^{h-e-1}(\ell+1)$ représentants 
d'orbites distinctes.
\end{proof}
...
\begin{proof}
\todo{preuve a faire}Question la plus importante dans quelle extension sont définis les points d'ordre $\ell^{h-i+e}$
\end{proof}
...
Par définition de ~$k$, on a ~$\ell^{2k-1} \in O(r\ell^2)$.
  ...
  $O(\mathsf{M}(r\ell^{2,5})\log(r)\log(\ell)\log(\ell q))$. \todo{ou ecrire $\sqrt{\ell^5}$}
