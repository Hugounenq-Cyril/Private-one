...
\begin{algorithm}
\label{alg:TRC:init}
\caption{Théorème des restes chinois sur une courbe elliptique}
\begin{algorithmic}[1]
\REQUIRE $E$: une courbe elliptique, $\ell_1,\ell_2$ deux nombres premiers distincts, $P_1$ un point d'ordre $\ell_1^{k_1}$.
\ENSURE $P$ un point de $E$ d'ordre $\ell_1^{k_1}\ell_2^{k_2}$  tel que $\ell_1^{h_1}P=0_E$ et $\ell_2^{k_2}P=P_1$.
% \STATE Calcul $(P_1,Q_1)$, a basis of $E[\ell]$
\STATE Calcul de $P'=\mathsf{divise}(P_1,\ell_2^{k_2})$;
\STATE Calcul de $Q=\ell_1^{k_1}P'$ un point d'ordre $\ell_2^{k_2}$;
\STATE Calcul de $R$ la pré-image de $Q$ par $\ell_1^{k_1}$;
\RETURN $P'-R$
\end{algorithmic}
\end{algorithm}
