...
\begin{prop}
Soit $E$ une courbe au niveau $h-e$ dans le volcan, soit $E_s$ sa courbe sommet associée, alors l'algorithme \ref{alg:rem:tri} calcule une base diagonale de $E_s[\ell^k]$ avec une complexité espérée de $O( \mathsf{R}(k-\beta+e) + \ell^{k-\beta+e}\mathsf{M}(\ell) + \ell\mathsf{M}(\ell^{k-\beta+e}))$ opérations sur $\mathbb{F}_q$.
\end{prop}
...
\`A l'étape~\ref{alg:rem:divide} dans le cas où l'on serait sur un volcan régulier on aurait les points $P$ et $Q$ situés au niveau $h-e+i$ définis dans $E(\mathsf{F}_{k-\beta+e-i})$ 
alors le côut de $\mathsf{divise}$ est de $O(\mathsf{R}(k+\beta+e+1-i))$
  opérations.

  L'évaluation de~$\pi(Q')$ et $\pi(P')$ à l'étape~\ref{alg:rem:frobenius} a un côut
  de~$O(\ell^{k-\beta+e+1-i}\mathsf{M}(\ell))$ par le théoèreme \ref{thm:frob-ell}.
  L'écriture~$\pi(P')$ comme une combinaison linéaire~$\lambda P' + [v] Q'$ nécessite au
  plus~$\ell$ additions de points, avec un côut de~$\ell
  \mathsf{M}(\ell^{k-\beta+e+1-i})$.
...
\begin{prop}
Soit $E$ une courbe au niveau $h-e$, situé au-dessus du premier niveau de stabilité dans le volcan alors l'algorithme \ref{alg:rem:tri} calcule une base diagonale de $E_s[\ell^k]$ avec une complexité espérée de $O(e( \mathsf{R}(k-\beta) + \ell^{k-\beta}\mathsf{M}(\ell) + \ell\mathsf{M}(\ell^{k-\beta})))$ opérations sur $\mathbb{F}_q$.
\end{prop}
...
que l'on travaille avec des points d'ordre $\ell^{h-e+1}$, ainsi cette première
 ...
 \ref{alg:ult:identical:divide} est de $\mathsf{R}(i+1+e-\beta)$. Le coût de calcul de 
 $\pi(P)$ est de $O(\ell^{i+1+e-\beta}\mathsf{M}(\ell))$ par le théorème 
 \ref{thm:frob-ell}. L'écriture de $\pi(P)=[u]P$, on 
 se limite à ce cas là car on est dans le cas où la matrice représentant 
 l'action du Frobenius est scalaire, a un coût de $O(\ell \mathsf{M}(\ell^{i+1+e-\beta}))$. 
