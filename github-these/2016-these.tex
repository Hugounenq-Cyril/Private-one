...
Cette opération est très importante pour déterminer des ensembles spécifiques de points d'une courbe elliptique (voir chapitre~\ref{cha:act:fro} \todo{ref à mettre plus tard}), c'est pourquoi on s'attache ici à montrer que l'on peut effectuer cette opération à un coût raisonnable. Le résultat ici est une généralisation de \cite{Doliskani-Schost15}.
...
\label{cha:act:fro}
\section{Cas Elkies}
La majeure partie du travail présenté dans cette section a été effectué en collaboration avec Luca De Feo, Jér\^ome Pl\^ut et \'Eric Schost. Il a fait l'objet d'une publication \cite{Defeo_Plut_Schost_2016}.

Dans cette partie on va étudier sur une courbe elliptique $E$ les sous-groupes 
de $E[\ell^{\infty}]$ que l'on peut déterminer à l'aide de l'action du 
Frobenius, on va montrer de plus comment on peut représenter ces sous-groupes à
l'aide du volcan de $\ell$-isogénies. On se place tout d'abord dans un cas 
particulier le cas Elkies que l'on définit de suite: 
...
sinon $h - e$~est le niveau de~$E$ dans le volcan.
\end{prop}
...
$\mathbb{Z}_{\ell}$ alors la matrice $\pi|T_{\ell}(E)$ est trigonalisable. \todo{justifier cela Morris Newman Integral Matrices III.16}
...
$\ell^{\min (h, v_\ell(a))}$ lui donnant la hauteur $\min (h, v_\ell(a))$ dans 
le volcan de $\ell$-isogénies.
\end{proof}

Ainsi avec la proposition \ref{pro:mat:fro} on a un moyen de déterminer la hauteur d'une courbe dans un volcan de $\ell$-isogénies à l'aide de l'étude de l'action du Frobenius sur la $\ell$-torsion. Le résultat suivant qui nous donne un moyen de savoir comment monter et descendre à l'intérieur d'un volcan de $\ell$-isogénies.

\begin{prop}
\label{pro:tri:dir}
Soient $E$ une courbe elliptique située au niveau $h-e$ du volcan de 
$\ell$-isogénies avec $\ell$ nombre premier de Elkies, $P,Q \in E$ tels que 
...
avec $c \in \mathbb{Z}/\ell \mathbb{Z}$ $\widehat{\varphi}$ dans cette 
base et donc par la proposition~\ref{pro:mat:fro} on a le codomaine de 
$\widehat{\varphi}$ qui se situe au niveau $h-e+1$. En effet un changement de
base de la forme $\left( \begin{smallmatrix} 1 & 0 \\ \gamma \ell^{k-1} & 1 
\end{smallmatrix} \right)$ avec $\gamma \in (\mathbb{Z}/\ell 
\mathbb{Z})^{\times}$ tel que $\ell^{k-1}(c+\gamma (\lambda -\mu))=0 \bmod 
\ell^k$ permet d'obtenir une matrice triangulaire supérieure de la forme de 
celles de la proposition~\ref{pro:mat:fro}.
...
\label{sub:ss-gr}
On va maintenant se focaliser sur la détermination des espaces propres du 
Frobenius, et voir comment les déterminer à précision finie. On travaille donc 
sur des courbes situées sur le cratère car c'est le seul cas où l'on peut 
diagonaliser le Frobenius. On verra par la suite comment utiliser cette 
diagonalisation pour avoir une détermination de sous-groupes cycliques de la 
$\ell^k$-torsion pour des courbes situées en dehors du cratère.
...
\begin{lem}
\label{lem:iso:con}
Soient $\langle P, Q \rangle$ une base de $E[\ell^k]$ avec $k>0$, $\phi$ 
l'isogénie de noyau engendré par $R=[x]P+[y]Q$  avec $x\wedge \ell =1$ et 
$y=\ell^my'$ avec $y' \wedge \ell =1$, $0 \leqslant m \leqslant k$, on note 
$E'$ le codomaine de $\phi$. $M_R$ la 
matrice de forme normale réduite de Hermite qui correspond au réseau engendré 
par $R$,  $\Pi$ la matrice du Frobenius sur $T_{\ell}(E)$ alors la matrice du 
Frobenius sur $T_{\ell}(E')$ est donnée par $M_R^{-1} \cdot \Pi \cdot M_R$ à 
conjugaison près.
\end{lem}
\todo{harmoniser les notations pour la matrice de $\pi$}
...
$\pi|T_{\ell}(E')$ est égale à $\left ( \begin{smallmatrix}\lambda& \ell^{h-k} 
...
\todo{faire un dessin pour cette proposition}
...
\end{proof}
\todo{Luca a raison là on s'éloigne du cadre que l'on s'était fixé}
La proposition suivante nous permet de faire le lien entre les bases horizontales et le contexte de l'algorithme de Couveignes~\cite{Couveignes96}.
...
\end{tikzpicture}
\end{center}
\caption{ \label{fig:par:hor:con} Construction d'un point partiellement horizontal à partir d'un point $P$ d'ordre $2^3$ tel que $[2]P$ soit horizontal de direction $\color{blue}{\mu}$ et $\psi:E_s \rightarrow E$ une $2$-isogénie descendante.}
\end{figure}

On voit sur la figure~\ref{fig:par:hor:con} la construction d'un point 
partiellement horizontal sur un volcan de $2$-isogénies. Sur la figure de 
...
pour construire le point partiellement horizontal $R=\psi(P) \in E$. Sur la 
figure de droite est représenté l'isogénie de noyau $\langle R \rangle$.

\todo{faire un dessin de construction}
...
\begin{proof}
Pour le point (1) montrons que l'image de $P$ par $\phi$ est encore d'ordre 
$\ell^k$, comme $P$ est un générateur d'une $\ell^{k-i}$-isogénie horizontale 
alors $[\ell^{k-1}]P$ n'est pas dans le noyau de $\phi$ et donc $\phi(P)$ est 
d'ordre $\ell^k$. On a alors $\langle [\ell^{k-i}]\phi(P) \rangle $ qui est le 
noyau de $\widehat{\phi}$.  
...
\todo{faire un beau dessin de correspondance}
\begin{prop}
...
\widehat{\psi'}(P') \rangle$ et donc les $\ell^{k}$-isogénies engendrées par 
...
courbes $r$-isogénes qui respectent l'action de la $r$-isogénie et qui ne se 
situent pas forcément sur le cratère du volcan de $\ell$-isogénies. 

La différence entre des points horizontaux $P_{\lambda}, Q_{\mu} \in E_s$ d'ordre $\ell^k$ et des points partiellement horizontaux $R_{\lambda}, R_{\mu} \in E$ d'ordre $\ell^k$ de profondeur $e>0$ de direction opposées c'est que l'on a $\langle R_{\lambda}, R_{\mu} \rangle \neq E[\ell^k]$ (en particulier$\langle [\ell^{k-e}]R_{\lambda} \rangle = \langle [\ell^{k-e}]R_{\mu} \rangle$) alors que pour deux points horizontaux de direction opposées on avait $E_s[\ell^k]= \langle P_{\lambda}, Q_{\mu} \rangle $. 


\subsection{Décomposition de la $\ell^k$ torsion sous l'action du Frobenius}
\label{sub:classes}
On va maintenant étudier la décomposition de la $\ell^k$-torsion en orbites 
selon l'action du Frobenius, afin de pouvoir ultérieurement calculer toute la 
$\ell^k$-torsion à partir des représentants des orbites et de l'action du 
Frobenius. 
...
On se place dans le cas où la courbe elliptique ordinaire $E$ admet un anneau 
des endomorphismes localement maximal par rapport à $\ell$.
Pour $\ell$ premier impair, on note~$\alpha= v_{\ell}(\lambda^{\ell-1}-1)$ 
et~$\beta=v_{\ell}(\mu^{\ell-1}-1)$; pour $\ell=2$, on 
note~$\alpha=v_2(\lambda^2-1)-1$ et~$\beta = v_2(\mu^2-1)-1$, on suppose sans 
perte de généralité que $\alpha \geqslant \beta$. On s'intéresse à ces 
valeurs-là car pour n'importe quel corps fini $\mathbb{F}_q$ on a $E[\ell] 
\subset E(\mathbb{F}_q^{\ell-1})$ pour $\ell \neq 2$ et $E[4] \subset 
E(\mathbb{F}_q^{2})$ pour $\ell=2$.  Comme $\lambda \neq \mu \bmod{\ell^{h+1}}$
...
La décomposition du groupe~$E[\ell^k]$ en calsse de Galois est la suivante:
\begin{enumerate}
\item pour~$i, j = 1, …, k-1$:
...
\section{Cas Atkin}
Dans cette section on s'intéresse aux courbes situées sur un volcan de 
$\ell$-isogénies dont le cratère est réduit à un point. Ce cas-là correspond 
au cas Atkin et l'on dira que $\ell$ est un nombre premier d'Atkin pour une 
courbe si l'on a un tel type de volcan de $\ell$-isogénies. On va donc étudier 
dans cette partie les groupes de points de $E[\ell^{\infty}]$ que l'on peut 
déterminer à l'aide de l'étude de l'action du Frobenius.
\todo{Dire ce qu'est le cas Atkin et contextualiser toute cette partie...}
\subsection{Determination de sous-groupes cycliques à l'aide du Frobenius}
...
ainsi le volcan de $\ell$-isogénies de $E$ de hauteur $h$ a un cratère réduit à
un point. Il existe un unique $e \in [ 0, h]$ tel que $\pi|T_{\ell}(E)$ est 
égale, sur~$\mathbb{Z}_{\ell}$, à la matrice 
\begin{equation*}
...
\todo{fixer le problème de profondeur et de hauteur}

Maintenant nous allons voir quelles informations on peut tirer de l'étude de 
l'action du Frobenius. Vu que l'on ne peut comme dans le cas Elkies distinguer 
deux sous-groupes cycliques de la $\ell$-torsion, on va voir quelles
...
P,R \rangle = \langle P,R' \rangle= E[\ell^{h+i}]$ alors $\pi(P,R)=\pi(P,R') $ si et seulement
si $[\ell^{h}]R=[\ell^{h}]R'$.
\end{prop}
...
cette égalité on déduit avec $c \wedge \ell = 1$ que $[\ell^{h}]R=[\ell^{h}]R'$
...
d'avoir $v_{\ell}(c)$ ou $v_{\ell}(b)$ qui appartient à $[2,2e]$ ce qui fait 
que l'implication $\pi(P,R)=\pi(P,R') \Rightarrow [\ell^{h-e}]R=[\ell^{h-e}]R'$ n'est plus 
assurée on peut seulement montrer que $\pi(P,R)=\pi(P,R') \Rightarrow 
[\ell^{h+e}]R=[\ell^{h+e}]R'$.
\end{rem}

Dés lors on va se concentrer à l'étude des points situés sur une courbe qui se 
situe sur le cratère.
...
\beta R)$ si et seulement si $[\ell^{h}]\beta R=[\ell^{h}]R'$ or en 
...
Montrons maintenant le second point. Soit $[\ell^{h}]([x]P+[y]Q)$ un point 
d'ordre $\ell^i$ engendré par $R$, 
le groupe $\langle [\ell^{h}]([x]P+[y]Q) \rangle$ engendré par ce point peut 
aussi s'écrire:$\langle [xy^{-1}]([\ell^{h-e}]P)+ ([\ell^{h}]Q) \rangle$. De 
même on peut écrire le groupe engendré par $[\ell^{h}]([x']P+[y']Q)$ comme 
...
forme une base avec $P$. \todo{a re rédiger}
...
$E'$ deux courbes situées au niveau du cratère du volcan de $\ell$-isogénies, 
...
Maintenant énonçons un résultat qui est une transposition de la 
...
résultats suivants, cependant ces résultats ne seront pas exploités par la 
suite.
...
Dans cette sous-section on va voir comment se décompose l'ensemble des 
points d'ordre $\ell^{h+i}$ sous l'action du Frobenius dans le cas Atkin pour 
une courbe se situant sur le cratère.
\begin{prop}
...
\begin{algorithmic}[1]
\REQUIRE $(P, Q)$: une base de~$E[\ell^{h-e+i}]$ avec $P$ un point fixé de $E$, $E$ une courbe située au niveau $h-e$;  un entier $i \geqslant e$; $M$ une matrice représentant l'action du Frobenius dans une base de la $\ell^{h-e+i}$-torsion d'une courbe elliptique située au niveau $h-e$.\\
...
$O(\ell^4(\mathsf{M}(\ell^{h-\beta+i+1})+(\beta-e)\mathsf{M}(\ell)))$ opérations sur 
$\mathbb{F}_q$ lorsque la courbe $E$ se trouve sur un volcan \emph{régulier} et
une complexité de 
$O(\ell^4(\mathsf{M}(\ell^{h-e+i+1-\beta})+(\beta)\mathsf{M}(\ell)))$
lorsque $E$ se situe au-dessus du premier niveau de stabilité.
\end{prop}

\begin{proof}
On traite tout d'abord le cas de volcan \emph{régulier}.
Par la proposition~\ref{pro:clas:fro:atk} les points $P_j,R_j$ sont définis
dans $\mathsf{F}_{j-\beta+e}$ lorsque $j-\beta+e>1$ et définis dans $\mathsf{F}_1$ sinon.
L'étape~\ref{alg:atk:bas:niv} est faite en tant que pré-calcul, ainsi on calcule
successivement des multiplications par $\ell$ de points définis dans 
$\mathsf{F}_{j-\beta+e}$ à $\mathsf{F}_{1}$ en décomposant la multiplication par 
$\ell$ par deux isogénies de degré $\ell$ qui coûte chacune 
$O(\mathsf{M}(\ell^{1+\max(j-\beta+e,1)}))$ opérations sur $\mathbb{F}_q$. On a donc 
un coût total pour l'étape~\ref{alg:atk:bas:niv} de 
$O(\mathsf{M}(\ell^{h-\beta+i+2})+(\beta-e)\mathsf{M}(\ell))$.
\`A l'étape~\ref{alg:atk:coef:int} le calcul de $R_j$ peut être actualisé à 
chaque itération de la boucle pour à l'aide 
de $\ell^2$ additions de points définis dans $\mathsf{F}_{j-\beta+e}$ 
lorsque $j-\beta+e>1$ ou $\mathsf{F}_1$ sinon. Ainsi le calcul de $R_j$ a 
un coût total de 
$O(\ell^{2}(\mathsf{M}(\ell^{h-\beta+i+1})+(\beta-e)\mathsf{M}(\ell)))$. Le 
calcul du Frobenius sur un point $R_j$ fixé défini dans 
$\mathsf{F}_{j-\beta+e}$ lorsque $j-\beta+e>1$, 
$\mathsf{F}_1$ sinon a un coût total de $O(\ell^{\max(j-\beta+e,1)}
\mathsf{M}(\ell))$ par le théorème~\ref{thm:frob-ell}. Le coût total du 
Frobenius pour cet algorithme est donc de 
$O(\ell^2\mathsf{M}(\ell)(\ell^{h-\beta+i+1}+(\beta-e)\ell))$ opérations sur 
$\mathbb{F}_q$.
...
$\ell^{j}$ définis dans dans $\mathsf{F}_{j-\beta+e}$ lorsque $j-\beta+e>1$, 
$\mathsf{F}_1$ sinon. Ce test d'égalité a donc un coût total de 
$O(\ell^4(\mathsf{M}(\ell^{h-\beta+i+1})+(\beta-e)\mathsf{M}(\ell)))$ opérations sur 
$\mathbb{F}_q$.

L'analyse du coût de l'algorithme pour une courbe située au dessus du premier
niveau de stabilité est la même excepté le fait que un point d'ordre 
$\ell^{j}$ est défini dans $\mathsf{F}_{j-\beta}$ lorsque $j-\beta>1$ et 
$\mathsf{F}_1$ sinon.
\end{proof}
...
\todo{Déplacer au bon endroit}
Afin d'avoir une présentation uniforme avec les points horizontaux et les 
points partiellement horizontaux on choisit de travailler avec les points 
d'ordre $\ell^k$(avec $k>h$), car avec deux points partiellement horizontaux 
d'ordre $\ell^k$ on peut tous les générer au contraire points de la 
$\ell^{e}$-torsion. En effet on ne peut pas générer tous les point de 
$\ell^e$-torsion et donc pour éviter toute redondance on ne travaille qu'avec 
les points d'ordre $\ell^k$. Si l'on souhaitait travailler avec les points de 
la $\ell^e$-torsion on utiliserait dans le cas de deux points partiellements 
horizontaux $R_{\lambda}, R_{\mu}$ (de profondeur $e>0$)
par exemple le groupe $\langle [\ell^{k-e}]R_{\lambda} \rangle$. 




cette précédente discussion justifie l'utilité de se concentrer sur les points d'ordre $\ell^{k}$ avec $k \geqslant h+1$ plutôt que sur la $\ell^k$ torsion dans le contexte de l'algorithme de Couveignes~\cite{Couveignes96}.
...
On s'intéresse ici au cas plus spécifique du "\emph{Problème du calcul 
explicite d'isogénie}" où les deux courbes en entrée sont situées sur un volcan
de $\ell$-isogénies à cratère cyclique, ce qui correspond au cas Elkies.
...
$[1;L]$ avec $L \in O(\log(q))$. On a vu dans la proposition~\ref{pro:cou:dec} 
...
$O(\mathsf{R}(k-\beta)+\ell^2\mathsf{M}(\ell^{k-\beta})+\ell 
\mathsf{M}(\ell^2)\log(\ell)\log(\ell q))$ (avec ~$\ell^{2k-1} \in O(r\ell^2)$)
...
rajouter le cout de la recherche de courbe sur le volcan: $O(\log(q)^6)$ par la 
...
\section{Cas Atkin}
\label{sec:Atk:Con}
On se restreint tout d'abord au cas où l'on travaille avec deux courbes 
$r$-isogènes, avec $r \wedge \ell=1$ qui se situent sur un
volcan de $\ell$-isogénies de hauteur $h$, dont le cratère est réduit à une 
courbe car on se place dans le cas Atkin. De plus on va dans un premier temps
se restreindre au cas où les courbes avec lesquelles on travaille se situent 
sur le cratère d'un volcande $\ell$-isogénies, on verra par la suite comment 
généraliser notre problème à des courbes non situées sur le cratère.
...
$[\ell^h]R'$ si la correspondance $(P,Q) \rightarrow (S',U')$ premet de définir
...
F=A_{S',U'} \bmod T
\]
avec $T$ le polynôme minimal qui s'annule sur tout point $[u]P +[v]Q_{\mu}$ 
...
Comme dit précédemment les résultats de la proposition~\ref{pro:bor:ell},
~\ref{pro:cou:dec} et du corollaire~\ref{cor:atk:elk:dist} sont 
...
similaire à celui de du théorème~{thm:gen:elk:fin}, vu le peu d'intérêt d'un 
