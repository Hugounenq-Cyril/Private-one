...
Comme vu précedemment on a besoin de calculer une solution de l'équation différentielle linéaire \ref{eq:LS08:diflin} de précision $4\ell-1$ si l'on ne connaît pas $p_1$ la somme des abscisses des points du noyau de l'isogénie. Dans ce cas qui nous intéresse on applique le résultat de Lairez Vaccon: pour obtenir une solution $p$-adique de précision $4 \ell-1$ on a besoin d'avoir en entrée des éléments de précision $ 1 + \lfloor \log_p(4 \ell - 1) \rfloor $.
\end{proof}

\begin{prop}
L'algorithme \ref{alg:LS08:modified} calcule une $\ell$-isogénie à l'aide $O(\ell^2)$  opérations sur $\mathbb{F}_q$.
\end{prop}
...
\subsection{Algorithme de Couveignes }
...
L'algorithme de Couveignes prend en entrée deux courbes elliptiques  ordinaires définies sur un corps fini $\mathbb{F}_q$ si ces deux courbes sont $\ell$-isogénes avec $\ell$ premier avec $p$, l'algorithme calcule la $\ell$-isogénie $\mathbb{F}_q$ rationnelle qui les relie, l'algorithme marche uniquement en caractéristique $p$ impaire. 


\subsubsection{Algorithme original de Couveignes}
...
Ainsi l'algorithme de Couveignes se sert du fait que la $p^k$-torsion de $E$ a pour image par l'isogénie $I$ la $p^k$-torsion de $E'$. C'est cette correspondance qui, lorsque $k$ est assez grand, donne une information suffisante pour interpoler l'isogénie. Comme la $p^k$-torsion est cyclique, alors cette correspondance est une correspondance entre deux groupes cycliques. Soient $P$ un générateur de $E[p^k]$ et $P'$ un générateur de $E'[p^k]$ tous deux calculés itérativement. L'algorithme fait alors l'hyptohèse que le point $P$ a pour image le point $P'$ par l'isogénie $I$. Ainsi si l'on représente $I$ à l'aide d'une fraction rationnelle (comme vu dans \textcolor{red}{ref à mettre}) on a:
\begin{equation}
\frac{g(x([k] \cdot P))}{h(x([k] \cdot P))}=x([k] \cdot P') \forall k \in \mathbb{Z}/p^{k}\mathbb{Z}.
\end{equation}
avec les formules de Vélu qui impliquent que l'on ait $\deg g = \ell$ et $\deg h = \ell-1$.
Ainsi pour calculer l'isogénie l'algorithme de Couveignes calcule un polynome $A \in \mathbb{F}_q[X]$ tel que l'on ait 
\begin{equation}
A(x([k] \cdot P))= x([k] \cdot P') \bmod T \quad \forall k \in \mathbb{Z}/p^{k}\mathbb{Z}
\end{equation} 
avec $T \in \mathbb{F}_q[x]$ qui s'annule sur $E[p^k]$ car on se sert d'une équation définie sur la $p^k$ torsion. En pratique l'algorithme de Couveignes demande que l'on choisisse $k$ tel que $p^k \geqslant 4 \ell+1$, cela est du notamment au fait que l'on veuille calculer l'isogénie sous la forme explicite de Vélu, en appliquant donc l'algorithme de Cauchy on a besoin d'avoir $p^k \geqslant 4 \ell +1$ et non $p^k \geqslant 2 \ell +1$ car tout point d'ordre différent de $2$ a son opposé de même abscisse on doit donc doubler le nombre de points à considérer. On vérifie alors si la fraction rationnelle obtenue est bien la restriction d'une isogénie sur les abscisses. Si ce n'est pas le cas on choisit alors un autre générateur $P'_2$ (par exemple $P'_2=\lambda P_2$ avec $\lambda \in \left( \mathbb{Z}/p^k \mathbb{Z} \right)^* $) de $E'[p^k]$ et l'on recommence l'algorithme en supposant que $I(P)=P'_2$.

\subsubsection{Coût de Couveignes}

\paragraph{Calcul de la $p$-torsion}
La première amélioration que l'on peut apporter à l'algorithme de Couveignes c'est la première étape de l'algorithme, le calcul de générateurs de la $p^k$ torsion. On a tout d'abord besoin de définir les points de $p$-torsion pour cela on se sert du travail de Gunji \cite{Gunji76} on se place dans le cas où $p \neq 2$.
...
\end{equation*}
avec $\Delta_0$ et $\Delta_1$ les detérminants des matrices données dans la figure \textcolor{red}{ref à mettre} avec $r=\frac{p-1}{2}, \alpha_{\nu}=\nu (\nu -1 ) a_6, \beta_{\nu}=\nu(\nu-\frac{1}{2})a_4 \nu_{\nu}=\nu^2a_2$ et $\delta_{\nu}=\gamma(\nu + \frac{1}{2})$ avec $\Delta_1=1$ lorsque $r=1$.
\end{thm}
Ainsi si on note $c$ l'une des racinces $p-1$ ème de $H_E$, ce résultat implique que les points de $p$-torsion sont définis dans $\mathbb{F}_q[c]$ et leurs abscisses sont incluses dans $\mathbb{F}_q[c^2]$.
...
[p]=\pi \circ V
\end{equation}
avec $\pi$ le Frobenius défini sur $\mathbb{F}_q$ et $V$ le verschebung. L'idée originale est donc d'inverser chacune  de ces isogénies pour calculer itérativement des antécédents de point de $p^i$-torsion par $[p]$. Inverser l'isogénie séparable $V$ revient notamment à factoriser un polynômes de degré $p$ dans une extension de $\mathbb{F}_q$, $V$ peut être explicité à l'aide de la $p$-torsion de $E: E[p]$. 

Une autre approche a été proposé par De Feo pour prendre avantage notamment des constructions de tour d'Artin Schreier en s'appuyant sur les travaux de Voloch \cite{Voloch90} suggéré dans l'article original \cite{Couveignes96}. On suppose $p\neq 2$ et l'on pose $E$ et $\tilde{E}$ d'équations respectives:
\begin{equation}
...
on pose alors:
\begin{equation}
\tilde{f}(x)^{\frac{p-1}{2}}=\alpha(x)+H_{\tilde{E}}X^{p-1}+X^{p}\beta(x)
\end{equation}
aec $\deg  \alpha <p-1$ et $H_{\tilde{E}}$ l'invariant de Hasse de $\tilde{E}$. Voloch a alors démontré le résultat suivant:
\begin{prop}
Soit $\tilde{c}$ une racine $p-1$ ème de $H_{\tilde{E}}$, le recouvrement de $\tilde{E}$ défini par:
\begin{equation}
\label{eq:vol:cov}
...
\end{equation}
avec $P_1$ un point primitif de $p$-torsion de $E$.
\end{prop}

La descente est alors effectuée de la façon suivante, à partir d'un point $P$ de $E$ on calcule un antécédent par $\pi$ ensuite on prend une de ses pré-images sur $C$ en résolvant l'équation \ref{eq:vol:cov}. On résoud alors l'équation \ref{eq:vol:iso} pour obtenir un point $P'$ sur $E$. La proposition nous assure que l'on a $[p]P'=P$. La descente est schématisée dans le dessin \textcolor{red}{ref à mettre}.

Cette méthode est plus efficace que la méthode standard car à l'aide des constructions de Tour d'Artin Schreier on peut résoudre plus rapidement l'équation \ref{eq:vol:cov} qui est une équation d'Artin Schreier cette équation peut être résolue à l'aide de méthodes de \textcolor{red}{ref à mettre} \cite[Ch.6]{DeFeo10} ou par algèbre linéaire comme dans \cite{Couveignes96}. Une fois qu'une solution $\tilde{z}$ de \eqref{eq:vol:cov} est trouvée il faut alors résoudre en $x$ et en $y$ le système de \eqref{eq:vol:iso} à l'aide d'un calcul de $\mathrm{pgcd}$ plus de détails peuvent être trouvés dans \cite[§6.2]{Ler97a} ou dans \cite[§9.1]{DeFeo10} alors que dans la méthode préconisée auparavant on devait facotriser un polynôme.

Comme le travail de \cite{DeFeo11} repose sur les constructions de tour d'Artin Schreier, dont une brève description sera faite \textcolor{red}{ref. à mettre} pour plus de détails voir \cite{DeFeo-Shost'12}, et que celles-ci sont construites comme des extensions de $\mathbb{F}_q$ alors les complexités seront exprimées en terme d'opérations sur $\mathbb{F}_p$ en utilisant la convention que l'on a $q=p^d$.
...
\begin{proof}
Le cout de calcul de la $p$-torsion nécessite tout d'abord, d'après les formules de Gunji de calculer $c$ et $\tilde{c}$ racines $p-1$ ème de $H_{E}$ et $H_{E'}$. On doit ensuite construire les corps $\mathbb{F}_q[c]$ et $\mathbb{F}_q[\tilde{c}]$, dans le pire des cas $\mathbb{F}_q[c]$ et $\mathbb{F}_q[\tilde{c}]$ sont des extensions de degré $p-1$ ainsi on aura besoin jusqu'à $p-1$ éléments de $\mathbb{F}_q$ pour représenter des éléments de $\mathbb{F}_q[c]$ et $\mathbb{F}_q[\tilde{c}]$. Le coût le plus important des formules de Gunji étant le calcul des déterminants d'une matrice $\frac{p-1}{2} \times \frac{p-1}{2}$ avec quatres diagonales non nulles cela a donc un coût de $\theta(p^2)$ opérations sur $\mathbb{F}_q[c]$ par élimination de Gauss ce qui a un coût de $\Omega(p^3d)$ opérations sur $\mathbb{F}_p$.
\end{proof}

Pour la calcul de la $p^k$-torsion on a tout d'abord besoin de savoir la taille du corps dans lequel on est ammené à effectuer les calculs. On introduit donc la définition de tours d'extensions $p$-adique, le résultat suivant issu de \cite{DeFeo10} est une généralisation de \cite[Proposition 26]{Ler97a} il montre en particulier que prendre une extension de degré $p$ dans une telle tour augmente exactement la valuation $p$-adique de la torsion rationnelle de la courbe de $1$.

\begin{defi}
\label{def:p-adic}
Soit $\mathbb{F}_q$ un corps fini sur lequel on définit la tour d'extension $p$-adique $\mathbb{U}_0,  \mathbb{U}_1, \mathbb{U}_2, \cdots \mathbb{U}_i$ telle que $\mathbb{U}_i$ soit la plus petite extension de corps de $\mathbb{F}_q$ dans laquelle on a $E[p^i] \subset E(\mathbb{U}_i)$. On pose $\mathbb{U}_0=\mathbb{F}_q$.
\end{defi}

\begin{prop}
\label{pro:ord:fro}
Pour une tour d'extension $p$-adique de $\mathbb{F}_q$ définie comme dans \ref{def:p-adic} on a:
\begin{itemize}
\item $[\mathbb{U}_1:\mathbb{F}_q] \mid p-1$
\item Pour tout $i \geqslant 1$ on a soit $[\mathbb{U}_{i+1}:\mathbb{U}_{i}]=p$ soit $\mathbb{U}_{i+1}=\mathbb{U}_{i}=\cdots = \mathbb{U}_{1}$.
\end{itemize}
\end{prop}
...
La preuve suivante n'est pas différente de celle dans \cite{DeFeo10} mais celle-ci est tout de même intéressante par son approche dont notamment l'étude de l'action du Frobenius sur le $p$ module de Tate d'où sa retranscription ici.

\begin{proof}
On a $[\mathbb{U}_1:\mathbb{F}_q] \mid p-1$ par le théorème \ref{thm:Gunji}. Prouvons maintenant le second point,
...
Regardons alors l'action du groupe de Galois $G$ de $\mathbb{F}_q$ sur $T_{p}(E)$. Comme $G$ est engendré par l'automorphisme de $\mathbb{F}_q$, la restriction de son action à $E[p^k]$ peut être représentée par la multiplication de $t_k=\mathfrak{t}_{\pi} \bmod p^k$. Ainsi on a $[\mathbb{U}_k:\mathbb{F}_q]=\mathrm{ord}(t_k)$.
...
On voit donc à travers cette preuve que si $E[p] \subset E(\mathbb{F}_q)$ alors il est possible que $\mathbb{U}_0=\mathbb{U}_1=\mathbb{U}_2$, de même lorsque l'on a $E[p] \nsubset E(\mathbb{F}_q)$ il est possible que $\mathbb{U}_1=\mathbb{U}_2$, dés lors la distinction suivante est nécessaire.
...
\begin{defi}
Soit $\mathbb{U}_0, \mathbb{U}_1, \mathbb{U}_2, \cdots, $ une tour d'extension $p$-adique de $\mathbb{F}_q$ définie comme dans \ref{def:p-adic} alors on définit l'indice de valuation rationnelle $p$-adique de la tour comme étant le plus grand indice $i$ tel que $\mathbb{U}_i \subset \mathbb{F}_q$. On le note $v_p(E(\mathbb{F}_q))$. On définit de la même manière l'indice de valuation $p$-adique de la courbe définie sur $\mathbb{U}_1$ comme étant le plus grand indice $i$ tel que $\mathbb{U}_i \subset \mathbb{U}_1$. On le note $v_p(E(\mathbb{U}_1))$.
\end{defi}

\begin{rem}
Il est à noter que si $v_p(E(\mathbb{F}_q)) \neq 0$ alors $v_p(E(\mathbb{F}_q))=v_p(E(\mathbb{U}_1))$.
\end{rem}

Ainsi on a $[\mathbb{U}_i:\mathbb{U}_1]=p^{i- v_p(E(\mathbb{U}_1))}$ pour tout $i \geqslant  v_p(E(\mathbb{U}_1))$. Ainsi asymptotiquement on a $[\mathbb{U}_k:\mathbb{U}_1] \in O(p^k)$
...
\paragraph{Interpolation de Cauchy} L'interpolation de Cauchy se fait en deux étapes: tout d'abord il faut calculer un polynôme d'interpolation $A$ d'après la donnée de valeurs prises en les points primitifs de $p^k$-torsion. Ensuite on doit reconstruire à l'aide de l'algorithme d'Euclide une fraction rationnelle $\frac{R}{V}$ égale à $A$ modulo le polynôme unitaire qui s'annule sur les abscisses des points de $p^k$ torsion primitifs. 

Dans l'algorithme de Couveignes \cite{Couveignes96} l'interpolation est effectuée distinctement sur les différentes orbites sous l'action du Frobenius puis en utilisant le théorème des restes chinois on obtient alors un polynôme d'interpolation modulo la $p^k$-torsion primitive.

En effet l'ensemble des abscisses des points primitifs de $p^k$-torsion est de taille $p^{k-1}(p-1)/2$. La taille d'une orbite sous l'action du Frobenius est quant à elle égale à $[\mathbb{U}_k:\mathbb{U}_1]$ (ou $\frac{[\mathbb{U}_k:\mathbb{U}_1]}{2}$ voir remarque \ref{rem:orb:fro} ). Ainsi si l'on note $f$ la taille de ces orbites et $d$ le nombre de représentants on a $d \times f=p^{k-1}(p-1)/2$.

Détaillons cela, on a vu dans la proposition \ref{pro:ord:fro} que pour tout $i\geqslant 1$ l'extension $U_{i+1}$, dans laquelle est définie $E[p^{i+1}]$, est soit une extension de degré $p$ de $\mathbb{U}_{i}$ soit égale à $\mathbb{U}_{i}$. Ainsi les points d'ordre $p^{v_p(E(\mathbb{U}_1)}$ de $E$ sont par définition de $v_p(E(\mathbb{U}_1))$ définis sur $\mathbb{U}_1$ (qui n'est pas forcément égal à $\mathbb{F}_q)$, on note $d$ le nombre d'abscisses différentes de ces éléments. Dès lors les points d'ordre $p^{v_p(E(\mathbb{U}_1))+i+1}$ avec $i \geqslant 1$ sont définis comme des antécédents par $[p]$ (la multiplication par $p$) d'un point $P_i$ d'ordre $p^{v_p(E(\mathbb{U}_1))+i}$ et chaque point de cet orbite sous l'action du groupe de Galois $[\mathbb{U}_{v_E(p)+i}:\mathbb{U}_{v_E(p)+i-1}]$ est un antécédent de $P_i$ par $[p]$, il y en a donc $p$. Sur la figure \ref{fig:p-tor:fro} on peut voir une répartition des points de $p^{i+v_p(E(\mathbb{U}_1))}$-torsion selon l'action du Frobenius et les différentes orbites. $d$ le nombre de représentants des orbites est égal aux nombres d'abscisses de points d'ordre $p^{v_p(E(\mathbb{U}_1))}:\frac{p^{v_p(E(\mathbb{U}_1))-1}(p-1)}{2}$ et n'est généralement pas égal à $[\mathbb{U}_1: \mathbb{F}_q]$. En effet $\mathbb{U}_1$ est construit comme une extension de $\mathbb{F}_q$ qui contient les points de $p$-torsion de $E$ et donc $d \mid p-1$ par le théorème de Gunji \ref{thm:Gunji}.
...
\begin{tikzpicture}[scale=0.60,domain=-10/3:4.5,samples=100,yscale=1/2]
          ...
          \draw (U0) node{$\mathbb{F}_{q} \subset \mathbb{U}_{v_p(E(\mathbb{U}_1))}$};
          \draw (U1) node{$\mathbb{U}_{v_p(E(\mathbb{U}_1))+1}$};
          \draw (U3) node{$\mathbb{U}_{v_p(E(\mathbb{U}_1))+i}$};
...
%          \draw [dashed,-] (P27) to (P28);
          ...
          \draw[color=black,decorate,decoration={brace,raise=0.2cm},above]
				(P210) -- (P200) node[color=black,above=0.2cm, pos=0.5] {f};
          \coordinate (P23) at (4,8);
          \coordinate (P24) at (2,8);
          \draw [dashed,-] (P1) to (P23);
          \draw [dashed,-] (P1) to (P24);
...
%          \draw [dashed,-] (P27) to (P28);
          ...
          \draw[color=black,decorate,decoration={brace},below]
				(16,-1) -- (5.5,-1) node[color=black,below,pos=0.5] {d};
        \end{tikzpicture}
...
\begin{rem}
\label{rem:orb:fro}
La taille des orbites des abscisses $f$ selon l'action du groupe de Galois $[\mathbb{U}_k:\mathbb{F}_q]$  est égale à $[\mathbb{U}_k:\mathbb{F}_q]$ si et seulement si $2 \nmid [\mathbb{U}_k:\mathbb{F}_q]$. En effet dans le cas où l'on a $2 \mid [\mathbb{U}_k:\mathbb{F}_q]$ alors si l'on regarde l'action du frobenius sur la $p^k$-torsion comme une multiplication par la trace du Frobenius $t_{\pi}$ modulo $p^k$ alors on a  si l'on note $e=[\mathbb{U}_k:\mathbb{F}_q]$ alors on a $\pi^{e/2}(x(P))=x(-P)=x(P)$ et donc $f \mid [\mathbb{U}_k:\mathbb{F}_q]/2$.
\end{rem} 

\begin{cor}
Soit $k$ tel que que $\mathbb{U}_k \neq \mathbb{F}_q$ avec $\mathbb{U}_k$ défini comme dans \textcolor{red}{ref à mettre}. 
La taille des orbites des abscisses des points d'ordre $p^k$ selon l'action du groupe de Galois $[\mathbb{U}_k:\mathbb{F}_q]$  est égale à:
\begin{enumerate} 
\item $[\mathbb{U}_k:\mathbb{F}_q]$ si $2 \nmid [\mathbb{U}_k:\mathbb{F}_q]$,
\item $[\mathbb{U}_k:\mathbb{F}_q]/2$ si $2 \mid [\mathbb{U}_k:\mathbb{F}_q]$.
\end{enumerate}
\end{cor}

\begin{proof}
Comme vu dans la preuve de la proposition \ref{pro:ord:fro} on peut considérer les points d'ordre $p^k$ commme des vecteurs propres pour le Frobenius associés à $t_{\pi}$. Ainsi lorsque  $2 \mid [\mathbb{U}_k:\mathbb{F}_q]$ on a $(t_{\pi}^{[\mathbb{U}_k:\mathbb{F}_q]/2})^2=t_{\pi}^{[\mathbb{U}_k:\mathbb{F}_q]}=1 \bmod p^k$. Ainsi l'action de $\pi^{[\mathbb{U}_k:\mathbb{F}_q]/2}$ sur la $p^k$ torsion correspond à une multiplication par une racine carrée de $1$, or par définition $[\mathbb{U}_k:\mathbb{F}_q]$ est l'ordre de  $t_{\pi}$ dans $(\mathbb{Z}/p^k \mathbb{Z})^*$ et donc $t_{\pi}^{[\mathbb{U}_k:\mathbb{F}_q]/2}=-1 \bmod \mathbb{Z}/p^k \mathbb{Z}$.
\end{proof}

\begin{rem}
Ce résultat peut aussi être démontré à l'aide du théorème de Gunji \ref{thm:Gunji}, notamment le fait que pour $c$ une racine $p-1$ème de $H_E$ les abscisses des points de $p$-torsion sont définies dans $\mathbb{F}_q[c^2]$.
\end{rem}

Dans l'algorithme original de Couveignes le coût minimal du calcul du polynôme d'interpolation a été sous-estimé. En effet on cherche à calculer un polynôme de degré $\frac{p^{k-1}(p-1)}{2}=\frac{\phi(p^k)}{2}$ à coefficients dans $\mathbb{U}_k$, ainsi ne serait ce que pour représenter ce polynôme on a besoin de $O(p^{2k})$ éléments de $\mathbb{F}_q$ et donc à minima un nombre de $O(p^{2k})=O(\ell^2)$ opérations à effectuer sur $\mathbb{F}_q$. De plus il faut calculer tous les points primitifs de $p^k$-torsion et donc effectuer $p^k$ additions de points de courbes elliptiques définis sur une extension de taille $[\mathbb{U}_k:\mathbb{F}_q] \in O(p^k) $, ce pré-calcul a un coût de $O(p^k\mathsf{M}(p^k)\log(p^k))$. Il faut donc utiliser des méthodes supplémentaires pour optimiser ce temps de calcul. Les travaux de \cite{DeFeo11} ont donc résolu cette question et seront présentés à \textcolor{red}{ref à mettre}.
...
\paragraph{Validation de l'isogénie} On doit ensuite tester si la fraction rationnelle $\frac{R}{V}$ définit bien une isogénie. Pour cela on doit tout d'abord regarder les degrés et vérifier que l'on a $\deg(R)=\ell$ et $\deg(V)=\ell-1$, si cela n'est pas vérifié alors ce n'est pas l'isogénie que l'on cherche à calculer et on peut passer à un nouvel essai. Ensuite on peut vérifier que $V$ est bien un carré (ou si $\ell$ est pair que c'est le produit d'un facteur du polynôme de $2$-division et d'un carré). Ces deux tests suffisent généralement à conclure et ils ont un coût de $O(\mathsf{M}(\ell) \log(\ell))$ opérations sur $\mathbb{F}_q$. Pour plus de sûreté on peut éventuellement évaluer la fonction sur différents points et vérifier que c'est bien un morphisme de groupe. Enfin si une preuve déterministique est souhaitée on peut vérifier que $V$ est bien un diviseur du polynôme de $\ell$-division, pour cela on calcule le polynôme de $\ell$ division en $O(\mathsf{M}(\ell)\log(\ell))$ opérations sur $\mathbb{F}_q$. 

Le cas $p=2$ n'est pas traité ici, le lecteur intéressé pourra se référer à \cite[8.5.2]{DeFeo10}

\subsubsection{Améliorations de Couveignes: Calcul de la $p^k$-torsion}
...
Pour le calcul de la $p^k$-torsion \cite{DeFeo11} utilise les calculs consécutifs aux travaux de \cite{Gunji76} et \cite{Voloch88} sur des tours d'Artin Schreier (tours $p$-adiques).

Faisons une brève présentation de celles-ci, pour plus de détails voir \cite{DeFeo-Shost'12}.
...
\begin{enumerate}
\item $[\mathbb{F_q} :\mathbb{U}_0]= d$
\item $[\mathbb{U}_{i+1} :\mathbb{U}_i]= p$ pour tout $i \geqslant 1$.
\end{enumerate}
\end{defi}

Une représentation efficace d'une tour $p$-adique est une tour dite \emph{primitive} dans laquelle tout corps de la tour est généré par un élément de celle-ci. Ainsi la multiplication de deux éléments de la tour peut être fait à l'aide de la multiplication de polynômes univariés à l'opposé de représentations multivariées qui ne sont pas efficaces.

On présente donc à cet effet comment est construite une telle tour:
\begin{thm}
Soit $\mathbb{U}_0=\mathbb{F}_p[X_0]/Q_0$, avec $Q_0$ un polynôme irréductible de degré $d$, soit alors $x_0=X_0 \bmod Q_0$ tel que $Tr_{\mathbb{U}_0/\mathbb{F}_p}(x_0)\neq 0$. On définit la suite de polynômes $G_i$ avec $ 1 \leqslant i \leqslant k$ par:
\begin{equation}
blabla
\end{equation}
\end{thm} 
...
\paragraph{$p$-torsion}
La construction de $\mathbb{F}_q[c]$ est faite comme une extension de $\mathbb{F}_p:\mathbb{F}_p[x]/Q_1$ avec $Q_1$ un polynôme irréductible et $Q_1$ qui vérifie l'une des deux conditions suivantes:
\begin{itemize}
\item $d \wedge p =1$
\item $\deg Q'_1+2 = \deg Q_1$
\end{itemize}
Généralement il est suffisant de sélectionner un polynôme $Q_1$ au hasard et de tester si il est irréductible pour avoir un polynôme qui vérifie une de ces conditions. Cela a un coût de $O(pd \mathsf{M}(pd)\log(pd)\log(p^2d))$ opérations sur $\mathbb{F}_p$ en utilisant l'algorithme de Ben-Or d'après \cite[Theoreme 14.42]{vzGJG03}.

Ainsi les complexités vont être exprimées en termes d'opérations sur $\mathbb{F}_p$. Avec une telle construction la complexité de calculer la $p$-torsion est de: 
\begin{equation*}
\tilde{O}(p^2d^3+pC(pd)+C(p)pd+(pd)^{\omega})
\end{equation*}
pour plus de détails sur le calcul de complexité voire \cite{DeFeo11}, la démarche étant la même que celle décrite dans la sous section précédente. 

Rappeler quelque part que $q=p^d$

\paragraph{$p^k$-torsion}
Pour le calcul de la $p^k$-torsion comme dit précédemment cela est effectué itérativement en résolvant successivement l'équation \eqref{eq:vol:cov} puis \eqref{eq:vol:iso}.
...
Il est à noter que dans ce cas-là on a plus besoin de stocker une matrice de taille $p^{k-1}d \times p^{k-1}d$ pour résoudre l'équation d'Artin Schreier \eqref{eq:vol:cov}, ainsi il n'y a plus besoin de stocker en mémoire $O(\ell^2)$ données.

\paragraph{Interpolation}

L'apport de l'utilisation de tour d'Artin Schreier \cite{DeFeo-Shost'12} est minime sur cette étape qui ne change que très peu de l'original. On commence donc par  calculer un polynôme d'interpolation de degré $p^{k-1}(p-1)-1$ à coeffcients dans $\mathbb{U}_k$ on utilise alors un push-down \cite[push-down]{DeFeo-Shost'12} pour avoir un polynôme de $\mathbb{F}_q[x]$ et ensuite utiliser une reconstruction rationnelle.

La première étape a un coût de $O(\mathsf{M}(p^{2k}d)\log(p^{k}))$ en utilisant les techniques de \cite[§10.2]{vzGJG03}. Ensuite on utilise une descente pour avoir une expression dans $\mathbb{U}_1[X]=\mathbb{F}_q[c][X]$, enfin à l'aide d'algèbre linéaire on exprime le polynôme sur $\mathbb{F}_q[X]$ en $O((pd)^2)$ opérations sur $\mathbb{F}_p$. La reconstruction rationnelle coûte $O(\mathsf{M}(p^kd)\log(p^k))$ d'après \cite[§11.1]{vzGJG03}, on a donc un coût total de:

\begin{equation*}
O(\mathsf{M}(\ell^2d)\log \ell + \ell L(k-1) + (pd)^2)
\end{equation*}

avec $L$ qui représente le coût d'effectuer un relèvement ou une descente dans une tour d'Artin Schreier. D'après \cite[Theorem 13]{DeFeo-Shost'12} $L(i) \in O(p^{i+1}d \log^2_p(p^id)+p \mathsf{M}(p^id))$, il est à noter que ce coût est linéaire en la taille de l'entrée (élément de $\mathbb{U}_i$). 

Cette étape, la plus coûteuse de l'algorithme de Couveignes, a à être répétée $O(p^k)=O(\ell)$ fois (à chaque fois on change l'hypothèse faite sur l'image du point générateur de la $p^k$-torsion) pour espérer trouver la bonne hypothèse et donc calculer la $\ell$-isogénie.
...
Cette partie est cruciale car comme on l'a vu précédemment la partie la plus coûteuse de l'algorithme de Couvegines reste l'interpolation de Cauchy et en particulier le calcul du polynôme d'interpolation. On peut déjà voir qu'interpoler $p^k$ paires de points $(P,I(P))$ avec $P$ et $I(P)$ à coordonnées dans $\mathbb{U}_k$ entraîne un cout minimal de $O(p^k p^kd)$. Bien que le polynôme que l'on souhaite calculer est de degré $p^k$ il est possible avec moins d'informations de calculer celui-ci. En effet comme préconisé par Couveignes \cite{Couveignes96} si on travaille avec des représentants d'orbite sous l'action du Frobenius alors on peut retrouver le polynôme d'interpolation en utilisant le théorème des restes chinois. Il manquait alors à l'époque des techniques efficaces pour justement faire agir le Frobenius sur ces représentants, ce que les constructions de tours d'Artin Schreier de \cite{DeFeo-Shost'12} permettent désormais. 

On va donc voir dans cette section comment à partir d'un polynôme d'interpolation défini uniquement à l'aide de représentants de l'orbite selon l'action du Frobenius on retrouve le polynôme d'interpolation. Cette raison est d'ailleurs l'une de celles pour laquelle on travaille uniquement avec des points primitifs de $p^k$-torsion puisque que eux seuls sont définis dans $\mathbb{U}_k$ par définition de $\mathbb{U}_k$. Bien évidemment on peut généraliser cette méthode à tous les points de $p^k$-torsion en utilisant le théorème des restes chinois, mais d'un point de vue pratique prendre $k$ tel que $p^{k-1}(p-1) > 4 \ell +1$ est suffisant, asymptotiquement le résultat reste le même.

Soient $P$ et $P'$ deux points primitifs de $p^k$-torsion de $E$ et $E'$. On veut donc calculer le polynôme d'interpolation $A \in \mathbb{F}_q[X]$ tel que:
\begin{equation}
A(x([n]P))=x([n]P') \bmod T(x) \quad \text{pour } n \in \left( \mathbb{Z}/p^k\mathbb{Z} \right)^*
\end{equation} 

avec $T(X)= \prod_{n \in \left( \mathbb{Z}/p^k\mathbb{Z} \right)^*} (X-x([n]P))$


Comme dit précédemment comme on cherche à calculer une fraction rationnelle définie sur $\mathbb{F}_q$, ansi le polynôme d'interpolation que l'on veut calculer est défini sur $\mathbb{F}_q$. 

On commence donc par le théorème des restes chinois pour ensuite se concentrer sur le calcul d'un seul polynôme d'interpolation. Comme préconisé dans \cite{Couveignes96} on regarde la factorisation de $T(X)$ sur $\mathbb{F}_q$:

\begin{equation*}
T(X)=\prod_{i=1}^dT^{(i)}(X)
\end{equation*}
avec $\deg(T_i)=f=[\mathbb{U}_k:\mathbb{F}_q]$ et $df=p^{k-1}(p-1)/2$.

Ainsi on travaille modulo chacun de ces $T_i$ et l'on définit:

\begin{equation}
A^{(i)}=A \bmod{T^{(i)}}.
\end{equation}

Avec l'utilisation du Théorème des restes chinois une fois chacun des $A^{(i)}$ déterminés on peut calculer $A$, dés lors on va se concentrer sur le calcul de $A^{(0)}$.

Comme dit précédemment on va prendre avantage du fait du faible coût du Frobenius dans une tour d'Artin Schreier, en effet cela a un faible coût à l'aide de l'algorithme \cite[Iter Frobenius,Theorem 17 et 18]{DeFeo-Shost'12}

Ainsi on fait l'hypothèse que l'abscisse du point $P$ générateur de la $p^k$-torsion de $E$ est envoyé sur l'abscisse du point $P'$ générateur de la $p^k$-torsion de $E'$. Dés lors on veut calculer 
\begin{enumerate}
\item le polynôme minimal $T$ de $x(P)$ défini sur $\mathbb{F}_q$ de degré strictement inférieur à $d$.
\item le polynôme d'interpolation $L$ défini sur $\mathbb{F}_q$ de degré strictement inférieur à $d$ tel que $L(x(P))=x(P')$.
\end{enumerate}

Ainsi avec un tel polynôme $L$ défini sur $\mathbb{F}_q$ on aura pour tout $\sigma \in \mathsf{Gal}(\mathbb{U}_k:\mathbb{F}_q)$:
\begin{equation*}
L(\sigma(x(P))=\sigma(L(x(P)))=\sigma(x(P'))
\end{equation*}
ainsi l'information de l'image d'un élément de $\mathbb{U}_k$ entraîne la connaissance des images des points de son orbite sous l'action de n'importe quel élément de $\mathsf{Gal}(\mathbb{U}_k:\mathbb{F}_q)$. On va donc se servir de l'action du Frobenius sur le polynôme d'interpolation défini uniquement pour un représentant de l'orbite (en l'occurence $L(x(P))=x(P')$) pour calculer successivement à l'aide d'un arbre de produit binaire le polynôme d'interpolation défini sur $\mathbb{F}_q$.

Parlons tout d'abord de l'arbre de produit binaire, cette idée a donc été utilisé dans \cite{DeFeo11}, mais avant été utilisé auparavant dans \cite{enge2008}.

Pour $P$ un point primitif de $p^k$ torsion on a $x(P) \in \mathbb{U}_k \setminus \mathbb{U}_{k-1}$ par définition de $\mathbb{U}_k$. On a alors pour tout $n \in \left( \mathbb{Z}/p^k\mathbb{Z} \right)^*$ $x([n]P) \in \mathsf{orb}(x(P))$. On a donc:
\begin{equation*}
T(x)=\prod_{n \in }(x-x([n]P))
\end{equation*}

\paragraph{Comportement de la $p^k$-torsion}
Soient $p^{i_0}$ tel que $E[p^{i_0}] \subset E[\mathbb{F}_q]$

 et calcule alors à l'aide d'une reconstruction rationnelle  ainsi pour tout générateur de la $p^k$-torsion il faudra tester jusqu'à $O(p^k)$ correspondances à travers les choix des générateurs de la $p^k$-torsion. 
Ainsi l'algorithme de Couveignes va utiliser cette information pour interpoler l'isogénie à l'aide des points de $p^k$-torsion.

L'algorithme de Couveignes est un algorithme qui calcule une isogénie en utilisant la remarque suivante. Soit $P$ un point de $p^k$-torsion alors son
