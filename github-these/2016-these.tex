...
l'action du Frobenius sur $\mathsf{T}_{\ell}(E)$. Le résultat suivant nous donne un 
moyen de savoir comment monter et descendre à l'intérieur d'un volcan de 
$\ell$-isogénies en étudiant l'action du Frobenius sur les points de la courbe 
qui génèrent leur noyau. C'est d'ailleurs l'approche que l'on aura tout au long
de cette section, pour déterminer les directions des isogénies on étudiera 
l'action du Frobenius sur un point générateur de leur noyau.
...
la preuve de la proposition~\ref{pro:tri:dir}. 
\todo{fixer le problème de profondeur et de hauteur}

Maintenant nous allons voir comment on peut distinguer les $\ell$-isogénies 
descendantes entre elles. Vu que l'on ne peut comme dans le cas Elkies distinguer 
deux sous-groupes cycliques de la $E[\ell^{h+i}]$ pour $i>0$, on va voir quelles
informations on peut tirer lorsque l'on fixe arbitrairement un point de la base
de la $E[\ell^{h+i}]$-torsion.
...
les deux points $R$ et $R'$ engendrent la même $\ell^i$-isogénie descendante de
noyau $\langle [\ell^h]R' \rangle = \langle [\ell^h]R \rangle$.
...
Ainsi à l'aide du corollaire~\ref{cor:mon:atk} on sait monter dans le volcan de 
$\ell$-isogénies pour réduire le problème de déterminations d'isogénies à celui
énoncé sur le cratère. 
Cependant si on travaille à partir d'une courbe située au niveau $h-e$ pour 
étudier la direction des $\ell^i$-isogénies on aura à travailler
avec une base de la $\ell^{h+i+e}$-torsion sur le cratère, ce qui comparé à la  
remarque~\ref{rem:atk:gen} est le mieux que l'on puisse espérer.

Dés lors on va se concentrer à l'étude des points sur une courbe qui se 
situe sur le cratère et montrer un résultat qui permet de distinguer les 
différents générateurs d'une même $\ell^i$-isogénie.
