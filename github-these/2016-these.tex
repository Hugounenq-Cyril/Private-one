...
\end{figure}
On peut donc voir sur le dessin \ref{fig:vol:3t} les 3 différents formes de volcan qui existent, celles-ci sont distinguées par la forme de leur cratère qui est déterminée par la valeur de $\left( \frac{d_K}{\ell} \right)$.
...
On va dans cette section mettre en avant les liens qui existent entre le niveau d'une courbe dans un volcan de $\ell$-isogénie et la structure de sa $\ell$-torsion. Les résultats exposés ici sont tirés de \cite{MiretMSTV08} et \cite{Ionica-Joux10}.

\begin{prop}[Lenstra]
\label{pro:len:str}
Soit $E$ une courbe elliptique ordinaire définie sur $\mathbb{F}_q$, $\pi$ l' endomorphisme de frobenius alors si $\pi \not \in \mathbb{Z}$ alors pour les extension de corps de la forme $\mathbb{F}_{q^r}$ $\mathrm{End}_{\mathbb{F}_{q^r}}(E)$ est un $\mathbb{Z}$-module de rang $2$ et il y a l'isomorphisme de $\mathbb{Z}$-module suivant:
\[
...
\begin{proof}
Cette preuve est issue de \cite{Ionica2010} elle est très importante pour comprendre la liaison entre la structure de la $\ell$-torsion d'une courbe et son niveau dans le volcan.
Tout d'abord on peut remarquer que deux courbes situées sur un même volcan de $\ell$-isogénies ont la même cardinalité. Par la proposition \ref{pro:len:str} la structure de la torsion rationnelle est alors de la forme :
\[
E(\mathbb{F}_{q}) \cong \frac{\mathrm{End}_{\mathbb{F}_{q}}(E)}{\pi-1}
\] 
On rappelle l'écriture $\pi=a + g \omega_K$, pour le plus grand $N$ tel que $E[N] \subset E(\mathbb{F}_q)$ on a par \cite[Lemma 1]{Ruck1987} $N=\mathrm{pgcd}(a-1,g/f)$ avec $f$ le conducteur de $\mathrm{End}(E)$.
\end{proof}

On a besoin du résultat intermédiare suivant:
...
\begin{prop}
Soit $E$ une courbe elliptique définie sur $\mathbb{F}_q$ telle que $E(\mathbb{F}_q)[\ell^{\infty}]=\mathbb{Z}/n_1\mathbb{Z} \times \mathbb{Z}/n_2\mathbb{Z}$, $\pi = a + g \omega_K$ l'endomorphisme de Frobenius alors si:
\begin{itemize}
...
Dés lors quand on descend d'un niveau dans le volcan la valuation $\ell$-adique de $f$ augmente de $1$ et par conséquent $v_{ell}(n_2)=v_{\ell}(g/f)$ diminue de $1$ aussi, en particulier cela implique que la valuation $\ell$-adique de $n_2$ pour une courbe sur le cratère est nulle. Ainsi on voit bien la bijection entre la valuation $\ell$-adique de $n_2$ et le niveau de la courbe dans le volcan.

\item On se place désormais dans le cas où l'on a $v_{\ell}(n_1)=v_{\ell}(n_2)$ (et par conséquent $2 \mid v_{\ell}(|E(\mathbb{F}_q)|)$) alors dans ce cas la structure de la $\ell$ torsion ne change pas du cratère jusqu'à un niveau où l'on tombe dans le premier cas, ainsi $E$ se trouve à un niveau supérieur à $v_{\ell}(|E(\mathbb{F}_q)|)/2$.
\end{itemize}
\end{proof}
...
\begin{rem}
On a appelé le niveau de stabilité premier niveau de stabilité car le second introduit et développé dans \cite{Ionica-Joux10} correspond lui à un niveau à partir duquel le pairing ne permet plus de déterminer des isogénies montantes sur le volcan de $\ell$-isogénies.  
\end{rem}

\begin{prop} 
\label{pro:iso:mon}
Soit $E$ une courbe elliptique définie sur $\mathbb{F}_q$ telle que l'on ait sa $l$-torsion infinie de la forme: $E(\mathbb{F}_q)[l^{\infty}]=\nicefrac{\mathbb{Z}}{l^{r}\mathbb{Z}} \times \nicefrac{\mathbb{Z}}{l^{s}\mathbb{Z}}= \langle P,Q \rangle $ avec $r>s +1 \geqslant  1$ et $P$ un point d'ordre $n_1$. Alors on a:
\begin{enumerate}
\item La $l$-isogénie engendrée par $l^{r-1}P$ est une isogénie montante qui mène à une courbe elliptique $E'$ ayant comme structure de $l$-torsion: \[E'(\mathbb{F}_q)[l^{\infty}]=\nicefrac{\mathbb{Z}}{l^{r-1}\mathbb{Z}} \times \nicefrac{\mathbb{Z}}{l^{s+1}\mathbb{Z}} \]
\item Les autres $l$-isogénies sont descendantes
\end{enumerate}
\end{prop}
...
Par le corollaire \ref{cor:tab:vol} on sait que l'on a $l+1$ $l$-isogénies et que si l'on a une $l$ isogénie montante alors les  autres sont descendantes.
\\
Considérons l'isogénie $\phi$ engendrée par $l^{r-1}P$. L'image de $P:\widetilde{P}$ par l'isogénie $\phi$ est alors d'ordre $l^{r-1}$. 

Montrons par l'absurde que l'on n'a pas $\nicefrac{\mathbb{Z}}{l^{r}\mathbb{Z}} \subset \phi(E)(\mathbb{F}_q)[l^{\infty}]$. Supposons qu'il existe un point $\widetilde{R}$ d'ordre $l^n$ avec $n \geqslant r$. Cela implique donc que l'on ait l'existence de $R \in E(\mathbb{F}_q)$ tel que $R$ soit d'ordre $l^n$ et $R \notin \langle P \rangle$ ou  $l^nR =l^{r-1}P$. La deuxième hypothèse est impossible car cela implique qu'il existe $S \in E(\mathbb{F}_q)$ tel que $l S=P$ et contredirait donc l'hypothèse de la structure de $E(\mathbb{F}_q)[l^{\infty}]$. De même la seconde hypothèse: $l^nR = \mathcal{O}$ contredit elle aussi la structure de $E(\mathbb{F}_q)[l^{\infty}]$. On a donc:
\[\phi(E)(\mathbb{F}_q)[l^{\infty}]=\nicefrac{\mathbb{Z}}{l^{r-1}\mathbb{Z}} \times \nicefrac{\mathbb{Z}}{l^{s+1}\mathbb{Z}} \].
...
Soit une courbe elliptique $E$ définie sur $\mathbb{F}_q$ telle que l'on ait $E(\mathbb{F}_{q})[l^{\infty}]=\mathbb{Z} / l^{r} \mathbb{Z} \times \mathbb{Z} / l^{s} \mathbb{Z}$, alors on a $E(\mathbb{F}_{q^l})[l^{\infty}]=\mathbb{Z} / l^{r+1} \mathbb{Z} \times \mathbb{Z} / l^{s+1} \mathbb{Z}$.
\end{cor}

\begin{proof}
%A l'aide des propositions \ref{desc-io} et \ref{ext-volc} le résultat est démontré dans \cite{IonicaJ10}
On a vu dans la proposition  que lorsque l'on prenait une extension de degré $\ell$ alors on augmentait la hauteur du volcan que de $1$, de plus en regardant la courbe $E_b$ située en bas du volcan défini sur $\mathbb{F}_q$ celle-ci a pour structure $\mathbb{Z} / l^{r+s} \mathbb{Z}$ par la proposition . Ainsi si l'on regarde $E_b$ définie sur $\mathbb{F}_{q^{\ell}}$  
\end{proof}
