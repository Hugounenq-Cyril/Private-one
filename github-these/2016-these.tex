...
L'objectif est de calculer une extension du corps fini $\mathbb{F}_q$, avec $q \wedge \ell =1$, de taille $\mathbb{F}_{q^n}$ avec $n=\ell^k$ et $k \geqslant 1$. 
Comme vu dans \cite[Theorem VI.9.1]{lang2002algebra} et mis en pratique dans \cite{Shoup93} pour calculer une extension quadratique d'un corps fini $\mathbb{F}_q$ on a deux cas à considérer: 
\begin{itemize}
\item pour $\ell \neq 2$:
\begin{itemize}
\item[$q=1 \bmod \ell$] alors tout résidu non $\ell$-adique $\alpha$ de $\mathbb{F}_q$ permet de construire le polynôme irréductible $X^n-\alpha$, duquel on déduit alors une construction de $\mathsf{F}_{n}$,
\item[$q \neq 1 \bmod \ell$] on se ramène au cas $q=1\bmod \ell$ en construisant une extension de degré $d_1=o_m(q)$ $\mathsf{F}_{0}$ de $\mathbb{F}_q$. Avec $o_m$ l'ordre multiplicatif de $q$ dans $\mathbb{Z}/\ell \mathbb{Z}$, cette construction sera explicitée dans la suite du document.
\end{itemize}
\item  pour $\ell=2$:
\begin{itemize}
...
\end{itemize} 
\end{itemize}
Ainsi pour construire une extension de degré $\ell$ d'un corps fini $\mathsf{F}_0$ un résidu non $\ell$-adique $\alpha$ défini sur $\mathsf{F}_0$ est nécessaire. On trouve celui-ci après $O(1)$ essais en prenant des éléments au hasard par \cite[Lemma 15]{Shoup93}, chaque essai a un coût de  on ne va donc pas considérer le coût de calcul de celui-ci. 
