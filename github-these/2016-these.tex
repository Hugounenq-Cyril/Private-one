...
\begin{defi}
\label{def:cas:atk}
Soit $E$ une courbe elliptique définie sur $\mathbb{F}_q$, $\ell$ est dit un nombre premier de Atkin si le polynôme caractéristique du Frobenius: $\pi^2-t_{\pi}\pi + q$ est irréductible dans $\mathbb{Z}_{\ell}$
\end{defi}
\todo{Faire explicitement le lien avec le chapitre 4}
Ce cas-là est donc appelé cas Atkin  si l'on a un tel type de volcan de $\ell$-isogénies. On
va donc étudier dans cette partie l'action du Frobenius sur 
$\mathsf{T}_{\ell}(E)$ $E[\ell^{\infty}]$ et voir si l'on peut déterminer des
directions d'isogénies.
