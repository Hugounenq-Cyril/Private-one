...
décisionnaires de cette bourse. Ensuite il y a eu une quatrième année qui a été 
...
copies à corriger durant ma quatrième année) ainsi que Nicolas, Johan et Valentin. 
...
supporté durant cette quatrième 
...
que Kohel a adapté et développé dans ce contexte; Kohel a aussi étudié le cas des 
...
\`A l'aide des méthodes de \cite{SchonhageStrassen71} et de leurs généralisations  \cite{Schonhage77} \cite{Cantor-Kaltofen91} utilisant des transformées de Fourier rapides, la multiplication de polynômes de degré $n$ appartenant à $R[x]$ pour un anneau $R$ quelconque est de \newline ${O(n\log(n) \log \log (n))}$ opérations arithmétiques dans $R$.
...
du Frobenius, il est naturel d'étudier la détermination de ses valeurs propres.
Ainsi en analysant l'équation de caractéristique de $\pi$ sur la 
$\ell$-torsion:
\begin{equation*}
...
abscisses de points d’ordre $p$ à l'aide d'éléments définis dans une extension 
...
Soit $K$ un corps de nombres quadratique, alors il existe un entier $N$ sans facteur carré tel que l'on peut écrire $K=\mathbb{Q}[\sqrt{N}]$. On définit alors alors le discriminant de $K$:
\begin{defi}
\label{def:dis:dK}
...
à un coût espéré de $O(\ell\mathsf{M}(\ell^2)\log(\ell)\log(\ell q))$
...
Les deux opérations les plus coûteuses des boucles sont les
...
\section{Algorithme de Couveignes $\ell$-adique dans le cas Elkies avec un point horizontal}
...
l’algorithme~\ref{alg:TRC:init:gen} qui prend en entrée un point $P_{i}$ d'ordre
