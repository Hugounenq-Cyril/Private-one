...
$\mathbb{Z}_{\ell}$ alors la matrice $\pi|T_{\ell}(E)$ est trigonalisable. \todo{justifier cela Morris Newman Integral Matrices III.\color{red}{15}}
...
\end{lem}
\todo{harmoniser les notations pour la matrice de $\pi$ mettre $\Pi$ pour la matrice}
...
\end{tikzpicture}
\end{center}
\caption{ \label{fig:poi:dia} Exemples de  différents points diagonaux $P \in E_s$ d'ordre $2^3$ tels que $\pi(P)=[\color{blue}{\mu}\color{black}]P$, ici sont représentées les différentes isogénies de noyau $\langle P \rangle$}
\end{figure}
...
\todo{faire un dessin pour cette proposition qui répond à l'autre dessin}
...
\end{tikzpicture}
\end{center}
\caption{ \label{fig:poi:hor} Exemple de construction de point horizontal. Sur le volcan de gauche le point $P \in E_s$ est représentée l'isogénie de noyau $\langle P \rangle$ tel que $[2^2]P$ est horizontal de direction $\color{blue}{\mu}$ et $\psi$ est une $2$-isogénie horizontale de direction  $\color{red}{\lambda}$. Sur le volcan de droite l'isogénie de noyau $\langle \psi(P) \rangle$ est représentée}
\end{figure}

On présente donc sur la figure~\ref{fig:poi:hor} un exemple de construction de 
point horizontal, on a donc repris pour celui-ci le premier exemple de point 
...
\rightarrow E'_s$ de direction $\mu$ différente de celle de $[2^2]P$, pour trouver
une telle i
...
\end{tikzpicture}
\end{center}
\caption{ \label{fig:par:hor:con} Construction d'un point partiellement horizontal à partir d'un point $P \in E_s$ d'ordre $2^3$ tel que $[2]P$ soit horizontal de direction $\color{blue}{\mu}$ et $\psi:E_s \rightarrow E$ une $2$-isogénie descendante. L'isogénie de noyau $\langle P \rangle$ est représentée en bleu sur le dessin de gauche, celle de noyau $\langle \psi(P) \rangle$ est représentée sur le dessin de droite.}
\end{figure}
...
Frobenius. On se place dans le cas où la courbe elliptique ordinaire $E$ admet 
un anneau des endomorphismes localement maximal par rapport à $\ell$.
...
J\'erome Pl\^ut et \'Eric Schost qui ont fait l'objet d'une publication 
...
(partiellement) horizontales par la proposition~\ref{pro:dia:hor}.
 ...
 proposition~\ref{pro:par:iso} et la proposition~\ref{pro:par:hor:par} les points de
...
\]
En effet afin d'avoir une présentation uniforme avec les points horizontaux et 
les points partiellement horizontaux on choisit de travailler avec les points 
d'ordre $\ell^k$(avec $k>h$), car avec deux points partiellement horizontaux 
d'ordre $\ell^k$ on peut tous les générer au contraire des points de la 
$\ell^{e}$-torsion ($e$ représentant la profondeur de la courbe). En effet on 
ne peut pas générer tous les point de $\ell^e$-torsion et donc pour éviter 
toute redondance on ne travaille qu'avec les points d'ordre $\ell^k$. Si l'on 
souhaitait travailler avec les points de la $\ell^e$-torsion on utiliserait 
dans le cas de deux points partiellements horizontaux $R_{\lambda}, R_{\mu}$ 
(de profondeur $e>0$)par exemple le groupe $\langle [\ell^{k-e}]R_{\lambda} 
\rangle$. 
