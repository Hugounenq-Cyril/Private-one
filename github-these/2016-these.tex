...
primordiaux\todo{?}, afin par exemple de définir les paramètres des cryptosystèmes. 
...
et Elkies \cite{Elkies91} \todo{check date} aboutissant à l'algorithme SEA \cite{Schoof95}.

Le calcul d'isogénies, morphismes de groupes de noyau fini entre deux courbes
elliptiques, apparaît dans SEA et a donc un intérêt central.\todo{scinder?} En effet deux 
courbes elliptiques reliées par une isogénie ont la même cardinalité \todo{Déplacer ?}. Les courbes elliptiques sont représentées, entre autres, à l'aide de leur 
...
complexité en le degré de l'isogénie que l'algorithme de Lercier et Sirvent\cite~{Lercier-Sirvent2008}, il n'est pas utilisé en pratique à 
...
Sirvent \cite~{Lercier-Sirvent2008}, avec une dépendance logarithmique en $p$, 
est préféré pour les plus grandes caractéristiques. 

En pratique déterminer lequel des deux est le plus pertinent doit se faire au cas par cas.

Ces deux algorithmes, 
n'étant pas limités par la caractéristique, on ne sait pas exactement 
quantifier la taille de la caractéristique, dans le cas moyen, rendant 
pertinente l'utilisation de l'algorithme de Lercier et 
Sirvent~\cite{Lercier-Sirvent2008} à la place de celui de 
Couveignes~\cite{Couveignes96}. 

Bien que la compléxité de SEA ne soit pas dominée par le calcul de l'isogénie explicite/Soulignons que, notons que/De plus comme dans SEA d'autres opérations ont 
...
la construction de fonctions de hachage \cite{CGL09}, l'accélération de la 
...
par l'action de l'isogénie que nous voulons calculer, tout en nous servant 
...
reliées par des arêtes symbolisant les isogénies d'un degré fixé. En 
particulier\todo{changer ce n'est plus un rappel}, toutes les courbes d'un même graphe d'isogénies ont la même 
cardinalité.
\begin{figure}
\begin{center}
...
Les graphes d'isogénies ont été étudiés pour la première fois par \todo{citer Piezer} 
\cite{Kohel96} afin de calculer l'anneau des endomorphismes d'une courbe 
...
Kohel a ainsi cherché à déterminer le niveau d'une courbe dans le graphe tout 
en connaissant la cardinalité de celle-ci. \todo{relier ça aux anneaux d'endomorphismes}
Fouquet et Morain \cite{FouquetMorain02} ont par la suite résolu le même 
problème, sans la connaissance préalable de la cardinalité de la courbe, afin d'améliorer
SEA, et ont nommé les graphes d'isogénies \emph{volcans d'isogénies}. Enfin les 
travaux de Miret \textit{et al}. \cite{MiretMRV05}  \cite{MiretMSTV06} \todo{Verifier que les 3 citations sont pertinentes}
...
sous certaines conditions. Ces travaux ont donc été faits dans le but de 
déterminer si une isogénie était\todo{pas d'imparfait, introduire direction} montante, descendante ou horizontale pour 
améliorer celui de Fouquet et Morain \cite{FouquetMorain02}, et donc SEA.

Concernant la construction de tours d'extensions, nos travaux se placent dans 
la suite de ceux de Doliskani et Schost \cite{Doliskani-Schost15}, De Feo, Doliskani et Schost 
\cite{DeFeo-Doliskani-Schost13},
...
Pour arriver à ce résultat nous avons développé de nouvelles méthodes pour définir et déterminer des directions dans les volcans de 
$\ell$-isogénies.
...
De Feo, Doliskani et Schost  Doliskani et 
Schost \cite{Doliskani-Schost15}, \cite{DeFeo-Doliskani-Schost13}, et nous présentons les tours d'extensions 
...
\cite{FouquetMorain02}, Miret \textit{et al.} \cite{MiretMRV05} \cite{MiretMSTV06} 
\cite{MiretMSTV08}, et Ionica et Joux \cite{Ionica-Joux10}. 

\todo{Dire que les chapitres 5 et 6 sont les principaux apports de la thèse}
...
On ne donne pas une preuve de cette estimation juste quelques pistes. Le 
...
opérations sur $\mathbb{F}_q$, on doit répéter celle-ci $p^{k-1} \in O(r)$ fois 
ainsi on obtient ce terme dominant.
...
elliptiques $E_a$ et $E_b$ $r$-isogènes telles que $j(E_a)=j_0$ et 
$j(E_b)=j_1$, alors le problème du calcul explicite d'isogénie est de calculer 
les courbes $E$, $E'$ telles que $j(E)=j_0$ et $j(E')=j_1$, et la fonction 
rationnelle qui définit la $r$-isogénie $\phi:E \rightarrow E'$.
...
%$\mathbb{F}_q$ 
nous avons une complexité cubique en $r$ supérieure à notre résultat.
...
\cite{DeFeo11} car nous utilisons des méthodes similaires. Nous voyons donc que 
comparé aux résultats des théorèmes~\ref{thm:C2:FP} et \ref{thm:C2:FQ} on 
obtient une complexité qui est aussi quasi-quadratique en le degré de 
l'isogénie et pour laquelle on s'est affranchie de toute dépendance polynomiale
...
parties qui sont en fait du précalcul avec notamment le calcul de bases 
...
Nous rappellons que nous travaillons avec deux courbes $r$-isogènes, avec $r 
...
Maintenant nons faisons une description de la méthode employée dans notre 
algorithme~\ref{alg:cou:atk} pour résoudre le problème du calcul explicite 
d'isogénie.
Soient $E$, $E'$ deux courbes elliptiques de $j$-invariants $j_0$ et $j_1$ 
reliées par une $r$-isogénie $\phi$, $P \in E$ et $S' \in E'$ deux points 
d'ordre $\ell^{k}$, avec $k$  plus grand que $h$, tels que $\phi(P)=S'$. Pour 
deux bases $(P,Q),(S',R')$ de $E[\ell^k]$ et $E'[\ell^k]$ telles que $\pi(P,Q)=
\pi(S',R')$ nous déterminons de façon unique $\phi([\ell^h]Q)=[\ell^h]R'$ par 
le corollaire~\ref{cor:atk:ess}. Nous déterminons ensuite le point 
...
une $r$-isogénie. Ce test consiste comme dans le cas Elkies (voir 
sous-section~\ref{ssec:cou:elk}) à calculer le polynôme d'interpolation 
$A_{S',U'}$ tel que 
\[
...
choisi $U'$ doit représenter l'action de la $r$-isogénie $\phi$. Nous cherchons 
...
\begin{proof}
Nous faison remarquer que la condition $\ell^h < \sqrt{r}$ signifie que $k>h$, 
...
dépendance polynomiale en la caractéristique. En particulier, nous avons présenté
 un algorithme qui 
résout le problème de l'isogénie explicite avec une complexité quasi-quadratique en le degré de l'isogénie, et ce uniquement dans le cas Elkies. Dans le 
cas Atkin nouus avons seulement obtenu un algorithme de complexité sous-cubique. Cet algorithme a une complexité trop grande \todo{moins bonne} par rapport à l'algorithme
de Couveignes \cite{Couveignes96}, mais reste meilleur que le coût de calcul 
du $\ell$-ème polynôme modulaire (cubique d'après \cite{BLS12}). \todo{citer plutot LS08}
Nous \todo{trop long!} avons aussi vu au travers de notre étude des moyens pour déterminer des 
directions dans le volcan de $\ell$-isogénies à l'aide de l'étude de l'action
du Frobenius sur le module de Tate, excepté pour le cas des 
isogénies descendantes dans le cas Elkies où l'on ne peut rien déduire. \todo{parler de la nouveauté et de la spécification des directions avec "les" + nouveauté dans le cas Atkin}

Voyons maintenant quelques améliorations possibles (ou à explorer) des 
résultats présentés dans cette thèse.

Le couplage n'a pas été abordé dans ce document mais seulement 
mentionné, certes dans \cite{Ionica-Joux10} il y a des résultats moins probants \todo{réfléchir a cette formulation}
...
%Un autre moyen pour distinguer des couples de points formant une base de 
%$E[\ell^k]$. Nous avons 
...
\cite{Kohel96},BissonSutherland11}, où l'on connaît déjà une 
factorisation de $d_{\pi}$, %avec les propositions ~\ref{pro:mat:fro} ou 
%\ref{pro:mat:fro:atk}, selon le cas Elkies ou Atkin, 
on peut déterminer la
valuation $\ell_i$-adique du conducteur de la courbe par rapport à 
$\mathbb{Z}[\pi]$ et donc on peut déterminer l'anneau des endomorphismes de
la courbe. Cela pourrait avoir par exemple une application dans l'algorithme de
...
anneau d'endomorphismes spécifique à partir d'une courbe au hasard dont 
$\mathbb{Z}[\pi]$ est fixé. %On pourrait alors à partir d'une telle courbe se servir des 
...
\cite{Doliskani-Schost15}, un moyen de calculer 
