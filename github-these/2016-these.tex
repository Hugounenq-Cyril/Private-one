...
\end{enumerate}
\end{thm}
Dans tous les cas on a $r$ qui correspond à l'ordre de la matrice $\pi$ dans $PGL_2(\mathbb{Z}/\ell \mathbb{Z})$ et on a le résultat suivant pour la trace du Frobenius $t_pi$:
\begin{equation*}
...
\subsection{Elkies' case} \label{sub:Elkies}
...
\subsection{Amélioration du cas Elkies par Couveignes et Morain}
...
En effet comme on est dans le cas Elkies alors la courbe $E_0$ dont on veut calculer le cardinal admet deux  courbes $\ell$-isogènes définies dans $\mathbb{F}_q$. Ainsi pour ces courbes isogènes le polynôme modulaire admet aussi deux solutions  dans $\mathbb{F}_q$, car ces courbes sont de même cardinal donc ont la même valeur $t_{\pi}$ et par conséquent le fait que $t_{\pi}^2-4q$ soit un résidu quadratique modulo $\ell$ reste préservé. Ainsi chaque courbe admet deux coubes isogènes définies sur $\mathbb{F}_q$, on définit alors un cycle qui est fini. En effet à chaque courbe on peut associer à isomorphisme près un ordre maximal pour l'indice $\ell$ (\textcolor{red}{ref a mettre plus tard}) dans $\mathbb{Q}[X]/(X^2-t_{\pi})X+q$ or comme le nombre de classe est borné on obtient le résultat. 

L'idée derrière cet article est donc de décomposer en deux espaces propres la $\ell^k$ torsion à l'aide des valeurs propres du Frobenius dans le cas Elkies, puis de calculer la valeur de la trace du Frobenius dans ces espaces propres. Pour décrire tout cela il est nécessaire de poser quelques notations. On généralise tout d'abord l'action du Frobenius $\pi$ sur $E[\ell]$ à une action sur $T_{\ell}(E)$, on a alors une décomposition de $T_{\ell}(E)$ en deux espaces propres: $T_{\ell}^1(E) \oplus T_{\ell}^2(E)$, avec $T_{\ell}^1(E)$ associé à $\lambda$ et $T_{\ell}^2(E)$ associé à $\mu$. 


Montrons maintenant que les isogénies peuvent êtres associées de façon canonique aux valeurs propres du Frobenius. On note $I_1$ (resp. $I_2$) l'isogénie engendrée par l'espace propre pour $\pi$ associé à la valeur propre $\lambda$ (resp. $\mu$). Ainsi en partant de $E_0$ on note alors $E_1$(resp. $E_2$) l'image de l'isogénie $I_1$ (resp. $I_2$). Lorsque l'on est sur $E_1$ on a alors à nouveau l'existence de deux isogénies associées aux deux espaces propres: $I_{11}$ pour l'iosgénie associée à la valeur propre $\lambda$ et $I_{12}$ pour l'isogénie associée à la valeur propre $\mu$. On a alors $I_{12}=\widehat{I_{1}}$, car l'isogénie $I_1$ étant rationnelle sa duale l'est aussi et on conclut alors en analysant son action sur $T_{\ell}^1(E)$. Ainsi on a des directions canoniques définies par les valeurs propres $\lambda$ et $\mu$. 

Il reste alors à montrer comment on détermine en pratique un espace propre de la $\ell^k$ torsion. Travaillons donc avec l'espace propre associé à $E_{\lambda}$, on a alors $T_{\ell}^1(E_0) \cap E_0[\ell]$ qui est contruit comme étant le noyau de l'isogénie qui va de $E_0$ à $E_1$. Pour calculer celui-ci on doit tout d'abord calculer à l'aide du polynôme modulaire les $j$-invariants des deux courbes, puis leur représentations sous forme de Weierstrass, puis l'isogénie est ensuite calculée. Ainsi on dénote $h_1$ le polynôme qui s'annule sur l'abscisse des points du noyau de l'isogénie $I_1$. 
Maintenant si l'on veut déterminer $T_{\ell}^1(E_0) \cap E[\ell^2]$, on remarque que cet ensemble c'est la préimage du noyau de l'isogénie $I_11$ ($h_{11}$) par l'isogénie $I_1$. Ainsi on injecte $I_1$ dans $h_11$ pour obtenir cet ensemble. On répète alors l'opération pour déterminer un espace propre de la $\ell^k$-torsion. Ainsi pour la $\ell^k$-torsion on va déterminer l'espace propre par un polynôme de degré $\ell^k  (\frac{\ell-1}{2})$ qui factorise le $\ell^k$-ème polynôme de division de degré $\ell^{2k}$.

Ainsi on a vu une façon de déterminer un espace propre de la $\ell^k$-torsion, à l'aide d'un cycle d'isogénies.
