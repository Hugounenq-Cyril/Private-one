...
\begin{thm}
Soit $\mathbb{U}_1=\mathbb{F}_p[X_1]/Q_1$, avec $Q_1$ un polynôme irréductible de degré $d$, soit alors $x_1=X_1 \bmod Q_1$ tel que $Tr_{\mathbb{U}_1/\mathbb{F}_p}(x_1)\neq 0$. On définit la suite de polynômes $G_i$ avec $ 1 \leqslant i \leqslant k$ par:
\begin{equation}
\begin{cases}
G_1=X_1 & \\
...
\end{cases}
\end{equation}
dans l'anneau de polynômes $\mathbb{F}_p[X_1, \cdots, X_i]$.    
\end{thm} 
...
\begin{itemize} 
\item si $\alpha \in \mathbb{U}_{k-1}$, alors il existe un polynôme $h \in \mathbb{F}_p[X_{k-1}]$ tel que $\deg(h)<|\mathbb{U}_1|^{p^{k-1}}$ $\alpha=h(x_{k-1})$ et le fait d'écrire $\alpha$ comme un élément de $\mathbb{U}_{k-1}$ s'appelle une descente (ou push-down dans la littérature anglophone \cite{DeFeo-Shost'12})
\item comme $\alpha \in \mathbb{U}_{k+1}$, alors il existe un polynôme $j \in \mathbb{F}_p[X_{k-1}]$ tel que $\deg(j)<|\mathbb{U}_1|^{p^{k+1}}$ $\alpha=j(x_{k+1})$ et le fait d'écrire $\alpha$ comme un élément de $\mathbb{U}_{k+1}$ s'appelle un relèvement (ou lift dans la littérature anglophone \cite{DeFeo-Shost'12}).
\end{itemize}
\end{defi}

\paragraph{$p$-torsion}
La construction de $\mathbb{F}_q[c]$ est faite comme une extension de $\mathbb{F}_p:\mathbb{F}_p[X]/Q_1(X)$ avec $Q_1$ un polynôme irréductible qui vérifie l'une des deux conditions suivantes:
\begin{itemize}
\item $d \wedge p =1$
...
Le seul apport de l'utilisation de tour d'Artin Schreier \cite{DeFeo-Shost'12} ne change pas la méthode utilisée dans l'algorithme original \cite{Couveignes96}, seul quelques opérations sont plus rapides grâce à celle-ci. On calcule donc un polynôme d'interpolation $A$ de degré $p^{k-1}(p-1)-1$ à coeffcients dans $\mathbb{U}_k$ à l'aide des couples $(P,I(P)))$, on utilise alors une descente (\cite[push-down]{DeFeo-Shost'12}) pour avoir un polynôme à coefficients dans $\mathbb{F}_q[c][x]$ ensuite on utilise l'expression du plongement de $\mathbb{F}_q$ dans $\mathbb{F}_q[c]$ pour obtenir, finalement on utilise une reconstruction rationnelle sur $\mathbb{F}_q$.

La première étape a un coût de $O(\mathsf{M}(p^{2k}d)\log(p^{k}))$ en utilisant les techniques de \cite[§10.2]{vzGJG03}. Ensuite on utilise une descente pour avoir une expression du coefficient à coeffients dans $\mathbb{U}_1[X]=\mathbb{F}_q[c][X]$, cela coûte $\ell L(k-1)$ opérations sur $\mathbb{F}_p$ avec $L$ qui représente le coût d'effectuer un relèvement ou une descente dans une tour d'Artin Schreier. D'après \cite[Theorem 13]{DeFeo-Shost'12} $L(i) \in O(p^{i+1}d \log^2_p(p^id)+p \mathsf{M}(p^id))$, il est à noter que ce coût est linéaire en la taille de l'entrée (élément de $\mathbb{U}_i$). Enfin à l'aide d'algèbre linéaire on exprime le polynôme sur $\mathbb{F}_q[X]$ en $O(\ell(pd)^2)$ opérations sur $\mathbb{F}_p$. La reconstruction rationnelle coûte $O(\mathsf{M}(p^kd)\log(p^k))$ d'après \cite[§11.1]{vzGJG03}, on a donc un coût total de:
\begin{equation*}
...
Cette partie est cruciale car comme on l'a vu précédemment la partie la plus coûteuse de l'algorithme de Couvegines reste l'interpolation de Cauchy et en particulier le calcul du polynôme d'interpolation. On peut déjà voir qu'interpoler $p^k$ paires de points $(P,I(P))$ avec $P$ et $I(P)$ à coordonnées dans $\mathbb{U}_k$ entraîne un cout minimal de $O(p^k p^kd)$ opérations sur $\mathbb{F}_p$. Bien que le polynôme que l'on souhaite calculer est de degré $p^k$ il est possible avec moins d'informations de calculer celui-ci. En effet comme préconisé par Couveignes \cite{Couveignes96} si on travaille avec des représentants d'orbite sous l'action du Frobenius alors on peut retrouver le polynôme d'interpolation en utilisant le théorème des restes chinois. Il manquait alors à l'époque des techniques efficaces pour justement faire agir le Frobenius sur ces représentants, ce que les constructions de tours d'Artin Schreier de \cite{DeFeo-Shost'12} permettent désormais.

On va donc voir dans cette section comment à partir d'un polynôme d'interpolation défini uniquement à l'aide de représentants de l'orbite selon l'action du Frobenius on calcule efficacement le polynôme d'interpolation sur toutes les orbites. C'est d'ailleurs l'une des raisons pour travailler uniquement avec des points primitifs de $p^k$-torsion puisque que eux seuls sont définis dans $\mathbb{U}_k$ par définition de $\mathbb{U}_k$. Bien évidemment on peut généraliser cette méthode à tous les points de $p^k$-torsion en utilisant le théorème des restes chinois, mais d'un point de vue pratique prendre $k$ tel que $p^{k-1}(p-1) > 4 \ell +1$ est suffisant, asymptotiquement le résultat reste le même.
...
\end{equation*}
avec $\deg(T_i)=f=[\mathbb{U}_k:\mathbb{U}_1]$ et $df=p^{k-1}(p-1)/2$.
