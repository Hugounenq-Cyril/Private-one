...
\todo{Point ascendant horizontal}
...
\label{eq:velu:sum}
\end{equation}
\todo{saut à la ligne fait, moche ?}
...
\end{itemize}
\end{defi}
	Avec cette distinction il va être possible selon les cas de déterminer des espaces propres pour le Frobenius appliqué à la $\ell$-torsion. Avant cela on a besoin d'énoncer la notation suivante:
\begin{nota}
...
\end{itemize}
	...
	On ne peut appliquer à priori ce résultat, car il suppose la connaissance de 
...
\begin{defi}
Soit $\ell$ un nombre premier, le terme volcan des $\ell$-isogénies désigne le graphe composé de courbes $\ell$-isogènes entre elles.
...
\] 
\end{prop}
\todo{@Luca dans le cas ordinaire $\pi$ n'est jamais dans $\mathbb{Z}$, et
...
\todo{ Luca: comment $R$ engendre-t-il un réseau} engendré par $R$, soit $\Pi$ la matrice du Frobenius sur $\mathsf{T}_{\ell}(E)$, alors la 
...
\end{figure}
%\todo{supprimer les couleurs dans la thèse}
...
\todo{reflexion de Luca sur ascendant horizontal}
...
partiellement horizontal sur un volcan des $2$-isogénies. Sur le volcan de 
...
descendantes entre elles dans un volcan des $\ell$-isogénies, quand $\ell$ est un 
...
\subsection{Calcul des directions dans le volcan des $\ell$-isogénies}
\label{sub:con:poi}
...
d'un volcan des $\ell$-isogénies, car c'est à ce niveau du volcan que l'on peut 
...
\subsubsection{Algorithmes pour le calcul des directions dans le volcan des $\ell$-isogénies}

L'objectif de cette sous-section est de calculer, pour une courbe se trouvant 
sur un volcan des $\ell$-isogénies une base horizontale (ou partiellement 
...
Dans le cas des volcans réguliers, la hauteur du volcan des $\ell$-isogénies 
...
le volcan des $\ell$-isogénies, la valuation $\ell$-adique de la torsion
...
dessous du cratère d'un volcan des $\ell$-isogénies, les points utilisés et 
...
volcan des $\ell$-isogénies; $P,Q \in E, \lambda, \mu, b \in \mathbb{Z}/
...
\begin{algorithmic}[1]
\REQUIRE $E_s$: une courbe elliptique ordinaire située sur le cratère d'un volcan des $\ell$-isogénies; $k$: un entier positif; $j<k$; $\lambda, \mu \in \mathbb{Z}/\ell^{j}\mathbb{Z}$; $P_j,Q_j$ tels que $E_s[\ell^j]=\langle P_j,Q_j \rangle$ $\pi(P_j)=[\lambda] P_j, \pi(Q_j)=[\mu]Q_j$;
...
\begin{algorithmic}[1]
\REQUIRE $E$: une courbe elliptique ordinaire située sur un volcan des $\ell$-isogénies avec cratère cyclique;
...
Soit $E$ une courbe située sur le cratère d'un volcan des $\ell$-isogénies dont 
...
\todo{Luca: Tu as déjà fait une remarque similaire. Il faut peut-être factoriser.}
\begin{rem}
...
\begin{prop}
\label{pro:par:hor}
Soit $E$ une courbe située au niveau $h-e$ dans un volcan des $\ell$-isogénies, 
...
Soit~$E$ une courbe elliptique définie sur un volcan des $\ell$-isogénies de 
...
\begin{proof}
Soit $E$ une courbe située au niveau $h-e$ du volcan des $\ell$-isogénies, alors
...
\end{tikzpicture}
\end{center}
\caption{\label{fig:atk:dif:niv:mat} Exemples de matrices du Frobenius obtenues sur les différents niveaux d'un volcan des $\ell$-isogénies}
\end{figure}
...
\begin{cor}
\label{cor:mon:atk}
Soit $E$ située au niveau $h-e \neq h$ d'un volcan des $\ell$-isogénies avec 
...
\begin{prop}
\label{pro:atk:equi}
Soient $P,R,R'$ trois points de $E$ située au niveau $h-e$ du volcan des 
...
\end{tikzpicture}
\end{center}
\caption{\label{fig:atk:dif:niv:iso} Exemples des isogénies obtenues à partir de différentes matrices du Frobenius calculées sur la courbe située au niveau du cratère du volcan des $\ell$-isogénies. Pour cet exemple on a travaillé avec une courbe de $j$-invariant $56$ définie sur $\mathbb{F}_{149}$.}
\end{figure}
...
niveau $h-e$ du volcan des $\ell$-isogénies tels que $\langle P,R \rangle= 
...
située au niveau du cratère du volcan des $\ell$-isogénies tels que $[\ell^{h-e}]P=
...
\subsection{Calculs de directions dans le volcan des $\ell$-isogénies.}
\label{sub:atk:cal}
On va maintenant étudier la structure de la torsion rationnelle des courbes 
...
volcan des $\ell$-isogénies. On note~$d_1$ le degré de la plus 
...
de degré $\ell^{\alpha}$ et cela contredirait la structure du volcan des 
...
Soit $E$ une courbe elliptique ordinaire située au niveau du cratère d'un 
volcan des $\ell$-isogénies. En reprenant les notations de la 
...
volcan des $\ell$-isogénies on est obligé de travailler avec une extension 
...
deux basées sur les directions dans le volcan des $\ell$-isogénies une pour le 
...
deux courbes situées sur le cratère du volcan des $\ell$-isogénies. Cependant la
...
$\ell$-isogénies ne permet pas de distinguer les $\ell$-isogénies entre elles.
 ...
 Q'_{\mu})$ (partiellement) horizontales de $E[\ell^k]$ et $E'[\ell^k]$.
...
\begin{algorithmic}[1]
\REQUIRE $E,E'$: deux courbes elliptiques $r$-isogènes ordinaires situées au niveau $h-e$ d'un volcan des $\ell$-isogénies avec cratère cyclique.
...
\begin{lem}
\label{lem:cou:dec}
Le coût de trouver un courbe sur le cratère d'un volcan des $\ell$-isogénies 
...
sommet du volcan des $\ell$-isogénies à l'aide de $O(\ell h^2)$ 
...
le volcan des $\ell$-isogénies  dans la preuve du 
...
allant d'une taille de $7$ bits à $252$ bits. Le volcan des $2$-iogénies de 
...
\ell=1$ qui se situent sur un volcan des $\ell$-isogénies de hauteur $h$, dont 
...
on travaille se situent sur le cratère d'un volcan des $\ell$-isogénies, on 
...
au niveau du cratère du volcan des $\ell$-isogénies telle que $E(\mathsf{F}_
...
Soit $E$ une courbe elliptique située au niveau du cratère du volcan des 
...
\begin{algorithmic}[1]
\REQUIRE $E,E'$: deux courbes elliptiques $r$-isogènes ordinaires situées au niveau du cratère d'un volcan des $\ell$-isogénies avec cratère réduit à une courbe.
\ENSURE $\phi$ la $r$-isogénie qui relie $E$ à $E'$.
% \STATE Calcul $(P_1,Q_1)$, a basis of $E[\ell]$
\STATE Calcul du plus petit $k$ tel que $\ell^{2k-2}(\ell+1)>4r$;
...
\begin{algorithmic}[1]
\REQUIRE $E,E'$: deux courbes elliptiques $r$-isogènes ordinaires situées au niveau $h_i-e_i$ d'un volcan des $\ell_i$-isogénies avec cratère cyclique.
\ENSURE $\phi$ la $r$-isogénie qui relie $E$ à $E'$.
% \STATE Calcul $(P_1,Q_1)$, a basis of $E[\ell]$
...
(P'_{\lambda,i},Q_{\mu,i}')$ bases (partiellement) horizontales de 
...
$(P_{\lambda,i},Q_{\mu,i})$ (partiellement) horizontale de $E[\ell_i^{k_i}]$ 
...
les  bases (partiellement) horizontales $(P_{\lambda_i},Q_{\mu_i})$ de 
...
vus dans le chapitre~\ref{cha:tour}.
 ...
 calcule les points (partiellement) horizontaux sur les volcans des 
...
Ainsi avec deux bases de points (partiellement) horizontaux $(P_{\lambda_1,
...
\begin{algorithmic}[1]
\REQUIRE $E,E'$: deux courbes elliptiques $r$-isogènes ordinaires situées au niveau $h_i-e_i$ d'un volcan des $\ell_i$-isogénies avec cratère cyclique pour $i$ allant de $1$ à $n$.
\ENSURE $\phi$ la $r$-isogénie qui relie $E$ à $E'$.
% \STATE Calcul $(P_1,Q_1)$, a basis of $E[\ell]$
...
qui se situent à une profondeur $e_i$ du cratère du volcan des 
...
cratère cyclique ($\ell$ est donc un nombre de Elkies) d'un volcan des 
...
cyclique ($\ell$ est donc un nombre de Elkies) d'un volcan des $\ell$-isogénies.
...
cyclique d'un volcan des $\ell$-isogénies ayant une base diagonale $(P,Q)$ de la
...
bas dans un volcan des $\ell$-isogénies pour le cas Elkies. 
...
\begin{prop}
Soit $E$ une courbe située au niveau $h-e$ du volcan des $\ell$-isogénies, 
...
\begin{algorithmic}[1]
\REQUIRE $E$: une courbe elliptique ordinaire située à un niveau $h-e$ d'un volcan des $\ell$-isogénies avec cratère cyclique;
...
\begin{algorithmic}[1]
\REQUIRE $E$: une courbe elliptique ordinaire au niveau $h-e \neq h$ d'un volcan des $\ell$-isogénies avec cratère cyclique;
$k$: une entier positif;
\ENSURE $(P_k, Q_k )$: une base de $E[\ell^k]$;
$\lambda, \mu, \delta \in \mathbb{Z}/\ell^k \mathbb{Z}$
...
\begin{prop}
L'algorithme~\ref{alg:bas:tri} calcule une base triangulaire de $E[\ell^k]$ située au niveau $h-e<h$ dans un volcan des $\ell$-isogénies régulier avec une complexité espérée de 
...
$\ell^i$ torsion n'est plus égale à une matrice scalaire tester l'égalité entre
 ...
 niveau du cratère, dans les deux cas le niveau de la courbe dans le volcan des 
