...
\end{smallmatrix}\right )$ avec soit $b, c$ premiers avec $\ell$ soit $b$ ou $c$ premier avec $\ell$ l'autre étant divisible par $\ell^{2e}$. Ce second cas correspond au cas où l'on a un point de la base dans laquelle est exprimée l'action du Frobenius qui est un générateur de l'isogénie "montante" sous-groupe isomorphe à $\mathbb{Z}/\ell^{h-e+1} \mathbb{Z} \times \mathbb{Z}/\ell^{X}\mathbb{Z}$ qui définit des $\ell^{h-e+i}$isogénies 
rationnelles. \todo{retravailler cela...} Il faut dire que l'on a $a=d \bmod \ell^h$ par transport à l'aide d'isogénie descendante...
\end{itemize}
...
est intéressant à analyser... On a $d_{\pi}/\ell^{2h}=(d-a)^2/{\ell^{2h}} + 4 bc$ \todo{est non nul, mais qu est ce que cela veut dire}
La conjugaison de la matrice $\left ( \begin{smallmatrix}a & b\ell^{h-e}\\c\ell^{h-e} & d
...
cette égalité on déduit avec $c \wedge \ell = 1$ que $[\ell^{h-e}]R=
...
On met tout d'abord en avant un résultat qui est une conséquence du 
chapitre~\ref{cha:volc:iso}. 

\begin{prop}
Soit $E(\mathbb{F}_q)$ une courbe elliptique située au niveau du cratère d'un volcan de 
$\ell$-isogénies alors:
\[
E(\mathbb{F}_q)[\ell^{\infty}] \simeq \mathbb{Z}/ \ell^{\beta} \mathbb{Z} \times \mathbb{Z}/ \ell^{\beta} \mathbb{Z} 
\]
et $\beta=h$ si et seulement si le volcan est \emph{régulier}.
\end{prop}
...
Dire juste après comment cela se comporte sur les extensions $\ell$-adiques...

\begin{prop}\label{pro:clas:fro:atk} \todo{nécessaire}
Soit $E$ une courbe elliptique ordinaire située au niveau du cratère d'un 
volcan de $\ell$-isogénies. Pour tout~$k$, on note~$d_k$ le degré de la plus 
...
% $d_k = \mathrm{lcm}(d_2, \ell^{k-\beta})$.
$d_k = \ell^{\max (0, k - \beta)}d_2$.
\item Soit $[F:\mathbb{F}_q]=d_1\ell^n$, le groupe $E[\ell^{\infty}](F)$ est isomorphe à~$(\mathbb{Z}/\ell^{n+\beta} \mathbb{Z}) \times (\mathbb{Z}/\ell^{n+\beta} \mathbb{Z})$.
%\item\label{prop:classes:count} Le groupe~$E[\ell^k]$ contient au plus
%$k \cdot \ell^{k+\beta}$ classes de conjugaisons de Galois sur~$F_1 = \mathbb{F}_{q^{d_1}}$.
\end{enumerate}
\end{prop}
...
\begin{lem}
Soit $P$ un point d'ordre $\ell^{\beta+i}$ de $E$ situé au niveau du cratère du 
volcan de $\ell$-isogénies telle que $E(\mathsf{F}_1)[\ell^{\infty}] \simeq 
\mathbb{Z}/ \ell^{\beta} \mathbb{Z} \times \mathbb{Z}/ \ell^{\beta}\mathbb{Z}$,
alors la taille de l'orbite de $P$ selon l'action de $\mathsf{Gal}(
\mathsf{F}_{\beta+i}:\mathsf{F}_1)$ est $\ell^i$. 
\end{lem}
...
\mathsf{F}_{\beta+i}:\mathsf{F}_1)$.
\end{prop}

On travaille tout d'abord avec une extension $\mathsf{F}_0$ sur laquelle on a 
$E[\ell]$ défini afin de faire cela 
...
\begin{lem}
Soit $P$ un point d'ordre $\ell^i$ de $E(\mathbb{F}_q)$ tel que $\phi$ la $\ell^i$-isogénie 
de noyau $\langle P \rangle$ ait pour codomaine $E_c$ tel que $E_c(\mathbb{F}_q)[\ell] \simeq 
\mathbb{Z}/\ell \mathbb{Z} \times \mathbb{Z}/\ell \mathbb{Z}$. Alors il existe
$\ell$ points $A_i$, $i \in (1,\ell)$ de $E$ tels que $\ell A_i=P$.
\end{lem}

\begin{proof}
Le plus dur est ici...
Parmi les $\ell+1$ sous-groupes cycliques de $E[\ell]$ il n'en existe qu'un $G$ tel 
que $G=\langle \ell^{i-1}P \rangle$ dés lors le seul groupe 
\end{proof}

Sur un volcan régulier de Atkin la $\ell$ torsion au niveau du cratère est paritaire...
en fait elle l'est quoiqu'il arrive (non régulier ou régulier...)
