...
elliptiques $E_0$ et $E_1$ $r$-isogènes telles que $j(E_0)=j_0$ et 
$j(E_1)=j_1$, alors le problème du calcul explicite d'isogénie est de calculer 
...
tour d'extensions $\ell$-adiques $\mathsf{F}_0\subset\cdots\subset 
\mathsf{F}_{k-\beta}$, et nous effectuons les précalculs nécessaires au 
...
O(r\ell^2)$ fois cela donne une complexité de $O(r \ell^2 \sqrt{r \ell}(\mathsf{M}(r\ell^4)\log(r)\log(\ell)+\mathsf{M}(\sqrt{r}\ell)\sqrt{r}\log(\ell)\log(\ell q))$ à laquelle il faut ajouter les précalculs pour obtenir le résultat.
\end{proof}


Maintenant nous abordons le cas général où les courbes $r$-isogénes ne sont
...
lemmme~\ref{lem:cou:dec}, et du corollaire~\ref{cor:atk:elk:dist} sont 
...
Détaillons comment nous procèdons, la méthode est restranscrite dans 
...
l'action de $\phi$ sur une base $(P_{\lambda,i},Q_{\mu,i})$ (ascendante) 
horizontale de $E[\ell_i^{k_i}]$ peut être représentée de la façon suivante:
$
\left(\begin{smallmatrix}
P'_{\lambda,i} \\
Q'_{\mu,i} 
...
coeffcients $a,b \in (\mathbb{Z}/\prod_{i=1}^n\ell_i^{k_i} \mathbb{Z})^{\times}$.
%\newline

\paragraph{Analyse du coût}
{Une telle approche a forcément des coûts de pré-calcul (pour le calcul des 
bases (asendantes) horizontales et des représentants) plus faibles que notre 
...
Nous énonçons une généralisation du thèorème des restes chinois pour une courbe 
...
$[\vartheta_{i_0}] P=P_{i_0}$ pour $i_0 \in [1,n]$.}\todo{reformuler}
\newline
...
et ont chacune des degrés $d_i=[\mathsf{F}_{(i,1)}:\mathbb{F}_q]$ qui lorsqu'ils 
...
Nous définissons alors les compositums qui nous intéressent dans le cas présent.
\begin{defi}
\label{def:con:com}
...
\end{defi}
\begin{rem}
Dans la définition~\ref{def:con:com} le cas où $d_i=d_j$ avec $d_i 
...
chapitre~\ref{cha:tour} $d_i | \ell_i-1$, ainsi il est possible que 
pour $d_i,d_j$ distincts $\mathrm{pgcd}(d_i,d_j) \neq 1$. Une 
...
\begin{nota} 
Nous posons $p_{i}=\prod_{j=1}^{i}d_j\ell_j^{k_j}$, en particulier $p_{i}$ est le 
...
Frobenius avec une méhtode similaire à celle du théorème~\ref{thm:frob-ell}. 
...
opération dominé par $O(d_i\ell_i^{k_i})\mathsf{M}(p_{i-1})$. 
...
d'un tel calcul par \[O(n\mathsf{M}(p_n \max_i(d_i\ell_i^{k_i}))
...
Avec $p_n \in O(r(\prod_{i=1}^n\ell_i)^{3,5})$ l'estimation est
de 
\[ 
...
$\max_i(d_i\ell_i^{k_i})p_{n-1} \in \Omega(p_n)$. Ainsi nous voyons que par 
...
donc de réduire les coûts apportés par l'étape d'interpolation dûs au choix des
...
uniquement dans le cas Elkies. Dans le cas Atkin nouus avons seulement obtenu 
...
Nous \todo{trop long!} avons aussi vu au travers de notre étude de nouveaux 
...
\todo{composita est le pluriel de compositum}
