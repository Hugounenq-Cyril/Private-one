...
Soit $a$ un élément de $F_i$ pour tout entier $j$ le calcul de la puissance $|\mathsf{F}_j|$ de $a$ coûte $O(\ell^{i-1}M(\ell))$ opérations sur $\mathbb{F}_q$ après un pré calcul de $O(\ell M(\ell)\log(q))$
\end{thm} 
...
Une méthode rapide est préconisée pour le calcul de $u \bmod |\mathsf{F}_0|-1$ et $v$, dans un premier temps il faut calculer $\rho=s \bmod \ell^i(|\mathsf{F}_0|-1)$ à l'aide de l'exponentiation rapide puis $u \bmod |\mathsf{F}_0|-1$ et $v$ sont obtenus comme le quotient et le reste de la division euclidienne de $\rho$ par $\ell^i$. Le coût booléen d'une telle opération est de $O(\log(j)\log(\ell^i|\mathsf{F}_0|))$.
Le calcul de $x_1^{u \bmod |\mathsf{F}_0|-1}$ se fait en utilisant $O(\ell M(\ell) \log(q))$ opérations sur $\mathbb{F}_q$ si et si , on conserve ce résultat sous la forme d'un monôme dans $F_1[x_i]$. Par hypothèse on a $a$ qui est représenté sous la forme d'un polynôme en $x_i$ de degré inférieur ou égal à $[F_i:F_0]$: 
\begin{equation*}
