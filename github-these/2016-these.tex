...
\end{matrix}\right ) \quad \text{avec }a=d \bmod \ell^{h-e}.
\end{equation*}
% where $a \in \mathbb{Z}$, $0 \leq a \leq \ell^{h} - 1$,
...
$\pi$ est irréductible alors l'action du Frobenius doit être scalaire.
...
\beta (c\ell^{h-e}) &  \beta(d-a) +\ell^{h-e} (b-c\beta)\\c\ell^{h-e} & d - c\ell^{h-e}
\end{smallmatrix}\right )$ pour laquelle les coefficients diagonaux sont encore
égaux modulo $\ell^{h-e}$. Le coefficient $\beta(d-a) +\ell^{h-e} (b-c\beta)$ \todo{est non nul, mais qu est ce que cela veut dire}
La conjugaison de la matrice $\left ( \begin{smallmatrix}a & b\ell^{h-e}\\c\ell^{h-e} & d
