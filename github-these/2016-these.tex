...
logarithme discret sur des corps finis. Malgré de nombreux progrès dans la 
résolution du logratihme discret sur les corps finis à l'aide de calculs 
d'indices 
...
utilisée malgré l'existence de méthodes post-quantiques permettant de résoudre
le problème logarithme discret. \todo{Trouver source}
...
elliptique. Cette méthode repose principale sur la détermination de la trace du
Frobenius sur un groupe de $r$-torsion. Ceci est fait en déterminant 
$t_{\pi}$ dans l'équation suivante obtenue à l'aide du polynôme 
caractéristique du Frobenius:
\begin{equation}
x^2-t_{\pi}x+q=0 \bmod r.
\end{equation} 
Des améliorations ont été apporté à cet algorithme notamment par Elkies 
...
Ainsi au cours de l'algorithme de S.E.A. (Schoof Elkies Atkin) on est amené à 
calculer le $r$-ème polynôme modulaire $\Phi_r$ avec un coût de 
$\tilde{O}(r^3)$ (par \cite[Algorithm 6.1]{BLS12}), ainsi q'une isogénie de 
...
d'isogénies de degré $r$ à l'aide de la $p$-torsion \cite{Couveignes96}
car celui-ci ne suppose pas la connaissance de
$\Phi_r$. Ce travaille de Couveignes fût amélioré par la suite par les travaux 
de De Feo et Schost \cite{DeFeo11}, \cite{DeFeo-Shost'12} qui se sert notamment
d'une construction efficace d'extensions $p$-adiques de corps finis. 

Cette thèse s'inscrit dans la continuation des travaux de \cite{Couveignes96} 
et \cite{DeFeo11} et cherche notamment à généraliser celui-ci à l'étude des 
points de $\ell$-torsion avec $\ell \neq p$ la caractéristique du corps sur 
lequel on travaille. Pour cela on doit notamment déterminer des invariants de 
l'isogénie de degré $r$ que l'on veut calculer tout en se servant de la 
construction de tours d'extensions $\ell$-adiques. 

Pour le calcul d'invariants de la $r$-isogénie on se sert de graphes d'isogénies 
étudiés pour la première fois par \cite{Kohel96} afin de calculer l'anneau 
...
d'expliciter un lien entre le niveau d'une courbe dans le volcan d'isogénies et
la structure de son groupe de Sylow.
...
courbes elliptiques et les endomorphismes de courbes elliptiques. Dans le 
...
par De Feo \cite{DeFeo11}. Dans le chapitre 3 nous présentons les tours 
$\ell$-adiques, dans la suite des travaux de \cite{Doliskani-Schost15}, sur 
...
notamment des résultats de Kohel \cite{Kohel96}, Fouquet et Morain 
...
Couveignes dans un cas favorable (i.e. on connaît un nombre de Elkies) puis 
dans le cas général où l'on utilise notamment un résultat de Shparlinski et 
Sutherland \cite{ShparlinskiSutherland14} afin de trouver un tel nombre de 
Elkies. On fait aussi une présentation moins détaillée du cas Atkin car 
celle-ci est moins performante.
