...
\begin{prop}
Soit $E$ une courbe au niveau $h-e$ dans le volcan, soit $E_s$ sa courbe sommet associée, alors l'algorithme \ref{alg:rem:tri} calcule une base diagonale de $E_s[\ell^k]$ avec une complexité de $O( \mathsf{R}(k+\beta+e) + \ell\mathsf{M}(\ell^{k+\beta+e})+\ell \mathsf{M}(\ell^2)\log(\ell)\log(\ell q))$ oéprations sur $\mathbb{F}_q$.
\end{prop}
...
\begin{prop}
Soit $E$ une courbe au niveau $h-e$, situé au-dessus du premier niveau de stabilité dans le volcan alors l'algorithme \ref{alg:rem:tri} calcule une base diagonale de $E_c[\ell^k]$ avec une complexité de $O(e( \mathsf{R}(k-\beta) + \ell\mathsf{M}(\ell^{k-\beta}))+\ell \mathsf{M}(\ell^2)\log(\ell)\log(\ell q) )$ oéprations sur $\mathbb{F}_q$.
\end{prop}
...
\STATE\label{alg:rem:iso}  $\widehat{\varphi} \leftarrow $ isogénie de noyau $\langle \ell^{i}P_{i+1} \rangle$
\STATE\label{alg:rem:divide}
...
réoudre équation~$\delta + x(\lambda - \mu) = 0$;
\STATE 
...
à un côut espéré \todo{vérifier orthographe côut, coût} de $O(\ell\mathsf{M}(\ell^2)\log(\ell)\log(\ell q))$
opérations en utilisant l'algorithme de Cantor-Zassenhaus.
...
Faire une proposition sur le côut d'évaluer un point partiellement horizontal.
\end{prop}
