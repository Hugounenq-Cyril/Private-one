...
Les nombres premiers de Elkies sont les plus favorables au calcul de $t_{\pi}$ car une fois que l'on a un nombre premier de Elkies alors par \eqref{pro:def-ss-grp} il existe un sous groupe cyclique $C$ de la $\ell$ torsion qui est un espace propre pour le Frobenius $\pi$, on résoud alors l'équation suivante dans ce groupe
\begin{equation*}
X^2-t_{\pi}X+q = 0 \bmod g_{\ell}
\end{equation*} 
ainsi en travaillant dans ce groupe, plus petit que celui de la $\ell$-torsion, on va diminuer la complexité du calcul à effectuer pour trouver la trace modulo $\ell$.

Par contre pour les nombres premiers $\ell$ de Atkin comme un sous groupe cylcique de la $\ell$-torsion ne peut être déterminé dans $\mathbb{F}_q$ alors on ne peut que spéculer sur les valeurs possibles de $t_{\pi}$ et l'on doit alors toutes les considérer. De telle sorte que pour les différents nombres premiers de Atkin on est obligé d'utiliser un algorithme à pas de bébé pas de géant pour lequel il existe des améliorations proposées par \cite{Ler97a}. Ces améloriations utilisent notamment un tri sur les nombres premiers de Elkies en fonction de leur côut, dû aux nombres de racines primites $r$-ième de l'unité, que ceux-ci ont pour l'algorithme.


On va donc dans la suite de ce document se concentrer sur le cas où $\ell$ est un nombre de Elkies car cela nous permettra de déterminer un sous espace particulier de la $\ell$-torsion et de la $\ell^k$ torsion en général avec $k$ un nombre premier.
...
Lorsque l'on est dans le cas Elkies, alors on cherche à déterminer un des deux sous-groupe de la $\ell$-torsion qui est aussi espace propre pour le Frobenius, le but étant au final de déterminer la trace du Frobenius à travers la détermination de son action sur cet espace propre. 

Ce sous groupe $C$ est déterminé comme étant le noyau de l'isogénie qui relie la courbe $E_0$ dont on veut calculer le cardinal et une des deux courbes $\ell$-isogènes à $E_0$. Pour calculer l'une des deux courbes isogènes il faut calculer les racines du polynôme modulaire $\Phi_{\ell}(X,Y)$ évalué en $j_0$ ($j$-invariant de $E_0$), on obtient alors le $j$-invariant des courbes $\ell$-isognèes définies sur $\mathbb{F}_q$, mais celles-ci sont définies sur $\mathbb{F}_q$ à tordue près. Il faut donc se servir du résultat suivant prouvé par Elkies afin de déterminer exactement la courbe isogène à l'aide des formes de Weierstrass.
...
Maintenant que la courbe isogène est entièrement déterminée, il reste à déterminer le noyau de l'isogénie de celle-ci c'est ce que des algorithmes tels que celui de \cite{BMSS08}  puis \cite{Lercier-Sirvent2008}   cherchent à résoudre en particulier dans ce contexte. Un présentation de ces algorithmes est faite \textcolor{red}{ref à mettre plus tard}.
...
Les complexités sont calculées en nombre d'opération ($+, \times,\div$) à coût identique dans $\mathbb{F}_q$, indépendamment de la construction de $\mathbb{F}_q$. On notera par $\mathsf{M}: \mathbb{N} \rightarrow \mathbb{N}$ l'application telle que la multiplication d'un polynôme de degré $n$ à coefficients dans $R$ coûtera $\mathsf{M}(n)$ opérations dans $R$ et telle que $\mathsf{M}$ est super-linéaire, en utilisant les résultats de \cite{Cantor-Kaltofen91} on a $\mathsf{M}(n)=O(n\log(n) \log \log(n))$.
