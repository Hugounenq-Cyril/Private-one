...
\begin{proof}
Vu que $a \in \mathbb{K}_{p_i}$ alors on a pour $s=r \bmod d_i \ell_i^{n_i}$ 
$a^{q^{r p_j}}=a^{q^{s p_j}}$ ainsi on considère que $r<d_i \ell_i^{n_i}$.
...
$O(d_i\ell_i^{n_i}\mathsf{M}(p_{i-1}))$ opérations dans $\mathbb{F}_q$. 
Pour ce calcul-là l'opération dominante est donc la conversion de 
$a \in \mathbb{K}_i$ en un élément de 
$\mathbb{K}_{i-1} \otimes \mathsf{F}_{(i,n_i)}$.
\end{proof}
...
minimal de $A$ dans la classe de polynômes $\mathbb{F}_q[x]/ \langle T \rangle$.
 ...
 d'abord $T$. On définit la suite de polynôme $T_i$ d'ordre $\frac{p_n}{p_{n-i}}$ 
 ...
 $\mathsf{Gal}(\mathbb{K}_{n-i-1}: \mathbb{F}_q)$. 
 On définit alors récursivement $T^{(i)}$ de la façon suivante:
 \begin{equation*}
T^{(i+1)}=\prod_{j=0}^{d_i\ell_i^{n_i}-1}T^{(i,j)}    
\end{equation*}
...
$T^{(i)}_{(0)}=T^{(i)}$, supposons que l'on connaisse  $T^{(i)}_{(j)}$ pour 
$i \in [0,n-1], j \in [0,n_i]$. La notation $T^{(i,k)}_{(j)}$ est définie comme
 auparavant par $T^{(i,k)}_{(j)}=\sigma^k_{\mathsf{Gal}(\mathbb{K}_{n-i-1})}
 (T^{(i)}_{(j)})$, on définit alors:
 \begin{equation*}
T^{(i)}_{(j+1)}=\prod_{k=0}^bT^{(i,k)}_{(j)} \qquad\text{avec}\qquad
  b = \begin{cases}
    \ell_i-1 &\text{si $j<n_i-1$,}\\
    d_i &\text{sinon.}
  \end{cases}
\end{equation*}
ainsi on a $T^{(i)}_{(j)}$ défini dans $\mathbb{K}_{n-i} \otimes \mathsf{F}_{(n-i+1,n_i-j)}$
en particulier on a $T^{(i)}_{(n_i+1)}=T^{(i+1)}_{(0)}=T^{(i+1)}$.  
...
\begin{proof}
  \`A l'étape $i$, à partir de la connaissance de $T^{(i)}$ on  calcule tous 
  les $T^{(i,j)}$ en utilisant la proposition~\ref{pro:fro:com}. Le coût du 
  Frobenius pour un polynome $T^{(i,j)}_{(k)}$ est de 
  $O(\frac{p_n}{p_{n-i}}(\mathsf{M}(p_{n_i})p_{n-i-1}^{1/2}+\mathsf{M}(d_i\ell_i^{n_i})p_{n-i-1}^{(\omega+1)/2}))$ opérations,
  \emph{i.e.} $O(\ell^n\mathsf{M}(\ell))$ pour chacun des $O(\ell)$ calculs.
  \`A partir des $T^{(i,j)}$ on calcule $T^{(i+1)}$ en utilisant un arbre de 
  sous-produits comme décrit dans~\cite[Lemma~10.4]{vzGJG03}. Le polynôme 
  résultant de cette opération est de degré $O(\ell^{i+1})$ et a ses 
...
\end{proof}
 ...
 l'action de $\mathsf{Gal}(\mathbb{K}_i:\mathbb{F}_{q})$ à un coût raisonnable.
