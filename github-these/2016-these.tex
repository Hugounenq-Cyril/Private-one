...
copies à corriger durant ma 4ème année) ainsi que Nicolas, Johan et Valentin. 
...
\nomenclature{$f_{E_1}$}{Application $f$ qui va de $E_1$ dans $E_1$}
...
%$j$-invariant.
Dans ces récentes applications le rapport des complexités avec le calcul 
d'isogénies est différent que celui de S.E.A..
...
Couveignes, qui fonctionnent pour presque tout $\ell$ premier différent de $p$.
...
bonnes propriétés nous est fourni d'avance), puis nous 
...
Nous présentons aussi une variante de l'algorithme qui nous permet alors de 
donner directement une borne sur $\ell$ pour résoudre le problème de l'isogénie
explicite.
...
Dans ce document, de nombreux algorithmes vont utiliser la multiplication rapide, nous notons alos: $\mathsf{M}:\mathbb{N} \rightarrow \mathbb{N}$ la \emph{fonction de multiplication} tel que le coût de la multiplication de deux polynômes de de degré $n$ définis dans $\mathbb{F}_q[x]$ coûte $M(n)$ opérations arithmétiques dans $\mathbb{F}_q$. En utilisant les résultats de \cite[§8.3]{vzGJG03} nous pouvons supposer que:
\begin{itemize}
...
\`A l'aide des méthodes de \cite{SchonhageStrassen71} et de leurs généralisations  \cite{Schonhage77} \cite{Cantor-Kaltofen91} utlisant des transformation de Fourier rapides, la multiplication de polynômes de degré $n$ appartenant à $R[x]$ pour un anneau $R$ quelconque est de \newline ${O(n\log(n) \log \log (n))}$ opérations arithmétiques dans $R$.
...
algorithmes sont des algorithmes probabilistes qui produisent un résultat 
correct. Dès que nous utilisons un algorithme de type Las Vegas alors pour la 
complexité énoncée nous parlerons de coût moyen. 
...
\begin{defi}
Une courbe elliptique $E$ est définie comme la variété projective associée à l'équation
\begin{equation}
\label{eq:weierstrass-proj}
E:Y^2Z+a_1XYZ+a_3YZ^2=X^3+a_2X^2Z+a_4XZ^2+a_6Z^3
\end{equation}
avec $a_1,a_2,a_3,a_4,a_6$ des éléments de $\overline{\mathbb{K}}$ et $(X,Y,Z) \in \mathbb{P}(\overline{\mathbb{K}})$. Dès lors que $a_1,a_2,a_3,a_4,a_6$ appartiennent à $\mathbb{K}$ alors la courbe est dite définie sur $\mathbb{K}$.
\end{defi}
...
On peut alors définir l'ensemble des points de la courbe définis sur $\mathbb{K}$ dans ce système de coordonnées affine comme:
\begin{equation*}
...
coordonnées affine des points d'une courbe elliptique.
...
sur $\mathbb{K}$ qui va de $E_1$ dans $E_2$, alors $\varphi$ est dite 
...
$O(\mathsf{M}(\deg(I))\log(\deg(I)))$ opérations sur $\mathbb{K}$ à l'aide d'un
algorithme de reconstruction rationnelle \cite[Théorème 7.5]{algeff17}.
...
L'ensemble des isogénies de $E_1$ vers $E_2$ adjoint à l'application nulle est appelé groupe des homomorphismes et est noté \emph{$\mathrm{Hom}(E_1,E_2)$}. 

Lorsque $E_1=E_2$ l'ensemble $\mathrm{Hom}(E_1,E_1)$ est appelé anneau des endomorphismes et est noté \emph{$\mathrm{End}(E_1)=\mathrm{Hom}(E_1,E_1)$}. Les éléments de $\mathrm{End}(E_1)$ inversibles sont appelés automorphismes et sont notés \emph{$\mathrm{Aut}(E_1)$}. 
...
\begin{enumerate}
\item Soit $E[p^e]=\{O_E\}$ pour tout $e \geqslant 1$ et alors $E$ est dite \emph{supersingulière},
...
\begin{defi}
Un ordre $\mathcal{O}$ dans un corps quadratique $K$ est un sous-ensemble de $K$ tel que:
\begin{enumerate}
...
L'endomorphisme de Frobenius $\pi$ relatif à $\mathbb{F}_q$ admet pour polynôme caractéristique:
\begin{equation}
\pi^2-t_{\pi}\pi+q=0
\end{equation}
...
De par la définition~\ref{def:mul:comp} toute courbe elliptique définie sur un corps fini est à multiplication complexe:
...
Dans cette sous-section une partie plus technique va être abordée. Celle-ci est
connue dans la communauté mais permettra au lecteur non initié de se 
familiariser avec la méthode employée pour déterminer un ensemble de points 
restrictifs à l'aide du Frobenius. Cette partie présente aussi l'avantage de 
montrer le contexte d'algorithmes (Elkies 1998 et BMSS2008 présentés dans le 
...
\begin{prop}
Soient $j_0 \in \mathbb{F}_q$ le $j$-invariant d'une courbe elliptique $E_0$ définie sur $\mathbb{F}_q$ de $j$-invariant $j(E) \neq 0,1728$ et  $\Phi_{\ell}(X,Y) \in \mathbb{Z}[X,Y] $ le $\ell$-ème polynôme modulaire. Les $\ell+1$ racines du polynôme $\Phi_{\ell}(X,j_0)$ sont les $j$-invariants de courbes $\ell$-isogènes à $E_0$. 
\end{prop}
...
Dans ce chapitre vont être présentés différents algorithmes pour résoudre le problème suivant: soient $E, E'$ deux courbes elliptiques définies sur $\mathbb{K}$, on cherche alors à calculer la fraction $\mathbb{K}$-rationnelle qui a pour domaine $E(\mathbb{K})$ et pour codomaine $E'(\mathbb{K})$  telle que  cette application  définisse une isogénie séparable.
...
telles que la $r$-isogénie associée à ces courbes est normalisée, on dit que ces 
...
$\mathbb{F}_q$ avec des équations de Weierstrass normalisées pour $p>2r-1$ 
...
\end{equation} 
On a alors à l'aide des sommes de Newton (somme des puissances successives des racines de $P$):$p_0=r-1,p_1, \cdots, p_{r-1}$:
\begin{equation}
\label{eq:pol:new}
...
$x+\sum_{i \geqslant 1}\frac{d_i}{x^i}$). En effet on peut déterminer les sommes
...
On définit alors l'interpolation de Cauchy qui va nous permettre de faire un 
...
sur les ordres d'un corps quadratique imaginaire car, comme vu précédemment 
...
Soit $K$ un corps quadratique, alors il existe un entier $N$ sans facteur carré tel que l'on peut écrire $K=\mathbb{Q}[\sqrt{N}]$. On définit alors alors le discriminant de $K$:
\begin{defi}
\label{def:dis:dK}
...
\begin{prop}
Soit $K$ un corps quadratique imaginaire, $\mathcal{O}_K$ l'anneau des entiers 
de $K$ alors $\mathcal{O}_K$ est l'ordre maximal de $K$.
\end{prop}
...
laissée dans le calcul de complexité afin de montrer l'influence de la boucle 
entre les étapes \ref{alg:cou:atk:div:h} à ~\ref{alg:cou:atk:test} et de 
pouvoir énoncer le corollaire suivant:

Nous pouvons énoncer un résultat particulier intéressant pour les volcans de 
Atkin de hauteur $0$.
...
\begin{cor}
  ...
  l'algorithme~\ref{alg:cou:atk} calcule une $r$-isogénie 
  ${\phi:E \rightarrow E'}$ avec un coût moyen de \[
  O(r \ell^2 (\mathsf{M}(r\ell^4)\log(r)\log(\ell)+\mathsf{M}(\sqrt{r}\ell^{1,5})\sqrt{r}\log(\ell)\log(\ell q))\]
auquel il faut ajouter un temps de pré-calcul de:
$O(r \mathsf{M}(\sqrt{r})\log(r)\log(\ell)+ \sqrt{r} \mathsf{M}(\sqrt{r})\log(\ell)\log(\ell q))$.
\end{cor}
...
l'analyse mais pour des applications cela peut rajouter un facteur non 
négligeable.
...
Nous avons donc vu deux cas où l'on avait un algorithme de Couveignes 
...
\begin{proof}
L'idée de la preuve est de montrer que l'on peut trouver un premier $\ell$ de
...
corollaire~\ref{cor:atk:full-complexity:h=0}. Les nombres de Atkin et de Elkies
de hauteur $h=0$ n'étant pas des diviseurs de $d_K$ et $d_K$ étant borné par 
$O(q)$ car $d_{\pi}$ l'est alors par un raisonnement similaire à celui de la 
preuve de la proposition~\ref{pro:bor:ell} on a le résultat. 
\end{proof}
...
dans des cas particuliers de nombre de Elkies et d'Atkin qui sont fréquents. 
