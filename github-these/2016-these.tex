...
\end{equation*}
On va par ailleurs noter $h=v_{\ell}(\lambda - \mu)$, ainsi on aura le polynôme caractéristique qui se scinde modulo une $\ell^k$ torsion avec $k \geqslant h$, $h$ représente ici la hauteur du volcan, car on rappelle que l'on a $h=\frac{v_{\ell}(d_{\pi})-v_{\ell}d_K}{2}$ (voir section \ref{sec:ann:end}).

Comme on travaille sur l'action du Frobenius sur la $\ell$-torsion sur une courbe elliptique alors on réprésente celle ci à l'aide d'une matrice $2 \times 2$. On présente tout d'abord un résultat intermédiaire sur la représentation d'un application sur la $\ell$-torsion et se représentation à conjugaison près. 
