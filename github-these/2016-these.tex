...
\todo{Ta thèse ne parle pas de crypto}
La cryptographie est la science de coder les informations afin de ne pas les
rendre intelligibles par toute personne non destinataire du message, tout en 
...
L'une des premières étapes à faire lorsque l'on veut utiliser une courbe
...
taille du corps de définition de la courbe 
elliptique. Cette méthode repose principalement sur la détermination de la 
trace de l'endomorphisme de Frobenius modulo un nombre premier $r$. Ceci est fait en déterminant 
$t_{\pi}$ dans l'équation suivante obtenue à l'aide du polynôme 
caractéristique du Frobenius:
\begin{equation}
x^2-t_{\pi}x+q=0 \bmod r.
\end{equation}
\todo{L'équation n'apporte aucune information supplémentaire au lecteur}
...
\todo{Si tu utilises le style de citation long (pas vraiment typique du domaine),
  il est inutile de donner le nom des auteurs avant la citation.
  Attention: tu as configuré ton bibtex en anglais: il ajoute des ``and'' entre les noms.}
avec des sous-groupes de la $r$-torsion lorsque le Frobenius admet deux 
espaces propres (cas Elkies) \todo{et Atkin?}.
...
\todo{Qu'est-ce que le polynôme modulaire?}
$\tilde{O}(r^3)$ (par \cite[Algorithm 6.1]{BLS12}),
\todo{Un peu rude que de parler déjà de complexité}
ainsi qu'une isogénie de
\todo{Qu'est-ce qu'une isogénie?}
...
\cite{Elkies1998}; celui-ci se limite au cas des corps de caractéristique 
suffisamment grande.
\todo{Puisque tu présentes en ordre historique, ne saute pas Couveignes ici.
  N'oublie pas Couveignes '94}
Ce travail a été ensuite amélioré par les travaux de 
Bostan, Morain, Salvy, et Schost \cite{BMSS08} puis par ceux de Lercier et Sirvent
...
Le désavantage avec toutes ces méthodes c'est qu'elles supposent la 
...
\cite{Teske06} sur un schéma
\todo{schéma de quoi?}
à trappe, les travaux de cryptanalyse de \cite{MaurerMenezesTeske01}, 
...
d'isogénies de degré $r$ à l'aide de la $p$-torsion \cite{Couveignes96}, avec 
$p$ la caractéristique du corps sur lequel on travaille,
\todo{``L'aide de la $p$-torsion'': pas clair ce que tu veux dire.
  Il est plus simple d'introduire Couveignes comme un algorithme d'interpolation.}
car celui-ci ne suppose pas la connaissance de
$\Phi_r$. Ce travail de Couveignes fût amélioré par la suite par De Feo  \cite{DeFeo11},  qui se sert notamment
d'une construction efficace d'extensions $p$-adiques de corps finis, développée conjointement avec Schost \cite{DeFeo-Shost'12}. 
...
points de $\ell$-torsion, dans le cas de courbes ordinaires, avec $\ell \neq p$.
\todo{Vaut mieux dire ici que Couveignes a une limitation que tu souhaites enlever.}
Pour cela on doit notamment 
déterminer des invariants de
\todo{Qu'est-ce qu'un invariant?}
l'isogénie de degré $r$ que l'on veut calculer, tout en se servant d'une 
construction efficace de tours d'extensions $\ell$-adiques.
\todo{Qu'est-ce qu'une tour $\ell$-adique?}

Pour le calcul d'invariants de la $r$-isogénie on se sert de graphes d'isogénies, 
étudiés (dans le cas de courbes ordinaires) pour la première fois par \cite{Kohel96}, afin de calculer l'anneau 
d'endomorphismes d'une courbe elliptique.
\todo{Il n'est pas clair si la finalité est la tienne, ou celle de Kohel}
Fouquet et Morain ont par la suite 
utilisé ces graphes, appelés \emph{volcans d'isogénies}, afin 
d'améliorer S.E.A. Enfin les travaux de 
Miret et al. \cite{MiretMRV05} \cite{MiretMSTV06} \cite{MiretMSTV08} ont permis
d'expliciter un lien entre le niveau d'une courbe dans le volcan de $\ell$-isogénies et
la structure des $\ell$-sous-groupes de Sylow.
Les travaux de \cite{Ionica-Joux10} ont permis, eux, à l'aide de couplages,
de déterminer la direction d'isogénies dans le volcan sous certaines
\todo{Qu'est-ce que la direction?}
conditions. 
...
intéressent, tout particulièrement le calcul du Frobenius.
\todo{Quel Frobenius? Il y a matière à confusion ici.
  Est-ce vraiment la peine de faire cette précision?}
...
chapitre des algorithmes pour calculer des points générateurs
\todo{Qu'est-ce qu'un point générateur?}
de certaines 
isogénies, spécifiées par leurs directions, dans le volcan. 

Ensuite, nous présentons dans le chapitre 6 comment ces directions peuvent être utilisées
dans l'algorithme de Couveignes $\ell$-adique.
Nous présentons et analysons donc un algorithme $\ell$-adique de
Couveignes
\todo{Tu utilises d'abord un article déterminé (l'algorithme), puis indeterminé (un algorithme).
  Il y en a combien d'algorithmes de Couveignes $\ell$-adiques, alors?}
dans un cas favorable (en particulier on connaît déjà un nombre de 
