...
\FOR{$j = 1$ to~$h-e+i$}
\STATE Calcul de $P_j=[\ell^{h-e+i-j}]P, Q_j=[\ell^{h-e+i-j}]P$; \label{alg:atk:bas:niv}
\STATE Calcul de $R_j=[\alpha + x\ell^{j-1}]P_j+[\beta +y\ell^{j-1}]Q_j$ avec $\pi(P_j,R_j)=M\bmod \ell^{j}$;
...
successivement des multiplications de points définis dans $\mathbb{F}_{i-j}$ 
à $\mathbb{F}_{1}$, la multiplication par $\ell$ d'un point dans 
Ainsi à l'étape $j$ de la boucle pour,
