...
\begin{tikzpicture}
\begin{scope}
    \coordinate (Eh) at (0, 0.1) ;
   	\coordinate (E'h) at (5, 0.1);
   	\coordinate (Eb) at (0, -0.1) ;
   	\coordinate (E'b) at (5, -0.1);
   	\coordinate (E) at (0, 0) ;
   	\coordinate (E') at (5, 0);
   	\draw[dashed, thick,->] (Eh) 	to[bend left=20] node[above left] {$\pi^{-1}$} (E'h);
   	\draw[dashed, thick,->] (E'b) 	to[bend right=-20] node[below right] {$V^{-1}$} (Eb);
   	\draw (E) node{$\bullet$};
   	\draw (E) node[left]{$E$};
    \draw (E') node{$\bullet$};
...
\begin{proof}
Le cout de calcul de la $p$-torsion nécessite tout d'abord, d'après les formules de Gunji de calculer $c$ et $c'$ racines $p-1$ ème de $H_{E}$ et $H_{E'}$. On doit ensuite construire les corps $\mathbb{F}_q[c]$ et $\mathbb{F}_q[c']$, dans le pire des cas $\mathbb{F}_q[c]$ et $\mathbb{F}_q[c']$ sont des extensions de degré $p-1$ ainsi on aura besoin jusqu'à $p-1$ éléments de $\mathbb{F}_q$ pour représenter des éléments de $\mathbb{F}_q[c]$ et $\mathbb{F}_q[c']$. Le coût le plus important des formules de Gunji étant le calcul des déterminants d'une matrice $\frac{p-1}{2} \times \frac{p-1}{2}$ avec quatres diagonales non nulles cela a donc un coût de $\theta(p^2)$ opérations sur $\mathbb{F}_q[c]$ par élimination de Gauss ce qui a un coût de $\Omega(p^3d)$ opérations sur $\mathbb{F}_p$.
\end{proof}
