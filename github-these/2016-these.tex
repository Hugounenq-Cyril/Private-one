...
On aborde ici une partie essentielle à la réussite de l'algorithme de calcul d'isogénies expliqué (\textcolor{red}{ref à mettre plus tard}), le calcul du Frobenius. Rappelons ici l'objectif d'une telle opération: soit $\gamma $ un élément de $\mathsf{F}_k$ et soit $s=q^d$ avec $d$ un entier quelconque, on veut alors calculer $\gamma^s$ dans $\mathsf{F}_k$. Pour plus de lisibilité on pose $2^k=n$.


On commence par le cas le plus simple: $\gamma=x_k$, ainsi en réécrivant $s=nu+v$, le fait que l'on ait $x_k^n=\alpha $ nous permet d'obtenir $x_k^s=\alpha^ux_k^v$. De plus comme $\alpha$ est un élément de $\mathbb{F}_q$ on obtient $\alpha^u=\alpha^{u \bmod q-1}$, cette opération est donc effectuée en $O(\log(q))$ opérations dans $\mathbb{F}_q$ à l'aide de l'exponentiation rapide. Une méthode rapide est préconisée pour le calcul de $u \bmod |\mathsf{F}_0|-1$ et $v$, dans un premier temps il faut calculer $\rho=r \bmod \ell^k(|\mathsf{F}_0|-1)$ à l'aide de l'exponentiation rapide puis $u \bmod |\mathsf{F}_0|-1$ et $v$ sont obtenus comme le quotient et le reste de la division euclidienne de $\rho$ par $\ell^k$. Le coût booléen d'une telle opération est de $O(\log(d)\log(nq))$.
...
Soit $\delta$ un carré non nul de $\mathsf{F}_k$ ayant pour racine carrée $\gamma$ (non connue a priori). Soit $\beta$ dans $\mathbb{F}_q$ défini de la manière suivante:
\begin{align}
...
&=\gamma(1+\delta^{(q-1)/2}+\cdots+ \delta^{(q^{n-1}-1)/2}) \\
&=\gamma \eta
\end{align}
en posant 
\begin{align*}
\eta=1+\delta^{(q-1)/2}+\cdots+ \delta^{(q^{n-1}-1)/2}
\end{align*}
...
Maintenant si l'on met au carré l'équation \ref{eq:trace-square-root} on obtient alors l'équation suivante dans $\mathbb{F}_q$: 
\begin{equation*}
\beta^2=\delta \eta^2. 
\end{equation*}
Dés lors si $\eta$ est connu, $\beta$ peut être déduit depuis cette équation et $\gamma$ calculé car on a alors $\gamma=\beta \eta^{-1}$.
...
Soit $\lambda$ un élément de $\mathsf{F}_k$ défini par $\lambda = \delta^{(q-1)/2} $ on a alors la relation suivante avec $\eta$:
\begin{equation*}
