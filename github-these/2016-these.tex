...
\begin{prop}
L'algorithme \ref{alg:bas:tri} calcule une base triangulaire de $E[\ell^k]$ avec une complexité de $O(\mathsf{R}(k) + \ell^2\mathsf{M}(\ell^{k}) + \ell\mathsf{M}(\ell^2)\log(\ell)\log(\ell
...
opérations en utilisant l'algorithme de Cantor-Zassenhaus.
  ...
  opérations.
  L'évaluation de~$\pi(P')$ à l'étape~\ref{alg:trigonal:frobenius} a un cout
  de~$O(\ell^{i}\mathsf{M}(\ell))$ par le théoèreme \ref{thm:frob-ell}.
...
\`A partir d'une base de la $\ell^k$ torsion pour une courbe $E$ située en dessous du cratère on peut se servir de celle-ci pour calculer une base diagonale de la $\ell^k$ torsion pour la courbe "sommet nom à trouver" \todo{Définir cela!} $Ec$  à l'aide de l'algorithme suivant:
...
\STATE\label{alg:rem:iso}  $\varphi \leftarrow $ isogénie de noyau $\langle \ell^{k-1}P \rangle$
...
\begin{prop}
Soit $E$ une courbe au niveau $h-e$ dans le volcan alors L'algorithme \ref{alg:rem:tri} calcule une base diagonale de $E_c[\ell^k]$ avec une complexité de $O( \mathsf{R}(k+\beta+e) + \ell^2\mathsf{M}(\ell^{k+\beta+e}))$ oéprations sur $\mathbb{F}_q$.
\end{prop}
...
alors le cout de $\mathsf{divise}$ est de $O(\mathsf{R}(k+\beta+e-i))$
  opérations.
  ...
  plus~$\ell$ additions de points, avec un cout de~$\ell
  ...
  Comme le cout de chaque boucle est géométrique
  la première boucle domine la complexité et donne le résultat.
  \todo{oubli cout du calcul de l'isogénie et de son application}
\end{proof}

Le résultat énoncé précédemment est valide pour les réguliers, pour les volcans non réguliers on a le résultat suivant:

\begin{prop}
Soit $E$ une courbe au niveau $h-e$, situé au-dessus du premier niveau de stabilité dans le volcan alors L'algorithme \ref{alg:rem:tri} calcule une base diagonale de $E_c[\ell^k]$ avec une complexité de $O(e( \mathsf{R}(k-\beta) + \ell^2\mathsf{M}(\ell^{k-\beta})) )$ oéprations sur $\mathbb{F}_q$.
\end{prop}
...
est calculée en factorisant le polynome de $\ell$-division
à un cout espéré \todo{vérifier orthographe cout, coût} de $O(\ell\mathsf{M}(\ell^2)\log(\ell)\log(\ell q))$
opérations en utilisant l'algorithme de Cantor-Zassenhaus.
  Une fois que $E[\ell]$ a été calculé,on factorise la
  multiplication-par-$\ell$ comme un produit de deux $\ell$-isogénies. Ainsi
  fpour tout $P$ défini dans $E(\mathsf{F}_{i-\beta})$, le calcul de
  $\mathsf{divise}(\ell, P)$ à l'étape~\ref{alg:diagonal:divide} coûte $O(\mathsf{R}(i-\beta+1))$
  opérations.
  L'évaluation de~$\pi(P')$ à l'étape~\ref{alg:diagonal:frobenius} a un cout
  de~$O(\ell^{i-\beta}\mathsf{M}(\ell))$.
  Ecrire~$\pi(P')$ comme une combinaison linéraire~$[u] P' + [v] Q'$ nécessite au
  moins~$\ell^2$ additions de points, avec un cout de ~$\ell^2
  \mathsf{M}(\ell^{i-\beta+1})$.
  Le cout de résolution des équations aux étapes~\ref{alg:diagonal:solve1}
  et~\ref{alg:diagonal:solve2} par recherche exhaustive est négligeable, comme
  les autres opérations restantes. Comme le cout de chaque boucle augmente
...
\begin{prop}\label{pro:alg:hor}
  ...
  avec un cout espéré de $O(\mathsf{R}(k-\beta) + k\mathsf{R}(h-\beta+1) + k\ell^2\mathsf{M}(\ell^{h-\beta+1}))$
  opérations dans $\mathbb{F}_q$.
\end{prop}
\begin{proof}
Soit $E_i$ le codomaine de $\psi_i$.
On check that at step~$i$ of the loop,
the points~$(P_i, Q_i)$ form a diagonal basis of~$E_i[\ell^{h+1}]$,
and $\psi_i$~has direction~$\lambda$.
Le fait que $R$~est horizontal est une conséquence
de la Proposition~\ref{pro:pus:hor}.
Les deux opérations les plus couteuses des boucles sont les
...
\begin{prop}
\label{pro:par:hor}
Soit $E$ une courbe située au niveau $h-e$ dans un volcan de $\ell$-isogénies soit $E_s$ sa courbe sommet dont on connaît un point horizontal $P_h$ d'ordre $\ell^k$, soit $\varphi_s:Es \ rightarrow E $ la $\ell^{e}$ isogénie descendante telle que l'on connaît une décomposition en $e$ $\ell$ isogénies descendantes: $\varphi=\varphi_1 \circ \varphi_{e}$ alors le coût de calculer $\varphi(P)$ est de $O(e\mathsf{M}(\ell^{k-\beta+1}))$ opérations sur $\mathbb{F}_q$.
Faire une proposition sur le cout d'évaluer un point partiellement horizontal.
\end{prop}

\begin{proof}
On rappelle que pour $P \in \mathsf{F}_{k-\beta}$ point horizontal alors on a  $\varphi(P) \in \mathsf{F}_{k-\beta}$ si le codomain. Le cout de l'évaluation d'une isogénie rationnelle de degré $\ell$ sur un point $P \in \mathsf{F}_{k-\beta}$ étant de $O(\mathsf{M}(\ell^{k-\beta+1}))$ opérations sur $\mathbb{F}_q$ on obtient le résultat.
\end{proof}

\begin{rem}
Il est normal que dans le coût de la proposition \ref{pro:par:hor} on ne prenne pas en compe le cout de calcul des isogénies descendants car si l'on veut calculer un point partiellement horizontal pour une coure non située sur le cratère alors il est raisonnable de penser que les algorithmes \ref{alg:bas:tri} et \ref{alg:rem:tri} ont été utilisé pour obtenir une base diagonale pour la courbe sommet de $E$, et ainsi au passage on a calculé les duales des isogénies dont on se sert dans l'algorithme \ref{alg:rem:tri}.
\end{rem}
...
\STATE\label{alg:identical:divide}
  $P' \leftarrow \mathsf{divise}(\ell, P_{i})$; $Q' \leftarrow \mathsf{divise} (\ell, Q_{i})$.
