...
\todo{reste à faire...}
\begin{defi}
Degré de séparabilité et degré d'inséparabilité.
\end{defi}
...
$[\mathbb{K}(E_1):\varphi^*(\mathbb{K}(E_2))]$ et l'on a $\deg{\varphi}=
...
\begin{rem}
On peut calculer l'isogénie à l'aide de la connaissance de son noyau. Une méthode pratique reste celle préconisée par Elkies~\cite{Elkies1998} et \cite[Section 2.4]{Kohel96} \todo{verifier reference} qui utilise les équations \eqref{eq:velu:gh} et \eqref{eq:velu:sum}:
\begin{equation*}
...
\end{equation}
avec $p_1$ la somme des abscisses des points du noyau. Ainsi à partir de $h(x)$ on est capable de calculer l'isogénie en $\mathsf{M}(\deg(I))$ opérations sur $\mathbb{K}$. \todo{Justifier tout cela...}
\end{rem}
...
\begin{defi}
Définir un ordre... \todo{definir un ordre}
\end{defi}
...
Le principal problème avec cette approche est que l'on travaille avec la $\ell$-torsion qui lorsque l'on a $\ell \neq p$ est de la forme $E[\ell] \simeq \mathbb{Z}/\ell \mathbb{Z} \times \mathbb{Z}/\ell \mathbb{Z}$. On doit alors travailler modulo le polynôme de $\ell$-division (le polynôme unitaire qui s'annule sur les abscisses des points de $\ell$-torsion) qui est de degré $(\ell^2-1)/2$. L'objectif de l'amélioration d'Elkies \todo{mettre la bonne référence ici...} est alors de considérer des sous-groupes de la $\ell$-torsion afin de diminuer le coût de l'algorithme.
...
\begin{equation}
\label{eq:rec:h_k}
\forall k \geqslant 3, h_k=\frac{3}{(k-2)(2k+3)}\sum_{i=1}^{k-2}h_ih_{k-1-i}-\frac{2k-3}{2k+3}Ah_{k-2}-\frac{2(k-3)}{2k+3}Bh_{k-3} \; 
\end{equation}
\todo{notation des $h_i$}
...
Comme on a $p \gg r$ on peut alors aussi appliquer la remarque suivante. \todo{verifier cela...}
