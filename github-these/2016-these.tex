...
On utilise alors l'homomorphisme induit de groupes finis $G'(\mathbb{F}_q) \to G(\mathbb{F}_q)$, si il est non surjectif alors il existe des points de $G(\mathbb{F}_q)$ avec leur fibrés définis dans une extension algébrique de $\mathbb{F}_q$. Ainsi si l'on est capable de déterminer un homomorphisme $\phi$ de telle manière que l'on puisse trouver un point contenu dans $U$, avec un fibre irréductible et ensuite appliquer une projection linéaire sur cette fibre, on détermine alors un polynôme irreductible de degré divisant $\ell$ (et espéré égal à $\ell$). Ainsi si l'on répète cette construction avec un nouvel homomorphisme $\phi':G''' \to G''$, et ainsi de suite, alors avec cette suite on est capable de construire des tours $\ell$-adiques sur $\mathbb{F}_q$. 
