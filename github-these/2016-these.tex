...
Lors de la boucle $h-e+i$, les étapes \ref{alg:ult:trigonal:solve1} et \ref{alg:ult:rem:solve1} sont résolues à l'aide d'une recherche exhaustive et ont un cout négligeable par rapport aux autres opérations. Lors de la boucle $h-e+i$ les points $P_{h-e+i+1}$ et $Q_{h-e+i+1}$ sont d'ordre $\ell^{h-e+i+1}$ et définis dans $\mathsf{F}_{h-e+i+1-\beta}$. En particulier par la structure des volcans réguliers (\proposition ref{pro:niv:str}) les antécédents de $P_{h-e+i+1}$ d'ordre $\ell^{h+1}$ sont définis dans  $ \mathsf{F}_{h-e-\beta}$ ainsi le coût de l'étape \ref{alg:ult:rem:divide} est majoré par le coût de l'étape  de $R(h-e-\beta+1)$, pour $P \in \mathsf{F}_{h-e-\beta+1}$ le coût de calcul de $\pi(P)$ est de $O(\ell^{h-e-\beta}M(\ell))$. L'écriture de $\pi(P)=[u]P+[v]Q$ à l'étape \ref{alg:ult:rem:frobenius}  a un coût de de $O(\ell M(\ell^{h-e-\beta}+1))$. 
\end{proof}
