...
\end{tikzpicture}
\end{center}
\caption{ \label{fig:poi:hor} Exemples de  différents points diagonaux $P \in E$ d'ordre $2^3$ selon la valeur propre $\color{blue}{\mu}$, ici sont représentées les différentes isogénies de noyau $\langle P \rangle$ }
\end{figure}
Sur le schéma~\ref{fig:poi:hor} sont représentés les isogénies qui ont pour noyau différents points $P \in E $ diagonaux d'ordre $2^3$ tels que $\pi(P)=[\mu]P$. On remarque tout d'abord que


En particulier il est plus facile de calculer une base diagonale qu'une base horizontale. Le résultat suivant nous permet de construire des bases horizontales de taille aussi grande que la $\ell$-torsion définie sur le corps sur lequel on travaille sans que l'on ait à travailler dans des extensions de corps supplémentaires.
