...
\begin{prop}
Soit $E$ une courbe elliptique définie sur $\mathbb{F}_q$, on note alors par $\mathbb{U}_i$ la plus petite extension de corps de $\mathbb{F}_q$ dans laquelle on a $E[p^i] \subset E(\mathbb{U}_i)$. Pour tout $i \geqslant 1$ on a soit $[\mathbb{U}_{i+1}:\mathbb{U}_{i}]=p$ soit $\mathbb{U}_{i+1}=\mathbb{U}_{i}=\cdots = \mathbb{U}_{1}$.
\end{prop}
...
Ce résultat montre donc que pour toute courbe elliptique il existe un $i_0$ tel que l'on ait $[\mathbb{U}_i:\mathbb{U}_1]=p^{i-i_0}$ pour tout $i \geqslant i_0$. Ainsi asymptotiquement on aura $[\mathbb{U}_k:\mathbb{U}_1] \in O(p^k)$
...
Enfin à l'aide de ces pré-calculs on effectue la résolution de l'équation d'Artin Schreier à l'aide de $O(\ell^2 )$ opérations sur $\mathbb{F}_q$, le coût de l'inversion de l'application $\mathbb{F}_q$ linéaire $x \rightarrow x^p - x$ étant négligeable, pour plus de détails le lecteur peut voir \cite[§2.4]{Couveignes96}.
\end{proof}

\paragraph{Interpolation de Cauchy} L'interpolation de Cauchy se fait en deux étapes: tout d'abord il faut calculer un polynôme d'interpolation $A$ d'après la donnée de valeurs prises en les points primitifs de $p^k$-torsion. Ensuite on doit reconstruire à l'aide de l'algorithme d'Euclide une fraction rationnelle $\frac{R}{V}$ égale à $A$ modulo le polynôme unitaire qui s'annule l'es abscisses des points de $p^k$ torsion primtifs. 

Dans l'algorithme de Couveignes \cite{Couveignes96} l'interpolation est effectué distinctement sur les différentes orbites sous l'action du Frobenius puis en utilisant le théorème des restes chinois on obtient alors un polynôme d'interpolation modulo la $p^k$-torsion primitive.

En effet on a l'ensemble des abscisses des point primitifs de $p^k$-torsion qui est de taille $p^k(p-1)/2$. La taille des orbites sous l'action du Frobenius est quant à elle égale à $[\mathbb{U}_k:\mathbb{F}_q]$. Ainsi si l'on note $f$ la taille de ses orbites et $d$ le nombre de représentants on a $d \times f=p^k(p-1)/2$.
...
A nouveau le détail des calculs de complexité peuvent être trouvés dans \cite{defeo2011}, il est à noter que la complexité obtenue pour résoudre \eqref{eq:vol:cov} est faite à l'aide de techniques spécifiques à la construction de tours $p$-adique, plus de détails peuvent être trouvés dans \cite{DeFeo-Shost'12}.
...
L'apport de l'utilisation de tour d'Artin Schreier \cite{DeFeo-Shost'12} est minime sur cette étape qui ne cahnge que très peu de l'original. On commence donc par  calculer un polynôme d'interpolation de degré $p^{k-1}(p-1)-1$ à coeffcients dans $\mathbb{U}_k$ on utilise alors un push-down \cite[push-down]{DeFeo-Shost'12} pour avoir un polynôme de $\mathbb{F}_q[x]$ et ensuite utiliser une reconstruction rationnelle.
