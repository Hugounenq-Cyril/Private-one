...
$\upsilon_n$ construit à l'aide de points d'ordre $\ell_i^{k_i}$ représentés 
...
$\prod_{j=1}^{i-1}d_j\ell_j^{n_j} \geqslant d_i\ell_i^{n_i}$, cette restriction
...
\begin{nota} Afin de simplifier les notations on va 
 noter $m_i=\prod_{i=j}^nd_j\ell_j^{n_j}/d_i\ell_i^{n_i}$ et \todo{vérifier notation de $m_i$} 
...
évaluer un tel coût est le \cite[Theorem 1]{BostanFlajoletSalvySchost06}.
...
\label{sss:crt:cou}
Nous avons abordé dans la sous-section~\ref{sub:TRCE:cou} l'intérêt 
...
sous-section~\ref{ssec:cou:elk} on a besoin du résultat qui nous permet de 
déterminer les représentants d'orbites sous l'action de groupes de Galois en 
rappelant que l'on note $\mathbb{K}_n=\otimes_{i=1}^{n}\mathsf{F}_{(i,k_i)}$ le
compositum qui contient tous les éléments $\mathsf{F}_{(i,k_i)}$ des tours 
d'extensions $\ell_i$-adiques construit d'après les restrictions énoncées dans 
la sous-section~\ref{sub:con:com}.
\begin{prop}
\label{pro:rep:com}
...
par rapport à l'action de $\mathsf{Gal}(\mathsf{F}_{(i,n_i)}:\mathbb{F}_q)$
 ...
 On va présente donc l'algorithme qui résulte de cette approche ainsi qu'une 
...
l'interpolation (étape~\ref{alg:cou:TRC-adique:int}) on estime les coûts du
Frobenius et du calcul de polynômes d'interpolation. 
...
\begin{proof}
Voir la preuve en annexe~\ref{cha:ann:comp}. \todo{En fait non pas de preuve du tout...}
\end{proof}
...
On obtient donc un résultant moins bon que celui du
théorème~\ref{thm:frob-ell}, or en adaptant les mêmes méthodes on aurait du 
avoir le coût de cette opération dominé par $O(d_i\ell_i^{k_i})$. Cependant 
ici, contrairement au cas du théorème~\ref{thm:frob-ell}, on doit prendre en 
...
section~\ref{sec:cou:elk} d'avoir une construction de compositum permettant un 
...
polynôme minimal $T=T^{(n)}$(en reprenant une notation similaire) d'élément de 
$\mathbb{K}_n \setminus \mathbb{K}_{n-1}$.
Ainsi comme dans la méthode développée dans la section~\ref{sec:interpolation} 
...
multiplication n'est que de $\tilde{O}(\mathsf{M}(p_n))$, ce qui en comparant 
avec les résultats du lemme~\ref{lemma:interpolation:minpoly} devrait être le 
coût dominant. Cependant comme dit précédemment le Frobenius n'a pas un coût 
similaire du fait des changements de représentation qui domine son coût. Ainsi 
l'application du Frobenius aux polynômes $T^{(i)}_k$ a un coût de $O(\mathsf{M}
(p_n)p_{n-i-1}^{(\omega-1)/2})$, or on doit appliquer le Frobenius 
$\ell_{n-i}(n_{n-i}+1)$ fois et donc on minore ce coût par $\Omega(\mathsf{M}
(p_n)p_{n-1}^{(\omega-1)/2}\max_i(\ell_{i}(n_{i}+1)))$.
...
encore une fois dominé par les Frobenius et on minore le coût d'un tel calcul
par $\Omega(\mathsf{M}(p_n \max_i(d_i\ell_i^{n_i}))\max_i(\ell_i)p_{n-1}^{(\omega-1)/2})$.

Ensuite pour $(v_1,w_1),\dots,(v_s,w_s)$ des paires d'éléments de $\mathbb{K}_n~\setminus~\mathbb{K}_{n-1}$, $t_i$ le degré des polynômes minimaux de $v_i$, on note  $t=\sum t_i$. 
On veut alors calculer les polynômes
  ...
  \item $A\in \mathbb{F}_q[x]$ de degré inférieur à $t$ tel que $A(v_i)=w_i$ pour
    tout $i$.
  \end{itemize}
On utilise à nouveau la même méthode que celle utilisée dans la 
section~\ref{sec:interpolation}, le calcul de $T$ à partir des $T_i$ a un même
coût que dans la proposition~\ref{prop:interpol} de $O(\mathsf{M}(t)\log(s))$. 
Cependant comme on a vu précédemment le coût de calcul des $T_i$ est de $\Omega(\mathsf{M}(t_i)(p_{n-1})^{(\omega-1)/2}\max_i(\ell_i))$ on majore donc le cout total par $\Omega(\mathsf{M}(t)(p_{n-1})^{(\omega-1)/2}\max_i(\ell_i))$ par superlinéarité de $\mathsf{M}$.

On peut donc minorer le coût de calcul de la partie de l'interpolation de cette
version $\ell$-adique de l'algorithme de Couveignes. On répète l'interpolation
$O(r \prod_{i=1}^n)\ell_i)$ en moyenne. On peut donc minorer le coût de cet 
algorithme par $\Omega(r (\prod_{i=1}^n)\ell_i)\Omega(\mathsf{M}(r \prod_{i=1}^n \ell_i^2)(p_{n-1})^{(\omega-1)/2}\max_i(\ell_i)) )$
