...
h(x)=\prod_{Q \in G \setminus \{0_E\}}(x-x(Q)),
\end{equation*}
avec $G=\ker(I)$ et $g$ un polynôme. On rappelle qu'en plus à l'aide de la reformulation d'Elkies on a:
\begin{equation}
\frac{g(x)}{h(x)}= \deg(I) x -p_1 - f'(x) \frac{h'(x)}{h(x)} -2f(x)  \left(\frac{h'(x)}{h(x)}\right)'
\end{equation}
avec $p_1$ la somme des abscisses des points du noyau que l'on peut lire sur le coefficient de $x^{\ell-2}$ dans $h$. Ainsi à partir de $h(x)$ on est capable de calculer l'isogénie en $\mathsf{M}(\deg(I))$ opérations sur $\mathbb{F}_q$. On va donc expliciter des algorithmes dans le reste de ce document qui calculent $h$.
...
L'idée de Elkies est d'à partir des équations de Weierstrass de $E:$
\begin{equation}
y^2=x^3+Ax+B
\end{equation}
 et $\tilde{E}:$
...
\begin{equation}
\frac{g(x)}{h(x)}=x+ \sum_{i\geqslant 1} \frac{r_i}{x^i}
\end{equation}
...
\begin{equation}
r_k=\frac{3}{(k-2)(2k+3)}\sum_{i=1}^{k-2}r_ir_{k-1-i}\frac{2k-3}{2k+3}Ar_{k-2}-\frac{2(k-3)}{2k+3}Br_{k-3} \quad \text{pour tout} k \geqslant 3
\end{equation}
, les termes $r_1,r_2$ étant déterminés par:
\begin{equation}
r_1=\frac{A-\tilde{A}}{5} \quad r_2={\frac{B-\tilde{B}}{7}}.
\end{equation}
Ainsi, à l'aide de cette récurrence, le coût de calcul de $r_3, \cdots , r_{\ell-2}$ est de $O(\ell^2)$ opérations sur $\mathbb{F}_q$. Ensuite à l'aide de récurrence sur les $r_i$ on peut calculer $h(x)$ en $O(\ell^2)$ opérations sur $\mathbb{F}_q$ voire en $O(\mathsf{M}(n))$ opérations sur $\mathbb{F}_q$ voir \cite{BMSS08} pour plus de détails. Il est à noter que la condition $p >> \ell$ est là pour nous assurer que $2, \cdots, 2\ell-1$ sont inversibles dans $\mathbb{F}_q$. 
\section{BMSS}
