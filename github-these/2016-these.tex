...
\begin{proof}
\todo{rédiger cette preuve}
Par la proposition~\ref{pro:elk:par:hor} on peut construire $P$ et $P'$ à partir de $R \in E_S$ et $R' \in E'_s$ des antécédents pas $[\ell^{e}]$ des points horizontaux $P_s \in E_s$, $P'_s \in E'_s$.
\end{proof}
...
Ainsi en pratique on est obligé de travailler avec les points d'ordre $\ell^{h+i}$ avec $i>0$, cette précédente discussion justifie l'utilité de se concentrer sur les points d'ordre $\ell^{k}$ avec $k \geqslant h+1$ plutôt que sur la $\ell^k$ torsion dans le contexte de l'algorithme de Couveignes~\cite{Couveignes96}.
...
\section{Approche multi-moduaire de l'algorithme de Couveignes $\ell$-adique avec l'ensemble de points (partiellement) horizontaux isomorphe à $\prod{i=1}^n \left( \mathbb{Z}/\ell_i^{k_i} \mathbb{Z} \right)^{2}$}
