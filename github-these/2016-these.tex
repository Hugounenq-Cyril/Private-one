...
cette égalité on déduit avec $c \wedge \ell = 1$ et de l'invariance de la trace
et du déterminant du Frobenius que $[\ell^{h}]R=[\ell^{h}]R'$ ce qui permet de 
conclure pour ce sens de l'équivalence.
...
[\ell^{h+e}]R=[\ell^{h+e}]R'$.
\end{rem}
\todo{choisir entre la remarque et la proposition}
...
\begin{lem}\todo{lemme et propotisition intéressantes mais inexploitées...}
\label{lem:fro:atk}
...
Par définition de ~$k$, on a ~$\ell^{2k-1} \in O(r\ell^2)$.
  ...
  $O(\sqrt{r}\mathsf{M}(\sqrt{r}\ell^{1,5})\log(r)\log(\ell)\log(\ell q))$. \todo{ou ecrire $\sqrt{\ell^5}$}
...
& r \ell (\mathsf{M}(r \log(q)^4) \log(r) \log(\log(q)) + \mathsf{M}(r \log(q)^3) \log(r)^2 \log(\log(q)) + \log(q)^6 )  \Bigr)
\end{alignedat}{1}
 \end{equation*}
%  \[
...
$r$-isogènes, avec $r \wedge \ell=1$ qui se situent au niveau d'un cratère d'un
volcan de $\ell$-isogénies de hauteur $h$, réduit à une courbe car on se place 
dans le cas Atkin. On verra par la suite comment généraliser cela
à des courbes non situées sur le cratère. \todo{Le faire!}
...
\begin{prop}
  ...
  En supposant que l'on a $\ell^h<\sqrt{r}$ \todo{trouver similaire}, 
...
On fait encore remarquer que la condition $\ell^h < \sqrt{r}$ signifie que l'on
a $k>h$.
...
Comme dit précédemment le résultat de la proposition~\ref{pro:bor:ell} est 
aussi valable dans le cas Atkin, on peut donc trouver un $\ell \in O(\log(q))$
nombre premier de Atkin \todo{regarder si c'est défini} tel que 
$\ell^h<\sqrt{r}$. 
...
\section{Variante de l'algorihme de Couveignes $\ell$-adique dans le cas Elkies avec un seul sous-groupe cyclique}
...
en appendice afin de ne pas surcharger ce document car comme vu juste au-dessus
il est très similaire à l'algorithme~\ref{alg:cou:ell-adique}.
...
\section{Approche avec l'ensemble de points isomorphe à $\cup_{i=1}^n \mathbb{Z}/\ell_i^{k_i} \mathbb{Z} \times \mathbb{Z}/\ell_i^{k_i} \mathbb{Z}$}
...
Dans cette section sont présentés les résultats énoncés dans la section, ils 
n'ont pas été présenté dans le document car en plus d'être technique ils sont 
juste une adaptation des résultats présentés dans la section et n'utilise 
aucune nouvelle idée. L'objectif de ce chapitre est donc de déterminer dans 
la proposition~\ref{pro:app:int} le coût de
calcul d'un polynôme d'interpolation à partir d'un ensemble de points qui 
sont des représentants sous l'action de groupes de Galois, ce résultat est le 
pendant de la proposition~\ref{prop:interpol} dans le cas du compositum.
\label{cha:ann:comp}
\begin{prop}
...
\chapter{Pourquoi la méthode Atkin ne marche pas sur le cas Elkies}
...
Soit une courbe elliptique $E$ définie sur $\mathbb{F}_q$ sur un volcan de $\ell$-isogénies à cratère cyclique ($\ell$ est donc un nombre de Elkies). Soit $(P,Q)$ une base diagonale de $E[\ell^{h+i}]$, soit $R=[a]P+[b]Q$ un point d'ordre $\ell^{h+i}$ tel que:
\begin{equation}
  \label{eq:mat}
...
\end{equation}
Alors pour les transformations:
\begin{equation*}
R=[a]P+[b]Q \rightarrow R'=[*]P+[yb]Q
\end{equation*}
tels que $\pi(P,R)=\pi(P,R')$ on obtient l'équation suivante:
\[  (* - b' \cdot b^{-1} \cdot a ) \cdot (1-q)  \equiv \alpha \cdot (1-b' \cdot b^{-1}) \bmod l^h \].
...
Soit une courbe elliptique $E$ définie sur $\mathbb{F}_q$ sur un volcan de $\ell$-isogénies à cratère cyclique ($\ell$ est donc un nombre de Elkies). Soit $(P,Q)$ une base diagonale de $E[\ell^{h+i}]$, soit $R=[a]P+[b]Q$ un point d'ordre $\ell^{h+i}$, alors pour toute $\ell^i$-isogénie descendante $\phi$ de domaine $E$ il existe une transformation \begin{equation*}
R=[a]P+[b]Q \rightarrow R'=[*]P+[yb]Q
\end{equation*}
...
valeur de $\ell^{h}b^{-1}y^{-1}*$, on rappelle de plus que l'on est obligé 
...
et $h-e+i > h$ car on a $\lambda = \mu \bmod \ell^{h}$.
...
\[\beta \ell^{h-e} + y\beta \ell^{h-e}  \]
et donc la valeur de la matrice ne dépend que que de $y$ ainsi en faisant 
...
$y$ et $*$ tels que $\ell^{h-e}\gamma=\ell^{h-e}*(by)^{-1}$ et que $\pi(P,R)=\pi(P,R')$.
...
\begin{eqnarray*}
\beta \ell^{h-e}(by)^{-1} + \ell^{e}\alpha \ell^{h-e} \gamma &=& b^{-1}(\beta \ell^{h-e}+\ell^{h}(\alpha)a) \\
\beta \ell^{h-e}(by)^{-1} &=& - \ell^{e}\alpha \ell^{h-e} \gamma + b^{-1}(\beta \ell^{h-e}+\ell^{h}(\alpha)a)
\end{eqnarray*}
