...
$r$-isogènes, avec $r \wedge \ell=1$ qui se situent au sommet d'un volcan de 
$\ell$-isogénies de hauteur $h$, on verra par la suite comment généraliser cela
à des courbes non situées sur le cratère.

Soient $E$, $E'$ deux courbes elliptiques de $j$-invariants $j$ et $j'$ reliées
par une $r$-isogénie $\phi$, telles que $E$ et $E'$ se situent au sommet d'un 
volcan de $\ell$-isogénies avec cratère réduit à une courbe (on est donc dans 
le cas Atkin) alors pour $k$  plus grand que $h$ la hauteur du volcan de 
$\ell$-isogénies avec $P \in E$ et $P' \in E'$ deux points d'ordre $\ell^{k}$ 
tels que $\phi(P)=P'$ deux bases $(P,Q),(P',Q')$ de $E[\ell^k]$ et $E'[\ell^k]$
telles que $\pi(P,Q)=\pi(P',Q')$ par la proposition~\ref{pro:atk:equi} cela 
détermine de façon unique $[\ell^h]Q$ et $[\ell^{h}]Q'$ or comme $\phi$ est une
$r$-isogénie qui commute avec le Frobenius on a $\phi([\ell^h]Q)=[\ell^h]Q'$ 
comme énoncé dans le corollaire~\ref{cor:atk:ess}. On détermine alors le point 
