...
\begin{algorithmic}[1]
\REQUIRE $E,E'$: deux courbes elliptiques $r$-isogènes ordinaires situées sur le cratère d'un volcan de $\ell$-isogénies avec cratère cyclique.
...
\IF{$(E_s,E'_s) \neq (E_s,E'_s)$}
\STATE Calcul de $(\color{red}P_{\lambda}\color{black},\color{blue}Q_{\mu}\color{black})=(),(\color{red}P'_{\lambda}\color{black},\color{blue}Q_{\mu}'\color{black})=()$ bases partiellement horizontales de $E[\ell^k],E'[\ell^k]$ avec ...;
\ENDIF
\STATE{$\lambda \leftarrow \lambda + \ell^{i} (a + bx)$; $\mu \leftarrow \mu + \ell^{i} (d + cy)$.}
\RETURN $(P_{k},Q_{k},\lambda,\mu).$
\end{algorithmic}
