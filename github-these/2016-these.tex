...
\todo{Mettre référence à $\mathsf{R}$ ici!}
...
\begin{prop}
L'algorithme \ref{alg:bas:tri} calcule une base triangulaire de $E[\ell^k]$ avec une complexité de $O()$ oéprations sur $\mathbb{F}_q$.
\end{prop}

\begin{proof}
Test. \todo{Mettre référence $\mathsf{R}$}
Pour~$i = 0$, la base de~$E[\ell](F_1)$ à l'étape~\ref{alg:point:divide}
est calculée en factorisant le polynome de $\ell$-division 
avec un coût attendu de $O(\ell\MM(\ell^2)\log(\ell)\log(\ell q))$
opérations en utilisant l'algorithme de Cantor-Zassenhaus.
  ...
  opérations.
  L'évaluation de~$\pi(P')$ in Step~\ref{alg:trigonal:frobenius} a un cout
