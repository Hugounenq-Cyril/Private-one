...
\end{equation*}
celui-ci se scinde modulo $\ell$. En effet comme $\ell \wedge q = 1$ on a le produit des valeurs propres qui n'est pas divisible par $\ell$. De plus comme le cardinal de la courbe: $|E|=q+1-t$ est divisible par $\ell$ alors on peut montrer que l'on a $t_{\pi}^2-4q$, le discrimant du polynôme charactéristique de $\pi$ qui est non nul. Enfin on montre que les deux valeurs propres $1$ et $q$ sont distinctes car $\ell \nmid q-1$.
Ainsi à l'aide de ces deux valeurs propres ont peut déterminer des sous-groupes cycliques de la $\ell$ torsion.
