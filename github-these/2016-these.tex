...
\begin{equation*}
\forall (X,Y,Z) \in \mathbb{K}^3 \setminus (0,0,0), (X,Y,Z) \mathrm{ssi} (X',Y',Z') \Leftrightarrow \exists c \in \mathbb{K}^*, X'=cX, Y'=cY, Z'=cZ.
\end{equation*}
\end{defi}
...
\end{equation}
avec $a_1,a_2,a_3,a_4,a_6$ des éléments de $\overline{\mathbb{K}}$. Le point $[0:1:0]$ est appelé le \emph{point à l'infini} et sera noté $O_E$. Dès lors que l'on a $a_1,a_2,a_3,a_4,a_6$ des éléments de $\mathbb{K}$ alors on dit que la courbe est définie sur $\mathbb{K}$.
\end{defi}

L'équation \eqref{eq:weierstrass-proj} est dite forme homogène de l'équation de Weierstrass. En appliquant le changement de variables $x=\frac{X}{Z}$, $y=\frac{Y}{Z}$, on passe alors en coordonnées affines, on obtient l'équation de Weierstrass suivante:
...
\begin{defi}
Soit une courbe $E$ définie sur $\mathbb{K}$ on définit $E(\mathbb{K})$, l'ensemble des points d'une courbe elliptique $E$ définit sur $\mathbb{K}$, comme étant l'ensemble:
\begin{equation*}
...
\subsubsection{Formule de Vélu}
Formule de Vélu à mettre ici pour définir $E \rightarrow E/C$.
 
Pour un sous-groupe $C$ de $E(\mathbb{F}_q)$ les formules de Vélu \textcolor{red}{Citer Velu} permettent d'exprimer de façon canonique une isogénie séparable $E \Rightarrow E'$ de noyau $C$ et son codomaine $E'$.  L'isogénie est $\mathbb{F}_q$-rationnelle si et seulement si le polynôme qui s'annule sur les abscisses des points du noyau $C$ est défini dans $\mathbb{F}_q[X]$.
...
\end{equation} 
On peut alors exprimer l'équation de la courbe codomaine de l'isogénie à l'aide des formules d'addition. Cependant par un souci de simplicité on va se restreindre au cas où l'on a $p \geqslant 3$, dans ce cas là on peut exprimer l'équation de $E$ par:
\begin{equation*}
y^2=x^3+a_2 x^2 + a_4 x + a_6
\end{equation*}
On note alors par $f$ et $f'$ les deux fonctions suivantes dans $\mathbb{F}_q(E)$:
\begin{align*}
f(P)&=& x(P)^3+a_2x(P)^2+a_4x(P)+a_6 \\
f'(P)&=& 3x(P)^2+2a_2x(P)+a_4
\end{align*}
Avec les formules d'addition on peut réécrire l'équation \ref{eq:Velu} de la façon suivante:
...
Alors si on note: 
\begin{equation*}
h(x)=\Pi_{Q \in G \setminus {0_E}}(x-x(Q)),
\end{equation*}
on obtient:
\begin{equation}\label{eq:velu:gh}
I(x,y)=(\frac{g(x)}{h(x)},y(\frac{g(x)}{h(x)})')
\end{equation*}
...
\begin{rem}
On peut calculer l'isogénie à l'aide de la connaissance de son noyau. Une méthode pratique reste celle préconisé par Elkies \textcolor{red}{ref a mettre} qui utilise les équations \eqref{eq:velu:gh} et \eqref{eq:velu:sum}:
\begin{equation*}
...
\end{equation}
avec $p_1$ la somme des abscisses des points du noyau. Ainsi à partir de $h(x)$ on est capable de calculer l'isoégnie en $\mathsf{M}(\deg(I))$ opérations sur $\mathbb{F}_q$.
\end{rem}

Une conséquence directe de ce théorème est le résultat suivant.

\begin{cor}
Soit $E_1$ une courbe elliptique, alors il y a une correspondance bijective entre les isogénies séparables qui ont pour domaine de définition $E_1$ et les sous-groupes finis de $E_1$:
\begin{equation*}
\begin{alignedat}{1}
...
X^2-t_{\pi}X+q=0
\end{equation*}
Une fois la valeur de la trace du Frobenius $t_{\pi}$ déterminée, on a alors $|E(\mathbb{F}_q)|=q+1-t_{\pi}$. En pratique cette équation est résolue modulo $\ell_i$, avec $\ell_i$ un nombre premier. Cette résolution est répétée pour un ensemble de nombres premiers $L$ tel que: $\Pi_{\ell_i \in L}\ell_i>4 \sqrt{q}$, le théorème des restes chinois est alors utilisé pour déterminer $t_{\pi}$ modulo $\Pi_{\ell_i \in L}\ell_i$. On utilise ensuite le théorème de Hasse pour déterminer la cardinalité de la courbe.
...
\end{equation*} 
on veut voir quand il est possible de déterminer des sous-groupes de la $\ell$-torsion définis sur $\mathbb{F}_q$.
On a plusieurs cas possibles selon les propriétés du discriminant $d_{\pi}=t_{\pi}^2-4q$.

\begin{defi}\label{def:dif-Atk-Elk}
Soit $d_{\pi}$ le discriminant du polynôme caractéristique du Frobenius $\pi$ défini sur $\mathbb{F}_q$. Soit $\ell$ un nombre premier différent de $p$ la caractéristique de $\mathbb{F}_q$ alors:
\begin{itemize}
...
\begin{prop}
Soient $j_0 \in \mathbb{F}_q$ le $j$-invariant d'une courbe elliptique $E_0$ définie sur $\mathbb{F}_q$ et  $\Phi_{\ell}(X,Y) \in \mathbb{Z}[X,Y] $ le $\ell$-ème polynôme modulaire. Les $\ell+1$ racines du polynôme $\Phi_{\ell}(X,j_0)$ sont les $j$-invariants de courbes $\ell$-isogénes à $E_0$. 
\end{prop} 
 Les $j$ invariants des courbes $\ell$-isogènes à $E_0$, racines du polynôme modulaire évalué en $j_0$, correspondent exactement aux $j$-invariants des  courbes générées par les $\ell$ isogénies $E \to E/C$ avec $C$(noyau de l'isogénie) un des $\ell+1$ sous groupe cyclique de la $\ell$-torsion. Dés lors avec cette remarque on voit un moyen de déterminer des sous groupes $C$ restreints de  la $\ell$ torsion.  Cependant il faut que celui-ci soit défini sur le corps sur lequel on travaille. On a alors le résultat suivant qui nous permet d'énoncer dans quel corps $C$ est défini en fonction des cas.
...
\begin{enumerate}
\item le polynôme $\Phi(X,j)$ a une racine $j_0 \in \mathbb{F}_{q}^r$ si et seulement si le noyau $C$ de l'isogénie $E \to E/C$ avec $j(E/C)=j_0$ est un espace propre de dimension $1$ pour $\pi^r$ appliqué à $E[\ell]$
\item le polynôme $\Phi(X,j)$ se scinde complètement sur $ \mathbb{F}_{q}^r[X]$ si et seulement si $\pi^r$ agit comme une matrice scalaire sur $E[\ell]$.
\end{enumerate}
\end{prop}
...
\end{enumerate}
\end{thm}
Dans tous les cas on a $r$ qui correspond à l'ordre de la matrice $\pi$ dans $PGL_2(\mathbb{Z}/\ell \mathbb{Z})$ et on a le résultat suivant pour la trace du Frobenius $t_{\pi}$:
\begin{equation*}
t_{\pi}^2=q(\zeta + \zeta^{-1})^2 \bmod \ell
\end{equation*}
avec $\zeta \in \overline{\mathbb{F}_q}$ une racine $r$-ième primitive de l'unité.

Ainsi en pratique pour déterminer la nature d'un nombre premier $\ell$ on calcule $g(X)=\mathrm{pgcd}(\Phi(X,\ell),X^q-X)$. Lorsque l'on a $g(X)=1$  alors $\ell$ est un nombre de Atkin, sinon $\ell$ est un nombre de Elkies.

Les nombres premiers de Elkies sont les plus favorables au calcul de $t_{\pi}$ car une fois que l'on a un nombre premier de Elkies alors par \eqref{pro:def-ss-grp} il existe un sous groupe cyclique $C$ de la $\ell$ torsion qui est un espace propre de dimension $1$ pour le Frobenius $\pi$. On résoud alors 
\begin{equation*}
X^2-t_{\pi}X+q = 0 \bmod g_{\ell}
\end{equation*} 
avec $g_{\ell}$ le polynôme qui s'annule sur les abscisses des points de ce groupe $C$.
...
Par contre pour les nombres premiers $\ell$ de Atkin comme un sous groupe cylcique de la $\ell$-torsion ne peut être déterminé dans $\mathbb{F}_q$ alors on ne peut que spéculer sur les valeurs possibles de $t_{\pi}$ et l'on doit alors toutes les considérer. De telle sorte que pour les différents nombres premiers de Atkin on est obligé d'utiliser un algorithme à pas de bébé pas de géant pour lequel il existe des améliorations proposées par \cite{Ler97a}. Ces améloriations utilisent notamment un tri sur les nombres premiers de Atkin en fonction de leur côut, dû aux nombres de racines primites $r$-ième de l'unité, que ceux-ci ont pour l'algorithme.
...
Voyons le cas Elkies plus précisément vu que l'algorithme que proposé dans ce document se compare à de la littérature qui se place dans ce contexte particulier.

\subsubsection{Cas Elkies} \label{sub:Elkies}
Lorsque l'on est dans le cas Elkies, alors on cherche à déterminer un sous-groupe de la $\ell$-torsion qui est aussi espace propre pour le Frobenius, le but étant au final de déterminer la trace du Frobenius à travers la détermination de son action sur cet espace propre. 

Ce sous groupe $C$ est déterminé comme étant le noyau de l'isogénie qui relie la courbe $E_0$ dont on veut calculer le cardinal et une des courbes $\ell$-isogènes à $E_0$. Pour calculer l'une des courbes $\ell$-isogènes il faut calculer les racines du polynôme modulaire $\Phi_{\ell}(X,Y)$ évalué en $j_0$ ($j$-invariant de $E_0$), on obtient alors le $j$-invariant des courbes $\ell$-isognèes définies sur $\mathbb{F}_q$. Celles-ci sont définies sur $\mathbb{F}_q$ par leur $j$-invariant à tordue près. Il faut donc se servir du résultat suivant prouvé par Elkies afin de déterminer exactement la courbe isogène à l'aide des formes de Weierstrass.
...
\end{equation*}
 qui nous est précisée par les équations suivantes: 
\begin{equation*}
\tilde{a_4}=-\frac{1}{48}\frac{\tilde{j'}^2}{\tilde{j}(\tilde{j}-1728)} \quad \tilde{a_6}=-\frac{1}{864}\frac{\tilde{j'}^3}{\tilde{j}^2(\tilde{j}-1728)}
\end{equation*}
avec $\tilde{j'} \in \mathbb{F}_q$ donné par l'équation suivante:
\begin{equation*}
\tilde{j'}=-\frac{18}{\ell}\frac{a_6}{a_4}\frac{\Phi_{\ell,X}(j,\tilde{j})}{\Phi_{\ell,Y}(j,\tilde{j})}j
\end{equation*}
avec la notation $\Phi_{\ell,X}$(resp. $\Phi_{\ell,Y}$) pour noter la dérivée partielle de $\Phi_{\ell}$ en $X$ (resp. $Y$): $\frac{\partial \Phi_{\ell}}{\partial X} $ (resp. $\frac{\partial \Phi_{\ell}}{\partial, Y} $ )
\end{prop} 

Maintenant que la courbe isogène est entièrement déterminée, il reste à déterminer le noyau de l'isogénie de celle-ci c'est ce que des algorithmes tels \cite{BMSS08}  puis \cite{Lercier-Sirvent2008}   cherchent à résoudre en particulier dans ce contexte. Un présentation de ces algorithmes est faite \textcolor{red}{ref à mettre plus tard}.


\subsection{Lien entre isogénie et son action sur le module de Tate}

\chapter{Construction efficaces de tours $\ell$-adiques}
La construction d'extension de corps finis est un problème naturel dès lors que l'on travaille sur des corps finis. On peut être ammené à construire des extensions de corps de taille arbitraire, pour lesquelles on voudra une construction dite compatible de telle sorte que l'on puisse projeter efficacement, lorsque cela est possible, les résultats dans les corps plus petits.

\begin{defi}[Tour \textit{$\ell$}-adique]
\label{def:tour-ell}
On appelle tour d'extension $\ell$-adique du corps fini $\mathbb{F}_q$ la suite d'extensions: $\mathbb{F}_q, \mathsf{F}_{0}, \mathsf{F}_{1}, \mathsf{F}_{2}, \cdots$ pour lesquels:
\begin{itemize}
\item $\mathsf{F}_{0}$ est une extension de $\mathbb{F}_q$ de degré $d_0$ avec $d_0 =r$ avec $r$ qui représente l'odre multiplicatif de $q$ dans $\mathbb{Z}/\ell \mathbb{Z}$,
\item $\mathsf{F}_{i}$ pour $i \geqslant 1$ est une extension de $\mathsf{F}_{i-1}$ de degré $\ell$.
\end{itemize}
\end{defi}


Les travaux de \cite{DeFeo-Shost'12} sur les tours $p$-adiques ont ouvert la voie à d'autres travaux sur les tours $2$-adiques tels que \cite{Doliskani-Schost15}, les tours $\ell$-adiques tels que \cite{DeFeo-Doliskani-Schost13}. Dans ces travaux l'idée pour obtenir des constructions de tours de corps finis compatibles est le passage de représentation univariée à bivariée. Ces idées ont permis d'obtenir des résultats satisfaisants en terme de complexité comparé aux résultats obtenus en utilisant de l'algèbre linéaire dans les constructions préconisées dans Magma \cite{BosmaCS97}. Ainsi certains algorithmes tels que le relèvement ou la descente d'éléments dans les corps de la tour $\ell$-adique (et $2$-adique) ont des complexités optimales et correspondent juste à une réécriture des éléments. Il y a aussi des opérations comme le Frobenius, crucial pour l'algorithme \textcolor{red}{ref à mettre plus tard}, ou le calcul de racine carrée qui ont des coûts satisfaisants. Tout cela va être détaillé dans ce qui suit. Les travaux de  \cite{DeFeo-Shost'12} ne seront pas étudiés ici, ces travaux sont en effet complémentaires de ceux étudiés et utilisés ici \cite{Doliskani-Schost15}, \cite{DeFeo-Doliskani-Schost13}.

Une façon naturelle de  construire une extension de corps fini $\mathbb{F}_q$ est de trouver un polynôme $P$ irréductible défini sur $\mathbb{F}_q$ et de degré égal à celui de l'extension désirée et de quotienter ensuite $\mathbb{F}_q$ par celui-ci.
Ainsi les éléments des tours d'extension seront toujours représentés comme des polynômes à coefficients dans $\mathsf{F}_0$, la taille des éléments de $\mathsf{F}_{k}$ sera donc $O(\ell^k)$ éléments de $\mathsf{F}_0$.

Les complexités sont calculées en nombre d'opérations ($+, \times,\div$) dans $\mathbb{F}_q$, indépendamment de la construction de $\mathbb{F}_q$. On notera par $\mathsf{M}: \mathbb{N} \rightarrow \mathbb{N}$ l'application telle que la multiplication d'un polynôme de degré $n$ à coefficients dans $\mathbb{F}_q$ coûte $\mathsf{M}(n)$ opérations dans $\mathbb{F}_q$ et telle que $\mathsf{M}$ est super-linéaire, en utilisant les résultats de \cite{Cantor-Kaltofen91} on a $\mathsf{M}(n)=O(n\log(n) \log \log(n))$. 


La totalité des travaux présentés ici proviennent du travail de \cite{Doliskani-Schost15}.
...
\section{Construction d'une tour d'extension $\ell$-adique}
L'objectif est de calculer une tour d'extension $\ell$-adique du corps fini $\mathbb{F}_q$, avec $q \wedge \ell =1$, de taille $\mathbb{F}_{q^{d_0n}}$ avec $n=\ell^k$, $k \geqslant 1$ et $d_0 \mid \ell-1$. 
On considère deux cas selon que $\ell=2$ ou $\ell \neq 2$.
\subsubsection{$\ell \neq 2$}
Pour $\ell \neq 2$ on considère deux cas: 
\paragraph{}{ Pour $q \neq 1 \bmod \ell$ on se ramène au cas $q=1\bmod \ell$ en construisant une extension $\mathsf{F}_{0}$ de $\mathbb{F}_q$ de degré $d_0=r$ avec $r$ qui représente l'ordre multiplicatif de $q$ dans $\mathbb{Z}/\ell \mathbb{Z}$, il divise $\ell-1$ et est premier avec $\ell$. La construction de $\mathsf{F}_0$ est faite en même temps que la détermination d'un non résidu $\ell$-adique.

On calcule le $\ell$-ème polynôme cyclotomique $\Phi_{\ell} \in \mathbb{Z}[X_0]$ et on le factorise sur $\mathbb{F}_q[X_0]$ par \cite[Th 9]{Shoup93} cela a un coût de $O_e(\mathsf{M}(\ell)\log(\ell q))$ opérations sur $\mathbb{F}_q$.
Sur $\mathbb{F}_q[X_0]$, $\Phi_{\ell}$ se scinde en facteurs irréductibles de même degré $r$. Soit $R_0$ l'un de ces facteurs, alors par construction il existe un non résidu $\ell$-adique dans $\mathbb{F}_q[X_0]/ \langle R_0 \rangle $. On trouve après avoir testé $O(1)$ éléments tirés au hasard par \cite[Lemma 15]{Shoup93}  un tel non résidu $\ell$-adique, chaque essai a un coût de $O_e(\mathsf{M}(\ell)\log(r)+\mathsf{M}(r)\log(\ell)\log(r)+\mathsf{M}(r)\log(q))$ opérations sur $\mathbb{F}_q$. Une fois un tel non résidu $\ell$-adique $y_0$ trouvé on calcule son polynôme minimal $P_0$, ce calcul coûte $O(r^2)$ opérations sur $\mathbb{F}_q$ d'après \cite[Th. 4]{Shoup93}. On construit donc $\mathsf{F}_0=\mathbb{F}_q[X_0]/\langle P_0(X_0) \rangle$ et le non résidu $\ell$-adique sera alors l'image de $X_0$ dans $\mathsf{F}_0$. $\mathsf{F}_0$ étant une extension de degré premier à $\ell$ de $\mathbb{F}_q$ alors lorsque l'on a une complexité exprimée en termes d'opérations sur $\mathsf{F}_0$ 
...
%Dans cette sous-section les cas $\ell=2$ et $\ell \neq 2$ vont être traités de façon unifiée. 
...
Les éléments de $\mathsf{F}_k$ peuvent être représentés de deux manières différentes: univariée et bivariée, de la même manière que dans \cite{DeFeo-Shost'12}. 
...
\[
T_1=X_1-\alpha \quad \textit{et} \quad T_k=X_k^{\ell}-X_{k-1} \quad \textit{avec $k$ entier naturel non nul}
\]  de la même manière la suite de polynômes $P_1,P_2, \cdots P_k$ est définie à l'aide de $\alpha$ de la façon suivante:
\[
P_k=X_k^{\ell^k}-\alpha \quad \textit{avec $k$ entier naturel non nul} 
\]
...
\]
Soit $k \geqslant 1$  on note $x_k$ l'image de l'indéterminée $X_k$ dans l'anneau résiduel $\mathsf{F}_0[X_1,X_2, ...,X_k]/A_k$, alors par inclusion naturelle de $\mathsf{F}_0[X_1,X_2, ...,X_k]/A_k$ dans $\mathsf{F}_0[X_1,X_2, ...,X_{k+1}]/A_{k+1}$, $x_k$ peut aussi être vu de façon indifférenciée comme un élément de $\mathsf{F}_{j}$ pour $j \geqslant k$.

De plus grâce à l'équation \eqref{eq:corres-repre-2}, représenter un élément de $\mathsf{F}_k$ peut être fait soit comme un polynôme en $x_1, ..., x_k$ de degré au plus un en chaque variable grâce au côté gauche de l'équation \eqref{eq:corres-repre-2}, soit comme un polynôme en $x_k$ de degré au plus $\ell^k$ grâce au côté droit de l'équation.

L'écriture par défaut des éléments de $\mathsf{F}_k$ sera donc la représentation univariée. Ainsi un élément de $\mathsf{F}_k$ sera représenté sous la forme $g(x_k)$ avec $g$ un polynôme de $\mathsf{F}_0[X_k]$ de degré strictement inférieur à $\ell^k$. 

Cependant lors des représentations d'éléments de $\mathsf{F}_k$ il ne va pas être fait des changements de base univariée à base multivariée mais seulement à une base bivariée pour permettre des passages un niveau plus bas dans la tour $\ell$-adique. On représentera par exemple alors $\mathsf{F}_k$ pour $k\geqslant1$ des deux façons suivantes:
\begin{equation*}
...
\end{equation*}  
La $\mathsf{F}_0$ base monomiale de $\mathsf{F}_k$ est donc:
\begin{equation*}
1,x_k,x_k^{\ell},...,x_k^{\ell^k-1}
\end{equation*}
la $\mathsf{F}_0$ base bivariée de $\mathsf{F}_k$ est alors:
\begin{equation*}
1,x_{k-1},x_{k-1}^{\ell},...,x_{k-1}^{\ell^{k-1}-1},x_k,x_kx_{k-1},x_kx_{k-1}^2,...,x_kx_{k-1}^{\ell^{k-1}-1}
\end{equation*}
En pratique ce changement de base se fait de la façon suivante à partir d'une expression $G(x_k)$ d'un élément de $\mathsf{F}_k$ avec $G$ un polynôme de degré inférieur à $\ell^k-1$:
\begin{equation*}
...
L'inversion d'un élément peut être alors considérée dans une base univariée en utilisant l'algorithme d'Euclide étendu. Le coût d'inversion d'un élément de $\mathsf{F}_k$ serait de $O(\mathsf{M}(\ell^k)\log(\ell^k))$ opérations sur $\mathsf{F}_0$ en utilisant les résultats de \cite{vzGJG03}[ch.11]. Une autre technique plus avantageuse est préférée, soit $G(x_k)$ un élément inversible de $\mathSf{F}_k$ alors en écrivant $G(x_k)=G_0(x_{k-1})+x_kG_1(x_{k-1})$ on obtient:
\begin{align*}
\frac{1}{G(x_k)} &=\frac{1}{G_0(x_{k-1})+x_kG_1(x_{k-1})} \\
 				&= \frac{G_0(x_{k-1})-x_kG_1(x_{k-1})}{G_0(x_{k-1})^2-x_{k-1}G_1(x_{k-1})^2}
\end{align*}
Ainsi le coût d'une inversion d'un élément de $\mathsf{F}_k$ est le coût d'une addition, de la multiplication d'éléments de $\mathsf{F}_k$ ainsi que le coût d'une inversion d'éléments dans $\mathsf{F}_{k-1}$, on a donc affaire à un algorithme récursif de coût  $T(k)=T(k-1)+O(M(\ell^k))$; avec la super linéarité de $M$ on obtient un coût de calcul de $O(M(\ell^k))$ opérations sur $\mathsf{F}_0$. La complexité de l'inversion est donc de $O(M(\ell^k))$ opérations sur $\mathbb{F}_q$ lorsque $\mathbb{F}_q \cong \mathsf{F}_0$ et de $O(M(\ell^k)\log(\ell))$ opérations sur $\mathbb{F}_q$ lorsque $\mathbb{F}_q \ncong \mathsf{F}_0$.
...
Soit $\mathbb{F}_q, \mathsf{F}_0, \mathsf{F}_1,  \cdots \mathsf{F}_n$ une tour d'extension $\ell$-adique
...
\begin{itemize}
\item lorsque $\mathsf{F}_0 \cong \mathbb{F}_q$ : $O(\ell^{i})$ opérations sur $\mathbb{F}_q$ après un pré-calcul de $O(\log(q))$ opérations sur $\mathbb{F}_q$
\item lorsque $\mathsf{F}_0 \ncong \mathbb{F}_q$ :
 $O(\ell^{i}M(\ell))$ opérations sur $\mathbb{F}_q$ après un pré-calcul de $O(\ell M(\ell)\log(q))$ opérations sur $\mathbb{F}_q$
\end{itemize}
auxquels il faut ajouter un calcul booléen de $O(\log(\ell^j)\log(\ell^i|\mathsf{F}_0|))$.
\end{thm} 
...
Une méthode rapide est préconisée pour le calcul de $u \bmod |\mathsf{F}_0|-1$ et $v$, dans un premier temps il faut calculer $\rho=s \bmod \ell^i(|\mathsf{F}_0|-1)$ à l'aide de l'exponentiation rapide puis $u \bmod |\mathsf{F}_0|-1$ et $v$ sont obtenus comme le quotient et le reste de la division euclidienne   de $\rho$ par $\ell^i$. Le coût booléen d'une telle opération est de $O(\log(\ell^j)\log(\ell^{i}|\mathsf{F}_0|))$. L'opération majeure étant l'exponentiation avec un exposant de taille $O(\ell^{j}\log(|\mathsf{F}_0|))$ modulo un entier de taille $O(\log(\ell^{i}|\mathsf{F}_0|))$.
Le calcul de $x_0^{u \bmod |\mathsf{F}_0|-1}$ se fait en utilisant $O(\ell M(\ell) \log(q))$ opérations sur $\mathbb{F}_q$ si $\mathsf{F}_0 \ncong \mathbb{F}_q $ et $O(\log(q))$ si $\mathbb{F}_q \cong \mathsf{F}_0$. On conserve ce résultat sous la forme d'un monôme dans $F_0[x_i]$. 

Comme préconisé dans la construction de la tour d'extension $\ell$-adique les éléments de la tour sont représentés comme des éléments à coefficients dans $\mathsf{F}_0$. Cependant il peut arriver que lorsque $\mathbb{F}_q \ncong  \mathsf{F}_0$ un élément puisse être représenté avec ses coefficients dans $\mathbb{F}_q$, on va donc à travers le calcul du Frobenius expliciter ces changements de représentation. On se place dans le cas où $\mathbb{F}_q \ncong \mathsf{F}_0$ avec $a$ représenté sous la forme d'un polynôme en $x_i$ de degré inférieur ou égal à $[\mathsf{F}_i:\mathbb{F}_q]$ : 
\begin{equation*}
...
\end{equation*}
avec les $a_w=a_w^* + \sum_{k=1}^{d_1d-1/d}a_{i+dk}x_1^k \in \mathsf{F}_0$ pour $w$ appartenant à $[0; d-1]$. Pour obtenir cette réécriture on s'est servi du fait que pour $t$ appartenant à $[1; d_1d-1]$ tel que $t=d u_t+v_t$ avec $0 \leqslant v_t<d$ on a $x_w^t=x_1^{u_t \bmod |\mathsf{F}_0|-1}x_w^{v_t}$ . Ainsi cette opération est juste un réarrangement des coefficients de $a$.

On a alors $a^s$ qui s'écrit sous la forme:
\begin{equation*}
a_0 + a_1 y + a_2 y^2 + \cdots + a_{d_1}y^{d-1}
\end{equation*}
On calcule alors $a(y)$ par la méthode de Horner. Les puissances $y^k$ étant des monômes dans $F_0[x_i]$, on calcule chacune des puissances successives à l'aide de la précédente en $O(M(\ell))$ opérations sur $\mathbb{F}_q$ si $\mathbb{F}_q \ncong \mathsf{F}_0$, sinon cela coute une multiplication dans $\mathbb{F}_q$. On a alors un coût total de $O(\ell^{i}M(\ell)))$ opérations dans $\mathbb{F}_q$ si $\mathbb{F}_q \ncong \mathsf{F}_0$ et $O(\ell^i)$ opérations sur $\mathbb{F}_q$ sinon. En réarrangeant les monômes $a_k y^k$ on obtient un polynôme en $(x_0,x_i)$ de degré $(d_1,d)$, on réécrit alors $a^s$ dans une base canonique de $\mathsf{F}_i$ à l'aide du changement de représentation inverse de celui effectué précédemment. En effet $a^s$ est représenté sous la forme:
\begin{equation*}
b_0 + b_1 x_i + b_2 x_i^2 + \cdots + b_{d-1}x_i^{d-1}
\end{equation*}
avec $b_k$ un élément de $\mathsf{F}_0$ pour tout $k$. En remarquant que
\begin{equation*}
 b_k=b_{k,0} + b_{k,1}x_0 + \cdots + b_{k,d_1-1} x_0^{d_1-1}=b_{k,0} + b_{k,1}x_i^d + b_{k,2} x_i^{2 \times d} + \cdots + b_{k,d_1-1} x_i^{(d_1-1)\times d} 
\end{equation*}
 avec $b_{k,i}$ un élément de $\mathbb{F}_q$ pour tous $k,i$. On représente alors $a^s$ sous la forme:
\begin{equation*}
b_0^* + b_1^* x_i + b_2 x_i^2 + \cdots + b_{d_1d-1} x_i^{d_1d-1}
\end{equation*}
 en  utilisant la transformation suivante $b_k^*=b_{u,v}$ pour tous $k,u,v$ tels que $k=ud+v$ avec $0 \leqslant v < d$. On a alors ici aussi un changement de représentation qui se fait à l'aide d'un réarrangement des coefficients qui premet d'exprimer le résultat sous la même représentation que celle donnée en entrée.
\end{proof}
...
Les résultats présentés sont issus d'idées développées dans \cite{Doliskani-Schost14} et sont adaptés à la construction de tour d'extension $\ell$-adique présentée ici où le coût du Frobenius est très faible. Le résultat à retenir est celui pour les extensions dyadiques du fait de son usage pratique même si une généralisation est présentée. 

\begin{prop}
Soit $\mathbb{F}_q, \mathsf{F}_0, \cdots \mathsf{F}_k$ une tour d'extension $\ell$-adique. Soit $\delta$ un carré non nul de $\mathsf{F}_k$, alors une racine carrée $\gamma$ de $\delta$ est calculée:
\begin{itemize}
\item lorsque $\mathsf{F}_0 \cong \mathbb{F}_q$ en $O(M(\ell^k)\log(\ell^kq))$ opérations sur $\mathbb{F}_q$,
\item lorsque $\mathsf{F}_0 \ncong \mathbb{F}_q$ en $O(M(\ell^k)\log(\ell)\log(\ell^kq^{\ell}))$ opérations sur $\mathbb{F}_q$.
\end{itemize}  
\end{prop}

\begin{proof}
Soit $\delta$ un carré non nul de $\mathsf{F}_k$ ayant pour racine carrée $\gamma$ (non connue a priori). Soit $\beta$ dans $\mathsf{F}_0$ défini de la manière suivante:
\begin{align}
...
\end{align*}
Sans perte de généralité on peut supposer que l'on a $\eta \neq 0$, si ce n'est pas le cas alors on peut remplacer $\gamma$ par $\gamma c^2$ avec $c$ choisi aléatoirement parmi les inversibles de $\mathsf{F}_k$.
$O(1)$ essais sont nécessaires en moyenne pour obtenir $T_{\mathbb{L}_k/\mathsf{F}_0}(\gamma c) \neq 0$. En effet il y a $|\mathsf{F}_0|^{\ell^k}/|\mathsf{F}_0|$ valeurs de $c$ pour lesquelles on a $T_{\mathsf{F}_k/\mathsf{F}_0}(\gamma c) = 0$ ainsi la probabilité d'avoir $T_{\mathsf{F}_k/\mathsf{F}_0}(\gamma c^2) \neq 0$ est de $1-(|\mathsf{F}_0|^{\ell^k}/|\mathsf{F}_0|)/(|\mathsf{F}_0|^{\ell^k})=1-1/|\mathsf{F}_0|>1/2$.
...
\begin{equation*}
 \gamma=\beta \eta^{-1}.
\end{equation*}
Le coût de calcul de $\beta$ est un nombre attendu de $O(\log(q))$ opérations sur $\mathbb{F}_q$ \cite{vzGJG03}[ch 14.5] si $\mathsf{F}_0 \cong \mathbb{F}_q$, et de $O(M(\ell)\log(\ell)\ell\log(q))$ opérations sur $\mathbb{F}_q$ si $\mathsf{F}_0 \ncong \mathbb{F}_q$. Ainsi seul le coût du calcul de $\eta$ est crucial pour obtenir une complexité satisfaisante.
...
\begin{equation*}
\zeta_1=\gamma^{|\mathsf{F}_0|} \quad \textit{et} \quad
\zeta_m=
\begin{cases} 
\zeta_{m/2}  \zeta_{m/2}\zeta_{m/2}^{|\mathsf{F}_0|^{m/2}} \quad \textit{si m est pair }\\
\zeta_{1}  \zeta_{m-1}^{|\mathsf{F}_0|} \qaud \textit{si m est impair}
\end{cases}
\end{equation*}
...
\begin{equation}
\label{eq:varepsilon}
\varepsilon_1=\lambda^{|\mathsf{F}_0|} \quad \textit{et} \quad
\varepsilon_m=
\begin{cases} 
\varepsilon_{m/2} + \zeta_{m/2}\varepsilon_{m/2}^{|\mathsf{F}_0|^{m/2}} \textit{si m est pair }\\
...
Le calcul de $\lambda$ a un coût de $O(M(\ell^k)log(q))$ opérations sur $\mathbb{F}_q$ si $\mathsf{F}_0 \cong \mathbb{F}_q$ et de $O(M(\ell^k)\ell\log(\ell)\log(q))$ opérations sur $\mathbb{F}_q$ si $\mathsf{F}_0 \ncong \mathbb{F}_q$. Le calcul de $\varepsilon_1$ et $\zeta_1$ se fait quant à lui à l'aide de $O(1)$ fois le Frobenius. Calculons alors par récurrence le coût total, en supposant que le calcul de $\varepsilon_m$ et $\zeta_m$ ait déjà été effectué. En utilisant les équations \ref{varepsilon} $\varepsilon_{2m}$ et $\zeta_{2m}$ ou $\varepsilon_{2m+1}$ et $\zeta_{2m+1}$ peuvent être calculés en utilisant $O(1)$ Frobenius et $O(1)$ multiplications sur $\mathsf{F}_{k}$. Ainsi $\varepsilon_n$ et $\eta=1+\lambda+\lambda\varepsilon_{n-2}$ sont calculés en $O(M(\ell^k)\log(\ell^k)+M(\ell^k)\log(q))=O(M(\ell^k)\log(\ell^kq))$ opérations sur $\mathbb{F}_q$ si $\mathsf{F}_0 \cong \mathbb{F}_q$ et en $O(M(\ell^k)\log(\ell^k)\log(\ell)+M(\ell^k)\ell\log(\ell)\log(q))=O(M(\ell^k)\log(\ell)\log(\ell^kq^{\ell}))$ opérations sur $\mathbb{F}_q$ si $\mathsf{F}_0 \ncong \mathbb{F}_q$.
Le coût de calcul de $\gamma$ et $\beta$ étant négligeables par rapport à celui de $\eta$, on a alors un coût total de $O(M(\ell^k)\log(\ell^kq))$ opérations sur $\mathbb{F}_q$ si $\mathsf{F}_0 \cong \mathbb{F}_q$ et de $O(M(\ell^k)\log(\ell)\log(\ell^kq^{\ell}))$ opérations sur $\mathbb{F}_q$ si $\mathsf{F}_0 \ncong \mathbb{F}_q$.

\end{proof}

Une implantation de cette construction de tours $2$-adique et des opérations du Frobenius et du calcul de racine carrée ont été réalisé en SageMath le code est disponible à cette adresse: \url{https://github.com/Hugounenq-Cyril/Two_curves_on_a_volcano/blob/master/Code/extension_corps.sage}.
...
Les coûts des différentes opérations dans de telles tours d'extensions $\ell$-adiques sont résumés dans le tableau suivant \ref{tab:complexite-degre2}.
\begin{figure}
\label{tab:complexite-degre2}
\begin{tabular}{|l|r|r|}
  \hline
  \textit{Opérations}  & \textit{Coûts} & \textit{Coûts} \\
  \hline
  &\textit{ $\mathsf{F}_0 \cong \mathbb{F}_q$ } &  \textit{ $\mathsf{F}_0 \cong \mathbb{F}_q$ } \\
  \hline
  \textit{addition/soustraction}  & $O(\ell^k)$ & $O(\ell^k)$\\
  \textit{multiplication}  & $\mathsf{M}(\ell^k)+ O(\ell^k)$ & $\mathsf{M}(\ell^k)\log(\ell)+ O(\ell^k)\log(\ell)$ \\
  \textit{division} & $O(\mathsf{M}(\ell^k))$ & $O(\mathsf{M}(\ell^k)\log(\ell))$ \\
  \textit{Frobenius} & $O(\ell^k + \log(q))$ & $O(\ell^k M(\ell)+ \ell \mathsf{M}(\ell)\log(q)$ \\
  \textit{Square-root} & $ O(\mathsf{M}(\ell^k)\log(\ell^kq)) (\textit{attendu}) $ & $ O(\mathsf{M}(\ell^k)\log(\ell)\log(\ell^kq^{\ell})) (\textit{attendu}) $\\
  \hline
\end{tabular}
\caption{Coûts des opérations dans $\mathsf{F}_{k}$ exprimées en nombre d'opérations sur $\mathbb{F}_q$}
\end{figure}
Il est à noter que pour le calcul d'une racine carrée le coût est un coût espéré, cela est dû à une partie de l'algorithme qui est de type non déterministe (Las Vegas).
