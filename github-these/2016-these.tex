...
[1,\deg(P)], j \in [1,\deg(Q)]$. On note $R= P\bigodot Q$.  
\end{defi}\todo{changer notation des degrés!}
...
voudrait dire que l'on a $E[\ell_i \ell_j] \subset E(\mathbb{F}_q)$ \todo{reflechir a cette remarque qui est fausse}
\end{rem}
...
\begin{nota} Afin de simplifier les notations on va 
 ...
 degré du composita $\mathbb{K}_i$ comme extension de $\mathbb{F}_q$.
...
\begin{prop}
 ...
 $O(p_n \mathsf{M}(p_nd)\log(d)\log(dq))$ opérations sur $\mathbb{F}_q$. \todo{trouver complexité bis}.
\end{prop}
...
$d_i\ell_i^{n_i},p_{i-1}$ premiers entre eux à coefficients dans $\mathbb{F}_q$
qui définissent $\mathbb{F}_q[x]/\langle P \rangle \cong \mathsf{F}_{(i,n_i)}$,
$\mathbb{F}_q[x]/\langle Q \rangle \cong \mathbb{K}_{i-1}$. Soit $R=P \bigodot 
...
a un coût de:
\begin{enumerate}
\item $O((d_i\ell_i^{n_i})^2\mathsf{M}(p_{i_1}))$ opérations sur $\mathbb{F}_q$ lorsque 
$d_i\ell_i^{n_i} \leqslant p_{i_1}$ et l'application de l'inverse de $\varrho$ 
a un coût de $O(\mathsf{M}(p_i)p_{i-1}^{1/2}+\mathsf{M}(d_i\ell_i^{n_i})p_{i-1}^{(\omega+1)/2})$ 
opérations sur $\mathbb{F}_q$,
\item $O(p_{i-1}^2\mathsf{M}(d_i\ell_i^{n_i}))$ opérations sur $\mathbb{F}_q$ lorsque 
$d_i\ell_i^{n_i} \geqslant p_{i_1}$ et l'application de l'inverse de $\varrho$ 
a un coût de $O(\mathsf{M}(p_i)(d_i\ell_i^{n_i})^{1/2}+\mathsf{M}(p_{i_1})(d_i\ell_i^{n_i})^{(\omega+1)/2})$ 
opérations sur $\mathbb{F}_q$.
\end{enumerate}
\end{prop}
...
$\mathsf{F}_{(i,n_i)}=\mathbb{F}_q[x]/\langle P \rangle$, $Q$ un polynôme 
irréductible de degré $p_{i-1}=\prod_{j=1}^{i-1}d_j\ell_j^{k_j}$
...
$q^{r p_{i-1}}$ est de $O(p_{i})$) opérations dans $\mathbb{F}_q$ avec un 
précalcul qui a un coût booléen de $O(\log(rp_{i-1})\log(\ell_i^{n_i}qd_i))$ et  
...
$\mathbb{F}_q[x,y]/\langle P,Q \rangle \cong \mathbb{F}_q[z]/\langle R \rangle$
 avec un coût de  opérations sur $\mathbb{F}_q$.
